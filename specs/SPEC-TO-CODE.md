...
| `SrvBind(cn, p)` | `kernel/devsrv.c::srv_conn_open_for_proc` + `kernel/srvconn.c::srvconn_create` (kernel; landed P5-corvus-srv-impl-a3b) + `kernel/syscall.c::sys_srv_connect_handler` (production client-open path; landed P5-corvus-srv-impl-b2) | The kernel mints a fresh per-Proc connection (transport pair + dedicated synchronous `p9_client`) when Proc p opens `/srv/corvus`, stamping the peer identity. `SRV_CONN_PER_PROC_MAX = 1` enforces "one connection per Proc" structurally. |
