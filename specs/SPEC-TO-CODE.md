...
- ⬜ **P5-attach-mount** — `kernel/sys_mount.c` + Territory mount table + `specs/namespace.tla` extension. Grafts any Spoor's tree at a target path. The architecturally substantive piece. Can land WITHOUT the user-visible attach_9p syscall, since `p9_attached_root_spoor` is a sufficient source-Spoor producer at v1.0.
