...
**RE-ENABLED for the fork-propagating legate scope (user-directed, 2026-09-07; the Imperium arc IM-2, I-25 strengthened / I-2).** The eighth instance of re-enabling point (a). `specs/imperium.tla` is RESERVED and lands BEFORE the IM-2 impl (`docs/IMPERIUM-DESIGN.md` 11.4): Procs with `scope` (0 = none), `root`, `propagating`, `caps` (the elevated set that flows), `alive`/`terminating`; actions `Redeem(p, propagating)` (a fresh scope iff `scope[p] = 0`; a PROPAGATING redeem refused iff `scope[p] # 0`; a further non-propagating redeem ORs caps and keeps the tag), `Fork(parent, child)` (the child joins the parent's scope; caps flow iff `propagating[parent]`; refused iff the parent is terminating), `Terminate(scope)` (root death / expiry / abdicate: every tag holder is marked terminating, then dead). Invariants `NoElevatedOutlivesScope`, `FlowOnlyUnderPropagatingScope`, `OneScopePerProc`. Buggy cfgs `imperium_buggy_straggler` (Fork skips the terminating check), `imperium_buggy_retag` (a second redeem allocates a fresh scope -- the A-4a F2 shape), `imperium_buggy_flow_without_flag` (caps flow under a non-propagating scope -- the v1.0 strip removed). Why re-enabled: the fork-vs-teardown race and the nested-redeem re-tag are exactly the interleavings a model checker finds and prose reasoning talks itself past. Spec-to-code rows land with IM-2; until then the row set is empty.
