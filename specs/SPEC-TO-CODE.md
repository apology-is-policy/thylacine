...
| (NO spec action -- a declared MODEL GAP, 2026-09-21) | `kernel/syscall.c::sys_seat_import_handler` (`SYS_SEAT_IMPORT` = 122) | The ONE kernel-mediated exception to I-5 (ARCH section 28 I-5 row; `docs/GRAPHICAL-SAK-OWNERSHIP.md`). It mints a SECOND `KObj_DMA` handle, in a Proc that is not the creator, on a buffer the creator still holds -- so `HwHandlesAtOrigin`, read literally, does not describe the tree any more, and `handles.tla` has no action for it (the model's hw partition has exactly one holder per kobj). It is NOT `BuggyHwTransfer`: nothing moves (the creator keeps its handle, the import is a refcount pin via `kobj_dma_ref`), nothing is caller-named (the buffer is resolved by `weft_share_claim_from(share_id, peer.stripes)` -- a share the ACCEPTED CONNECTION PEER's live incarnation registered, never a PA or a handle index from the caller), and the admissible set is two kinds only (`dma->weave` / `dma->gpu_bo`; MMIO, IRQ, PCI, plain DMA, ANON and hostmem are refused). The caller must be the boot-designated seat service (`proc_is_seat_service`), hold `CAP_HW_CREATE`, and pass its own I-34 allowance (`allowance_permits` + `allowance_handle_alloc`, the revoke-raced two-step). Validated by PROSE + the in-session review (spec-first is suspended for new surfaces); if spec-first is re-enabled here, the owed change is an `ImportPin(svc, peer, k)` action plus `HwHandlesAtOrigin` weakened to "origin, OR the designated seat service through a live peer share", with a buggy cfg for the caller-named-buffer form. Runtime witness: `ls-graphical-sak*.exp` (the positive path); the refusal arms have NO kernel test yet -- owed. |
...
| `CloseMaster` / `CloseSlave` (an endpoint fully closes) | `server.rs::Conn::close_endpoint` at every opened-endpoint drop (`fid_clunk` / `fid_set` / `teardown` / `drop_all_fids` — a dying emulator connection IS carrier loss); `ring_drain`'s `other_open == 0` arm returns `Eof` only on an EMPTY ring | drain-then-EOF as above. The `CloseMaster` → `tty:hup` raise (PTY-2d): `open_dec` reports the `n_master` 1→0 edge and `close_endpoint` raises `TTY_SIG_HUP` before the unref. `HupAtMostOnce` is BY CONSTRUCTION — exactly one master fd per pts can ever exist (mint-only; no walk resolves a master path; 9P forbids walking FROM an opened fid; a walk to an existing newfid is rejected), so the edge fires at most once per pts lifetime. |
...
- `Register` ↔ `SYS_LOOM_REGISTER` of a payload page within the per-flow shared Burrow (the I-30 buffer pin) — Weft-2 (cross-Proc Burrow-share) + Weft-6 (the per-flow binding). The per-flow ring registration is **Weft-6a-2**: `SYS_WEFT_SHARE` (`kernel/syscall.c::sys_weft_share_for_proc`) takes the I-30 **registration pin** (`kernel/weft.c::weft_share_register` → `burrow_ref`) + mints the kernel-scoped `share_id`; `SYS_WEFT_MAP` (`sys_weft_map_for_proc` → `weft_map_claimed`) claims it (`weft_share_claim`, consume-once) and `burrow_share_into`s the ring into the guest, recording the binding in `dev9p_priv->weft`. The **netd side** that supplies the registered ring is **Weft-6b-1**: `usr/netd/src/server.rs::h_weft` (dispatched on `P9_TWEFT`) → `Net::weft_ensure(N)` lazily `SYS_BURROW_ATTACH`es the per-flow ring, writes its geometry mirror (`libthyla_rs::weft::init_ring`), and `SYS_WEFT_SHARE`s it — one ring + one `share_id` per flow, idempotent on a repeat `Tweft`; the kernel's claim then realizes the guest-side map above.
...
- `NetdAct` ↔ the consumer reads the payload IN PLACE at the validated `out[].addr` under the flow pin (no per-op re-check) + queues the smoltcp send — the validated snapshot is **Weft-3** (`weft_ring_drain`'s `out[]`); the F_NOTIF holder-set arm (`holders := Holders`) is **Weft-5** (`kernel/weft.c::weft_notif_arm`); the **live netd in-place read + smoltcp queue is Weft-6b-2** — `usr/netd/src/server.rs::h_weftio` reads `ring_va + payload_off + off` for `len` and calls `data_send`, driven by the `Tweftio(off,len,WRITE)` the kernel write fast-path issues (`kernel/dev9p.c::dev9p_weft_try_write` validates the syscall-arg descriptor via `kernel/weft.c::weft_binding_validate_write` against the binding's `weft_ring_view` — the synchronous sibling of the descriptor-ring `Consume`, sourced from a trusted register-passed arg). The batched descriptor-ring drive (`weft_ring_drain` live via Loom) + the F_NOTIF posting are Weft-6c/6b-3.
...
spec-first section; they moved here verbatim when CLAUDE.md was slimmed.
Later re-enablements (cons_poll, net_poll, fs_cache, sched_tickless,
pty_stop, reader_frame, ...) are recorded per-row in
...
**RE-ENABLED for the fork-propagating legate scope (user-directed, 2026-09-07; the Imperium arc IM-2, I-25 strengthened / I-2).** The eighth instance of re-enabling point (a). `specs/imperium.tla` is RESERVED and lands BEFORE the IM-2 impl (`docs/IMPERIUM-DESIGN.md` 11.4): Procs with `scope` (0 = none), `root`, `propagating`, `caps` (the elevated set that flows), `alive`/`terminating`; actions `Redeem(p, propagating)` (a fresh scope iff `scope[p] = 0`; a PROPAGATING redeem refused iff `scope[p] # 0`; a further non-propagating redeem ORs caps and keeps the tag), `Fork(parent, child)` (the child joins the parent's scope; caps flow iff `propagating[parent]`; refused iff the parent is terminating), `Terminate(scope)` (root death / expiry / abdicate: every tag holder is marked terminating, then dead). Invariants `NoElevatedOutlivesScope`, `FlowOnlyUnderPropagating`, `OneScopePerProc` (+ as landed: `ScopeTraitsSetOnce`, `MembersNeverRoot`, `PropagatingIsScopeWide`, `FlowNeverWidens`, the `ScopeEventuallyEmpty` liveness). Buggy cfgs `imperium_buggy_straggler` (Fork skips the terminating check), `imperium_buggy_retag` (a second redeem allocates a fresh scope -- the A-4a F2 shape), `imperium_buggy_flow_without_flag` (caps flow under a non-propagating scope -- the v1.0 strip removed), `imperium_buggy_nest` (a PROPAGATING further redeem admitted -- added at the build). Why re-enabled: the fork-vs-teardown race and the nested-redeem re-tag are exactly the interleavings a model checker finds and prose reasoning talks itself past. **LANDED 2026-09-07 with IM-2**: the module went through TLC before the kernel was touched; the first run showed the retag cfg tripping on a root re-tagging ITSELF, two steps deep, so `NoElevatedOutlivesScope` was re-keyed on the ANCHOR root's identity (captured at the join) and the trace became the member escape. The rows:

| Spec action / invariant | Code |
|---|---|
| `RedeemFresh(p, G, prop)` | `kernel/devcap.c::cap_redeem_grant_for_writer` (the CLEARANCE arm, under `g_cap_grants.lock`) -> `kernel/proc.c::proc_become_legate` FRESH arm: `legate_scope_alloc` + `legate_caps = G` + `legate_flags = prop` written before the `legate_scope_id` RELEASE store; `PROC_FLAG_LEGATE_ROOT` |
| `RedeemFurther(p, G)` | `proc_become_legate` FURTHER arm (`legate_scope_id != 0`): caps OR'd; tag, root status, `legate_caps`, `legate_flags` kept; `legate_valid_until` = the earlier nonzero |
| the refused PROPAGATING further redeem (no clean action; `BuggyRedeemNest` is admitting it) | `cap_redeem_grant_for_writer`: `writer->legate_scope_id != 0` with `CAP_GRANT_FLAG_PROPAGATING` -> -1 WITHOUT consuming; `proc_become_legate`'s defensive twin |
| `Fork(parent, child)` / `Flow(parent)` | `kernel/proc.c::rfork_internal`: `pscope` ACQUIRE-loaded first; `flow = legate_caps iff LEGATE_FLAG_PROPAGATING`; `child->caps = (parent & mask) & ~(CAP_ELEVATION_ONLY & ~flow)`; the tag inherit keyed on `pscope`; `child->legate_caps = flow & child->caps` |
| `ForkOK(parent)` (the straggler close; `BUGGY_STRAGGLER` is its absence) | `rfork_internal`: `parent->group_exit_msg` ACQUIRE-loaded under the SAME `g_proc_table_lock` hold as `proc_link_child`; set -> `rfork_rollback_unpublished` + -1 |
| `Exit(p)` / `Die(p)` + `Swept` | `proc_become_zombie_locked` -> `proc_legate_teardown_if_root` (the tag walk under `g_proc_table_lock`; `proc_group_terminate` each member) |
| `Terminate(p)` | `proc_group_terminate` (the flag; the death at `el0_return_die_check`) |
| `Expire(p)` | `el0_return_die_check`'s `legate_valid_until` arm (`proc_for_each(legate_teardown_cb, except = NULL)`) |
