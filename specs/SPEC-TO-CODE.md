...
- ⬜ **P5-fd-rw** — SYS_READ + SYS_WRITE handlers. User-VA buffer copy into kernel scratch; route to dev->read / dev->write. When this lands, userspace can actually exchange bytes through pipes + 9P-mounted files.
- ⬜ **P5-fd-close** — SYS_CLOSE handler. Thin wrapper over handle_close.
- ⬜ **P5-fd-dup** — SYS_DUP handler. Wraps handle_dup; spoor_ref via the KOBJ_SPOOR acquire path wired at P5-fd-pipe.
