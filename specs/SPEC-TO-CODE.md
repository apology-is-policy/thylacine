...
| `SendIO(t, fid)` — other families | `Tgetattr` / `Tsetattr` / `Tlock` / `Tstatfs` / `Tsync` / `Treflink` / `Tfallocate` / `Tfadvise` / xattr family on tag `t` for `fid` | Wire builders deferred to P5-wire-meta / -mutation / -lock / -xattr / -stratum-ext; session-side dispatch handlers extend `p9_session_dispatch_rmsg` at each landing. |
...
- ✅ **P5-wire-io** (landed this chunk) — codec extension: Tlopen/Rlopen, Tlcreate/Rlcreate, Tread/Rread, Twrite/Rwrite. Session-level send APIs (`p9_session_send_lopen` / `_lcreate` / `_read` / `_write`) with fid-exclusivity rules (lopen/lcreate exclusive; read/write concurrent-permitted). R111 caller-cap-bound applied at `p9_parse_rread`. 15 new unit tests (7 wire + 8 session); test count 276 → 291.
- ⬜ **P5-wire-meta / -mutation / -lock / -xattr / -stratum-ext** — extends the wire codec with the remaining message families. Each wave adds session-side dispatch handlers.
