...
| `scheduler_liveness_hwake.cfg` (P4-Ic6) | Spec_Live_Hwake + SF on Wake | `Invariants` + `LatencyBound` + `HardwareWakeProgress` | clean (2T × 1C) | 23 |
| `scheduler_buggy_hwake.cfg` (P4-Ic6) | Spec_Live (no SF on Wake) | `HardwareWakeProgress` | stuttering (depth 4) | 23 |
...
- **Impl mapping deferred**: replacing the current ELE-on-deadlock-check (`kernel/sched.c::sched()`, the "sched: deadlock — current is blocking, no runnable peer system-wide" extinction call) with a WFI-and-retry pattern requires architectural decisions about the `on_cpu` protocol's intersection with WFI-in-place. Two viable directions:
  1. **Refactor on_cpu**: when a CPU enters WFI-in-place, define semantics that let `wakeup()`'s on_cpu spin terminate without context-switching prev away. Likely requires a new thread sub-state (THREAD_HALTED or similar) + a wakeup() path that doesn't spin on the new state.
  2. **Introduce a separate boot-CPU idle thread**: today the boot CPU's `cs->idle` is `kthread` (sched_init line 136, `thread_init_per_cpu_idle` line 130 explicitly reserves cpu_idx 0 for kthread). Adding a dedicated boot-CPU idle thread (distinct from kthread) lets the deadlock path switch-to-idle cleanly via `cpu_switch_context`, matching the secondaries' per_cpu_main pattern. Architecturally clean (mirrors Linux); requires updating boot sequence + sched_init.

  The spec captures the *intent* (SF on Wake → HardwareWakeProgress holds). The kernel impl picks an approach. Either approach is fairly modeled by the existing spec since the spec abstracts over on_cpu entirely.
- **Pre-existing finding**: `scheduler_liveness_wfi.cfg` (P3-G) violates LatencyBound — has done since the P3-G commit at `7a61651` (verified by replaying TLC at that commit). The cfg uses 2T × 2C with MaxIPIs=2, and the trace lassoes through a state where one CPU's wfi flag flips TRUE while the IPI budget is exhausted, leaving a RUNNABLE thread starved. The SPECIFICATION + INVARIANTS + Liveness_Wfi spec extension itself is sound (proven by `scheduler.cfg` safety + per-action soundness); only the *cfg parameters* are too tight to admit the wake path. Tracked as R12-wfi-cfg (P3-G-era cfg regression, distinct from R12-sched). Recommended follow-up: re-derive a correct cfg parameter set (likely MaxIPIs=4-6 + careful EnterWFI placement) or refactor `EnterWFI`'s preconditions to avoid the lasso. Not in scope for P4-Ic6 spec extension.
