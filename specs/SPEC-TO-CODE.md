...
- ⬜ **P5-pipe-blocking** — extends `kernel/pipe.c` with rendez wait queues: read sleeps when buffer empty; write sleeps when buffer full. Producer/consumer wakes each other on state change. Spec extension needed — `specs/pipe.tla` or extend `specs/scheduler.tla` — to pin the missed-wakeup-freedom invariant (ARCH §28 I-9) at the pipe-specific composition. Required for actual cross-Proc IPC + shell pipelines.
