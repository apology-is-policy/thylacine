...
(*                                                                         *)
(*   atomically, under the Burrow lock:                                    *)
...
(*                                                                         *)
(* The count is a per-slot array beside filepages[], NOT struct             *)
(* page.refcount -- LINEAGE 2.8 measures why (written per-block-head, so    *)
(* every tail page of an order>0 block is stale; already double-booked as   *)
(* SLUB's inuse count; and promoting it still would not give eager anon the *)
(* per-page ownership 2.9 shows COW needs).                                *)
(*                                                                         *)
