...
(* that hook list -- `poll_waiter_list_wake` -- which nests a `wakeup` per *)
(* poller inside the list lock: O(pollers) work the per-byte IRQ must not  *)
(* do, and (until audit round 4) under a PLAIN spinlock no IRQ could take. *)
(* Round 4 F3 made that lock irqsave, for a reason this spec does not      *)
(* model -- the list lock nests under g_cons.lock, which the RX IRQ takes, *)
(* so a list-lock holder with IRQs on could be interrupted by an RX IRQ    *)
(* spinning on g_cons.lock while another CPU held g_cons.lock waiting for  *)
(* the list lock (an ABBA through the IRQ edge). The relay below stays:    *)
(* it is what keeps the IRQ's work O(1).                                    *)
(*                                                                         *)
