...
(*   Both share the same dual-count NoUseAfterFree semantics, which is   *)
(*   the invariant this spec proves. The MMIO type's additional          *)
(*   relationship (holding a ref on the underlying KObj_MMIO so the PA   *)
(*   claim survives the user-handle's close) is modeled in handles.tla's *)
(*   HwResourceExclusive — the cross-spec link is impl-level, not spec-  *)
(*   level. See `kernel/burrow.c::burrow_free_internal`'s type switch.   *)
(*                                                                         *)
