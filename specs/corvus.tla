...
(*                                                                         *)
(* AdminElevate models corvus's side: the passphrase verify plus the      *)
(* console-attached precondition. The cap grant itself runs through the   *)
(* kernel `cap` device (CORVUS-DESIGN §5.5.1) — corvus registers a grant, *)
(* the target Proc redeems it, and the cap lands on the Proc's cap set    *)
(* (not the session record; this spec models that mechanical form). The   *)
(* console-attached check is enforced REDUNDANTLY: by corvus here AND by  *)
(* the kernel at redemption. The kernel copy is load-bearing — it makes   *)
(* HostownerRequiresConsole hold even against a buggy corvus, which is    *)
(* why BuggyElevateWithoutConsole must stay caught: it models corvus      *)
(* omitting its check, and the kernel is the real-system backstop.        *)
(*                                                                         *)
