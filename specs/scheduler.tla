...
(*   P4-Ic6 — R12-sched hardware-wake fairness. Pins the spec-level       *)
(*           invariant for the WFI-and-retry semantics intended to        *)
(*           replace the current impl's ELE-on-blocking-with-no-peer      *)
(*           deadlock check (kernel/sched.c, the "sched: deadlock —       *)
(*           current is blocking, no runnable peer system-wide"           *)
(*           extinction call). Adds:                                       *)
(*                                                                         *)
(*           - HardwareWakeProgress property: every non-cond-waiter        *)
(*             SLEEPING thread eventually leaves SLEEPING. Formal          *)
(*             encoding of "hardware IRQs eventually deliver" — the       *)
(*             external-event guarantee the impl currently denies by      *)
(*             ELEing before the IRQ can arrive.                          *)
(*                                                                         *)
(*           - Liveness_Hwake: extends Liveness_Wfi with SF_vars(Wake(t)) *)
(*             for every thread t. SF here encodes "hardware delivers     *)
(*             the IRQ eventually," independent of any kernel-thread     *)
(*             scheduling decision.                                        *)
(*                                                                         *)
(*           - cfg matrix:                                                 *)
(*             scheduler_liveness_hwake.cfg (correct): Spec_Live_Hwake;   *)
(*               2T × 2C; Invariants + LatencyBound + HardwareWakeProgress *)
(*               all hold.                                                 *)
(*             scheduler_buggy_hwake.cfg (buggy): Spec_Live_Wfi (no SF on *)
(*               Wake); HardwareWakeProgress counterexample. A thread     *)
(*               Block-transitioned to SLEEPING never leaves SLEEPING —   *)
(*               the impl-level analogue of the ELE-on-deadlock-check     *)
(*               killing the system before hardware can deliver.          *)
(*                                                                         *)
(*           Wake(t) itself is the existing P2-Bb action. Its semantics   *)
(*           — "transition a SLEEPING-non-cond-waiter thread to           *)
(*           RUNNABLE + enqueue" — already model what kobj_irq_dispatch's *)
(*           wakeup() call effects at the abstract level. R12-sched is   *)
(*           a FAIRNESS refinement, not a new mechanism: it asserts that *)
(*           Wake fires eventually under fairness, which the impl must   *)
(*           guarantee by WAITING (WFI-and-retry) rather than EXTINCTING. *)
(*                                                                         *)
(*           Impl deferred (the on_cpu protocol intersection with         *)
(*           WFI-in-place requires either an on_cpu refactor or a        *)
(*           separate-idle-thread refactor on the boot CPU; choice is    *)
(*           architectural and warrants its own session).                 *)
(*                                                                         *)
...
(*                              not satisfy I-17.                          *)
(*   scheduler_liveness_hwake.cfg (P4-Ic6 / R12-sched) — Spec_Live_Hwake; *)
(*                              2T × 2C; HardwareWakeProgress holds: a   *)
(*                              Block-transitioned SLEEPING thread is    *)
(*                              eventually woken by SF on Wake.           *)
(*   scheduler_buggy_hwake.cfg (P4-Ic6 / R12-sched) — Spec_Live_Wfi (no  *)
(*                              SF on Wake); HardwareWakeProgress violated:*)
(*                              Block creates a SLEEPING thread which then *)
(*                              stutters in SLEEPING forever. Models the  *)
(*                              impl's ELE-on-deadlock-check (the kernel  *)
(*                              extincts before the hardware IRQ can      *)
(*                              deliver its wakeup).                       *)
(*                                                                         *)
...
(*                                                                         *)
(* P4-Ic6 / R12-sched: this action is also the abstract model of the      *)
(* kobj_irq_dispatch wakeup() path. A thread t blocked on a hardware-IRQ  *)
(* Rendez sits in state[t]=SLEEPING with t \notin waiters (since the spec *)
(* models only ONE shared cond, while the impl has many Rendez; threads   *)
(* blocked on the irq Rendez are NOT in the spec's `waiters` set, which   *)
(* is reserved for the cond-waiter model). Wake(t) transitions such a    *)
(* thread to RUNNABLE + enqueues. SF_vars(Wake(t)) (Liveness_Hwake)       *)
(* asserts that hardware IRQs eventually deliver — the property the impl  *)
...
(***************************************************************************)
(* P4-Ic6 / R12-sched: HardwareWakeProgress.                               *)
(*                                                                         *)
(* Every non-cond-waiter SLEEPING thread eventually leaves SLEEPING.       *)
(* Spec-level encoding of the external-event guarantee that hardware      *)
(* IRQs always eventually deliver. The Wake(t) action models the impl's   *)
(* kobj_irq_dispatch → wakeup() path; SF on Wake (in Liveness_Hwake)      *)
(* asserts the fairness.                                                   *)
(*                                                                         *)
(* Why we need this distinct from LatencyBound: LatencyBound is a         *)
(* RUNNABLE-only property (RUNNABLE => <>RUNNING). A SLEEPING thread is   *)
(* not RUNNABLE; LatencyBound is vacuously satisfied for it. The R12-     *)
(* sched bug is that a hardware-blocked SLEEPING thread is ELE'd by the  *)
(* scheduler's deadlock check before it has a chance to wake — so the    *)
(* thread never reaches RUNNABLE, and LatencyBound never has anything to *)
(* check.                                                                  *)
(*                                                                         *)
(* HardwareWakeProgress captures exactly the bug: a non-cond-waiter       *)
(* SLEEPING thread (i.e., a thread blocked on a hardware-IRQ Rendez)     *)
(* must eventually transition out of SLEEPING. The buggy variant         *)
(* (scheduler_buggy_hwake.cfg) drops SF on Wake and produces a           *)
(* counterexample where the thread stutters in SLEEPING forever.          *)
(*                                                                         *)
(* The exclusion `t \notin waiters` matches Wake(t)'s precondition:       *)
(* cond-waiters are woken by WakeAll, not Wake, so they're outside       *)
(* HardwareWakeProgress's scope. (The spec models only ONE shared cond + *)
(* waiters; in the impl, the irq Rendez is one of many Rendez with its   *)
(* own per-Rendez cond — but the abstraction is that "in waiters" maps   *)
(* to "cond-blocked" and "not in waiters" maps to "hardware-blocked or   *)
(* other Block path".)                                                    *)
(***************************************************************************)
HardwareWakeProgress ==
    \A t \in Threads :
        [](state[t] = "SLEEPING" /\ t \notin waiters
            => <>(state[t] # "SLEEPING"))

(***************************************************************************)
(* P4-Ic6 / R12-sched: Liveness_Hwake — strong fairness on Wake(t).        *)
(*                                                                         *)
(* Extends Liveness_Wfi with SF_vars(Wake(t)) for every thread. The SF    *)
(* encodes "hardware IRQs are external events that always eventually      *)
(* deliver" — independent of any kernel scheduling decision. With SF on  *)
(* Wake, HardwareWakeProgress holds: a SLEEPING-non-cond-waiter thread    *)
(* is guaranteed to eventually leave SLEEPING.                             *)
(*                                                                         *)
(* The buggy variant uses Spec_Live_Wfi (no SF on Wake); Wake is enabled  *)
(* but can stutter indefinitely. HardwareWakeProgress fails. This is the *)
(* spec-level analogue of the impl's "ELE on no runnable peer" deadlock  *)
(* check: the system halts before hardware can deliver, so the SLEEPING  *)
(* thread starves.                                                         *)
(***************************************************************************)
Liveness_Hwake ==
    /\ Liveness_Wfi
    /\ \A t \in Threads : SF_vars(Wake(t))
