...
build_kernel() {
    ...
    # (mirrors build_go_goroot's opt-out). THYLACINE_BAKE_DOSBOX=0 opts out for
    # a fast iteration loop; an absent LLVM C++ fork skips gracefully
    # (build_dosbox_x returns 0).
    [[ "${THYLACINE_BAKE_DOSBOX:-1}" == "1" ]] && build_dosbox_x
    # DX-5/DX-8 (Cryptid): fetch + stage the game data BEFORE the pool fixture
    # (populate_stratum_pool puts the stages at /duke3d + /tombraider). Gated on
    # dosbox too: the game DATA is useless without the emulator that runs it, so
    # an emulator opt-out also skips the game data. THYLACINE_BAKE_DUKE3D=0 /
    # THYLACINE_BAKE_TOMBRAIDER=0 opt out of just one game (keep the emulator).
    ...
    # DX-8: render the DOSBox-X system default config (-> /lib/dosbox-x) from the
    # build config's CPU preset (THYLACINE_DOSBOX_CPU_PRESET). Rides the
...
EOF
    ...
    # opts out for a fast iteration loop; an absent LLVM C++ fork leaves no
    # binary, so the -f guard below stages nothing.
...
# Sound plays through Nocturne on Thylacine (N-2a-4): DOSBox-X's mixer reaches
# the SDL thylacine audio backend, so a card is detected and heard.
[sdl]
output=surface
...
populate_stratum_pool() {
    ...
    # emulator is opted out (a stale stage from an earlier build must not bake
    # past an opt-out). READ-ONLY MASTER (SYSTEM-owned): DOSBox-X opens the
    # group file READ-WRITE and a non-owner cannot open a SYSTEM-owned file
    # read-write even at 0666, so Duke3D is not run in place -- a user (and the
    # ls-gfx-dosbox-duke3d gate) copies /duke3d into a writable home and runs
    # from there (the quake per-user game-dir shape). ---
    ...
    # Staged by build_tombraider_fixture; baked iff TOMB.EXE is staged AND
    # neither the game nor the emulator is opted out. The 3dfx build drives
    # Glide -> DOSBox-X's built-in GLIDE2X.OVL -> the software-emulated Voodoo
    # (voodoo_card=software; C_OPENGL off). Read-only SYSTEM master; the gate
    # copies it to a writable home (the quake shape). ---
    ...
    # (rendered by stage_dosbox_sysconf from DOSBOX_CPU_PRESET). /lib exists by
    # now (the ndb bake above); mkdir is single-level. Rides the emulator's
    # opt-in AND the rendered file, so a stale render never bakes past an
    # opted-out emulator.
...
build_clade() {
    ...
    if [[ ! -f "$bdir/CMakeCache.txt" || "${BASH_SOURCE[0]}" -nt "$bdir/CMakeCache.txt" ]]; then
        ...
        local cmake_args=(
            ...
            # RTTI is ON for Mesa/llvmpipe (CL-7a; LLVM-DESIGN section 16.19b).
            # Upstream LLVM defaults it OFF, and that default is unusable here: with
            # RTTI off Mesa's configure refuses outright -- measured, in its own
            # words, "LLVM was built without RTTI, so Mesa must also disable RTTI"
            # -- and its suggested escape (-Dcpp_rtti=false) then breaks gallivm's
            # ORC backend on a dynamic_cast over llvm::orc types. So the ORC path,
            # which is the seam CL-7k's DualMapMemoryMapper plugs into, CANNOT be
            # built against an RTTI-less LLVM. Every distro shipping llvmpipe sets
            # this. Cost: a slightly larger clang/clangd (typeinfo emission), which
            # is why it landed WITH the CL-7a rebuild + device re-gate rather than
            # as a drive-by that would desync this recipe from the baked pool.
...
stage_clade() {
    ...
    for cxxa in libc++.a libc++abi.a libunwind.a; do
        # #156 recovery on a partial (fetch-only) clade tree: cxx-rt is a builder
        # artifact the fetch does not pull, but build_libcxx has just produced the
        # identical trio in the local stage2 sysroot -- the exact source this gap's
        # message names below. Fill from there rather than refuse, so a mac clade
        # stage (fetched llvm-build + local sysroot) is reproducible without a
        # builder round-trip.
        if [[ ! -f "$cxxrt/$cxxa" && -f "$sysroot/lib/$cxxa" ]]; then
            mkdir -p "$cxxrt"
            cp "$sysroot/lib/$cxxa" "$cxxrt/$cxxa"
            echo "    clade stage: filled cxx-rt/$cxxa from the local stage2 sysroot (#156)"
        fi
