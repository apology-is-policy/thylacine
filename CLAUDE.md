...
| I-44 | Address-space integrity under sharing + COW (`docs/LINEAGE.md`): an AddrSpace's pages live until the last referencing Proc drops it AND the last mapping is gone; a COW break yields a PRIVATE page equal to the shared page at fault time, leaves every other sharer's view UNCHANGED, and no page is writable through one mapping and executable through another. **RESERVED; ENFORCED at L-4/L-5** | **spec-first RE-ENABLED**: the model lands TLC-green BEFORE the L-4 impl (buggy cfgs: break-vs-break, break-vs-teardown, lost-VFORK-wake) + LINEAGE.md + the L-7 audit + the SMP gate |
...
**Rewritten 2026-08-16 (main#237) after measurement found EVERY operative claim here false.** The prior text described a 2026-06 charter — "owns `usr/apps/**` only", "never touches `kernel/`/`arch/`/`mm/`/`specs/`/`tools/`", "no edits to `docs/reference/*`", "never boots QEMU", plus a constitution / worklist / deliverable under `usr/apps/` — that the tree flatly contradicts: `aux-2` changes 41 `kernel/`, 17 `docs/reference/`, 12 `tools/`, 1 `specs/` and **zero** `usr/apps/` files; aux boots HVF VMs and runs its own SMP gates; `AUX-DIRECTIVE.md` and `DOC-GAP-REPORT.md` exist nowhere in either tree, and `usr/apps/AUX-ROADMAP.md` was superseded by `docs/AUX-ROADMAP.md` without this pointer following. Do not restore any of it from memory of an older session. **Note the shape, it is the recurring one:** the drift survived two months because the section reads as settled fact and nothing ever checked it — and a file-existence probe at a cited path can only ever tell you the PATH is stale, never whether the DOCUMENT still exists (asking the first question is how the roadmap was almost recorded as deleted when it had merely moved).

- **The aux track is a full second engineering track, not a userspace-apps annex.** Its charter is **`docs/AUX-ROADMAP.md`** (authoritative; the aux agent maintains it) — as of 2026-07-28 the graphics arc G-0..G-9, the Aurora environment, and VIVARIUM. It works the same surfaces main does. **Read the branch off the worktree, never off a doc**: this file said `aux/userspace-apps`, the roadmap says `gfx-4`, the worktree is on `aux-2` — three answers, and only `git -C ../thylacine-aux rev-parse --abbrev-ref HEAD` is ever current.
...
- **HOST CONTENTION IS REAL, AND IT MAY EXPLAIN NOTHING BUT WALL CLOCK.** The contended resource is **CPU CORES**, not QEMU-ness: TCG is a pure CPU emulator, an HVF guest runs real vCPU threads, and `cargo build -j` / a TLC run / a sanitizer build each saturate every core — so builds, model checks and guest boots are **ONE contention class**. This host has 8 cores; the SMP gate's `smp8` configs alone ask for 8 vCPU threads, so a concurrent build is direct core-for-core competition. The deleted clause claimed aux "never contends with the main track's QEMU boots, the host-oversubscription flake source" — false in both halves, and its parenthetical licensed, in always-loaded scripture, the exact "host load" non-explanation that §"Whole-system stewardship" forbids ~1000 lines above. **Slowness may be attributed to contention; a RED RESULT never may.** When a timing-thin gate needs a quiet host, ASK for one — that negotiation has been run cleanly (yip 0009 turns 7-11: a ~2h hold, offered and honored).
