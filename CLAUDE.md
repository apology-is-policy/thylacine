...
- **HALCYON-INSTRUMENT I-5: type and the rich document -- I-5a the substrate: three Plex cuts and two Cornucopia subsets embedded, the free-running mono face, the Italic cell face, the box-glyph precedence in the cell, the Sheet's role table, the chrome on the type map (halcyond's in-process face, I-32)** -- `usr/halcyond/src/raster.rs` (`FACE_SANS` / `FACE_SANS_MEDIUM` / `FACE_SANS_SEMIBOLD` / `FACE_MONO_ITALIC` / `FACE_MONO_TEXT`, `prop_slot`, `VENDORED_FACES`, `italic_shares_the_cell`, the box glyphs before the face), `usr/halcyond/src/lib.rs`, `usr/lib/cornucopia/src/{lib.rs,cornucopia-subset.ttf,cornucopia-subset-italic.ttf}`, `tools/subset-cornucopia.py` (`--extra`, `--match`), `usr/halcyond/src/{layout,rail,chrome,menu,main,session}.rs` ...
