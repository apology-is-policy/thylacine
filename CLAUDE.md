...
- **HALCYON-INSTRUMENT I-5: type and the rich document -- I-5a the substrate: three Plex cuts and two Cornucopia subsets embedded, the free-running mono face, the Italic cell face, the box-glyph precedence in the cell, the Sheet's role table, the chrome on the type map; I-5b the document: the profile-keyed fractional flow, the Sheet's document column, the terminal view and the code block, the position indicator, the raw grid's ground (halcyond's in-process face, I-32)** -- `usr/halcyond/src/raster.rs` (`FACE_SANS` / `FACE_SANS_MEDIUM` / `FACE_SANS_SEMIBOLD` / `FACE_MONO_ITALIC` / `FACE_MONO_TEXT`, `prop_slot`, `VENDORED_FACES`, `italic_shares_the_cell`, the box glyphs before the face, `is_instrument_sans`), `usr/halcyond/src/layout.rs` (`Flow` / `Rhythm`, the document column, `flow_box`), NEW `usr/halcyond/src/indicator.rs`, `usr/halcyond/src/lib.rs`, `usr/lib/cornucopia/src/{lib.rs,cornucopia-subset.ttf,cornucopia-subset-italic.ttf}`, `tools/subset-cornucopia.py` (`--extra`, `--match`), `usr/halcyond/src/{tile,rail,chrome,menu,main,session}.rs`, `tools/interactive/ls-halcyon-instrument.exp` ...
