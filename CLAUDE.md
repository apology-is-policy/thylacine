...
| Group termination / cross-thread shootdown (`SYS_EXIT_GROUP`) | `kernel/proc.c` (`proc_group_terminate` + `group_exiting`/`group_exit_status` on `struct Proc`; the group-status last-Thread-out ZOMBIE reap; `exits`/`thread_exit_self` group-status path), `kernel/include/thylacine/thread.h` (`die_requested` + `cpu` on `struct Thread`), `kernel/include/thylacine/proc.h` (`group_*` fields), `arch/arm64/exception.c` (die-check at the sync-from-EL0 tail + the NEW IRQ-from-lower-EL return tail -- `#713`-sensitive), `arch/arm64/vectors.S` (IRQ-from-EL0 return path), `kernel/torpor.c` (`torpor_wake_all_for_proc`), `kernel/smp.c` / `arch/arm64/gic.c` (IPI-kick a running peer via `IPI_RESCHED`), `kernel/syscall.c` (`SYS_EXIT_GROUP = 60` handler + the `kill`-cascade replacing the multi-thread refusal in `sys_postnote`), `kernel/include/thylacine/syscall.h` (`SYS_EXIT_GROUP = 60` ABI), `usr/lib/pouch/patches/0001-pouch-syscall-seam.patch` (`__NR_exit_group` 0 -> 60), `usr/lib/libt` + `usr/lib/libthyla-rs` wrappers | SYS_EXIT_GROUP (#809; pulls the documented v1.x lift forward -- #808 audit F3). **Invariant I-24** (group termination atomic + exactly-once + no-lost-wakeup; ARCH §7.9.1). Privilege/lifetime boundary AND a wait/wake surface -- prosecute hard. Flag-and-self-terminate at the EL0-return checkpoint (Plan 9 / Linux / Zircon convergent model; seL4 synchronous-stall rejected). Invariants: I-9 (sleeper-wake = register-then-observe under the rendez lock; lock order `g_proc_table_lock -> r->lock`, as `proc_become_zombie_locked` already does); #713 (the new IRQ-from-EL0 die-check sits before the DAIF-masked eret window; the die path is noreturn); #788 (`on_cpu`-spin before any peer `thread_free`); I-8 (every flagged Thread reaches its checkpoint -- IPI-kick + timer-tick floor + sleeper-wake). Closes the `exits`-with-live-peers extinction (the test.sh flake) + `kill -> -EIO in multi-thread Proc` (13b R1-F9). **v1.0 envelope** (ARCH §7.9.1 OPEN Q 7.9.A): a peer blocked indefinitely in a non-torpor/non-note kernel sleep (bare pipe read) dies at call completion, not instantly; universal-interruptible-sleep is the v1.x lift. Multi-thread **fault** path (`proc_fault_terminate`) is a tracked follow-up, NOT this chunk. **No new spec** per the 2026-05-23 spec-to-code broadening -- prose validation in ARCH §7.9.1 + this row + the audit + the runtime tests. |
