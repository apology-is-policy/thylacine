...
- **I-47 inline media: the `/srv/halcyon` place channel + the `view` decoder (the console spike; format-fuzz; docs/HALCYON.md 14.7)** -- NEW `usr/halcyond/src/placesrv.rs` (the minimal 9P server the console renderer posts) + NEW `usr/halcyond/src/inlineaccum.rs` (the PURE host-tested validate-before-alloc accumulator, `PLACE_MAX_PIXELS`) + `usr/halcyond/src/main.rs` (post + the unified-poll fds + the service/`inject_image` step) + NEW `usr/view/{src/lib.rs,src/main.rs}` (sniff + zune `decode_png` -> ARGB + `place_on_halcyon`) + NEW `usr/lib/inlinewire/src/lib.rs` (the shared place-request wire) + `usr/joey/joey.c` (renderer spawn `| MAY_POST_SERVICE`) + `tools/build.sh` (stage `view`; bake `/test.png`) ...
...
0. **Check the vault first.** Before writing or extending a `docs/reference/NN-*.md` section, run:
   ...
   **Since 2026-09-06 (operator-ratified) the answer is ALWAYS the vault** — `docs/reference` is frozen and being retired (Part A). **The vault AGENT is INACTIVE (operator-directed 2026-09-09); every agent manages its own dossiers via quaestor** — there is no separate vault track to ring; the code track that changes a surface authors the dossier itself and co-stages it. **Exit 0** — the vault carries that surface: update the owning dossier yourself (edit `vault/system/<dossier>.md`, then `vault/meta/quaestor/quaestor render`, then co-stage it with the code). **Exit 1** — no dossier yet: a **new dossier** is owed for the surface — author it yourself (`quaestor new <type> <id>` from its template, then fill it), never a new `docs/reference` section. **With several paths the answer is usually MIXED and the exit status reports only half of it** — read the summary line, which names both sets; a dossier is owed for each either way (update for the covered, create for the uncovered).
