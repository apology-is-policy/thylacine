...
- **HALCYON-SCALE: the display scale (the EDID-derived percent the compositor owns; `Metrics::at`; the gated `scale` verb + chords; halcyond follows)** -- `usr/lib/libhalcyon/src/{scale,theme}.rs`, `usr/tapestryd/src/{gpu,server,pane,chords}.rs`, `usr/lib/libtapestry/src/lib.rs`, `usr/tapestry-battery/src/main.rs` (SC-2); halcyond's `Sheet.scale` sweep (SC-3) ...
