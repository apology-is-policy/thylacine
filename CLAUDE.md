...
| I-28 | Path resolution contained at `root_spoor` + per-component X-search; mount-cross keyed by full Spoor identity; symlink expansion contained by the SAME machinery (DISTRO D-1, design 2026-08-05: absolute targets re-anchor at `root_spoor`, expanded components re-enter the gate family, follows bounded) | ARCH §9.6.7 + STALK-DESIGN prose + `docs/DISTRO.md` + tests |
