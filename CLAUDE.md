...
| I-15 | Hardware view derives entirely from DTB (documented PL011 QEMU-virt fallbacks are the argued exceptions) -- incl. each SPI's interrupt TRIGGER, read from the PCIe interrupt-map flags cell (level for PCI INTx; SPIs absent from the map default to edge, the argued fallback, ARCH 9.3.1) | code review + audit |
...
| I-25 | Legate authority scope-bounded + fully revoked on scope exit; **PROPAGATING scopes (IM-2, `IMPERIUM-DESIGN.md` §11.4)**: `rfork` children inherit exactly `legate_caps` (never HOSTOWNER, never wider than parent ∩ mask), the rfork-vs-teardown straggler closed under `g_proc_table_lock`, ONE scope per Proc set once (propagating never nests) | IDENTITY-DESIGN §3.1/§9.8 prose + tests; `specs/imperium.tla` (spec-first, IM-2) |
| I-26 | Cross-Proc kill is explicit two-axis (owner-identity OR CAP_HOSTOWNER/CAP_KILL) | IDENTITY-DESIGN §9.8 prose + tests |
| I-27 | Trusted path: SAK unspoofable; console-ATTACH (elevation gate) distinct from console-OWNER (Ctrl-C target) per `@2608c88` (the SAK revokes ATTACH + sets `owner=NULL` + attaches corvus, NEVER owns corvus); the `/dev/cons` namespace front-door gates at `devdev.open` identically to `SYS_CONSOLE_OPEN` (#57b). **MEDIUM-INDEPENDENT** (`TRUSTED-PATH.md`, 2026-06-15): generalized off serial -- same SAK+ATTACH-to-corvus rides any renderer; framebuffer = corvus cell-grid -> kernel trusted sink (sole painter, renderer suspended); graphical SAK = kernel-scanned key-combo via the MENAGERIE trusted-tier keyboard; framebuffer enforcement reserved-then-enforced (at impl); serial live (A-4c). **ENFORCED on serial at IM-1 (`IMPERIUM-DESIGN.md` §11.3)**: the trusted EPISODE -- non-attached console reads/writes FROZEN, RAW forced, the renderer feed refused, the `sak` note to corvus; ended only by `SYS_CONSOLE_EPISODE_END` or the trusted Proc's death (no kernel timeout); the serial sink = the kernel exclusivity gate + userspace rasterization (fork F1). **GRAPHICAL AS BUILT on QEMU (2026-09-21, `docs/GRAPHICAL-SAK-OWNERSHIP.md`)**: the boot-designated Lictor service alone owns GPU + keyboard; the KERNEL scans the chord (Ctrl+Alt+Delete or Ctrl+Alt+F10) in `proc_seat_op`, not any compositor; grants are held until restoration; a failed episode recovers and confers nothing; serial SAK needs the boot token `thylacine.serial-sak=1` | IDENTITY-DESIGN §9.8 + ARCH §17.1/§9.4 + `docs/TRUSTED-PATH.md` + `IMPERIUM-DESIGN.md §11.3` prose + tests |
...
| I-46 | RESERVED (2026-09-05): Nocturne, the audio system (`docs/NOCTURNE.md` §6.13; design RATIFIED 2026-09-06, §13) — audio authority = namespace + owner-or-clearance; the audio cycle never waits on an untrusted node (no-stall; substituted output; ring integrity); the cadence lease bounded + revocable. Reserved here + in ARCH §28 so the number cannot be double-allocated | planned: `nocturne_cycle.tla` + `cadence.tla` (spec-first) + the per-chunk audits |
| I-47 | RESERVED (2026-09-09): Inline media — the `view` / out-of-band pixel path (`docs/HALCYON.md` §14.7; RATIFIED 2026-09-09, native `view` inline (A) + `gallery` fullscreen (B), spike-first) — image DECODE runs in the short-lived `view`/`gallery` Proc (NEVER in halcyond — the blast-radius amendment), the raster crosses to halcyond as a BOUNDED WRITE via a per-pane control-endpoint place-request (Weft is the wrong trust direction for inline -- the sharer needs CAP_HW_CREATE; zero-copy lives on the gallery/(B) side, where the app MAPS a compositor-shared buffer like DOSBox), a placement shows a whole raster or nothing (format/dims validated + copy-before-live), a per-pane image-table quota bounds it (I-32). Composes I-1/I-7/I-32/I-40; the fullscreen `gallery` (B) rides I-40/I-45 (a Tapestry pane app), not I-47. Reserved here + in ARCH §28 so the number cannot be double-allocated | planned: prose HALCYON §14.7 + BEACON §10 reconciliation + the per-chunk format-fuzz audit + the PNG-inline spike witness (thyla-pi V3D); no spec module (inline handoff = a bounded copy) |
...
**THE VAULT AGENT IS RETIRED (operator-directed 2026-09-15). There is no separate vault track to ring.** Every agent now **owns the documentation for the surfaces it touches**: when your chunk changes a surface, you update its owning dossier **yourself**, in the same chunk, and commit it (in the vault repo at `../thylacine-vault`). For an **unowned or vault-orphaned** surface, the touching agent **takes ownership** — create or adopt the dossier and maintain it thereafter. The `quaestor owner` check and the doc-update-per-PR discipline below are UNCHANGED; only the delegation is gone. **Everywhere below that says "ring vault" now means "update it yourself"** (the vault repo is a separate git repo, so a main-repo code commit still cannot literally co-stage a vault dossier — it carries the `No-dossier-change: <why>` trailer to pass the hook, and the `<why>` names the same-chunk vault-repo update, e.g. `dossier sub-kernel-irqfwd updated in the vault repo`).
