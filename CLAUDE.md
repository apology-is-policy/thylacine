...
0. **Check the vault first.** Before documenting a changed surface, run quaestor
   ...
   **Since 2026-09-06 (operator-ratified) the technical reference IS the vault** — `docs/reference` is frozen legacy (Part A). **Exit 0** — the vault carries that surface: update the owning dossier **yourself, in this worktree, co-staged with the code** — unless the verdict reads HARNESS or OUTSIDE THE CODE CENSUS (a probe/smoke/test program, a `tools/` script, the vault's own tooling), where the coverage view owes no dossier. **Exit 1** — no dossier yet: a **new dossier** is owed — **author it yourself** under `vault/system/`, never a new `docs/reference` section. **With several paths the answer is usually MIXED and the exit status reports only half of it** — read the summary line, which names both sets; a dossier is owed for each either way (update for the covered, create for the uncovered).
