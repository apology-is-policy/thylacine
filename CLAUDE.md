...
| I-46 | RESERVED (2026-09-05): Nocturne, the audio system (`docs/NOCTURNE.md` §6.13; candidate, not yet ratified) — audio authority = namespace + owner-or-clearance; the audio cycle never waits on an untrusted node (no-stall; substituted output; ring integrity); the cadence lease bounded + revocable. Reserved here + in ARCH §28 so the number cannot be double-allocated | planned: `nocturne_cycle.tla` + `cadence.tla` (spec-first) + the per-chunk audits |
