...
| I-47 | RESERVED (2026-09-09): Inline media — the `view` / out-of-band pixel path (`docs/HALCYON.md` §14.7; RATIFIED 2026-09-09, native `view`+Weft, spike-first) — image DECODE runs in the short-lived `view` Proc (NEVER in halcyond — the blast-radius amendment to §14.7's original "decode in halcyond"), the raster crosses to halcyond by Weft (I-37) + a per-pane control-endpoint place-request, a placement shows a whole raster or nothing (format/dims validated + copy-before-live), a per-pane image-table quota bounds it (I-32). Composes I-1/I-7/I-32/I-37/I-40. Reserved here + in ARCH §28 so the number cannot be double-allocated | planned: prose HALCYON §14.7 + BEACON §10 reconciliation + the per-chunk format-fuzz audit + the PNG-inline spike witness (thyla-pi V3D); no spec module (the handoff rides `weft.tla`) |
