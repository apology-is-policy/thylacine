...
| `stratum-mkfs --populate-from` (host build infra) | (Stratum branch `thylacine-pouch-arm`) `src/cmd/stratum-mkfs/run.c` (new `--populate-from <dir>` flag); paired Thylacine side: `tools/build.sh::build_stratum_pool_fixture` (passes the boot binary corpus directory) | P6-pouch-stratumd-boot 16c -- the host-side "installer" that pre-populates `pool.img` with Thylacine's boot binary corpus (joey, corvus, stratumd, pouch-hello-*, /system.key) before QEMU boot. Equivalent in role to a Linux distro's installer copying squashfs contents onto a fresh disk, run at HOST build time instead of at runtime. Without it, joey cannot pivot to the disk-backed FS (the binaries would be unreachable post-pivot, breaking every joey spawn that resolves a `/sbin/*` or `/pouch-hello-*` path from devramfs). **Invariants**: `--populate-from` only consumes the host directory tree (read-only); it does NOT modify Thylacine kernel state or affect runtime behavior beyond the static contents of pool.img; the populated FS is a snapshot of the host directory at build time (no incremental update -- stratum-mkfs rebuilds the pool from scratch each call). v1.x runtime installer (a Thylacine `/sbin/installer` that runs in an alternate boot path) would PROVE the same flow via the runtime 9P write surface (Twrite + Tcreate + Tmkdir) -- 16c v1.0 ships the bake-at-build-time expedient. NOT audit-bearing on the Thylacine kernel surface (host build infra; the only Thylacine-side artifact is the pool.img bytes); cross-project rights-mirror discipline applies for any new RIGHT_* the populate path queries. **No new spec** -- prose validation in `docs/reference/88-pouch-stratumd-boot-16c.md` + the Stratum-side session-handoff doc + this row. |
