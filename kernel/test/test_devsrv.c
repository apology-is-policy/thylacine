...
void test_devsrv_post_listener(void) {
    ...
    int h = devsrv_post_listener(p, root, "corvus", 6, SRV_MODE_9P, false, false);
    ...
    int hb = devsrv_post_listener(p, root, "byter", 5, SRV_MODE_BYTE, false, false);
    ...
    struct Proc *u = make_test_proc();
    TEST_ASSERT(u != NULL, "unmarked proc");
    TEST_EXPECT_EQ(devsrv_post_listener(u, root, "nope", 4, SRV_MODE_9P, false, false), -1,
    ...
    walkqid_free(w);
    TEST_EXPECT_EQ(devsrv_post_listener(p, svc_ref, "x", 1, SRV_MODE_9P, false, false), -1,
