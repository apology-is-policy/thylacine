...
//     proc_alloc_in(as) gives the Proc that exact space and takes a reference,
...
void test_addrspace_proc_alloc_in_shares(void) {
    ...
    struct Proc *p = proc_alloc_in(as);
    ...
    struct Proc *q = proc_alloc_in(NULL);
    TEST_ASSERT(q != NULL, "proc_alloc_in(NULL) returned NULL");
    TEST_ASSERT(q->as != NULL, "proc_alloc_in(NULL) allocates a fresh space");
