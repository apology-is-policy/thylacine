...
//     vma_drain frees every VMA + decrements VMO mapping_count.
...
void test_vma_alloc_free_smoke(void) {
    ...
    struct Vmo *vmo = vmo_create_anon(VA_PAGE);
    TEST_ASSERT(vmo != NULL, "vmo_create_anon failed");
    ...
    TEST_EXPECT_EQ(v->vmo,         vmo,               "vmo");
    TEST_EXPECT_EQ(v->vmo_offset,  0u,                "vmo_offset");
...
void test_vma_alloc_constraints(void) {
    ...
    // Zero-length range.
    TEST_ASSERT(vma_alloc(VA_BASE, VA_BASE, VMA_PROT_RW, vmo, 0) == NULL,
        "zero-length range should be rejected");

    // Reversed range.
    TEST_ASSERT(vma_alloc(VA_BASE + VA_PAGE, VA_BASE, VMA_PROT_RW, vmo, 0) == NULL,
        "reversed range should be rejected");

    // Unaligned start.
    TEST_ASSERT(vma_alloc(VA_BASE + 1, VA_BASE + VA_PAGE, VMA_PROT_RW, vmo, 0) == NULL,
        "unaligned start should be rejected");

    // Unaligned end.
    TEST_ASSERT(vma_alloc(VA_BASE, VA_BASE + VA_PAGE + 1, VMA_PROT_RW, vmo, 0) == NULL,
...
void test_vma_insert_lookup_smoke(void) {
    ...
    struct Vmo *vmo = vmo_create_anon(VA_2MIB);
    TEST_ASSERT(vmo != NULL, "vmo_create_anon failed");
...
void test_vma_insert_sorted_invariant(void) {
    ...
    for (unsigned i = 0; i < sizeof(starts) / sizeof(starts[0]); i++) {
        ...
        struct Vma *v = vma_alloc(s, s + VA_PAGE, VMA_PROT_RW, vmo, 0);
...
void test_vma_drain_releases_all(void) {
    ...
    u64 alloc_before = vma_total_allocated();
    u64 free_before  = vma_total_freed();
    u64 mapping_before = vmo_mapping_count(vmo);
    ...
    TEST_EXPECT_EQ(vmo_mapping_count(vmo), mapping_before + N,
        "vmo mapping_count advanced by N");
    ...
    TEST_EXPECT_EQ(vmo_mapping_count(vmo), mapping_before,
        "vmo mapping_count returned to baseline");
