...
//
//   vmo.unmap_proc_smoke
//     vmo_map followed by vmo_unmap removes the VMA + decrements
//     mapping_count.
//
//   vmo.unmap_proc_no_match
//     vmo_unmap with non-matching range returns -1 without disturbing
...
void test_vmo_map_proc_smoke(void) {
    ...
    int mapping_before = vmo_mapping_count(v);

    int rc = vmo_map(p, v, TEST_VA, ONE_PAGE, VMA_PROT_RW);
    TEST_EXPECT_EQ(rc, 0, "vmo_map should succeed on a clean Proc");
    TEST_EXPECT_EQ(vmo_mapping_count(v), mapping_before + 1,
        "vmo_map should increment mapping_count via vma_alloc");
    ...
    TEST_EXPECT_EQ(vma->vmo,         v,                      "vmo backref");

    // vma_drain handles the cleanup — also exercised by proc_free below.
    vma_drain(p);
    TEST_EXPECT_EQ(vmo_mapping_count(v), mapping_before,
...
void test_vmo_map_proc_constraints(void) {
    ...
    // NULL Proc.
    TEST_EXPECT_EQ(vmo_map(NULL, v, TEST_VA, ONE_PAGE, VMA_PROT_RW), -1,
        "NULL Proc rejected");

    // NULL VMO.
    TEST_EXPECT_EQ(vmo_map(p, NULL, TEST_VA, ONE_PAGE, VMA_PROT_RW), -1,
        "NULL VMO rejected");

    // Zero length.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA, 0, VMA_PROT_RW), -1,
        "zero length rejected");

    // Unaligned vaddr.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA + 1, ONE_PAGE, VMA_PROT_RW), -1,
        "unaligned vaddr rejected");

    // Unaligned length.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA, ONE_PAGE + 1, VMA_PROT_RW), -1,
        "unaligned length rejected");

    // W+X prot.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA, ONE_PAGE,
...
void test_vmo_map_proc_overlap_rejected(void) {
    struct Proc *p = make_proc();
    TEST_ASSERT(p != NULL, "proc_alloc failed");
    struct Vmo *v = vmo_create_anon(TWO_PAGES);
    TEST_ASSERT(v != NULL, "vmo_create_anon failed");

    int mapping_before = vmo_mapping_count(v);

    // First map: succeeds.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA, ONE_PAGE, VMA_PROT_RW), 0,
        "first vmo_map should succeed");
    TEST_EXPECT_EQ(vmo_mapping_count(v), mapping_before + 1, "mapping_count = +1");

    // Adjacent (touching at boundary) — half-open ranges, NOT overlap.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA + ONE_PAGE, ONE_PAGE, VMA_PROT_RW), 0,
        "adjacent VMA accepted");
    TEST_EXPECT_EQ(vmo_mapping_count(v), mapping_before + 2, "mapping_count = +2");

    // Exact overlap with first VMA — rejected, mapping_count unchanged.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA, ONE_PAGE, VMA_PROT_RW), -1,
        "exact overlap rejected");
    TEST_EXPECT_EQ(vmo_mapping_count(v), mapping_before + 2,
        "rollback after vma_insert overlap: mapping_count UNCHANGED");

    // Partial overlap — rejected, mapping_count unchanged.
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA - ONE_PAGE, TWO_PAGES, VMA_PROT_RW), -1,
...
void test_vmo_unmap_proc_smoke(void) {
    ...
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA, ONE_PAGE, VMA_PROT_RW), 0,
        "vmo_map");
    TEST_EXPECT_EQ(vmo_mapping_count(v), mapping_before + 1, "+1 after map");

    // Exact unmap.
    TEST_EXPECT_EQ(vmo_unmap(p, TEST_VA, ONE_PAGE), 0,
        "vmo_unmap exact match should succeed");
    TEST_EXPECT_EQ(vmo_mapping_count(v), mapping_before,
        "vmo_unmap returns mapping_count to baseline");
    TEST_ASSERT(vma_lookup(p, TEST_VA) == NULL,
        "VMA gone after vmo_unmap");
...
void test_vmo_unmap_proc_no_match(void) {
    ...
    int mapping_before = vmo_mapping_count(v);
    TEST_EXPECT_EQ(vmo_map(p, v, TEST_VA, ONE_PAGE, VMA_PROT_RW), 0, "map");

    // No VMA at this address.
    TEST_EXPECT_EQ(vmo_unmap(p, TEST_VA + ONE_PAGE, ONE_PAGE), -1,
        "unmap miss returns -1");

    // Wrong start within an existing VMA's range.
    TEST_EXPECT_EQ(vmo_unmap(p, TEST_VA + 1, ONE_PAGE), -1,
        "unmap unaligned in existing VMA returns -1");

    // Wrong length on an existing VMA.
    TEST_EXPECT_EQ(vmo_unmap(p, TEST_VA, TWO_PAGES), -1,
