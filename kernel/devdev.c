...
static int devdev_stat_native(struct Spoor *c, struct t_stat *out) {
    ...
    case DEV_KIND_WARP:
        ...
        return 0;
    case DEV_KIND_CONS:
        // The cons leaf delegates to the ONE is-a-cons fill (#55), so this
        // door and devcons cannot drift: same mode/uid/gid AND the same
        // CONS_STAT_QID_FLAG marker, which is what makes isatty() true here.
        // (It re-zeroes `out` itself; the fill above is harmless.)
        return cons_stat_native_fill(c, out);
    case DEV_KIND_CONSCTL:
        // consctl is a character device but NOT a console: it is the control
        // file. It deliberately does NOT get cons_stat_native_fill, because
        // that stamps the is-a-cons qid flag and a program would then read
        // consctl as a tty.
        out->qid_type = QTFILE;
...
struct Dev devdev = {
    ...
    .walk     = devdev_walk,
    .stat     = devdev_stat,
    .stat_native = devdev_stat_native,   // #55 is-a-cons contract (cons leaf)
                                         // + CL-4 F2: every other leaf too, so
                                         // `clang++ < /dev/null` can fstat it
