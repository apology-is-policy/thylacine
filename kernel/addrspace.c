...
struct AddrSpace *addrspace_clone(struct AddrSpace *src, bool exempt) {
    ...
    // I-32 (A): a cloned space inherits the SOURCE space's cap. fork gives
    // the child a copy of the parent's address space, so it must give it the
    // same bound -- deriving it from the child Proc instead would let a fork
    // silently widen or narrow what the parent was actually running under.
    struct AddrSpace *dst = addrspace_alloc(src->page_budget);
    if (!dst) return NULL;
...
bool addrspace_charge_pages(struct AddrSpace *as, u32 npages, bool exempt) {
    ...
    for (;;) {
        ...
        if (__atomic_compare_exchange_n(&as->page_count, &cur, cur + npages,
            ...
            // Telemetry only; monotonic non-decreasing. A racing peer may make
            // this lag by one charge, which is acceptable for a high-water mark
            // and is why it is not part of the CAS above.
            u32 want = cur + npages;
            u32 peak = __atomic_load_n(&as->page_peak, __ATOMIC_RELAXED);
            while (want > peak &&
                   !__atomic_compare_exchange_n(&as->page_peak, &peak, want,
