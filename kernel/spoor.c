...
//
// The wrap after 2^32 attaches stays benign, but for a DIFFERENT reason than
// before: a collision can only ever ADD MNOEXEC coverage (a queried file
// matches some other MNOEXEC entry's source and is refused), never remove it,
// so it is fail-closed over-restriction rather than a bypass. A Proc cannot
// choose or influence its devno either -- the counter is kernel-internal and
// monotonic.
