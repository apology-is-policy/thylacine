...
//
// Run npxf's own two halves against each other over a socketpair and check the
// claim from the outside: the keys must cross, and a record must actually
// travel client -> server under the key the fixture calls c2s. The ephemerals
// are random here, which is exactly why this is a check and not a vector.
static void check_direction_binding(std::span<const uint8_t> token) {
    int sv[2];
    ...
    srv.join();
    if (!server_err.empty()) die(server_err.c_str());

    if (ckeys.send != skeys.recv || ckeys.recv != skeys.send)
        die("the two halves disagree on direction: client.send != server.recv");
...
int main() {
    ...
    check_direction_binding(byte_span(std::string_view(token)));
