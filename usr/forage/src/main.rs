...
//
// THE SECURE CHANNEL (`-t` / `--token-env`). With a token, forage runs npxf's
...
/// synchronously inside the syscall). A forage that is merely slow and a forage
...
macro_rules! step {
    ($($a:tt)*) => {{
        if VERBOSE.load(Ordering::Relaxed) != 0 {
            let mut s = alloc::string::String::from("forage: .. ");
...
/// writer. Exiting is correct rather than merely convenient: forage IS the
    ...
    while read_frame(ctx.src, &mut msg) {
        let ok = match ctx.sealer.as_mut() {
            ...
            Some(s) => match s.seal(&msg, &mut rec) {
                ...
                Err(e) => {
                    say!("forage: sealing a {}-byte message failed ({:?})", msg.len(), e);
                    false
                }
            },
        };
...
fn read_record(src: i64, o: &mut npxf::Opener, buf: &mut Vec<u8>) -> RecordIn {
    ...
    let body = match o.body_len(&hdr) {
        ...
        Err(e) => {
            say!(
                "forage: refusing a record claiming {} bytes ({:?})",
                u32::from_le_bytes(hdr),
                e
            );
            return RecordIn::Ended;
    ...
    let n = match o.open(&hdr, buf) {
        ...
        Err(e) => {
            ...
            say!("forage: record authentication failed ({:?}) -- the channel is over", e);
            return RecordIn::Ended;
        }
    };

    if n < MSG_MIN as usize || u32::from_le_bytes([buf[0], buf[1], buf[2], buf[3]]) as usize != n {
        say!("forage: a record's 9P size field disagrees with the record length");
        return RecordIn::Ended;
    }
    RecordIn::Message(n)
}
...
const USAGE: &str = "\
usage: forage [-a aname] [-t file | --token-env VAR] host!port mountpoint [cmd ...]
  Mount a remote 9P2000.L tree served over TCP.

  With a command, forage mounts, runs it WITH THE TREE VISIBLE, and exits with
  ...
  so a backgrounded `forage ... &` is NOT visible to the shell that started it.
...
Example:
  forage -t /cfg/npxf.token '10.0.2.100!7820' /n/host
";
...
fn run(argv: Args) -> Result<(), &'static str> {
    ...
    let (host, port) = forage::addr::split_dial(&args.addr)
    ...
    let port = forage::addr::parse_port(port).ok_or("the port must be 1..=65535")?;
    ...
    // `forage ... &` from a shell mounts into forage's OWN namespace and the
    // shell sees nothing; measured, on the first working channel:
    //   cat: /home/cora/host/hello.txt: no such file or namespace entry
    // while forage sat happily mounted. This is the Plan 9 answer -- rfork,
    ...
    // chunk and a design question, recorded in docs/FORAGE-DESIGN.md.
