...
fn run(args: Args) -> i64 {
    let (p1, p2) = match (args.get_str(1), args.get_str(2)) {
        (Some(a), Some(b)) => (a, b),
        _ => {
            aux_rt::eprintln!("cmp: missing operand");
            return 2;
        }
    };

    let d1 = match File::open(p1).and_then(|mut f| aux_rt::slurp(&mut f)) {
        Ok(d) => d,
        Err(e) => {
            aux_rt::eprintln!("cmp: {}: {}", p1, e);
            return 2;
        }
    };
    let d2 = match File::open(p2).and_then(|mut f| aux_rt::slurp(&mut f)) {
        Ok(d) => d,
        Err(e) => {
            aux_rt::eprintln!("cmp: {}: {}", p2, e);
            return 2;
        }
    };
    ...
    for i in 0..min {
        if d1[i] != d2[i] {
            aux_rt::println!("{} {} differ: byte {}, line {}", p1, p2, i + 1, line);
            return 1;
        }
        if d1[i] == b'\n' {
            line += 1;
    ...
    if d1.len() != d2.len() {
        ...
        aux_rt::eprintln!("cmp: EOF on {}", shorter);
        return 1;
    }
    0
}
