pub use crate::gpu_api::{HostRing, FenceVindication, FenceTag, FencedErr};
// The virtio-gpu backend shared during the Lictor migration (Tapestry G-3; TAPESTRY.md
...
use libdriver::Error;
use crate::skein::Seg;   // WEAVE-SKEIN: the scatter-gather backing entry
...
const _: () = assert!(
    PROBE_CTX_ID_VIRGL > crate::limits::MAX_WARP_CTXS as u32
        && PROBE_CTX_ID_VENUS < crate::limits::COMPOSITOR_CTX
...
const _: () = assert!(
    BLOB_PROBE_RES_ID < crate::limits::SCREEN_RES,
...
const _: () = assert!(
    HOST3D_PROBE_RES_CTX < crate::limits::SCREEN_RES
        && HOST3D_PROBE_RES_GLOBAL < crate::limits::SCREEN_RES
        ...
        && HOST3D_PROBE_CTX_ID > crate::limits::MAX_WARP_CTXS as u32
        && HOST3D_PROBE_CTX_ID < crate::limits::COMPOSITOR_CTX
...
const _: () = assert!(
    HOSTMEM_PROBE_RES < crate::limits::SCREEN_RES
        && HOSTMEM_PROBE_RES_2 < crate::limits::SCREEN_RES
        ...
        && HOSTMEM_PROBE_CTX_ID > crate::limits::MAX_WARP_CTXS as u32
        && HOSTMEM_PROBE_CTX_ID < crate::limits::COMPOSITOR_CTX
...
const _: () = {
    ...
    assert!(FREQ_LEN >= (crate::limits::SRV_MSIZE as u64 - 23) + 8 + GPU_CTRL_HDR_LEN as u64);
};
...
impl Controlq {
    ...
    fn verify_sync_fence(&mut self, offset: u64, expected: u64) -> Result<(), ()> {
        let at = self.ring_va + offset;
        let flags = unsafe { r32(at + 4) };
        let id = unsafe { r32(at + 8) as u64 | ((r32(at + 12) as u64) << 32) };
        if flags & VIRTIO_GPU_FLAG_FENCE == 0 || id != expected {
            self.dead = true;
            say!("lictor: GPU synchronous completion fence mismatch");
            return Err(());
        }
        Ok(())
    }
    ...
    fn submit_and_wait(&mut self, req_len: u32, resp_len: u32) -> Result<u32, ()> {
        ...
        virtio_rmb();
        self.verify_sync_fence(RESP_OFF, fence)?;
    ...
    fn submit_pair_and_wait(
        ...
        virtio_rmb();
        self.verify_sync_fence(RESP_OFF, fence1)?;
        self.verify_sync_fence(RESP2_OFF, fence2)?;
...
pub struct Gpu {
    ...
    condemned_n: usize,
    collect_retirements: bool,
    retired_resources: alloc::vec::Vec<u32>,
...
impl Gpu {
    ...
    pub fn probe(
        ...
        let mut gpu = Gpu {
            ...
            condemned_n: 0,
            collect_retirements: false,
            retired_resources: alloc::vec::Vec::new(),
    ...
    fn resource_unref_raw(&mut self, resource_id: u32) -> Result<(), Error> {
        if self.collect_retirements && self.retired_resources.len() >= crate::objects::MAX_OBJECTS {
            return Err(Error::Hardware);
        }
        ...
        let result = self.ctrl.step(
            "RESOURCE_UNREF",
            GPU_CTRL_HDR_LEN + 8,
            GPU_CTRL_HDR_LEN,
            VIRTIO_GPU_RESP_OK_NODATA,
        );
        if result.is_ok() && self.collect_retirements {
            self.retired_resources.push(resource_id);
        }
        result
    }
    ...
    pub fn mint_host3d_ring(
        ...
        let map_info = match self.map_blob(res_id, off) {
            Ok(mi) => mi,
            Err(e) => {
                // MAP may have reached the host before its reply was lost.
                // Reuse only after explicit unmap AND unref acknowledgements.
                if self.unmap_blob(res_id).is_ok() && self.resource_unref_raw(res_id).is_ok() {
                    self.hostmem_free(off, size);
                }
                return Err(e);
            }
    ...
    /// mapping_count. Takes the handle by value: it cannot be retired twice.
    /// A refused step retains the reserved extent; only full acknowledged
    /// teardown returns it to the allocator.
    pub fn drop_host3d_ring(&mut self, ring: HostRing) {
        // Every step is a prerequisite of address reuse. An uncertain device
        // completion deliberately leaks this reserved extent until service
        // restart/reset; logging an error is not permission to alias it.
        let rc = unsafe { t_burrow_detach(ring.va, ring.size) };
        if rc < 0 {
            say!("lictor: hostmem detach refused {}; extent remains reserved", rc);
            return;
        }
        if self.unmap_blob(ring.res_id).is_err() {
            say!("lictor: hostmem unmap refused; extent remains reserved");
            return;
        }
        // Unlike resource_unref(), the raw operation cannot return success
        // for deferred scanout retirement. Host rings are never scanout targets.
        if self.resource_unref_raw(ring.res_id).is_err() {
            say!("lictor: hostmem unref refused; extent remains reserved");
            return;
        }
    ...
    /// The broker enables this after boot probes, then releases imported DMA
    /// only for IDs returned here. A deferred unref never publishes retirement.
    pub fn enable_retirement_journal(&mut self) { self.collect_retirements = true; }
    pub fn take_retired_resources(&mut self) -> alloc::vec::Vec<u32> {
        core::mem::take(&mut self.retired_resources)
    }

    /// Device completion proof for trusted takeover and backing retirement.
    ...
    pub fn exclude_all_outputs(&mut self) -> Result<(), Error> {
        ...
        if len < 12 { return Err(Error::Hardware); }
        ...
        for output in 0..outputs {
            ...
            unsafe {
                ...
                write_rect(req + 24, 0, 0, 0, 0);
                w32(req + 40, output);
                w32(req + 44, 0);
            }
            self.ctrl.step("TRUSTED_EXCLUDE_OUTPUT", GPU_CTRL_HDR_LEN + 24,
                GPU_CTRL_HDR_LEN, VIRTIO_GPU_RESP_OK_NODATA)?;
        }
        Ok(())
    }
