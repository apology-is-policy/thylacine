...
pub mod effects {
    ...
    /// A MENU's drop shadow -- the tile verb menu, the theme picker, the
    /// workspace list: the kit's `.theme-menu`, black .32 at (0, 20), blur
    /// 55. A menu carries nothing else (section 10, revised 2026-09-16).
    pub const MENU_SHADOW: Argb = 0xFF00_0000;
    pub const MENU_SHADOW_ALPHA: u8 = pct256(320);
    pub const MENU_SHADOW_DY: i32 = 20;
    pub const MENU_SHADOW_BLUR: i32 = 55;

    /// A DIALOG's drop shadow -- the 14.5 confirmations and the keyboard
    /// reference: the kit's `.help-card`, black .35 at (0, 24), blur 80,
    /// over the backdrop below.
    ///
    /// These two were once collapsed into one (the dialog's), because the
    /// compositor could not tell the cards apart: `menu place` took only
    /// coordinates. The placement now carries the class, so each card takes
    /// the kit's own literal again.
    pub const DIALOG_SHADOW: Argb = 0xFF00_0000;
    pub const DIALOG_SHADOW_ALPHA: u8 = pct256(350);
    pub const DIALOG_SHADOW_DY: i32 = 24;
    pub const DIALOG_SHADOW_BLUR: i32 = 80;

    /// A dialog's backdrop: `rgb(3,4,4)` at .72 over a 3 px blur of the
    /// scene, across the whole display (the kit's `dialog::backdrop`). A
    /// menu has none.
...
mod tests {
    ...
    fn the_effect_literals_are_not_theme_tokens() {
        ...
        // The two card shadows are the kit's two, not one (section 10 as
        // revised 2026-09-16: the placement carries the class again).
        assert_eq!(effects::MENU_SHADOW_ALPHA, 82, ".32 -- the theme menu's");
        assert_eq!(effects::MENU_SHADOW_DY, 20);
        assert_eq!(effects::MENU_SHADOW_BLUR, 55, "clamped to GLOW_RADIUS_MAX at paint");
        assert_eq!(effects::DIALOG_SHADOW_ALPHA, 90, ".35 -- the help card's");
        assert_eq!(effects::DIALOG_SHADOW_DY, 24);
        assert_eq!(effects::DIALOG_SHADOW_BLUR, 80, "clamped to GLOW_RADIUS_MAX at paint");
        assert_eq!(effects::MENU_SHADOW, 0xFF00_0000, "black");
        assert_eq!(effects::DIALOG_SHADOW, 0xFF00_0000, "black");
