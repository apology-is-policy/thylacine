...
| 2026-09-09, I-47 inline media slice 2 (the `view` decoder) | IN: `zune-png` 0.4.10, `zune-core` 0.4.12, `zune-inflate` 0.2.54 (each MIT / Apache-2.0 / Zlib) + `simd-adler32` 0.3.10 (MIT; zune-inflate's Adler32). The image-decode closure for the inline `view` (HALCYON.md 14.7): pure-Rust PNG decode, chosen over a ported C codec for the format-fuzz posture (H-7). All four build `no_std` on `aarch64-unknown-none` (`zune-*` `default-features = false` drops their `std`), verified at vendor time by the `view` guest build. | 143 |
