...
//
// Returns the epoch in seconds, or 0 when no plausible wall clock is
// readable (no DTB node + an implausible fallback read, or a map failure).
// 0 is the FAIL-SOFT signal: the caller anchors CLOCK_REALTIME to the epoch
// so a 0 yields "1970 + uptime" rather than a fabricated time, and the boot
// path never extincts on a missing/odd RTC. CLOCK_MONOTONIC is unaffected.
//
