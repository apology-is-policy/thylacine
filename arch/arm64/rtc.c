...
// Plausibility floor: 2020-01-01T00:00:00Z. A real PL031 returns the host's
// Unix time (well past this); a dead/absent device reads 0 or garbage. Any
// value below the floor is treated as "no wall clock" -> fail-soft to 0, so
// CLOCK_REALTIME reads 1970 + uptime (the honest signal) instead of a
// fabricated plausible-but-wrong time.
#define RTC_EPOCH_FLOOR 1577836800ull
...
u64 rtc_read_epoch_seconds(void) {
    ...
    if ((u64)secs < RTC_EPOCH_FLOOR) return 0;   // implausible -> fail-soft
    return (u64)secs;
}
