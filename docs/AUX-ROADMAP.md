...
| **DOSBox (DX-0..DX-5) -- the DOS-emulation arc** | **DESIGN (scoped 2026-09-03; `docs/DOSBOX.md`).** Port DOSBox Staging via Pouch (C++/musl + the proven `SDL_thylacine`->Tapestry backend + libc++) to open the DOS library -- "TyrQuake in C++"; a flagship of the emulation story (beside VIVARIUM + the planned x86-xlat / Wine). Sound STUBBED (v1.0 non-goal). The novel part: the dynamic recompiler emits through **CAP_JIT (I-42)** -- reviewed a CLEAN fit for the as-built dual-map surface with NO kernel change (DOSBox is same-thread emit/execute + software-SMC, dodging the only designed-but-unbuilt JIT caveats; ORC `DualMapMemoryMapper` is the C++ template); `core=normal` interpreter is the always-works first-light floor. Sequence: DX-1 vendor+C++ build -> DX-2 FIRST LIGHT (`Z:\` in a Tapestry pane + `ls-gfx-dosbox` gate) -> DX-3 sound/input polish -> DX-4 CAP_JIT dynarec (AUDIT-BEARING) -> DX-5 a real DOS game + close. |
