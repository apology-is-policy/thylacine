...
| **Nocturne (N-0..N-7) -- the audio system (operator-named 2026-09-05)** | **N-0 LANDED (2026-09-05): the research pass + design CANDIDATE `docs/NOCTURNE.md` (heritage: Plan 9 audio(3) + 9front mixfs; SOTA: PipeWire/PulseAudio/JACK2/CoreAudio/WASAPI; peers: Fuchsia effects + Zircon deadline profiles, Genode 24.02 play/record inversion, seL4 sDDF sound; substrate: virtio-snd + QEMU 10.0.x on both hosts, the Pi 400/500 HDMI-only facts). Candidate: a file-server audio graph (`/dev/nocturne`), client nodes over Weft-shared rings, program-supplied DSP nodes (descants) isolated in their own Proc, a never-wait cycle with per-node substitution, policy as data, and ONE kernel lift (the cadence lease, N-4, spec-first). I-46 RESERVED. **AWAITING OPERATOR RATIFICATION** (section 9 decisions + section 10 questions + the sub-names). NEXT after ratification = N-1 (virtio-snd driver + the `wav` capture witness). |
