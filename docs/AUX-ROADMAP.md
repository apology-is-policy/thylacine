...
| **X-8** | **`tar -z` (compressed) is still blocked: `tar: vfork: Function not implemented`.** busybox spawns `gzip` for `-z`, so `.tar.gz` -- the format every real download uses -- does not work even though `tar` itself now does. Uncompressed `tar` is proven end to end (`tools/interactive/abin-tar.exp`). Sequence after N-3 (`CLONE_THREAD`), whose clone-flag plumbing this shares. | **W2-a is NOT closed without it** |
