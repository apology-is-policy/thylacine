...
| **Inline media / `view` (IM-view) — images in the scrollback** | **DESIGN RATIFIED (2026-09-09, operator vote): native `view` + Weft, spike-first; reserved I-47.** Scripture landed (HALCYON section 14.7 mechanism + BEACON section 10 reconciliation + ARCH/CLAUDE section 28 I-47). `view test.png` -> a letterboxed PNG inline in the transcript, reflow-on-resize; DECODE runs in the short-lived native `view` (zune-png/jpeg, no_std -- NOT halcyond, the blast-radius amendment), the RGBA raster crossing to halcyond by Weft + a per-pane control-endpoint place-request; a new `transcript::Item::Image` -> the already-built + tested `cartoon::Op::Image` compositor; the obj-verb is a one-line verbs rule; `--fullscreen` = whole-content placement. Video/audio later = split-by-medium (a sandboxed ported codec on the Embed surface; audio -> Nocturne). **Research (2026-09-09): 3-agent fan-out (Halcyon feasibility + native/ported boundary + external SOTA) found the compositor floor already exists + is unit-tested, the channel is the ONLY new IPC, and there is NO production-grade pure-Rust H.264/HEVC/VP9 decoder as of 2026-09 (so "video later" forces ported C).** NEXT = build-verify the eb26e8b8 main-merge, then the PNG-inline spike (channel + Item::Image + view + reflow) proven on thyla-pi V3D, then expand (JPEG + verb + fullscreen + DoS quota + audit). |
