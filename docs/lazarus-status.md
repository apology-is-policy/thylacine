...
- **W2 — the GICv2 driver** (audit-bearing; scheduler/IPI, I-18). `PORTABILITY.md
  ...
  `kernel/smp.c`. The HVF-on-Apple enabler (no `ICC_*` sysregs) + what the Pi's
  GIC-400 needs. HVF-MMIO `ISV=0` timing mitigation (a `dsb` between consecutive
  GIC MMIO) validated empirically when it lands. Add the audit-trigger row.
