...
A kernel **extinction** (ELE — Extinction Level Event; the thematic name for kernel panic) prints `EXTINCTION: <message>` as a recognizable prefix. Use `extinction(msg)` or `extinction_with_addr(msg, addr)` from `kernel/extinction.c`; `ASSERT_OR_DIE(expr, msg)` for assert-style checks. The prefix is part of the kernel ABI with the development tooling, and so is each message body a tool matches (today the six that `tools/test-fault.sh` checks; the vault note `abi-boot-banner` keeps the set). Changing one of those is a **format break** (§"Autonomy + escalation"): surface it, do not just sweep. Any other body is prose and may be reworded, for example to follow a program that moves (operator vote 2026-09-25, `dec-2026-09-25-extinction-bodies`); a body that gains a consumer joins the set.
