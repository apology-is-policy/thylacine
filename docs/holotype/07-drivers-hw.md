...
**Fix**: `proc_quiesce_owned_devices` walks the dying Proc's `KObj_MMIO` handles and resets every probed virtio transport in each claimed range (via the kernel `.base` mapping, independent of the user mapping `vma_drain` tears down), BEFORE the `KObj_DMA` pages free, at BOTH proc-exit close sites (`proc_close_handles_at_exit` single-thread; `proc_free` multi-thread reap + orphan/rollback). *(Since #68 a multi-thread Proc's last live Thread closes its handles in `proc_close_handles_at_exit` too, so `proc_free` covers only the orphan/rollback paths.)* The structural close is a per-device `KObj_VIRTIO_DEV` device-session model (R3-F8, Phase 5+). Regression `virtio.proc_death_quiesces_device` (+ `reset_in_range_no_match`, `vq_size_for`).
