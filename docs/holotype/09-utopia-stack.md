...
| HT09.R4-F2 | coreutils | S | **P2** | tail/sort/cut/grep/uniq/wc/cmp `.rs` | Seven utils slurp whole inputs into the 4 MiB fixed heap -> silent OOM-abort (exit 1, no message) on real /sbin binaries; cmp's OOM exit-1 collides with its "differ" verdict (a silently-wrong result) | **FIXED** `@dcc4d08` -- `io::slurp` now capped at `SLURP_CAP` (2 MiB, chunked via `slurp_capped`) -> a graceful `NoMemory` error (which every caller already surfaces) instead of an OOM-abort, fixing all seven at the shared helper. Streaming the seven is the deferred proper fix. B-1c (2026-09-24) lifted the cap with the fixed heap it was sized to, and streamed the six that do not need the whole input (`coreutils::stream`, host-tested at every read boundary): cmp in two buffers; grep, cut and uniq a line at a time; wc a buffer at a time; tail a window of its answer. Their input has no size limit, and a line is bounded at `LINE_MAX` (64 MiB; B-1c round 1's F1: a line with no end grew to the fault and read as "no match"): a longer line is refused, grep exiting 2 and cut, uniq and tail 1. `sort` alone still holds its input: past free memory it ends at a fault, v1.0's exit status 1 (docs/ERRORS.md), which is its own error status (no verdict collides), with the cause on the kernel's uart line rather than stderr; the v1.x exit-status lift lets a caller tell a fault from an error. The INPUT-driven half is CLOSED (streaming + `LINE_MAX` + round 2's F1: nothing built from a line may outgrow it -- grep's list of a line's matches and cut's of its fields were 16 B an entry, up to 1-2 GiB for one line at `LINE_MAX`, so both now hand each on as it is found, `coreutils::find` / `coreutils::select`, host-measured to allocate nothing; and cat's line mode, which held a whole line, streams through `CatLines`); the PRESSURE-driven half stays **OPEN for the operator**: with no victim selection, another program exhausting the pool can end grep or cmp at any page, which reads as "no match" or "differ" (the exit-status ABI lift, or a commit check; OPEN-BUGS). |
...
- **R3-F6** [P3]: history has an entry cap but no byte cap -> a large paste OOMed the shell (4 MiB heap) into logout. Since B-1c the heap grows, so the history grows into the user pool instead, with no bound of its own (one of the unbounded consumers owed a per-consumer cap or OOM victim selection).
...
- **R4-F7..F17** [P3]: head/tail `-N` overflow->10 + `-nN` form (tail's FIXED at B-1c: an overflowing count is refused and `-n +N` counts from the start; head's stands); `-` stdin claimed-not-implemented (head and tail read it since B-1c); rm `.`/`..`/`/` refusal + per-child error continuation; cp depth note + `cp -r ..`; cut `-d` empty/multibyte; tr empty-SET2/reversed-range; sort -n i64 + -nu; ls exit-status gaps; uniq 2nd operand silently ignored; syscall-per-fragment output. (See the R4 report for the full list.)
