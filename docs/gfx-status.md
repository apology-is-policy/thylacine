...
the substrate. G-3 is CLOSED; G-4a/b/c are LANDED (the fbcon is live). Next: the G-4 close (gates + audit)
(Aurora renderer MVP).
...
| 9a9b6d29 | G-4c | `usr/aurora` -- the renderer MVP: VT subset (truecolor SGR, real alt-screen, DECSTBM-ignored seam) + Bonfire palette + atlas blit + procedural U+2500-259F + blinking cursor; the loop BLOCKS on the Loom CQ (the non-SQPOLL pump -- a poll-only loop starves its own completions, measured); joey spawns it perm-granted as the resident boot presenter (demo stays baked, no longer spawns); run-vm binds kbd-pci0 `display=gpu0` (targeted QMP injection) | first integrated boot: "Thylacine login:" in Cornucopia at 128x36 + blink + QMP-typed "xyz" echoed on BOTH sinks |
