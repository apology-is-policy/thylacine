...
and surfaced four real bugs, all fixed same-day: #31 (controlq live-display
desync) + #30 (cora's home / the full SrvRegistry) + #35 (the blend lane
bug) + #36 (bare-LF session output). Interactive graphical mode is UP:
`THYLACINE_DISPLAY=cocoa tools/run-vm.sh`. Next: G-5 (the graphics audit
round; the #31 controlq rework + the gate's saturated-color blindness join
its scope).
