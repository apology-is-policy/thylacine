...
| &nbsp;&nbsp;**N-2a-2** the SDL audio backend | `usr/ports/sdl2/thylacine/SDL_thylacineaudio.c` (a voice via a direct `/srv/nocturne` conn for per-exit lifetime); patch 0004 + `-nosound` retired; DOSBox/Quake sound | W-4 (DOSBox/Quake audio arm), LS-CI | a boundary-line port: routine unless max |
