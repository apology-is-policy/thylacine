...
| &nbsp;&nbsp;**N-2b-2b** the back-pressure wake poke + edge cases + the N-2b FORMAL AUDIT | the producer->consumer poke on a stopped stream (`ready_signal` or `torpor::wake_one`, removing the 100 ms `IDLE_POLL_MS` latency); the batched N-2b holotype audit (N-2b-1 + N-2b-2a; I-37 cross-Proc DATA race, memory ordering, teardown-under-inflight); lays the descant substrate | the audit close (0 P0/P1) + a poke-latency measurement | ring back-pressure: **ASK unless max** |
