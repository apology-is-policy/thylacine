...
`SDL_Init` did not `E_Exit` (0004 stood until N-2a-4, RETIRED 2026-09-06 -- DOSBox-X's SDL now auto-selects the thylacine audio backend when `/srv/nocturne` exists); TyrQuake ran `-nosound` until N-2a-3 and carries a guard
...
/dev/nocturne/
  ...
  audio            write: the Plan 9 audio(3) playback file -- a write is an s16le stereo voice at the graph rate
                   (the `#A`-compatible default). READ is REFUSED here (N-3c-1): a sink tap carries eavesdropping
                   authority and a shared mount cannot carry per-writer identity (the N-3a-2 F1 lesson), so recording
                   is the gated `tap` on /srv/nocturne-ctl (6.8), never this file. `bind /dev/nocturne/audio
                   /dev/audio` serves PLAYBACK.
  nodes/
...
- **`/srv/nocturne-ctl`** -- the **sink-authority** surface, reached by a
  ...
  writable `volume` (N-3a-3) and the gated sink `tap` (N-3c-1 -- a read is an ear
  on the mixed sink output: single-reader [EBUSY on a second concurrent open, a
...
- **Read a tap** (an ear on the sink, or on another program's voice): the owner
  of what is tapped, or the clearance — recording is eavesdropping otherwise.
  The **sink tap** is realized (N-3c-1) as the gated `/srv/nocturne-ctl/tap`
  file (the sink-authority gate below), NOT as a `/dev/nocturne/audio` read:
  9front's loopback is an ungated `/dev/audio` read, but a shared mount cannot
  carry per-writer identity (the N-3a-2 F1 lesson), so the mount `audio` read is
  refused and the tap is reached over the caller's own `-ctl` connection (ours is
  a security decision — and it must therefore be an authenticated one). An ear on
  another program's *voice* (owner-or-clearance) is the `ear` node kind, deferred
  with the full node model (N-4).
...
**The sink tap reads through the identical gate (N-3c-1).** Opening
`/srv/nocturne-ctl/tap` runs the same four-axis check (`sink_authorized`, the
N-3a-3 gate — renamed from `volume_authorized` because it now gates both the
volume write and the tap read), and because a recording is long-lived while caps
and the console owner mutate, every `tap` read re-checks the peer FRESH and fails
closed (EPERM) the instant authority is lost — so a recording started by the
person at the keyboard stops when they cease to own the console, and one riding a
...
`sink_authorized`, and the `SYS_SRV_PEER` `SRV_PEER_FLAG_CONSOLE_OWNER`
...
on the mounted playback post); **the gated sink `tap` (N-3c-1): the mixed-output
mirror + the single-reader guard + the parked tap read, and the per-READ fresh
...
| &nbsp;&nbsp;**N-2b-1** the Weft ring SUBSTRATE + map witness -- **LANDED 2026-09-06 (aux-3)** | `nodes/<id>/data` + `h_weft` (the Tweft responder, netd `weft_ensure` template) allocates + shares an ANON ring (weftlib layout, K=8 period slots) and the client `SYS_WEFT_MAP`s it THROUGH the mount; owner-gated (the F1 gate) + the kernel's consume-once claim (SPSC producer-uniqueness); `RingVoice` Drop teardown. USERSPACE-only -- the kernel Weft mechanism is reused unchanged, so `weft.tla` is not in play. `/ring-voice-probe` maps + validates the geometry + runs 2 controls (idempotent re-map; voice-0 map refused) | `tools/test-ring-voice.sh` GREEN (`joey: ring-voice-probe OK`; mapped ring K=8 payload=20160) | ran at max; the formal holotype audit is batched to the N-2b close |
| &nbsp;&nbsp;**N-2b-2a** the ring PERIOD protocol + the ring-fed mixer -- **LANDED 2026-09-06 (aux-3)** | fixed-slot SPSC over the ring (`weft::slot_produce`/`slot_consume`/`slot_pending`, K=8 period slots, CONSUMER-computed offsets, the I-30 len snapshot; Release/Acquire on `prod_tail`/`cons_head`); `next_period` consumes ONE finished slot per ring voice into the float32 mix; `has_playable` starts+holds the stream on ring data (100 ms idle poll -- the poke is 2b); `t_yield` back-pressure with a stall backstop | `tools/test-ring-audio.sh` GREEN (`PASS(chord): 70 windows carry 1000+2000 Hz at once`, ring path, `dropped=0`); test-ring-voice + test-audio (byte) regressions GREEN | ran at max; the formal holotype audit is batched to the N-2b close |
| &nbsp;&nbsp;**N-2b AUDIT** the batched holotype (N-2b-1 + N-2b-2a) -- **CLOSED CLEAN 2026-09-06 (aux-3)** | Opus fallback (Fable out of credits); the prosecutor RE-DERIVED the cross-Proc SPSC memory ordering, the consumer's bounds against a hostile ring, the #847 dual-count lifetime + the consume-once SPSC backstop -- all sound | 0 P0 / 0 P1 / 0 P2, 3 P3 fixed (probe control-(b) attribution; time-based stall bound; "sole producer thread" note; a compile-time K*PERIOD_BYTES assert) | [[audit-nocturne-closed-list]] round 3 |
| &nbsp;&nbsp;**N-2b-2b** the back-pressure wake POKE -- **v1.x (needs a kernel primitive)** | the producer->consumer wake on a full ring / a stopped stream. RESEARCH FINDING: `torpor` is per-Proc-keyed (`hash(Proc*, user_va)`), so it CANNOT do the cross-Proc consumer->producer wake; the weft ready-ring's park leg is validated-not-wired. A real cross-Proc park+wake needs a NEW kernel primitive (a physical-page-keyed futex OR the weft park leg wired). The v1.0 answer is N-2b-2a's `t_yield` busy-poll (realtime-paced, correct) + the 100 ms `IDLE_POLL_MS` start poll | a poke-latency measurement, when built | the kernel-primitive fork: **surface to the operator** (not v1.0) |
...
| &nbsp;&nbsp;**N-2a-4** DOSBox-X + glquake game audio -- **LANDED 2026-09-06 (aux-3)** | retiring `0004-thylacine-force-dummy-audio` was the whole code change (the DX-1 mixer was never sound-compiled-out; 0004 setenv'd `SDL_AUDIODRIVER=dummy` to a null sink). This chunk also: RESTORES the DOSBox build+bake subsystem the 292-commit main merge (`8b28327f`) silently deleted from `build.sh` (config/executor mismatch -- `CHUNK_DOSBOX=y` with no executor; `bug-main-merge-deleted-dosbox-subsystem`); FIXES `DUKE3D.CFG` `FXDevice`/`MusicDevice` 13 (Apogee PC speaker) -> 0 (SoundBlaster) + `NumBits` 1 -> 16; fills the #156 `cxx-rt/libc++.a` fetch gap from the local stage2 sysroot so `stage_clade` bakes `/clade/bin/tyr-glquake`; teaches `test-game-audio.sh` the `--production` (`THYLA_BOOT_PROBES=OFF`) clean-wav case (a clade-baked game image must boot `--production` to skip the boot-fatal clade gates) | W-4 `--music` GREEN both games (mac HVF + QEMU wav): `ls-gfx-dosbox-duke3d` 1779/3319 windows, flatness 0.09, 14 dominant bins + `ls-gfx-glquake` 761/1096, flatness 0.20, 18 dominant bins | ran at max. The DOSBox SB16+OPL -> SDL thylacine backend -> Nocturne path had NEVER run on Thylacine (0004 nulled it from DX-1 on). SILICON audio (thyla-pi) is **N-6**: the game `.exp` force `hvf`, so `--music` is a mac/HVF+wav witness; thyla-pi is the graphics/GL host |
...
| &nbsp;&nbsp;**N-3c-2** device capture (the mic / line-in `source`) — **LANDED 2026-09-07 (aux-3): scripture @7a9d69e6 + impl this commit** | Design ratified 2026-09-07 (operator `AskUserQuestion`: (1) the CI witness is DETERMINISTIC now [`audiodev=none`: a monotonic period COUNT + the authority deny/allow pair -- the security-critical I-46 half in CI, real-audio-content proof deferred to a thyla-pi PipeWire-loopback follow-up]; (2) ONE chunk [driver RX + `source` + gate + witness together]). The virtio-snd driver gained a D_INPUT (capture) RX stream + the rxq (`VQ_RX`=3), negotiated OPTIONALLY at open -- absent (streams=1 default) => playback-only, N-1 unaffected (MEASURED: default boot `nocturne-probe OK` + `capture disabled (playback-only)`). The device-capture read is the gated `source` on `/srv/nocturne-ctl` (the singleton form; the `<name>/` subtree is N-3b/N-4), reusing the N-3c-1 `sink_authorized` gate + single-reader + bounded-drop-oldest + parked-read machinery verbatim, sourced from the RX pump instead of the mixer. Capture is ON-DEMAND (open STARTs the RX stream, close STOPs it -- a privacy property) via `Graph.source_open` + a cycle-thread poke (the device lives in the cycle thread). RX driver-adversarial safety: the device-controlled captured LENGTH clamped to PERIOD_BYTES before copy-out; the used-id validated like TX. The `none`-clocks-capture contingency was RESOLVED: it does -- the deterministic witness holds | witness GREEN: `tools/test-nocturne-capture.sh` (streams=2 + audiodev=none): SYSTEM `source` open + BYTES DELIVERED off `source` (the discriminating positive) + single-reader EBUSY + a user-principal `source` open DENIED + `source` absent on the mount; default boot `nocturne-probe OK` (playback intact) + 1512/1512 kernel tests + boot OK | EFFORT GATE fired (capture = eavesdropping + a driver surface), confirmed max. **Round-8 audit (Opus fallback -- Fable out arc-wide) 0 P0 / 0 P1 / 1 P2 / 3 P3: F1 (capture-state leak on the world-readable mount `info`, defeating "presence not probeable" + side-channelling live recording) FIXED (removed from render_info; the witness reads `source`, not info); F2 (fixed-threshold count witness) DISSOLVED by the byte-flow witness; F3 (PCM_START-fail retry-forever + reader-hang) FIXED (has_capture cleared -> ENODEV); F4 (lock-across-device-RPC) DEFERRED with N-2c-F2. NOT dirty -> no re-audit. Fable-diversity pass owed arc-wide.** |
...
## 15. N-3c-1 ratification record (2026-09-07): the gated sink tap

N-3c (capture) opens the eavesdropping surface I-46's authority half names.
Research — Plan 9's `/dev/audio` loopback is *ungated*, while the modern SOTA
(PipeWire's per-sink monitor source, CoreAudio's TCC-gated system-audio capture,
Fuchsia's `AudioCapturer` capability route, Android's `RECORD_AUDIO` +
opt-in `AudioPlaybackCapture`) makes capture a *distinct, mediated authority*
from playback — plus the as-built facts (the mixer already produces a mixed S16
period; the virtio-snd driver negotiates only `D_OUTPUT`, so device mic capture
is hardware-blocked) split the chunk cleanly: the sink tap is pure software and
buildable now; device sources are a separate, driver-audit-bearing sub-chunk.

**Operator decisions (`AskUserQuestion`, 2026-09-07), all as recommended:**

1. **Tap shape = a `tap` file on `/srv/nocturne-ctl`.** A gated read streams the
   mixed sink output; the mount `/dev/nocturne/audio` read is refused
   (playback-only). This reconciles the §6.4/§6.8 tension the way N-3a-3 taught:
   an eavesdropping authority cannot ride the shared mount (its peer is always
   the mounter=SYSTEM — the F1 bug), so it lives on the caller's own `-ctl`
   connection where the peer is the reader. Rejected: making the mount `audio`
   read the sink tap (the F1 bug redux — the mount cannot gate); the full `ear`
   node kind now (larger, overlaps the N-4 descant-ring machinery — deferred).
2. **Device `sources/` (mic capture) deferred to N-3c-2.** The virtio-snd driver
   negotiates only a playback stream, and a device-capture witness needs a
   non-wav QEMU backend; the work is separable and driver-audit-bearing, and the
   sink tap does not depend on it. Surfaced, not silently dropped (the
   deferral-needs-signoff rule).
3. **N-3c before N-3b.** The eavesdropping surface first; the node gain→dB
   grammar (N-3b) follows.

The gate is the N-3a-3 `sink_authorized` (renamed from `volume_authorized` — it
now gates both the volume write and the tap read), re-checked FRESH per tap read
so a mid-recording authority loss fails closed. The tap is single-reader (EBUSY
on a second concurrent open, a v1.0 bound), bounded drop-oldest (the DoS floor),
