...
| &nbsp;&nbsp;**N-2b-2** the ring PERIOD protocol + the ring-fed mixer | the producer/consumer over the ring (`prod_tail`/`cons_head`, K period slots; the I-30 snapshot discipline); `next_period` consumes FINISHED slots into the float32 mix; the back-pressure poke (ready_signal busy-poll or torpor); lays the descant substrate | a ring-voice chord witness (wav `--chord` played THROUGH the ring) | ring consumer: **ASK unless max** |
