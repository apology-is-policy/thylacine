...
| 16 | **pouch-stratumd-boot** | joey spawns real stratumd; `/sysroot` mount; ramfs pivot; retire the stub. **Sub-chunked 16a/16b/16c** (in-session 2026-05-24); **16b further sub-chunked 16b-α/16b-β** (in-session 2026-05-25, user signoff). **16a LANDED 2026-05-24**: binary-load probe in joey (`t_spawn` + `t_wait_pid`; verifies libc init + argv parsing + clean exit); not audit-bearing. **16b — stratumd-as-driver architecture** (chosen 2026-05-25 over alts: separate `virtio-blkd` daemon / writable-in-RAM pool / defer): stratumd holds `CAP_HW_CREATE` and drives the virtio-blk hardware directly via a new Stratum-side `src/io/bdev_thylacine.c` arm. Plan-9-canonical: the FS server owns its disk. Persistent: writes go to QEMU disk.img (second virtio-blk-device). Reversible: process isolation can be re-introduced later by factoring the driver back out into a daemon if the LOC growth in stratumd or the fault-isolation calculus changes. **16b-α**: argv pass-through kernel surface; pouch arm; joey passes richer argv to stratumd; the probe verifies argv is observable from inside stratumd. Audit-bearing. **16b-β**: Stratum-side `bdev_thylacine.c` (port of the userspace Rust `usr/virtio-blk-rw` driver) + pouch HW-syscall arm (`SYS_MMIO_*` / `SYS_IRQ_*` / `SYS_DMA_*` numbers in `bits/syscall.h.in`) + joey spawns stratumd with `CAP_HW_CREATE` + end-to-end pool mount + `/srv` socket bind. Audit-bearing. **16c** (kernel 9P client + `/sysroot` mount + chroot + stub retire) remains audit-bearing. | **yes** (16b-α, 16b-β, 16c) |
