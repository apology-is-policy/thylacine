...
- **Dynamic linking / shared objects** — v1.0 is static-only. pouch ships `libc.a`; `.so` support is deferred. **AMENDED 2026-09-23 (ARCH §6.5 "Dynamic loading"; lands at B-1d): static stays the DEFAULT; a program that needs `dlopen` links dynamically against `libc.so` (musl's loader), and the sysroot ships `.so` only for runtime-loaded objects and for `libc.so` itself. Decided 2026-09-24 (ARCH §6.5): a dynamically linked program is a PIE (`-pie`) and a static one stays a non-PIE `ET_EXEC` (`-static-pie` is refused); `libc.so` is PT_INTERP `/lib/libc.so` on the device; the handle form is `fdlopen` plus directory handles endowed at spawn, built at B-6.**
...
- **Memory** — the `malloc` family via musl's `mallocng`, resting on the native `burrow_attach` / `burrow_detach` syscalls (ARCHITECTURE.md §6.5, Tier 1). [RESOLVED 8.1: the lower half retargets musl's `__mmap` / `__munmap` onto the burrow-attach pair; `brk` / `madvise` / `mprotect` / `mremap` are tolerated no-ops — `mallocng` needs none of them.] [AS BUILT at B-1b (2026-09-23, patch 0044 `pouch-mman-protect-decommit`): `mmap` mints at the prot asked for over `SYS_BURROW_RESERVE` under a mint-time ceiling of RW (PROT_NONE reservations, read-only, W implying R; X is EACCES — I-12 never degraded); `mprotect` → `SYS_BURROW_PROTECT` with Linux's normalisation (a bit beyond R|W|X EINVAL, a hole ENOMEM, a raise past the ceiling or a guard EACCES; four arguments, so no stray flags word can SEAL); `madvise` DONTNEED / FREE → `SYS_BURROW_DECOMMIT` (the pure hints answer 0; any other advice EINVAL); `MAP_FIXED` over one's own anonymous mapping = discard + reprotect (never a creation: a hole is ENOMEM; `MAP_FIXED_NOREPLACE` ENOSYS); partial `munmap` = the range detach; `mallocng`'s `USE_MADV_FREE` is on, so freed slots inside a retained group return their pages; `mremap` stays the copy fallback. `__NR_mmap` is back at the seam's sentinel — a raw Linux-shaped `mmap` no longer reaches 83, whose Clade CL-4 arm reads the length alone and maps RW whatever prot is asked, so musl's one raw caller, `__init_tls`, maps through `__mmap` (patch 0046, required by the parking) — and the four numbers live in `src/internal/_pouch_mman.h`. The witness is `pouch-hello-mem` (twelve legs; the mallocng-trim leg measured 4263 → 2339 → 11 pages full / half-freed / all-freed, and is RED on a libc built with `USE_MADV_FREE 0`).]
...
1. **auxv population in `exec_setup`** — argc / argv / envp / auxv on the initial user stack, with at minimum `AT_PAGESZ`, `AT_RANDOM` (16 bytes), `AT_PHDR`/`AT_PHENT`/`AT_PHNUM`, `AT_NULL`. (The research pinned the minimum.) This is owed to the future `exec` syscall regardless. [AS BUILT at B-1b (2026-09-23): eleven entries — `AT_PHDR`, `AT_PHENT`, `AT_PHNUM`, `AT_PAGESZ`, `AT_HWCAP`, `AT_RANDOM`, `AT_ENTRY`, the private pair `AT_STACK_BASE` (0x5342) / `AT_STACK_SIZE` (0x5353) stating the initial stack's extent (8 MiB, decision 4 of the browser arc; a libc DERIVES it — patch 0045's `pthread_getattr_np` — and refuses with ENOSYS a kernel that writes neither tag, never mirroring `exec.h`), the optional `AT_VDSO_CLOCK`, and `AT_NULL`.]
...
| 16 | **pouch-stratumd-boot** | joey spawns real stratumd; `/sysroot` mount; ramfs pivot; retire the stub. **Sub-chunked 16a/16b/16c** (in-session 2026-05-24); **16b further sub-chunked 16b-α/16b-β/16b-γ** (in-session 2026-05-25, user signoff). **16a LANDED 2026-05-24**: binary-load probe in joey (`t_spawn` + `t_wait_pid`; verifies libc init + argv parsing + clean exit); not audit-bearing. **16b — stratumd-as-driver architecture** (chosen 2026-05-25 over alts: separate `virtio-blkd` daemon / writable-in-RAM pool / defer): stratumd holds `CAP_HW_CREATE` and drives the virtio-blk hardware directly via a new Stratum-side `src/io/bdev_thylacine.c` arm. Plan-9-canonical: the FS server owns its disk. Persistent: writes go to QEMU disk.img (second virtio-blk-device). Reversible. **16b-α LANDED**: argv pass-through kernel surface; SYS_SPAWN_FULL_ARGV=49 + struct sys_spawn_args; audit closed clean (0 P0 + 0 P1 + 2 P2 + 9 P3). **16b-β foundation LANDED**: Stratum-side `bdev_thylacine.c` (~610 LOC port) + pouch HW-syscall arm (0008) + pouch openat() multi-component walk (0009) + host stratum-mkfs build + pool fixture + QEMU 2nd virtio-blk-device + joey integration; non-fatal probe. **16b-γ-syscalls LANDED**: native fstat + lseek kernel ABI (SYS_FSTAT=50, SYS_LSEEK=51, struct t_stat 72 bytes) + Dev `.stat_native` vtable slot + devramfs_stat_native impl + kernel walk fixes (devramfs walk reuse-nc + partial-walk reject in sys_walk_open + kproc territory chroot to devramfs root in joey_run) + libt + 0010-pouch-fstat-lseek.patch (3-file: bits/syscall.h.in numbers + src/stat/fstat.c t_stat→struct stat + src/fcntl/open.c→openat redirect) + system.key placed at ramfs root (v1.0 flat-cpio expedient; FHS-shaped /etc/stratum/ awaits devramfs subdir walks). stratumd reaches `stm_bdev_open(THYLACINE) OK` then fails at `stm_sb_mount_scan` (bdev I/O read-path); paired audit deferred. **16b-γ-mount-close LANDED 2026-05-25**: bdev_thylacine `T_RIGHT_SIGNAL` constant fix (Stratum-side, was `(1u<<3)` = `RIGHT_TRANSFER`; corrected to `(1u<<5)` matching kernel `RIGHT_SIGNAL`); pouch musl `0011-pouch-abort.patch` overrides `abort()` to `_Exit(127)` (v1.x extension landed early per `docs/reference/83-pouch-signals.md`); joey retry+drain+reap workaround for the not-bound NON-FATAL path (6000-retry srv_connect with non-blocking pipe drain on each retry + bounded post-failure drain + final `t_wait_pid` that prevents the unreparented-zombie / kproc-wait_pid `wrong pid` extinction race). After fix: bdev's full read/write/fsync path runs end-to-end (STRATUM2 magic read at offset 0x4000); mount_scan succeeds; downstream stratumd mount path proceeds into pool_open / alloc_open / sync_create + exits clean rc=-N (separate downstream failure tracked as next sub-chunk). joey probe still NON-FATAL pending that fix. Paired audit covers Stratum-side bdev fix + Thylacine 0011 patch + joey workaround + carried-over 16b-γ-syscalls surfaces. **16b-γ-mount-bind CLOSED prior session 2026-05-26**: AEGIS-256 / mallocng corruption arc closed via the F1 memory-model fix (`61d5da3` SYS_BURROW_DETACH stale-PTE/TLB) -- root cause was kernel-side, not Stratum-side. Downstream mount path now runs clean end-to-end. **16c-pre LANDED 2026-05-26**: two pouch boundary-line patches unblock stratumd's listen path on /srv/stratum-fs -- `0014-pouch-srv-stubs.patch` stubs `lstat`/`unlink`/`chmod` for `/srv/*` paths (return ENOENT/0/0); `0015-pouch-poll-tag.patch` adds pouch-tag dispatch to `poll(2)` (closes a sub-chunk 12 gap where the only fd-consuming syscall not given tag translation was poll, leaving stratumd's accept loop to bail on POLLNVAL). NOT audit-bearing (pure pouch-side; gap-fills audited patterns). Stratumd now binds + serves indefinitely; joey reaps cleanly. **16c design LANDED 2026-05-26** (scripture-only, this commit): re-framed under the **live-medium -> installer -> boot-from-installed-disk** pattern that every real OS distro embodies. Thylacine's `disk.img` cpio is the "live medium" (equivalent in role to a Linux ISO's squashfs); the boot binary corpus (joey + corvus + stratumd + pouch hellos) is in the cpio. The "installer" pre-populates pool.img with the same corpus so the post-pivot root has every binary joey or its peers might spawn. **Choice: host-bake at build time (over runtime installer + over no-pivot)** -- `tools/build.sh::build_stratum_pool_fixture` orchestrates the already-shipping Stratum v2 binaries (stratum-mkfs creates the empty pool, stratumd starts on a temp Unix socket, stratum-fs write copies each corpus file, stratumd stops); NO new Stratum-side code; pool.img ships pre-populated. The v1.1 runtime-installer lift (a Thylacine `/sbin/installer` that runs in an alternate boot path and copies devramfs -> stratumd via 9P Twrite/Tcreate) would PROVE the install loop end-to-end; the v1.0 expedient ships the headline cleanly without that complexity. **Binary-corpus problem solved by joey pivoting LAST**: joey runs all bringup spawns (corvus + probes + stratumd itself) BEFORE pivoting, since those spawn-path resolutions go through devramfs; pivot is the final bringup step before `Thylacine boot OK`. Kernel surfaces (3, all audit-bearing): `kernel/9p_srvconn_transport.{c,h}` (new transport adapter wrapping byte-mode SrvConn rings into p9_transport_ops); `SYS_ATTACH_9P_SRV` (new syscall composing SrvConn handle + adapter + p9_attached_create + dev9p root); `SYS_PIVOT_ROOT` + `territory_pivot_root` (atomic root_spoor swap with displaced-ref tear-down -- closes the v1.x "long-running Proc release chroot" note in `usr/joey/joey.c:293-304`). NO Stratum-side coordination needed -- the host-bake uses already-shipping `stratumd + stratum-fs` binaries on the `thylacine-pouch-arm` branch unchanged. Stub retirement scope: the userspace `usr/stub/` stratumd-stub binary + joey's `do_stratumd_stub_bringup` call retire; `kernel/dev9p.c` + the kernel-internal `test_stratumd_stub.c` tests are KEPT (the kernel-side dev9p machinery is still exercised by tests; only the user-facing stub usage retires). **16c** (this session): kernel + host-bake + libt + joey + retire, single audit-bearing chunk. Detailed scripture in `docs/reference/86-pouch-stratumd-boot.md` "## 16c design" section + the new `docs/reference/88-pouch-stratumd-boot-16c.md` once impl lands. | **yes** (16b-γ-mount-bind, 16c) |
...
- **R3 — Stratum-side coordination.** The `pouch-libsodium` + `pouch-stratumd-build` + `pouch-stratumd-boot` (16b-β) sub-chunks need Stratum changes (the `peer_creds` Thylacine arm; the `bdev_thylacine.c` virtio-blk arm under the stratumd-as-driver decision). User-owned; sequencing risk if Stratum and Thylacine drift. Mitigation: `thylacine-pouch-arm` branch off Stratum `main` carries all Thylacine-specific Stratum code; coordination via `docs/session-handoff-*.md` artifacts; minimal-targeted-change discipline.
