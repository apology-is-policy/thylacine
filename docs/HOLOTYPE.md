...
| RW-2 | Sched/SMP/threads/death/wait-wake | **CLOSED `@<close-sha>`** — 0 P0; **4 P1** all FIXED `@e504e8b` (2C-F1 poll-waiter-outlives-obj-ref UAF; 2A-F1 vd_t wake-no-rebase starvation; 2B-F1/F2 wait_pid single-waiter-extinction + lockless-cond UAF — all the multi-thread-Proc lift P6 landed but the wait/poll machinery never caught up to) + 5 P3 (sched/smp loud-fail + 4 doc) + 2 regressions. **1 P2 SURFACED** (2B-F3 orphan→kproc leak / ARCH §7.9 PID-1 divergence — init-reaping policy, user's call). Dirty-close round-2 (2 Fable on the fixes): **clean** (0 P0/P1/P2; 3 P3 doc/test fixed). The #809/#811/LS-5/#926 death cascade is SOUND under composition. 808/808; SMP gate 0 corruption. Catalog: `docs/holotype/{00-register,02-sched-death-waitwake}.md` + `audit_holotype_rw2_closed_list.md`. REGISTERED → RW-13: 2A-F6 (not-real-EEVDF), SA-1 (no death-wake spec — user vote), 2C-F2; TRACKED: SA-2/SA-3, #17/#18. |
