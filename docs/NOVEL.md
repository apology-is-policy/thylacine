...
### Post-v1.0 candidates (captured, deferred)
...
The mechanism is built on one small FS primitive (FS-delta, the `T_OPATH` walk-without-open mode) the kernel otherwise already supports: the spawner builds the storage dir (`mkdir -p` via FS-delta), `handle_dup`s the non-opened handle down to `R|W` (dropping `TRANSFER` as least-authority hardening — authority is bounded `<=` the grant + subtree and **monotonic**; a grantee may delegate within its grant but never exceed it; `TRANSFER` gates the Phase-5+ 9P-transfer surface, not spawn-endow at v1.0), and endows it as a child fd at spawn; the service **chroots to the handed handle as its first action** so its filesystem world IS the capability (confinement is cooperative at v1.0 — spawner-set-root is the v1.x form); optionally it `SYS_MOUNT`s the handed Spoor into its Territory at a path (e.g. `/state`) for a service that must keep a broader root.
