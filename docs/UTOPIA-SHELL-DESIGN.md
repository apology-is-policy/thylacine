...
**As-built (U-7c-b).** At v1.0 there is no PTY line discipline (step 1) and no process group (step 2): the kernel console owner posts the catchable **`interrupt`** note (not `snare:int` — `snare:*` is kernel-synthetic-fault-only and terminates; see §10.8) to the shell Proc on a `/dev/cons` Ctrl-C (the A-4c console path). The forward happens NOT from the main `poll()` loop but during the *foreground command's* wait: a foreground command runs while the shell is not reading the console, so its reap (`wait_pids_interruptible`, `libutopia::eval`) polls the shell's own note fd between WNOHANG reap sweeps and, on a drained `interrupt`, forwards `send(Pid(pid), "interrupt")` to each still-live foreground pid — it does NOT run the shell's own `on note` handler while a foreground job is present (step 4's first branch). The "no foreground job → clear the line / fresh prompt" branch (step 4's second branch) is realized at **LS-8c**: LS-8a made `/dev/cons` pollable, so the main loop (`usr/utopia/shell/src/main.rs`) polls cons + the shell's note fd together, and a Ctrl-C at an idle prompt -- delivered as `interrupt` to the shell while the editor is mid-read -- wakes the loop and `Repl::on_notes_ready` cancels the in-progress edit + redraws (an `on note 'interrupt'` handler, if registered, fires instead and the edit is preserved -- the LS-5 catchable semantics). Before LS-8c the single-fd loop blocked in read(), so the idle-prompt interrupt was drained only at the next sync point by `deliver_pending_notes`. A forwarded `interrupt` to a *native* non-notes-reading child is inert (it queues); it is meaningful to a pouch/musl child as an async SIGINT.

**Stale-idle drain + loop interrupt (RW-9 R3-F1 / R2-F3).** Two refinements pin the as-built so a queued `interrupt` cannot leak across the command boundary or be lost in a child-less loop:

- **Drain at the top of the Accept cycle, not only the bottom.** An `interrupt` queued while idle at the prompt (the editor never sees the cooked `0x03`) MUST be consumed *before* the next command is evaluated — otherwise that command's `wait_pids_interruptible` finds the stale note and spuriously forwards it to the just-spawned child, killing an unrelated command. `Repl::feed` therefore calls `deliver_pending_notes` at the *top* of the `Accept` arm (before `run_line`) as well as at the bottom: the idle interrupt fires its `on note` handler (if any) or is discarded as benign at the sync point, and the new command starts on a clean queue.
- **A runaway shell-eval loop is interruptible.** A loop whose body never blocks in a foreground wait (`while (1) { ... }`, pure shell eval) has no child to forward to, so without an explicit check it would never observe the `interrupt` and would spin forever. `eval_while` / `eval_for` poll the shell's note queue every `LOOP_INTERRUPT_STRIDE` (128) iterations (`poll_loop_interrupt`); a terminate-intent `interrupt` latches a one-shot `Env::interrupt_pending` flag that `eval_block` checks after every statement — so it unwinds every enclosing loop / block / function to the prompt, propagating across function-call boundaries exactly as `pending_exit` (`exit`) does. Non-`interrupt` notes read past while polling are deferred to the post-command drain (not lost). The REPL clears the flag and sets `$status = 130` (128 + interrupt, shell convention) once the command has unwound. A loop whose body *does* run a foreground command is already interruptible via that command's `wait_pids_interruptible`, so the strided poll only governs the child-less case.
...
Spawns the command; the shell prints `[N] PID` where N is the jobspec; adds the child's notes fd to the poll set; continues the prompt. When the child exits, the shell prints `[N]+ Done <cmd>`. **As-built (through LS-8c):** the notification is reported ASYNCHRONOUSLY while the user is idle at the prompt -- LS-8a made `/dev/cons` pollable, and the LS-8c multi-fd loop (`usr/utopia/shell/src/main.rs`) polls cons + the shell's note fd together, so the child's `child_exit` note wakes the idle loop and `Repl::on_notes_ready` reaps + prints the `[N]+ Done` line the instant the job exits (bash-style immediate, not the fish-style lazy "at next prompt cycle" the U-7a interim used while cons had no `.poll` hook).
