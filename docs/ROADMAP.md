...
- **D3 — The on-system toolchain is IN v1.0** (NEW Phase-8 scope). clang/lld + make + git ported via Pouch; the self-hosting / build-storm (W2) story is part of v1.0. This makes VISION section 397 "no subset" + the Phase-9 "parallel make" criterion coherent — and is the single largest Phase-8 pole. **The binding design is the "Clade" LLVM keystone arc (`docs/LLVM-DESIGN.md`, SIGNED OFF 2026-07-23):** clang/lld is one instance of a keystone LLVM port whose ~80%-committed cost (clang is C++, so the C++ runtime port is forced either way) carries four near-free riders — the C++ runtime via Pouch (the hard prerequisite; no C++ program is portable today), Mesa's llvmpipe (software GL for the graphics arc, realizing `JIT-ON-WX-DESIGN.md` + minting invariant **I-42**), release-grade Rust, and clangd for Nora — sequenced as workstreams CL-0..CL-9.
...
**The 2026-06-15 scope extension (the Menagerie + real-hardware + Aurora arc; user-ratified):** four interlocking design docs extend v1.0 scope beyond the QEMU-textual target:
- **The real-hardware track is IN v1.0** via the **Menagerie driver framework** (`docs/MENAGERIE.md`): the binding layer (discovery sources -> the warden -> a narrowed hardware allowance -> capability-sandboxed userspace driver Procs) that brings RPi4/RPi5 up. The one kernel lift is the **hardware allowance** (reserves I-34); `devhw` publishes the DTB to userspace; QEMU `virt` is a peer board in a **universal image** (the DTB selects at boot). This realizes §3.5 for real hardware and supersedes the §22.4 "RPi5 post-v1.0" framing.
- **Aurora is a first-class textual environment** (`docs/AURORA.md`), a proper alternative to Halcyon — it ships at v1.0 *regardless* of Halcyon (de-risks the ship; the VISION two-environments statement lands with its scripture commit).
- **The trusted path is unified across renderers** (`docs/TRUSTED-PATH.md`): the SAK lifted off the serial substrate (generalizes I-27); framebuffer-only production posture.
- **The installer** ("the founding", `docs/INSTALLER.md`): the host-bake promoted to a runtime program; a passphrase-derived disk key (the FileVault model via the corvus A-5c system identity).
  Adoption sequence (scripture-first, one commit each): **Menagerie** (this commit) -> trusted-path -> Aurora -> installer. Then the Menagerie build arc (`devhw` -> the allowance, model-first -> the warden/libdriver -> the QEMU-virt proof), **then net-2 resumes on the Menagerie substrate** so `netd` is born discovery-driven + allowance-narrowed.

**The forward sequence** (the arc emerges here; the system resumes building):
RW-13 reconciliation pass (the long-deferred section-2.1 renumber propagation + the present-tense honesty edits — pure scripture) -> the D4 pre-rc sub-arc -> the container keystone (exec-from-namespace + ns substrate) -> the **LS line, Phase-7 completion** (LS-K id/whoami/date + the wall clock -> LS-8 termios/PTY -> LS-7 the nora editor -> LS-6 login echo) -> **the virtio-PCI transport sub-arc (pci-0..3, DONE 2026-06-15)** -> **the Menagerie + Aurora + trusted-path + installer arc (scripture-first, then the driver-framework build)** -> **Phase 8** (net-2 onward [netd + smoltcp on the PCI NIC + Menagerie substrate -> net-3..net-8] -> **the net-optimization (Weft) arc** [the per-flow capability-scoped zero-copy network dataplane + the Tier-A window win; `docs/NET-THROUGHPUT.md` + I-37 — pushes throughput 16 MiB/s -> hundreds of MiB/s AND closes the RX-wake latency floor (NET-PERF N1) on one shared-ring mechanism; the Snap-transport + Arrakis-framing NOVEL, gap-confirmed by a five-lineage literature pass] -> **the REVENANT arc (R-1..R-5 DONE 2026-06-22; #229 RESOLVED)** [file-backed demand-paged exec + the static-linking conviction; `docs/EXEC-LOAD-DESIGN.md` + I-36 — retired the 1-MiB whole-ELF slurp cap, demand-pages + shares text via the qid-keyed Image cache; a binary of any size execs (the unstripped >1.11-MiB `net-echo` boot proof); resolved #229, the on-system toolchain's large-binary-exec prerequisite] -> **the Imperium/Authority arc** [the serial trusted-path episode + imperium self-elevation -> the mandate/censor delegation; `docs/MANDATE-DESIGN.md` + I-35] -> container runner -> on-system toolchain -> Linux binary shim) -> **Phase 9** (hardening -> v1.0-rc.1) -> **Phase 10** (Halcyon -> v1.0 final). The section 10/11 fallback stands: if Halcyon slips, the Phase-9 rc ships as v1.0 — and Aurora makes that fallback a *complete* textual OS, not a degraded one.

**The 2026-07-05 insertion (the concurrent-FS arc; user-charter 2026-06-25, ratified 2026-07-05):** between the on-system toolchain (whose go-build clean+perf mission closed 2026-07-05: correctness fully clean; wall-clock ~36x host, gap proven FS-concurrency-shaped — a `-p 4` build executes at serial wall-clock behind the one serial 9P server loop) and the Linux binary shim, the **concurrent-FS arc** runs: `docs/CONCURRENT-FS.md` (binding). CF-1 the Stratum 9.8-BE lockless write half (the R171 soundness gate + Bε small-write batching; `stratum docs/phase-9.8-design.md` §5.1.1) -> CF-2 the stratumd per-connection worker pool (the Plan 9 `exportfs`/diod per-request model) -> CF-3 measurement-gated transport levers -> CF-4 the commit-path cost (Area-G follow-ups) -> CF-5 measure/audit/close. Soundness-first: CF-2 must not ship before CF-1. NOVEL.md records the angle (concurrent-9P-server x lockless-metadata-core fusion).
...
**The Menagerie driver framework (`docs/MENAGERIE.md`, scripture 2026-06-15) is the full realization of this principle for real hardware.** It makes "drive it from userspace" the *default and only* path for everything outside a minimal kernel-resident TCB floor (the GIC, the timer, the console UART, the CSPRNG seed source, and the `devhw` DTB-inventory source). Every other device — including third-party drivers — binds as an isolated, capability-sandboxed userspace Proc holding only its device's **narrowed hardware allowance** (I-34), so a driver's blast radius is exactly its own device. This is what lets the driver ABI be *stable* (a third-party driver cannot crash the kernel or escalate) — a NOVEL position a monolithic kernel structurally cannot offer.
...
**On-system toolchain (v1.0 scope per HOLOTYPE RW-13 / D3, 2026-06-11; the binding design is the "Clade" LLVM keystone arc — `docs/LLVM-DESIGN.md`, SIGNED OFF 2026-07-23)**:
...
- **Clade** reframes the pole as ONE keystone LLVM port (LLVM 22.x, host-parity, vendored as the `~/projects/llvm-thylacine` fork — the `go-thylacine` precedent) with near-free riders: the **C++ runtime** (libunwind/libc++abi/libc++ static over the pouch sysroot — the hard prerequisite, no C++ is portable today), the **on-device optimizing C toolchain** (the static `llvm` multicall + `Triple::Thylacine` + lld default — this D3 deliverable, retiring the `pouch-clang`/`pouch-ld` wrappers), **Mesa llvmpipe** (software GL for the graphics arc, realizing `JIT-ON-WX-DESIGN.md` + minting **I-42** JIT-as-a-capability, with GLQuake the acceptance gate), and **release-grade Rust** (a cross `aarch64-thylacine` target now; on-device rustc staged). Workstreams CL-0..CL-9; the two audit-bearing kernel touches are the **F4 spawn-time page-budget** (CL-5) and the **CL-7k JIT capability** (the code Burrow + `SYS_ICACHE_SYNC` + `CAP_JIT`).
- The single largest Phase-8 pole; sequenced after the net arc + the container runner (F7: post-gfx-milestones; CL-1/CL-2/CL-3 parallelizable with late gfx work — disjoint surfaces).
...
- Kernel preemption enabled (the Phase 2 deferral). Audit-gated.
  - **Status (2026-09-22): this item never reached `docs/phase7-status.md`.** Its precursor was also never built: ARCH section 8.1 designs syscalls that are non-preemptible but service interrupts, and they were implemented with interrupts masked (the as-built note there). Building that precursor is scheduled as its own spec-first kernel chunk, before the browser arc's kernel work (operator decision 2026-09-22); full kernel preemption stays this item. **The precursor's design is ARCH section 8.12** (2026-09-22): a separate per-thread in-syscall marker (not `preempt_count`, which three live assertions forbid), the unmask confined to the SVC body so the #713 eret window stays masked, a lock sweep that MEASURED clean, and a kernel-stack measurement that pulls one prerequisite forward (`viv_tier2`'s frame).
