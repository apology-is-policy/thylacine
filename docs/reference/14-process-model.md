...
1. Re-parent any orphan children to **init** (`g_init_proc`, published by `joey_thunk` via `proc_publish_init` before exec — 2B-F3, aligning the code with ARCH §7.9 step 6), falling back to **kproc** while init is not up (early boot, the in-kernel test phase, or after init itself died — `proc_become_zombie_locked` clears `g_init_proc` at init's own ZOMBIE transition so it never dangles). Children adopted **already-ZOMBIE** trigger one `poll_waiter_list_wake(&adopter->child_waiters)` inside `proc_reparent_children` (their original exits-side wakeup went to the now-dead parent — without the fresh wake, an adopter blocked in a wait-any could sleep over a reapable zombie). joey reaps adoptees with a `t_wait_pid_for(-1, WAIT_WNOHANG, …)` sweep per getty iteration (`usr/joey/joey.c::reap_adopted_orphans`); its session/bringup waits are all **by-pid** (`t_wait_pid_for(pid, 0, …)`) so an adopted orphan's zombie can never be mistaken for a tracked child (and conversely the sweep never steals a tracked child's exit — the by-pid waits claim their targets).
   ...
   **#80 — naming the orphan.** `proc_reparent_children` emits one line per adoption: `proc: orphan pid=N name="X" (parent pid=M name="Y" exiting) -> adopted by pid=A`. It exists because joey's sweep sees only a **pid**, which made its report undecidable from the log alone — this is the one point where the kernel still holds both Procs, so the pair of lines resolves *which* Proc later shows up in the sweep. Rare and notable by construction: Thylacine has no daemonize idiom (joey spawns its servers directly), so an adoption means some Proc exited with a live child — and a **kproc**-adopted one (`adopted by pid=0`, init not yet up) is never reaped at all. `uart_puts`/`uart_putdec` are the direct bounded-FIFO path (no P1-F TX ring, no sleep, no lock), so this is safe under `g_proc_table_lock`. On the joey side the sweep now **decides** rather than hedges: a pid in joey's own daemon registry (corvus / stratumd / ptyfs / diorama / aurora — the five it spawns and never waits) reports `DAEMON EXITED: <name>`; anything else reports `reaped adopted orphan`; an overflowed registry reports UNKNOWN rather than downgrading a possible server death to routine hygiene. The registry supersedes the earlier audit-F3 neutral wording, which existed *only* because joey could not tell the two apart.

   Three adoptions are expected every boot, all benign and all self-explaining in the log: `netd` and `tapestryd` (the warden spawns its drivers, then exits — the Menagerie leaf-driver shape), and `ambush-child` (the `/dap-probe` debuggee, terminated by **EXITKILL** when its launcher exits — §I-39; it is `main.parkLoop` and can never exit voluntarily, so `status=1` is forced, not a failure). That last line was the whole of #80: the sweep's report is the *footprint of the EXITKILL fix* (`e5e60df3`), not a defect. Before EXITKILL the debuggee was resumed and orphaned to run forever — the measured HVF idle leak — so it never died and joey never reported it.
...
**fd-close-at-exit (#926, completed by #68).** A `Proc`'s handle table is normally closed + freed by `proc_free` at *reap* (step 4 of `wait_pid`). Since #926 (U-6f command-substitution prerequisite, user-voted 2026-06-08) a single-thread `Proc` closed it at *exit* (a `thread_count == 1` gate at the top of `exits()`); since **#68** (2026-07-14, whose round-2 audit RETIRED that gate — `thread_count` counts unreaped `THREAD_EXITING` peers, so a joined-then-exits native multi-thread program was skipped by it) the LAST live Thread out closes the table at **both** exit sites — `exits()` (single-thread AND joined-then-exits multi-thread voluntary exit) and `thread_exit_self` (`SYS_EXIT_GROUP` + the killed paths) — each gated on `proc_count_live_peers_locked(p, t) == 0` under `g_proc_table_lock`, in a deliberately-opened RUNNING+ALIVE window — it drops `g_proc_table_lock`, runs `proc_close_handles_at_exit`, re-takes the lock (asserting the last-out determination held; structurally guaranteed — every peer has committed `THREAD_EXITING`, whose remaining execution never touches the handle table, and no new peer can spawn without a RUNNING thread in the Proc), and only then transitions the Proc to ZOMBIE. The effect is correct Unix/Plan 9 semantics on **every** exit path: inherited fds close when the **process terminates**, not when the parent later reaps it. Pre-#68 this held only for single-thread voluntary exits; a multi-thread Proc (every Go binary exits multi-thread via `SYS_EXIT_GROUP`, since `thread_count` decrements only at reap) delivered EOF only at reap, so a parent draining the child's stdout to EOF *before* reaping deadlocked — the nora gofmt-on-save hang and the ut `$(go ...)` substitution wedge (EOF needed reap, reap needed the parent's wait, the wait waited on EOF). The close runs under `exit_close_active` (#68 F1, the audit's catch): `group_exit_msg` is set on **every** `SYS_EXIT_GROUP` — a clean `exit_group(0)` included — so without the flag `thread_die_pending()` read true for the closer and every sleep-capable close hook short-circuited: the dev9p write-behind close-flush silently dropped its staged bytes (data loss for a file left open at a multi-thread exit; accepted `write()`s must survive exit, the page-cache contract) and the close-time Tclunk was never sent (a server-side fid leak per open dev9p fd). The flag makes `thread_die_pending()` read false for the closer during the one bounded close pass, restoring exactly the pre-#68 reap-time behavior (the reaper was never dying); it is set/cleared *inside* `proc_close_handles_at_exit` (round-2 F1) so `exits()`'s site is covered too — that site is reachable with the LS-5 interrupt-terminate latch deliberately still armed (the default-terminate path leaves the note queued), and a racing cross-Proc kill can set `group_exit_msg` mid-close. The re-admitted wedged-server strand equals the old reap-time exposure, relocated from the parent's `wait_pid` (where it hung the shell) onto the already-dying Proc — and, unlike the old strand, it is NOT breakable by a further kill (the flag suppresses both death legs for the closer): precondition = a wedged *trusted* server, an already system-degraded state; a bounded/abortable close-flush is the recorded v1.x seam (round-2 F3). The window's soundness rests on the same three properties at both sites (sleep-capable close hooks legal while RUNNING; no reap possible while ALIVE; a single table toucher). `proc_free`'s `handle_table_free` no-ops on the NULLed table and remains the fallback for the direct `state=ZOMBIE; proc_free()` orphan/rollback paths and for hooks a death-flagged close skipped. The handle-close-at-exit / vma-drain-at-reap ordering inversion is safe by the #847 per-Burrow dual refcount (a Burrow frees only when `handle_count == 0 && mapping_count == 0`). Regression: `sys_spawn.killed_child_delivers_pipe_eof_before_reap` (a killed pipe-parked child's write end delivers EOF before any reap — bounded poll, revert-probed; the `thread_exit_self` site) + `sys_spawn.joined_multithread_child_delivers_pipe_eof_before_reap` (a `/thread-probe` child — worker Threads joined via clear-child-tid, so `thread_count > 1` and `live_peers == 0` at its `t_exits()` — delivers EOF before any reap; the `exits()` site, revert-probed) + the go6.exp E2E (nora gofmt-on-save over an all-piped Go child). Audit-trigger surface (ARCH §25.4); SMP-gated; focused audits per landing (#926: 0 P0/P1/P2; #68: see the close list).
...
**The kernel-side consumer (#94).** U-7-pre built the filter and converted the userspace callers, but left the site that motivated it — `kernel/joey.c`'s kproc-waits-for-joey — on the reap-any `wait_pid`, so the hazard the section above describes stayed live in the kernel. It is reachable, not hypothetical: when joey exits early its daemon children re-parent to kproc, `proc_reparent_children` splices each onto the **front** of `kproc->children` (ahead of joey), and the scan breaks on the first ZOMBIE it meets — so any daemon that had already died was reaped *instead of* joey, and the `reaped != pid` check extincted with "wrong pid". That discarded joey's exit status, i.e. the boot-failure diagnostic, precisely on the branches where a daemon dying is *why* joey is exiting. kproc now waits by pid. Orphan fate is unchanged: kproc has never reaped orphans as a service (see `proc_reparent_children` — init is the reaper; kproc adopts only once init is gone), and this site performed a single incidental reap before extincting on it. Pinned by `proc.wait_pid_for_skips_adopted_orphan_zombie`, which builds the boot's exact arrangement — an adopted orphan ZOMBIE ahead of the target, with distinct statuses so the pid and the status each discriminate.
