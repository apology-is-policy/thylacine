...
- **`warp-prove ctl` / `warp-prove tctl [key]`** -- the two census
  ...
  per WRITE, not per stream. The read side ALSO had a real splice
  class -- the ctl composes fresh on every read(), so a reader looping
  512-byte reads to EOF (the r7-F6 fix) crossed generations --
  and tapestryd's contract is now: ONE OPEN READS ONE GENERATION
  regardless of read count (the offset-0 read pins the compose; the
  mechanism is dossier material). But the impossible triple that
  motivated the pin turned out to be CAPTURE-side: an expect regex
  with no trailing line anchor fires on a PARTIAL serial chunk, and
  the exp's own tagged `puts` then interleaves into the log mid-row,
  severing the last field (`max=3` for a real `36322`; both runs'
  "impossible" maxes were truncations, and the corrected values make
  every row arithmetically possible). The composed-screen exp calls
  its trailing `[\r\n]` load-bearing for exactly this; every cost-row
  capture now carries it. A row that violates `sum <= n * max` is a
  READ- or CAPTURE-coherence defect, never the writer -- that
  inequality is a theorem of the census accumulator.
