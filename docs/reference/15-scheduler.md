...
- **`tools/ci-smp-gate.sh`** (`make smp-gate`) builds the needed kernels once and runs the matrix — `default-smp4` / `default-smp8` / `ubsan-smp4` (the #860 amplifier) / `ubsan-smp8` — at **N≥10** boots each (env `SMP_GATE_N`, `SMP_GATE_CONFIGS`). It composes **`tools/smp-multiboot.sh`**, which re-runs `tools/test.sh` against one built kernel and classifies each failure: **CORRUPTION** (a ctx/stack-corruption signature — `invalid prev state`, `stack canary mismatch`, `kernel stack overflow`, `already on_cpu`, `#860`, …) FAILS the gate; **EXTERNAL-KILL** FAILS the gate under its honest label, reporting the signal, the guest's health at the kill, and — where recoverable — the sender pid (`ps`'d at classify time while it may still be alive). It has **two detectors, because the first is structurally blind to the signal that matters most**: (1) QEMU's own `terminating on signal N from pid M` report (#88) catches the *catchable* signals and names the sender; (2) **SIGKILL is uncatchable, so QEMU never gets to print that report** — the arm that most needed the bucket could not reach it, and two real #200 sightings therefore landed in OTHER (and, in the corner where the guest log also matched the inject-miss shape, in the *non-failing* inject-miss class — a fail-open, not merely a mislabel). Detector (2) reads the shell's job notification (`line N: PID Killed: 9`) out of the **harness** stream, and requires `test.sh`'s `qemu_alive_at_teardown=0` alongside it. That conjunct is load-bearing rather than defensive: bash emits the notification whenever a job died by signal and a command boundary elapsed before the shell reaped it, so the harness's OWN teardown kill prints it too (measured — `tools/test-smp-classify.sh` case G); `alive=0` proves the harness's kill hit an already-dead process, so the signal death cannot have originated there. The ladder and both detectors are pinned by `tools/test-smp-classify.sh`, which drives the real classifier (sourced, not re-declared — #143) over real captures plus synthetic cases, and whose every arm is A/B-proven to redden exactly its own cases; an **inject-miss** (#362 — the QMP key-injector missed delivery, guest proven green) is noted, not failed; benign host-**TIMING** fragility is reported, not failed; **OTHER** is surfaced for investigation. `test.sh` itself is the *primitive* the gate multi-boots — it is deliberately NOT a soundness gate, and the gate is a separate CI entry point to avoid recursion. `smp-multiboot.sh` clears its own label's prior `build/multiboot-fails/*.log` at the start of each run, so the captures dir only ever reflects the latest run — stale logs (including any written by a since-fixed classifier) cannot masquerade as current findings. It **archives** them to `build/multiboot-fails/archive/<label>-<utc>/` rather than deleting them (#223): the clearing rationale is right, but a `rm -f` made the standard response to a rare failure — re-running that label in isolation — destroy the only copy of the thing being diagnosed, which is how the #200 sighting-2 harness log was lost. Every boot also records its wall clock (`PASS (20s)`, plus a per-label `exposure … mean/boot` line), so the open #200 question of whether the UBSan asymmetry is a sanitizer effect or merely a longer exposure window is answerable from ordinary gate runs instead of a bespoke experiment.
