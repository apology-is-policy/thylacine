...
- `sched_runnable_count()` aggregates runnable WORK across all CPUs — diagnostic-only; not a hot path; consulted by no scheduling decision. It EXCLUDES `SCHED_BAND_IDLE`: the per-CPU idle threads are always-runnable infrastructure (each secondary's idle lives in `run_tree[BAND_IDLE]` per the "always re-inserted on yield" note above; the boot CPU's is off-tree), not pending work. Counting them reported a phantom backlog on an idle multi-CPU system and made the in-kernel `sched_runnable_count()==0` quiescence assertions race a secondary idle thread that — under host load — was in-tree (not the running thread) at the check instant. That was the #857 "smp8 cons.* flake": never `console_mgr`, never a kernel fault, just a benign idle thread miscounted as work. `sched_runnable_count_band(b)` is the unfiltered per-band query (still counts IDLE). Real work (`BAND_INTERACTIVE`/`BAND_NORMAL`) is always counted, so a genuinely stranded work thread is never masked. Regression: `scheduler.runnable_count_excludes_idle`.

### Per-CPU idle thread

Two flavors at v1.0 P2-Cd:
- **CPU 0 (boot)**: `g_bootcpu_idle`, a dedicated `Thread` created by `thread_create` in `boot_main` (registered via `sched_set_bootcpu_idle`). It owns a **real kstack** — cpu 0's boot stack belongs to `kthread` (which after bringup blocks forever in `joey_run`/`wait_pid`), so cpu 0's idle cannot be a bare-boot-stack thread the way the secondaries' are. It is dispatched **only** by the deadlock-path fallback in `sched()` (taken on cpu 0 when the current thread blocks and no peer is runnable); on yield it goes RUNNABLE but stays **off the run tree**, and the deadlock path re-selects it.
  - **#860 invariant (load-bearing, not cosmetic): `g_bootcpu_idle` MUST stay off-tree AND unstealable.** Because it owns a real kstack it would pass `try_steal`'s sole gate (`kstack_base != NULL`) — so a tree entry makes it stealable by a secondary while cpu 0's deadlock path concurrently re-picks it, running one thread on one kstack/ctx across two CPUs → cross-CPU context corruption (wild PC / smashed stack; surfaced as an intermittent UBSan-smp4 crash in the post-test production bringup, a true residual of the #788/#801 saga). Enforced three ways: the yield path never inserts it (`sched.c`), `try_steal` skips it explicitly, and `insert_sorted` carries a permanent `extinction` tripwire if it ever reaches a tree (the durable regression guard — the timing race itself can't be made deterministic, but its *precondition* — the tree insertion — is, and the tripwire fires on that).
- **CPU N (secondary)**: a fresh `Thread` allocated by `thread_init_per_cpu_idle(idx)` in `per_cpu_main`. **`kstack_base == NULL`** — it runs on the per-CPU boot stack assigned by `secondary_entry` — so `try_steal` correctly skips it (CPU-pinned). Lives in `SCHED_BAND_IDLE` so it sorts below any NORMAL-band runnable thread. (This `kstack_base==NULL` unstealability is the contract `g_bootcpu_idle` structurally cannot share, hence the explicit #860 guards above.)
