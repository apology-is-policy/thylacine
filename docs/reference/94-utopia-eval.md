...
**Status:** U-6a..U-6d-b LANDED (evaluator scaffold + value model + expression eval; statement eval + control flow; external command spawn; multi-element pipeline + pipefail; I/O redirection `<` `>` `>>` heredoc). Built-ins + glob argv expansion (U-6e, needs the native-argv runtime), substitution/process-sub (U-6f), and the poll() main loop (U-6g) follow.
