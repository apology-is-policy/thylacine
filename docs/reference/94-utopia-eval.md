...
8. **`Repl::run_script` script mode (D2)** -- `run_script("/s.ut",
   ["foo","bar"], "exit 9")` binds the positionals (names `0`/`1`/`2`/`*`)
   at the global scope, switches the Env to script mode (`interactive =
   false` -> fail-fast, §8.9), and propagates `exit 9`; a clean script
   (`let a = 1`) returns 0 with state intact. Proves the `ut SCRIPT`
   evaluator path in QEMU (§13).
...
| ~~Stdio inheritance: replace v1.0 Piped-then-drop with `Stdio::Inherit`~~ external **stdout/stderr** now inherit the console when the shell holds one (`env.stdio_inherit`, set by `ut::main` via an fd-1 probe) -- LANDED **LS-2**. stdin + termios (ECHO, raw/cooked) | LS-2 (stdout/stderr) / LS-8 U-PTY (stdin + termios) |
...
|---|---|---|
| **GO6** | *(this chunk)* | **Stage 6 `$path` extension**: `resolve_command`'s static search list gains `/goroot/bin` as the LAST entry (`["/bin/", "/", "/goroot/bin/"]`) so the Go toolchain shipped in the default image (`go`, `gofmt`) resolves bare at the prompt. `/bin` stays authoritative; reachability remains namespace-governed (the existence probe only finds what the Territory exposes); the user-settable `$path` stays the recorded v1.x seam. `Repl::install_completion` scans the same list so a resolvable command is a completable one (`/goroot/bin` absent on a non-bake image degrades silently). login seeds `/env/PATH=/bin:/goroot/bin` to MIRROR this list for programs doing their own `$PATH` lookup (os/exec, `which`) -- the two lists must not drift. PURE USERSPACE; proven by `tools/interactive/go6.exp` (`which go` + bare `go build`). |
| **D2** | `675a7d8` | **`#!` shebang + `ut SCRIPT` script mode** (§13; UX-polish batch). The five external-spawn sites collapse to one `build_command(argv)` chokepoint that resolves `argv[0]` through `$path` and -- via `peek_shebang` + the pure, host-tested `parse_shebang_line` -- expands a `#!interp [arg]` line into `[resolve(interp), arg?, prog, args...]` (kernel exec stays ELF-only; `#!` is shell-side, Plan 9 lineage). A non-`#!`/unreadable head passes through unchanged (no recursion; the kernel ELF-loads or 127s). `Repl::run_script(arg0, args, src)` runs a file non-interactively: binds positionals (names `0`/`1`/`2`/`*`) at the global scope, sets script mode (`interactive=false` -> §8.9 fail-fast), `eval_source`s the whole file, returns the exit code (`exit N` wins, else last `$status`); no banner. The `ut` binary's `parse_script` selects the first non-flag operand as the script. **PURE USERSPACE** (libutopia eval + repl + the `ut` binary; kernel byte-identical) -> no SMP gate, not a formal-audit-trigger surface (self-reviewed: panic-free `build_command` fallback, bounded 128-byte peek, every File dropped, no recursion on the interpreter). The `$N`/`$*` *reference* syntax is a separate language item (#138). Verified: `parse_shebang_line`+`trim_ascii_ws` host units (7/7), `Repl::run_script` host units, `/u-6-test` flow 8 in QEMU, **880/880 + boot OK + 0 EXTINCTION**. |
| **LS-2** | `8d3c13b` | **External command output reaches the console** (Life Support arc; `docs/LIFE-SUPPORT.md`). A session `ut` DOES hold a terminal-backed fd 0/1/2 (login spawns it with `Stdio::Inherit` over the SYS_CONSOLE_OPEN handle), so the U-6c "Piped-then-drop" convention -- chosen when no fd was ever terminal-backed -- silently DROPPED every external's stdout/stderr (`echo`/`cat`/coreutils ran but you saw nothing). New `Env::stdio_inherit` (default **false**), set by `ut::main` via `io::stdout_is_live()` (a zero-length `SYS_WRITE` to fd 1 -- the kernel validates the handle even for len 0; cheaper + more universal than `fstat`, which the console Dev lacks `stat_native` for). When true, the non-redirected stdout/stderr slots in `exec_external` / `exec_external_redirected` / `spawn_pipeline_elements` (the last element's outer stdout + every element's stderr) + `capture_external_pipeline`'s stderr use `Stdio::Inherit` (via the `out_stdio` helper) instead of Piped-drop -- the kernel installs the shell's fd 1/2 into the child and its output reaches the terminal. **Conditional is load-bearing**: every fd-less boot-check / test harness (`t_spawn`, empty handle table) keeps the default false -> Piped-drop -> unchanged (Inherit would make `SYS_SPAWN_FULL_ARGV` reject the spawn). **stdin stays Piped-drop** in both modes at LS-2 (a foreground external does not yet read the console; the shell's wait loop owns fd 0 -- interactive child stdin is LS-5/LS-8). Command substitution `$(cmd)` keeps its captured stdout Piped (the shell reads it); only its stderr inherits (bash-correct: `$(cmd)` captures stdout, stderr goes to the terminal). The `drop(child.std*.take())` epilogues stay correct under Inherit (`resolve_stdio(Inherit)` retains no parent end -> take() is None -> the drop is a no-op). **PURE USERSPACE** (libthyla-rs io + libutopia eval + `ut`; kernel byte-identical) -> no SMP gate, not a formal-audit-trigger surface. Verified: `make test-tcg` x2 (all boot self-test harnesses -- `coreutil-smoke`/`u-test`/`u-6-test`/`u-7-test`/`u-subst-test`, all `stdio_inherit=false` -- green, **0 regression** + login E2E + `Thylacine boot OK`) + an **expect/PTY interactive proof**: log in as `michael`, `echo MARKER` shows MARKER as command output (not just the line-editor echo), `echo X | cat` shows the pipeline tail's output, and `cat`'s error on a denied file is now VISIBLE (pre-LS-2 it was dropped). Durable interactive regression net = **LS-CI** (next). |
...
## 13. External resolution + `#!` shebang + script mode (D2)

### 13.1 `build_command` — the single external-spawn chokepoint

Every external-command spawn site (foreground `exec_external`, the raw-mode
TUI path `exec_external_raw`, `exec_external_redirected`, a pipeline element,
and a background pipeline -- five sites) routes through `build_command(argv)`,
which resolves `argv[0]` and expands a `#!` interpreter line into the
`libthyla_rs::process::Command` to spawn. Centralizing it means a script runs
identically in every position (`./s.ut`, `./s.ut | grep x`, `./s.ut > out`,
`./s.ut &`).

### 13.2 `#!` shebang (Plan 9 lineage, shell-side)

The kernel exec loads ELF only (ARCH §9.6.8); `#!` is a **shell-side**
convention, kept out of the kernel exactly as Plan 9 keeps it out of its exec.
`prepare_argv` resolves `argv[0]` through `$path` (`/bin`), then `peek_shebang`
reads the first 128 bytes of the resolved program and `parse_shebang_line`
(pure, host-tested) recognizes `#!interp [arg]`:

- A match rewrites the argv to `[resolve($path, interp), arg?, prog, args...]`
  -- the interpreter (itself `$path`-resolved, so `#!ut` works as well as
  `#!/bin/ut`) runs with the script path as its argument. At most ONE argument
  after the interpreter is recognized (the Linux/BSD convention): the trimmed
  remainder of the first line verbatim, no further word-splitting.
- A non-match -- an ELF (`\x7fELF`), an unreadable file, or any non-`#!` head --
  passes through unchanged; the kernel then loads it (or fails, status 127) as
  before. The shell never re-peeks the interpreter, so there is no recursion
  (a non-ELF interpreter just fails the kernel ELF load).

The cost is one extra `open`+`read`+`close` of the program file per external
spawn (negligible beside `exec`). Reading the script needs the caller's read
permission; the kernel still gates the actual exec on the `OEXEC` X-bit -- so a
script needs both R (to peek) and X (to exec), matching Unix.

### 13.3 `ut SCRIPT` script mode -- `Repl::run_script`

`ut SCRIPT [args...]` runs a file non-interactively (the `ut` binary's
`parse_script` selects the first non-flag operand as the script; a `#!/bin/ut`
spawn invokes `ut <script> <args>` with no flags). `Repl::run_script(arg0,
args, src)`:

- binds the positional parameters (the names `0`/`1`/`2`/`*`) at the script's
  global scope, mirroring `invoke_function` (the `$N` / `$*` *reference* syntax
  is a separate Utopia language item -- the lexer's var-name-start is
  `[a-zA-Z_]`, so a digit cannot begin a `$`-reference today; functions use
  named args -- tracked as a follow-up);
- switches the Env to **script mode** (`interactive = false`) so a non-zero
  `$status` fail-fast-propagates (scripture §8.9), unlike the interactive REPL;
- evaluates the whole source via `eval_source` (a multi-statement parse), with
  no line editor / prompt / notes loop -- a script reads no fd 0;
- returns the exit code: an explicit `exit N` wins, else the last statement's
  `$status`; a parse/eval error is reported to the UART and yields a non-zero
  code. No banner is printed (a script is not an interactive session).

Validation: `parse_shebang_line` + `trim_ascii_ws` host unit tests (the pure
parsing); `Repl::run_script` host tests + the `/u-6-test` flow 8 (§8.4, in
QEMU); the interactive `./script.ut` is the user-facing E2E.
