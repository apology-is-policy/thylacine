...
- **Primitive inventory** (the "added on demand" clause realized): `load_u8` (R12; SYS_PUTS), `store_u8` (P5-fd-rw), `load_u32` / `store_u32` (P6 torpor + clear-child-tid), and since CF-3 A the BULK pair `uaccess_copy_out(user_va, src, len)` / `uaccess_copy_in(dst, user_va, len)` — the u64-stride copies the byte-I/O handlers stage through. Shape: byte head until the USER-side VA is 8-aligned, 8-byte body, byte tail; the kernel-side unaligned partner access is legal (Normal memory, `SCTLR_EL1.A = 0` per `INIT_SCTLR_EL1_MMU_OFF = 0x30D00800`). Each has THREE fault-prone instructions (head/body/tail) = three `.uaccess_fixup` entries sharing one fault label; the faulting instructions use NO writeback, so a demand-page resume re-executes them with unchanged registers. Mid-copy fault semantics: copy_out may leave a partial prefix in user-VA (the same caveat the per-byte loop had — the caller returns its EFAULT-equivalent); copy_in leaves the kernel buffer's suffix unspecified (callers must not consume it on -1). `uaccess.copy_out_unmapped_faults_all_arms` / `copy_in_...` / `copy_fixup_entries_present` pin all six arms; joey's boot-fatal `cf3-bulk` probe proves the positive path end-to-end (a 40 KiB round-trip: the first write moves the full Twrite payload 32745 in one call, all bytes verified).
...
- **The fixup-table walker is O(table size)**: 10 entries after CF-3 A (4 single-access + 2×3 bulk arms); the linear scan is still trivial and runs only on an actual kernel-mode user-VA fault. If future syscalls grow the table past ~16 entries, consider sorting + binary search (Linux's `__ex_table` is sorted at link time and binary-searched).
