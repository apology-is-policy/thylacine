...
- **Multi-thread Proc + SIG_DFL non-SIGCHLD note: NDFLT refuses; bootstrap falls back to NCONT** (R1-F2 fix). The kernel's R3-F2 gate (sub-chunk 13a) refuses NDFLT in multi-thread Procs because `exits()` would extinct on live peer Threads (cross-thread shootdown is v1.x). Pouch's bootstrap retries with NCONT on NDFLT refusal — the Thread resumes pre-handler context rather than wedging permanently. POSIX-divergent: SIG_DFL for SIGINT in a multi-thread Proc should terminate the whole Proc; pouch's v1.0 behavior is "resume the Thread, signal is silently dropped." v1.x: cross-thread shootdown.
...
- **`raise(SIGKILL)` in a multi-thread Proc returns -1 with errno=EIO** (R1-F9). The kernel's kill-vs-multi-thread gate refuses (cross-thread shootdown is v1.x). The user-visible failure surfaces as a generic EIO rather than the precise ESRCH/EPERM POSIX expects.
