...
### The tx end MUST be non-blocking (CNBFRAME) — the round-B F1 fix (2026-08-18)

The 9P client (`kernel/9p_client.c`) holds `c->lock` **across** `p9_transport_send` (documented at `9p_client.h`: "never held across a blocking wait") and recovers from a full transport by returning `P9_TRANSPORT_EAGAIN` — `client_send_flow` then drops `c->lock` in `client_pump_or_park_locked` and retries. The srvconn ring backend is inherently non-blocking; the **pipe** backend is not: `devpipe_write` sleeps on a full pipe. A blocking write under the held `c->lock` is the `#360` lock-across-sleep **extinction** (an unprivileged multi-threaded container filling the 4096-byte `c2s` with concurrent `/proc` opens reaches `sched()` with a counted lock held → the whole guest dies). This was the diorama channel's exposure — the spoor transport's first production consumer under the shared `p9_client`.

The fix: `p9_spoor_transport_init` sets the **`CNBFRAME`** flag (`spoor.h` bit 6) on the **tx** Spoor. `devpipe_write` then commits the whole buffer or returns `-T_E_AGAIN` having written **nothing** — never partial (a stranded 9P-frame fragment desyncs the shared stream; `do_send` treats a mid-frame EAGAIN as fatal, `#349`), never sleeps. `spoor_transport_send` maps `-T_E_AGAIN` (which equals `P9_TRANSPORT_EAGAIN` = -11, but the mapping is **explicit** so a divergence of either constant cannot silently become -1) to `P9_TRANSPORT_EAGAIN`. Only the tx end is CNBFRAME: the `recv` on rx blocks, but runs with `c->lock` DROPPED (`#841`), so it is sound. Progress is guaranteed — a frame ≤ msize ≤ `PIPE_BUF_SIZE` always fits an empty pipe, and the elected reader drains `s2c`, freeing the diorama to drain `c2s`. Regression: `pipe.cnbframe_atomic_nonblocking`.

**The follow-up round's F1 — the CNBFRAME fix was backend-specific, so the syscall now enforces pipe-only (2026-08-18).** CNBFRAME is honored by `devpipe_write` alone. `p9_spoor_transport_init` sets it unconditionally, but a **non-pipe** tx (a `/srv` byte-conn whose `devsrv_write` → `srvconn_client_send_blocking` tsleeps; a dev9p file whose write is a nested blocking RPC) silently ignores the flag and **blocks** — re-opening the identical `#360` lock-across-sleep extinction. `SYS_ATTACH_9P` accepts any writable Spoor as tx, so the pipe-only precondition is enforced at the **handler**, not the transport: `sys_attach_9p_handler` refuses the attach unless `sys_attach_9p_ends_are_pipes(tx, rx)` (both `dev == &devpipe`). `p9_spoor_transport_init` itself stays **Dev-generic** — it is sound over *any* non-blocking tx, and the transport's own unit tests drive it over a non-blocking linear-buffer mock; the pipe-only constraint is a property of the EL0 boundary (EL0's only sound tx is a real pipe), not of the transport. Kernel-internal callers are trusted to pass a non-blocking tx. Regression: `pipe.attach_9p_admits_pipes_only` (exercises the handler's actual predicate — a pipe pair admitted, any non-pipe / NULL end refused).
