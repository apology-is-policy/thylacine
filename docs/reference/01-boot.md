...
14. **`test_run_all()`** (test harness). Walks `kernel/test/test.c`'s `g_tests[]` array; each test reports PASS/FAIL on UART. `boot_main` extinctions if any test fails. See `docs/reference/09-test-harness.md`. **Gated by `KERNEL_TESTS` (#61):** this step is compiled in only in the dev/CI shape (the default); the production shape (`tools/build.sh --production`, `KERNEL_TESTS=OFF`) drops `kernel/test/*.c` from the link and `#ifdef`-skips this step + the boot-stack probe, printing `tests: DISABLED (KERNEL_TESTS=OFF production build)` instead. The lean shape boots to `Thylacine boot OK` in ~21.5 ms (TCG) vs ~525 ms with the suite. See `TOOLING.md §9` "The production boot shape".
...
- Exception vector table: `arch/arm64/vectors.S` (~150 LOC) — 16-entry table at `_exception_vectors`, page-aligned (0x800), with KERNEL_ENTRY save (288-byte context: 31 GP regs + sp_el0 + elr + spsr + esr + far). KERNEL_EXIT defined for Phase 2's recoverable handlers (unused inline at P1-F).
- `arch/arm64/exception.{h,c}` (~250 LOC). `exception_init()` sets VBAR_EL1. `exception_sync_curr_el` decodes ESR/FAR; routes stack-overflow detection to `extinction("kernel stack overflow", FAR_EL1)`; W^X violation on kernel image to `extinction("PTE violates W^X")`; other sync faults to `extinction` with diagnostic. `exception_unexpected` catch-all for the 15 non-live entries with descriptive vector-name table.
- The deferred fault paths from P1-C-extras Part A (boot-stack guard) and P1-C (W^X PTE constructors) finally close. See `docs/reference/08-exception.md`.

**Not yet implemented**:

- GIC + IRQ-driven UART (P1-G or P1-F-extras). IRQ vector entries currently route to `exception_unexpected`.
- ARM generic timer (P1-G).
- Hardening flags (P1-H).
- Phase 1 exit verification (P1-I).

**Landed**: P1-A at commit `2b332d8`; P1-B at commit `d3e33a8`; P1-C at commit `6462227`; P1-C-extras Part A at commit `ff22ca3`; P1-C-extras Part B at commit `74fd391`; P1-D at commit `198c48c`; P1-E at commit `e867e3b`; P1-F at commit `67a6b16`.
