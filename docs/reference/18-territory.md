...
- **MCREATE**: recorded in the entry's `flags`; names the union member a create lands in (ARCH §9.5 "create in the first writable mount"). **AS-BUILT** (UM-5a): the `STALK_CREATE` amode crosses a union create-parent to the first MCREATE member (`stalk_union_create_member`); a REMOVE (unlink/rename-source) instead targets the member HOLDING the leaf via the `STALK_REMOVE` amode (UM-8c-2, F3 -- see "Union remove" below). `territory.tla::CreateTargetCorrect` / `RemoveTargetCorrect`.
...
The mechanism (`kernel/syscall.c::union_readdir_run`, reached from `spoor_readdir_run` when the open Spoor carries `union_snap`):

- **Tagging**: a `STALK_OPEN` of a `>= 2`-member mount point captures a `Spoor.union_snap` on the opened fd (ref-held, freed at `spoor_free_internal`, NEVER inherited by `spoor_clone`). It holds (a) the OPENED + R-gated members captured once at open (each a `struct union_member {opened, walkable}` -- the `opened` OREAD fid for the per-member readdir, and an UNOPENED `walkable` clone minted BEFORE the open for the dedup Twalk, because a 9P server rejects a Twalk from an opened fid, R2-F1) and (b) the retained mount `point` for fd-relative ops off the handle (F5). The fd's OWN identity stays member[0] (the final cross), so `fstat`/type see member[0]; readdir consults `union_snap->m[]`, the F5 fd-mutation/resolution consumers `union_snap->point`. Set only for a real union (`mount_member_at(_, 1) != NULL`); an O_PATH (STALK_WALK) union base gets a POINT-ONLY snap (`n == 0`, point set, no member opens -- R2-F2).
...
- **Dedup** is a raw `Dev.walk` existence probe into earlier members' UNOPENED `walkable` clones (`stalk_union_has_child`: no perm check, no symlink follow -- name presence only; the walkable fid because a 9P server rejects a Twalk from the opened readdir fid, R2-F1). No persistent seen-set: `union_snap` is IMMUTABLE after the open, so a fork/dup-SHARED union fd needs no lock.
...
The parent of a create resolves via a new **`STALK_CREATE`** amode (Plan 9 `Acreate`, `stalk.h`): identical to `STALK_WALK` (resolve-only, no open, quarry crossed) EXCEPT at a union final quarry, where it crosses to the first `MCREATE` member (`stalk_union_create_member`) instead of member 0. A non-union parent (plain dir or single-member mount) crosses to the mounted root exactly as `STALK_WALK`, so every non-union create is unchanged. `sys_stalk_parent` takes the amode: its create callers (`open_create_try_create` for `SYS_OPEN_CREATE`, and the phenotype openat-O_CREAT/mkdirat path) pass `STALK_CREATE`; its remove caller (`viv_mutation_parent`, the THIRD caller -- the earlier "both create callers" undercounted it, F11) passes `STALK_REMOVE`.
...
### Union remove: the holder member (UM-8c-2, F3, AS-BUILT)

A remove (unlink / rmdir / rename SOURCE) on a union acts on the member that HOLDS the entry, NOT the MCREATE member a create picks. The parent resolves via the **`STALK_REMOVE`** amode: identical to `STALK_WALK` EXCEPT at a union final quarry, where it returns the mount point UNCROSSED (like `STALK_MOUNT`). The caller then selects the member whose `Dev.walk` FINDS the leaf, first-hit (`stalk_union_member_holding`), else `-T_E_NOENT`. `territory.tla::RemoveTargetCorrect` pins the selection rule (`RemoveSel = FirstHolder`). A union RENAME is within the source member (Plan 9 -- a 9P `Trenameat` is one server): when old and new name the SAME union point, the destination lands in the member holding the source (`spoor_same_mount_identity`), not the MCREATE member; a genuine cross-member move falls to `spoor_rename_in_dirs`'s same-Dev guard (EXDEV -> EINVAL). Tested: `test_stalk_union_member_holding`, `test_stalk_union_remove_uncrossed`.

### Union off an fd base (UM-8c-3 / UM-8c-4, F5 / R2-F2 / R2-F3, AS-BUILT)

A handle to a union directory holds member[0] plus the `union_snap` (whose `point` is the retained mount point). Fd-relative resolution and fd-based mutation off that handle reach EVERY member via the point, not just member[0]:

- **fd-relative walk/open** (`stalk` with the union handle as base): the base cross sets `union_base` to `base->union_snap->point`, so the first component runs the union branch (`stalk_union_child`).
- **fd-based unlink/rename/create** (`sys_unlink_handler` / `sys_rename_handler` / `sys_walk_create_handler` -> `sys_union_dirfd_member`): route the dirfd through `union_snap->point` -- the holder of the leaf (remove) or the first MCREATE member (create).
- **O_PATH base (R2-F2)**: native `fs::` opens its mutation parent `T_OPATH` (STALK_WALK), which carries a POINT-ONLY snap (no member opens), so every consumer keying on `union_snap->point` reaches the union off it.
- **zero-component base (R2-F3)**: a resolution off a union base whose path nets to zero components (`.`, or a bare-leaf create's `.` parent) clones the POINT (not member[0]), so the final-quarry dispatch keeps the union (create -> MCREATE, not member[0]).

Tested: `test_stalk_union_fd_base`, `test_stalk_union_opath_base`, `test_stalk_union_zero_component`. **Known limitations (TRACKED)**: the single-hop `SYS_WALK_OPEN` is union-blind (R2-F5, pre-existing; native nav uses `SYS_OPEN`), and a union grafted at a mounted-ROOT identity (mount-over-mount) is first-hop-only (R2-F6, pre-existing).
...
| Mount-union readdir (merge + dedup first-member-wins) | **Landed (UM-5, R2-F1)** -- `union_readdir_run` + `Spoor.union_snap` ({opened, walkable} pairs); `territory.tla::ReaddirDedupFirstWins` |
...
| Mount-union remove (holder member) | **Landed (UM-8c-2)** -- `STALK_REMOVE` + `stalk_union_member_holding`; `territory.tla::RemoveTargetCorrect` |
| Mount-union off an fd base (walk / mutation / create) | **Landed (UM-8c-3/4)** -- `union_snap.point` + point-only O_PATH snap; F5 / R2-F2 / R2-F3 |
