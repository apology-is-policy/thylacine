...
10. **SYS_DMA_MAP + SYS_MMIO_MAP accept vaddr ≥ 2^47 that later extincts the kernel on first access (R12-DMA F180 + R10 F158 cross-reference)**. A CAP_HW_CREATE caller passing `vaddr = 2^47` (or anywhere in the bit-47-set TTBR0 range) reaches `burrow_map`'s existing overflow check (`vaddr + length` doesn't wrap), the VMA installs successfully, and the syscall returns the PA cleanly. First userspace access → page fault → `mmu_install_user_pte`'s `if (vaddr >> 47) return -1;` (R10 F158 close) → `FAULT_UNHANDLED_USER` → `extinction_with_addr("EL0 fault: no VMA covers vaddr / permission denied", ...)`. **R10 F158 chose MMU-layer reject** rather than syscall-layer reject; R12-DMA F180 inherits the same shape for SYS_DMA_MAP. The proper fix lifts the bound to `burrow_map` (a single `if (vaddr + length > (1ULL << 47)) return -1;` benefits MMIO + DMA + anon Burrows uniformly) — deferred to a separate R12-vaddr chunk so the unification is visible and auditable. Phase 5+ note delivery converts the remaining cases of extinction-on-bad-userspace-input into SIGSEGV-equivalent. At v1.0 the CAP_HW_CREATE gating means only trusted procs can reach this path; the symptom is loud (kernel extinction) rather than silent (memory corruption / DOS).
