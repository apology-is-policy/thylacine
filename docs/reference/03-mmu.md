...
**Cross-CPU concurrent demote — the residual that motivated #808.** The IRQ-mask above closes only the *same-CPU* race. A *peer* CPU that dereferences any direct-map PA inside the 1 GiB / 2 MiB block while this CPU is mid-demote takes the identical translation fault, and a per-CPU IRQ mask cannot prevent it — reachable on the **default `-smp 4` + work-stealing** harness (concurrent `rfork` → concurrent `directmap_walk_*` demotes; any peer's running-kstack / `current_thread` / `struct page` deref into the block being demoted faults). A global `mmu_lock` alone does **not** fix it — the faulting peer holds no lock. The audit (F1) corrected an earlier "demote is single-CPU at v1.0" claim: this was reachable today, not future-only.

### Direct-map page-mapped from boot — no runtime block split (#808)

**#808 eliminates the runtime demote entirely**, closing *both* the same-CPU IRQ-in-window race and its cross-CPU sibling **by construction**. `mmu_pagemap_directmap(base, end)` (in `mmu.c`) is driven once from `phys_init` — **single-CPU, IRQs masked at PSTATE** (`boot_main` has not yet run `msr daifclr, #2`), with the buddy allocator live (it allocates the L2/L3 tables) and **before the first `thread_create`**. It forces every 2 MiB block of the buddy zone (`l1_directmap[1..8]`) through `directmap_walk_to_l3`, so afterwards **every** `l1_directmap[1..8]` entry is a table descriptor and **every** 2 MiB block within is an L3 table. The boot pass's own BBMs are safe by construction: no concurrent CPU, no IRQ handler can observe the transient unmap, and the running code + boot stack live in the kernel-image 2 MiB blocks (already L3 from `build_page_tables`, never re-BBM'd).

**Invariant (post-boot):** the buddy direct map is page-mapped at L3 granularity, so the runtime kstack-guard path (`mmu_set_no_access_range` / `mmu_restore_normal_range` → `directmap_walk_to_l3`) only flips already-present L3 leaf entries and **never** does a block→table break-before-make. The `mmu.c` IRQ-mask in `directmap_walk_to_l2` / `_l3` therefore becomes **belt-and-suspenders** — dormant for the kstack path, retained as defense for any hypothetical demote outside the pre-mapped zone (none exists at v1.0). The boot pass + the runtime fast path are exercised by `directmap.pagemapped_808` (sweeps every zone block, asserts page-mapped).

This is the Linux-arm64 move — it page-maps the linear map (no block mappings) so per-page linear-map permission changes (`rodata_full` / `DEBUG_PAGEALLOC` / KFENCE) never need a runtime block split.

**Rejected alternative (audit F2): non-BBM, OA-preserving refinement.** Because the demote reproduces the same output address + attributes, the `= 0` break looks architecturally unnecessary — but installing a table descriptor over a block descriptor without the intervening invalidate is only safe with **FEAT_BBM level 2** (ARMv8.4+). The RPi 400 (Cortex-A72, ARMv8.0; Lazarus's first board) lacks it → TLB-conflict UB. The page-map approach is portable (no FEAT_BBM dependency), the most verifiable, and the one chosen (user vote 2026-05-31). The other rejected options were a segregated page-mapped kstack arena (new allocator) and a cross-CPU stop-the-world quiesce (new SMP coordination + deadlock-freedom proof).
