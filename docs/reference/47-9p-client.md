...
`client_run` and `p9_client_clunk_async` are the pump/park EAGAIN-aware callers. The two `Tflush` sends (the DIED path + `abandon_async`) are EAGAIN-aware **without** pumping (#53): a dying thread must not park and an abandoner is in a teardown, so on EAGAIN they roll the flush back (`p9_session_flush_rollback`: the never-sent flush tag is freed, the victim's `awaiting_flush` clears) and fall to the pre-#845 ownerless reclaim — the victim tag stays ACTIVE until its late original reply drains ownerlessly or session teardown reclaims it; only a genuine (non-EAGAIN) break latches the session. Pre-#53 a single EAGAIN'd flush latched the **whole shared session** dead — the #349 collapse on a different send path. The async `p9_client_submit_async` (the Loom path) keeps its fail-close (the pre-existing, tracked **#350** seam).

### Never-sent aborts (#52): the tag is reclaimed, the session stays live

`client_send_flow` distinguishes **never-sent** exits (`CLIENT_SEND_NEVER`: a self-dying sender refusing to park; the session observed already dead; a spill-OOM) from a **genuine break** (`-P9_E_IO`): on every never-sent exit the transport's all-or-nothing contract means **zero bytes** of the frame reached the wire, so the server has never seen the tag and `p9_session_abort_unsent` reclaims it immediately — I-10-safe by construction (no late reply can arrive for a request never sent). The same reclaim covers the reply-buffer-OOM path in `client_run` (the op is marked outstanding by `session_send_*` before the buffer alloc). Pre-#52 each such abort leaked one of the 64 `outstanding[]` slots on a LIVE shared session (64 accumulated → `alloc_tag` fails → every mount resolving through the client wedges `-P9_E_IO`), and the spill-OOM latched the whole session dead. `p9_client_clunk_async` captures its tag **before** `client_send_flow` (the #375 discipline — `out_buf` is not re-readable after a park). Regressions: `9p_session.abort_unsent_reclaims_tag`, `9p_session.flush_rollback_restores_victim`, `9p_client.abandon_async_eagain_keeps_session_alive` (revert-probed).
