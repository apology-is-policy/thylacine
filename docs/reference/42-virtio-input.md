...
- **P4-K-events** (this chunk): adds event consumption via `t_irq_create` + bounded busy-poll on `used.idx`. The kernel test pre-fires SPI 77 before spawning the child (P4-Ic-latency / P4-Ic5-IRQ-probe pre-pend pattern); the child consumes that one wake via a single `t_irq_wait`, prints the `AWAITING_QMP_KEY` sentinel, then transitions to a bounded busy-poll. `tools/test.sh` spawns a QMP injector (`tools/qmp-inject-key.py`, launched at QEMU start; #362) that connects to QMP during boot, tail-follows the boot log for the sentinel, and issues `send-key` with qcode `a` within ~25 ms of it; QEMU's virtio-keyboard-device fills the eventq with `{type=EV_KEY, code=KEY_A=30, value=1}` (press) + SYN + `{value=0}` (release) + SYN; the driver drains, validates, recycles descriptors, and exits PASS. Without QMP injection (`tools/run-vm.sh` interactive run, or `THYLACINE_INPUT_INJECT=0`), the busy-poll runs out its wall-clock budget (`POLL_BUDGET_MS` = 3 s via CNTVCT_EL0, with an iteration backstop per the #188 discipline; #362 — an iteration-count cap is substrate-speed-dependent and under-sized the window on HVF exactly when the injector was slowest) and exits SKIP — boot continues either way.
...
```
   ...
   print "AWAITING_QMP_KEY" sentinel
               ↓
   busy-poll on used.idx (POLL_BUDGET_MS = 3 s wall-clock; iteration backstop)
        ┌─────────────┴─────────────────────────────────┐
        ↓                                               ↓
   used.idx advanced                          poll budget reached
        ↓                                               ↓
...
CI verification (P4-K-events; injector reworked by #362): `tools/test.sh` spawns `tools/qmp-inject-key.py` at QEMU launch. The injector connects to QEMU's QMP Unix socket (`build/qmp.sock`) and negotiates `qmp_capabilities` DURING boot (paying interpreter startup + connect outside the delivery window), then tail-follows the boot log for the `virtio-input: AWAITING_QMP_KEY` sentinel and issues `send-key` with qcode `"a"` within ~25 ms of it (one re-send if the success marker lags 2 s). The pre-#362 shape (a bash grep-poll spawning python3 only on sentinel match) paid its whole startup inside the guest's poll window and missed delivery in 23/40 ci-smp-gate boots. QEMU's virtio-keyboard-device translates that into 4 eventq writes (KEY press + SYN + KEY release + SYN); the driver drains, validates `{type=EV_KEY, code=KEY_A, value=1}`, and exits PASS. After boot completes, `tools/test.sh` greps the log for `virtio-input: saw target key` — absence (when injection was expected) fails the run. Disabled via `THYLACINE_INPUT_INJECT=0` or `THYLACINE_NO_QMP=1`.
...
- The bounded busy-poll on `used.idx`: in the CI path (P4-K-events), QMP injection lands ~25-100 ms after the sentinel print (#362 injector), so the driver exits PASS almost immediately. In the interactive (no-injection) path, the poll runs the full `POLL_BUDGET_MS` (3 s wall-clock, substrate-independent; #362 — previously a 200M-iteration cap whose wall-clock meaning varied ~10-50x between TCG and HVF).
...
- **P4-K-events (landed)**: full event consumption pipeline. New mechanisms: (1) kernel test pre-fires SPI 77 via `gic_set_pending_spi` before rfork (interactive-mode forward-progress guarantee — child consumes a guaranteed first wake then SKIPs after the poll cap rather than hanging indefinitely); (2) driver adds `t_irq_create` + a single `t_irq_wait` (consumes the pre-fire wake) + bounded busy-poll (`MAX_POLL_ITER = 200M iterations`); (3) used-ring drain helper that parses 8-byte event records, validates `(type=EV_KEY, code=KEY_A=30, value=1)` as the target, and recycles descriptors via the standard avail-ring re-publish pattern with DSB-fenced idx bump; (4) `tools/run-vm.sh` adds `-qmp unix:build/qmp.sock,server,nowait`; (5) `tools/test.sh` spawns a background injector that polls the log for the `AWAITING_QMP_KEY` sentinel and issues `send-key` for qcode `"a"` via QMP; (6) post-boot grep enforces that `virtio-input: saw target key` appears when injection was expected (otherwise the run fails). **Does NOT close ROADMAP §6.2** `Userspace virtio-input: keyboard input from VirtIO console reaches user processes via /dev/cons` — that exit criterion requires a `/dev/cons` 9P surface bridging the userspace driver's eventq output to other processes (Phase 5+ or P4-Id). What this chunk DOES close: the event-injection-to-userspace-consumption end-to-end pipeline (CI-visible PASS). Binary 10944 → 12536 B (within 16-KiB blob cap). 243/243 tests PASS × default + UBSan; test count unchanged.
- **P4-Z (this chunk; landed)**: cumulative driver-model audit close. R14 prosecutor pass surfaced F217 (P1) — missing VIRTIO 1.2 §2.7.13.2 LoadLoad barrier between reading `used.idx` and the subsequent `used.ring[k]` / event-pool reads. The hazard would surface on out-of-order ARM Cortex-A cores (v1.0 deployment target) as a phantom-EV_SYN classification of the genuine key event (the speculative read returns the populate_eventq-zeroed pre-advance state). Fix: `libthyla_rs::virtio_rmb()` (`dmb ishld`) inserted in `drain_used` immediately after `read16(used_va + 2)`. Also closed: F220 (P2) — `avail_idx_at_entry` capture replaces the read-from-self gate (cleaner intent + multi-thread-safe); F223 (P3) — pre-fire `t_irq_wait` return is now warned-on rather than silently discarded; F226 (P3) — `used.elem.len` is now validated against `VIRTIO_INPUT_EVENT_LEN`; F224 (P3) — doc comment on intentional discard. Binary 12536 → 12936 B (within 16-KiB blob cap). 243/243 tests PASS × default + UBSan; test count unchanged.
- **#362 (this chunk; landed)**: wall-clock poll budget + injector rework. The SKIP budget moved from a fixed iteration cap (`MAX_POLL_ITER = 200M`) to `POLL_BUDGET_MS = 3 s` via CNTVCT_EL0/CNTFRQ_EL0 (EL0 counter reads; irq-bench idiom) with a 4G-iteration unconditional backstop (the #188 discipline: a frozen counter must not hang the probe) and a `CNTFRQ == 0` fallback to the old pure-iteration cap. Root cause: an iteration cap's wall-clock meaning varies ~10-50x between TCG and HVF-native, so on HVF the window closed in well under a second — exactly when the host-side injector was slowest (ci-smp-gate load: 4-8 vCPU threads starving host-side helpers; 23/40 gate boots missed delivery, all ground-truthed green guests). Paired harness fixes: `tools/qmp-inject-key.py` (single process spawned at QEMU launch; QMP connect + capabilities paid during boot; ~25 ms sentinel-to-send; one re-send if the success marker lags) and the `tools/smp-multiboot.sh` classifier (inject-miss gets its own green-guest-proven category; `TIMING_RE` anchored on emitted `[SOFT-WARN]` strings, never test names — the old `stalk.*lifetime` entry matched a passing test's NAME line, making TIMING a catch-all).
