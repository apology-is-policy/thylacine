...
- **Phase 3 — in progress.**
  - **T1 / #117 — LANDED (2026-06-14):** the Kaua ANSI terminal-size handshake. A new `kaua::query` module: the pure, host-tested `parse_cpr` (the CPR reply `ESC[<rows>;<cols>R` → `(cols, rows)`) + the backend-gated `terminal_size(timeout_ms)` (writes `ESC[s ESC[9999;9999H ESC[6n` to fd 1, reads the reply from fd 0 within the deadline, restores the cursor). nora measures the console at launch and fills it, falling back to 80×24 when the terminal does not answer (dumb terminal / the non-interactive harness). I-27-safe (fd 0/1 only, never consctl, never console-attach; the read stops at `R` so a launch-window keystroke stays for `PollSource`; fixed `[u8; 32]` staging; #811 death-interruptible) → a focused self-audit + the I-27-adjacent discipline applies. Verified: kaua host tests 63→68, device build clean, 880/880, boot OK, 0 EXTINCTION, login→ut session clean.
