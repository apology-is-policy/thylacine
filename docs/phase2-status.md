...
17. **P2-Eb: kernel/namespace.c impl**. `Pgrp` struct (refcount + mount table), `bind`/`unmount` syscalls, cycle-detection (DFS over existing bind graph), `pgrp_clone` for rfork(RFPROC). `struct Proc` extended with `struct Pgrp *pgrp`. New tests: `namespace.bind_smoke`, `namespace.cycle_rejected`, `namespace.fork_isolated`. Mount union semantics (MBEFORE/MAFTER/MREPL) deferred to Phase 5+. `mount` (9P-server-attaching variant) deferred to Phase 4 (9P client landing).
