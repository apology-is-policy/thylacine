...
8. ⚠️ **P2-Cd: per-CPU run trees + idle threads (Cda+Cdb landed; Cdc deferred)**. Per-CPU CpuSched + idle Thread descriptors landed; sched()/ready()/sched_tick all index by smp_cpu_idx_self. **P2-Cdc** (GIC SGI + IPI types + IPI handlers + per-CPU GIC redistributor init) is the next sub-chunk — currently secondaries WFI in pure park mode without IRQ wake source. `IPI_RESCHED`, `IPI_TLB_FLUSH`, `IPI_HALT`, `IPI_GENERIC` per ARCH §20.4.
