...
6. **P2-Cb: per-CPU init (next)**. Refactor PAC init in primary's start.S to populate globals; secondary trampoline initializes its own PAC keys + MMU + VBAR_EL1 + TPIDR_EL1 (idle thread); reaches per_cpu_main C function which runs sched_init for that CPU and enters idle. Per-CPU run trees (g_run_tree → per-CPU array indexed by MPIDR/cpu_idx). `smp_cpu_idx()` accessor.
