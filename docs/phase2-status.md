...
|---|---|---|
| `f98b602` | **P2-Eb**: kernel namespace impl. New `kernel/include/thylacine/pgrp.h` + `kernel/namespace.c` (~150 LOC). struct Pgrp (80 bytes; PGRP_MAX_BINDS=8) with bind/unmount + alloc/clone/ref/unref. Cycle-detection in bind matches spec's `WouldCreateCycle` exactly: fixed-point reachability over existing edges; rejects if `dst` is already reachable from `src`. struct Proc grew 80→88 bytes with the pgrp pointer. main.c bootstrap order: slub_init → pgrp_init → proc_init → thread_init. rfork(RFPROC) calls pgrp_clone(parent->pgrp) for child; proc_free calls pgrp_unref. PGRP_MAX_BINDS deliberately kept small (8) to keep the struct under 80 bytes — SLUB's KP_ZERO byte-loop on a larger struct inflated rfork_stress_1000 boot time past the 500 ms budget under QEMU. 3 new tests: namespace.bind_smoke / cycle_rejected / fork_isolated. New reference doc 18-namespace.md. Phase tag P2-Ea → P2-Eb. | 28/28 tests (3 new); ~270 ms boot; ~320 ms UBSan; 4/4 fault matrix; KASLR 5/5 distinct; scheduler.tla + namespace.tla both TLC-clean. |
| `49d5e7f` | **P2-Ea**: namespace.tla spec. Spec-only chunk. New `specs/namespace.tla` (~165 LOC TLA+) + `namespace.cfg` + `namespace_buggy.cfg`. Models Plan 9 `bind` as a directed graph; cycle-freedom (ARCH §28 I-3) is the primary state invariant. Isolation (I-1) is structural in the data model. RFNAMEG sharing deferred to Phase 5+. `BuggyBind` skips cycle check; counterexample at depth 4 / 95 states. `Unbind` mirrors `unmount`; `ForkClone` mirrors `rfork(RFPROC)`'s namespace copy. SPEC-TO-CODE.md updated mapping each action + invariant to its (deferred) impl site at P2-Eb. Phase tag P2-Dc → P2-Ea. **Two open follow-ups carried in**: (1) wait_pid on_cpu spin defensive hardening (`9b8761f`); (2) try_steal rotate-start (`81e5f74`). Neither is P2-E-related but both landed in the same session; documented for reference. | 25/25 tests; 4/4 fault matrix; ~265 ms boot (production); ~312 ms UBSan; **scheduler.tla TLC-clean (10188 states); namespace.tla TLC-clean (625 states); namespace_buggy.cfg counterexample at depth 4**. |
...
16. ✅ **P2-Ea: namespace.tla spec**. Landed. Plan 9 `bind` modeled as a directed graph; cycle-freedom (ARCH §28 I-3) the primary invariant. Isolation (I-1) structural. RFNAMEG sharing deferred to Phase 5+. namespace_buggy.cfg produces a 2-bind cycle counterexample.
17. ✅ **P2-Eb: kernel/namespace.c impl**. Landed. `struct Pgrp` (80 bytes; PGRP_MAX_BINDS=8) with bind/unmount + pgrp_alloc/clone/ref/unref. Cycle-detection matches spec's `WouldCreateCycle`. struct Proc grew 80→88 bytes. rfork(RFPROC) clones parent's namespace. 3 new tests + new reference doc 18-namespace.md. Mount union semantics (MBEFORE/MAFTER/MREPL) deferred to Phase 5+. `mount` (9P-server-attaching variant) deferred to Phase 4.
18. **P2-F: handle table + VMO**. `kernel/handle.c` — typed kernel object handles per ARCH §18 (Process / Thread / VMO / MMIO / IRQ / DMA / Chan / Interrupt); rights monotonicity (I-2, I-6); transfer-via-9P invariant (I-4) — placeholder pending Phase 4's 9P client; hardware-handle non-transferability (I-5). `kernel/vmo.c` — VMO manager per ARCH §19; refcount + mapping lifecycle (I-7). New spec `handles.tla` (mandatory; Phase 2 close).
19. **P2-G: ELF loader + minimal init**. `arch/arm64/elf.c` — ELF loader; reject RWX segments per W^X invariant. `init/init-minimal.c` — first userspace process; UART debug shell.
20. **P2-H: Phase 2 closing audit + spec finalization**. Round-4 audit on the entire Phase 2 surface; spec mappings in `specs/SPEC-TO-CODE.md` populated and CI-checked. ROADMAP §5.2 exit criteria all met.
