...
13. **P2-Db: per-thread guard pages + 1000-iter cross-CPU stress**. Closes P2-A trip-hazard #1 (kstack underflow detection). 1000-iteration rfork+exits+wait stress with cross-CPU placement (verifies work-stealing carries new processes across CPUs cleanly). Multi-thread Procs (RFMEM-style — same address space, multiple threads via rfork(RFMEM | RFPROC)) deferred to Phase 5+ with the syscall surface.
14. **P2-E: namespace + bind/mount**. `kernel/namespace.c` — `Pgrp`, `bind`, `mount` (stub: ramfs only at this phase), `unmount`. Namespace cloned on rfork(RFPROC); shared on rfork(RFPROC | RFNAMEG). New spec `namespace.tla`: cycle-freedom (I-3), isolation (I-1).
15. **P2-F: handle table + VMO**. `kernel/handle.c` — typed kernel object handles per ARCH §18 (Process / Thread / VMO / MMIO / IRQ / DMA / Chan / Interrupt); rights monotonicity (I-2, I-6); transfer-via-9P invariant (I-4) — placeholder pending Phase 4's 9P client; hardware-handle non-transferability (I-5). `kernel/vmo.c` — VMO manager per ARCH §19; refcount + mapping lifecycle (I-7). New spec `handles.tla` (mandatory; Phase 2 close).
16. **P2-G: ELF loader + minimal init**. `arch/arm64/elf.c` — ELF loader; reject RWX segments per W^X invariant. `init/init-minimal.c` — first userspace process; UART debug shell.
17. **P2-H: Phase 2 closing audit + spec finalization**. Round-4 audit on the entire Phase 2 surface; spec mappings in `specs/SPEC-TO-CODE.md` populated and CI-checked. ROADMAP §5.2 exit criteria all met.
