...
| **U-2h-hardware** | `t::hardware::{Mmio, Irq, Dma}` typed wrappers (RAII over SYS_MMIO_CREATE/SYS_IRQ_CREATE/SYS_DMA_CREATE; impl AsFd for poll integration; type-system enforces I-5 non-transferability). Migrates 7 virtio-* binaries in the same commit. | U-2c-io, U-2d |
