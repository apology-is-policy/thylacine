...
shape now; the implementation lands in the Venus chunk.** Also required was a fix to `PciDev::claim`'s eager
map-every-BAR policy (§3) — **and that fix already landed, at Warp-2a (#166).** **This sentence read "Also required and currently
broken" until 2026-08-18, while §3 — the section it cross-references — had
recorded the fix as `[FIXED at Warp-2a (#166)]` for weeks.** The document
contradicted itself, and Warp-6 V-0 planned a pull-forward of a fix that was
already in the tree before checking. **A cross-reference pointing AT the
correction is not the same as being corrected**: the pointer text keeps its own
verdict, and a reader who follows the pointer has already believed the pointer.
A "currently broken" note is a claim about the tree and ages exactly like a
status field — nobody's step flips it.

So the V-2 delta is precisely the one §3 already names, and no more:
**discovery is DONE** (the kernel cap walk resolves `cfg_type = 8` into
`t_pci_info.shm[2]`, surfaced as `PciDev::shm_region(shmid) -> (PA, length)`;
`pci.walk_caps_shm` is a passing test), an oversized BAR is claimed-but-unmapped
...
|---|---|---|---|
| **thyla-pi** (RPi 400, KVM) | **`V3D 4.2.14.0` / V3DV Mesa** — real hardware | **guest-observed capset id=4**, gate VERIFIED | the certification host |
| **thyla-gl** (Parallels on the Mac, TCG) | `llvmpipe` / lavapipe — software | **guest-observed capset id=4**, gate VERIFIED | the fast-iteration host |

The pairing mirrors the GL arc: iterate where it is quick, certify where the
silicon is real. **thyla-gl was PROMOTED 2026-08-18** — it had been checked only
to the property/`strings` level, and V-0's remaining half was to boot it. It
does. Both hosts return **byte-identical feature words**
(`lo=0x30000013` without venus, `0x3000001b` with), which is a stronger
cross-host agreement than the arc needed and worth having on record.
...
| **Warp-6** | **Venus — OPENED 2026-08-18; the V-0 gating probe PASSED (§6.2, §9.1).** Hostmem mapping (§6.2), `vn_renderer_thylacine`, blobs — **also owes the §4.5.5 generalization: the blob-mediated capset-neutral blit, extending Warp-C's mechanism without reshaping its model**. Sub-chunks: **V-0** the gating probe — §9.1 is binding, so *nothing structural lands until a host is proven to RUN Venus*, the Warp-C C-0 precedent. **MEASURED on thyla-pi**: the guest enumerates **capset `id=4` (VENUS), max_size 160 B**, and the **control one variable away** — same tree, same fixtures, same boot path, device declared without `venus=on` — enumerates only `id=1` + `id=2`. Both arms `BOOT: PASS` (220 s / 225 s), so the third capset is attributable to the declaration and not to one arm failing differently. No guest change was needed: `probe_capsets` already enumerates to `GPU_CAPSET_ENUM_MAX=8` and logs each `id`. **Both halves CLOSED**: thyla-gl was PROMOTED the same day — its first run (the host's own stale Aug-12 artifacts) HUNG tapestryd under the venus declaration (`Readiness::Timeout`, 3 restarts, then gave up), and two hypotheses for it were killed by measurement rather than argument (the Aug-12 tree carries the identical #166 oversized-BAR skip, dated 2026-06-15 by `git log -S`; and lavapipe enumerates in 248 ms, with `SUBMIT_DEADLINE_MS=500` already bounding the controlq wait). Re-run on the CURRENT build the same host and the same declaration came up clean and VERIFIED — **so the attribution is the stale artifacts, not the host**, though one sample each way across two builds means the old hang is *unexplained*, not *explained*. V-0 also made the device's feature offer a per-boot fact (it was read and discarded): **`CONTEXT_INIT` is offered on a PLAIN `-gl` device with no venus and no blob** — so V-0b's negotiation is not gated on the Venus configuration — while **`RESOURCE_BLOB` appears only with `blob=on`**, which means V-1's blob work cannot be exercised on the default local dev device at all (`virgl=0`), the same shape as #166's inert-hostmem-under-HVF constraint; **V-0b** `CTX_CREATE` with `capset_id=4` — **MEASURED on thyla-pi (KVM, real V3D), committed `bf448929`, round owed before push**: a Venus context CREATES (`ctx-capset id=4 CREATED` with venus; `skipped` without — the negative control, since a device ignoring `context_init` would create it anyway; `id=2` virgl the positive control on both legs). **So the absent render server does NOT block context creation** — virglrenderer's in-process venus init handles it, settling the §6.2/§9.2 question EMPIRICALLY rather than by inference. It is a FEATURE-BIT change, not a field change: `ctx_create` wrote `context_init=0` under "F_CONTEXT_INIT not negotiated", and the device honours the field only when the feature is negotiated — which the driver never offered back, so the naive "pass capset 4" is a false pass (`RESP_OK_NODATA` over an implicitly-virgl context). The venus gate grew the both-directions ctx-capset arms; **V-1** blobs — **AS-BUILT (guest-blob create MEASURED on thyla-pi, KVM, real V3D 4.2)**: `RESOURCE_CREATE_BLOB` (`blob_mem = GUEST`, one guest-page `mem_entry`, `blob_flags = 0`) returns `RESP_OK_NODATA` — F2 sequenced blobs here, and Venus's command ring IS a guest blob (section 2.4). A driver-internal probe negotiates `F_RESOURCE_BLOB`, creates+unrefs a device-global guest blob at init (before any client), and the venus gate's V-1 arm discriminates `blob-create guest CREATED` with venus / `skipped` without. The **guest** blob is the whole V-1 scope: host3d blobs + the hostmem-BAR mapping (`MAP_BLOB`) stay **V-2**, the client blob-BO type + the coherent ring stay **V-3**; **V-2** hostmem — and **scoped DOWN by measurement, 2026-08-18**: discovery is ALREADY BUILT (Warp-2a #166 walks `cfg_type=8` into `t_pci_info.shm[2]`, surfaced as `PciDev::shm_region(shmid)` -> `(PA, length)`, with `pci.walk_caps_shm` passing), and the `PciDev::claim` eager-map-every-BAR policy is ALREADY FIXED there too (an oversized BAR is claimed-but-unmapped, `region()` fails closed). §6.2 called that fix "currently broken" until today while §3 recorded it as fixed — the doc contradicted itself and this ladder briefly planned to re-do it. **What V-2 actually owes is the MAPPING half**: an owner-minted, client-mappable, revocable, budgeted map of a *subrange* of the shm window into a client VA at the host-dictated cache attribute, unmapped before `UNMAP_BLOB`. **Not orderable later: QEMU refuses to realise the device without hostmem**; **V-3** `vn_renderer_thylacine` (~1–1.5 kLOC per §2.3) + the shmem ring (head/tail/status cachelines; §2.4 — no syncobj object model, no timeline primitive, no dma-fence graph is owed). **The seam is already built to receive this**: `WarpCtx.capset` exists in `usr/tapestryd/src/server.rs` with the `capset <id>` ctl verb writing it, documented as *"Recorded at the seam from day one; the device sees them when F_CONTEXT_INIT / per-ring fencing are negotiated (Venus)"* — nothing reads it today (verified: one occurrence outside the declaration and the zero-init), so the client ABI does not change, only the plumbing. **The I-45 obligation rides the SAME change, not the audit**: the instant that field goes live an arbitrary client-declared `u32` reaches `CTX_CREATE` through an unvalidated ctl verb, so rejecting a capset the device never enumerated belongs in V-3 itself — V-6 should be confirming that check, not discovering its absence; **V-4** the §4.5.5 capset-neutral blit, whose obligation Warp-C was built to preserve; **V-5** the Vulkan prover in-guest (the gate); **V-6** the focused audit — **V-2 maps MMIO pages into a client VA, a NEW kernel memory-authority path**, so it is audit-bearing on I-45 and I-32 independently of the rest of the arc | a Vulkan prover in-guest |
