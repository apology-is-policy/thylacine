...
| B-1d | dlopen: PT_INTERP for native execs; `burrow_map_file`; driver `-shared` / PIE / `-dynamic-linker`; `libc.so`; ldso boundary-line. Before B-3. **Four votes 2026-09-24 (row 14 above)**; the scripture commit precedes the code. **B-1d-u** (Plan 9 unions, row 15) lands first. | a Pouch `.so` loaded on the device; the two deny paths |
