...
- Demand-zero-pages anonymous mappings on first touch — **BUILT as the overcommit model (2026-06-23): a dedicated `SYS_BURROW_ATTACH_LAZY` syscall reserves the VA + a sparse `BURROW_TYPE_ANON_LAZY` Burrow and demand-zero-fills + charges each page on first touch; `SYS_BURROW_DECOMMIT` releases pages back. See "The overcommit model" below.** The eager `burrow_create_anon` backing stays for kernel-internal copy-target callers (exec data segments, DMA buffers, the Weft rings) that need pages present at create; the unconditional migration of every `SYS_BURROW_ATTACH` to demand-zero is a recorded follow-up. **That follow-up is TAKEN for the exec half at LINEAGE L-4a (user-voted 2026-08-02; landed)**: exec's stack + every NON-EXECUTABLE segment moved to `burrow_create_anon_lazy` (pre-populating only the `ceil(filesz/PAGE_SIZE)` leading slots; the stack, only the run its argv/auxv frame occupies at the top), because a COW break is per-page and eager anon owns one indivisible buddy block. The gate is NOT-executable rather than writable because the demand-zero arm does no I-cache maintenance (#107's hazard); W^X makes that a superset of writable, so nothing COW must break is left out. **This put exec-image pages on the I-32 page axis** -- `burrow_map_in` charges only the VMA axis, so they were uncharged while eager -- which makes `page_count` track true RSS across exec and makes stack growth failable (gracefully, per Proc) — see the §25.4 "Process creation" row's *L-4 AS-DESIGNED* clause. **DMA buffers and the Weft rings stay eager permanently**, and for a different reason than "pages present at create": they need *physical contiguity* (a device DMAs into a PA range; a Weft ring is registered whole). Exec's segments never needed contiguity — they had it only because `burrow_create_anon` was the one anon constructor when they were written.
...
JIT compilers get the **dual-mapped code Burrow** (`docs/JIT-ON-WX-DESIGN.md`, realized by the "Clade" LLVM arc — `docs/LLVM-DESIGN.md` §8, invariant **I-42**): one anonymous Burrow whose physical pages are mapped **RW at `VA_w`** (the JIT writes) AND **RX at `VA_x`** (it executes), so no PTE is ever W∧X and I-12 holds at page granularity — the kernel's own Lazarus W1.5 LSE alternatives-patcher already does exactly this transient dual-alias of `.text` internally. Emission publishes via an explicit **`SYS_ICACHE_SYNC(range)`** (`dc cvau`/`ic ivau`/`dsb ish`/`isb`), and code-Burrow creation is gated by **`CAP_JIT`** (elevation-only, non-rfork-grantable, the `CAP_HW_CREATE` class). This **supersedes** the earlier post-v1.0 `pkey_alloc`/`pkey_mprotect`-shaped note — two fixed-permission aliases of one region is the design-true W^X answer, not a dynamic W↔X flip of a single page. The first consumer is Mesa's llvmpipe shader JIT (Clade CL-7); the kernel half lands at CL-7k. This is post-v1.0 (the Clade arc, a Phase-8 pole).
...
|---|---|---|---|
| 44 | `SYS_NOTE_OPEN` | (none) | fd of the calling Proc's note Spoor read end |
| 45 | `SYS_NOTIFY` | `handler_va` | 0 / -EINVAL — register the async handler (0 clears) |
| 46 | `SYS_NOTED` | `arg` (0 = NCONT, 1 = NDFLT) | NEVER RETURNS NORMALLY (or -EINVAL if outside a handler) |
| 47 | `SYS_POSTNOTE` | `pid`, `name_va`, `name_len` | 0 / -EINVAL / -EPERM / -ESRCH / -EAGAIN |
| 48 | `SYS_NOTE_MASK` | `new_mask`, `old_mask_out_va` | 0 / -EINVAL |
...
> **RESOLVED 2026-08-03 (task #149); recorded because the SHAPE recurs.**
> Resolution had always worked; the LOAD that followed refused every
> real-world binary. `kernel/exec.c` rejected a `PT_LOAD` whose `p_vaddr` was
> not page-aligned -- but ELF requires only `p_vaddr == p_offset (mod
> p_align)`, and on aarch64 `p_align` is 64 KiB, so ordinary linkers place the
> data segment mid-page (Alpine's `busybox-static` has its at `0x51d2e0`).
> Every binary Thylacine's own toolchain emits is page-aligned, so the
> precondition was universally true until the first FOREIGN ELF was exec'd --
> the #136 shape. A segment now maps from the page-aligned FLOOR of its vaddr
> with its own bytes placed `lead` bytes in, and the leading slack reads ZERO
> (those bytes lie outside `[p_vaddr, p_vaddr+p_memsz)`, so filling them from
> the file would map content no segment claims -- an exposure Linux accepts
> for mmap fidelity and we decline). The **file-backed arm keeps requiring an
> aligned vaddr**, gated in `exec_load_into`, so REVENANT §4.6's
> `burrow-offset-0 == seg->vaddr` identity stays true BY CONSTRUCTION rather
> than by assumption; an unaligned non-writable segment degrades to the
> private eager arm (measured: 0 of 20 Alpine ELFs has one). Two `PT_LOAD`s
> sharing a page are refused by name -- the page would need the W+X union I-12
> forbids, and Linux's answer (later mapping wins, trailing text silently
> loses X) is exactly the silent-wrong this fix exists to remove.
>
> The durable lesson is the DIAGNOSTIC, not the arithmetic. The reject ran in
> the child thunk *after* the pid was returned, so the parent saw a healthy
> spawn and a child that exited non-zero having executed nothing -- no
> syscall, no fault, no log line naming the pid, indistinguishable from a
> program that ran and chose to fail. Three instruments had to come back EMPTY
> before the shape of the evidence (a process making no syscalls at all cannot
> have run) pointed at the loader. `exec_report_fail` now names every refusal
> and `exec_setup_from_spoor` names the pid. **Any new refusal on this path
> must report itself**; a silent one re-opens the same hole.
...
- ~~**`SYS_PUTS` (`t_puts`/`t_putstr`) is an EL0-reachable diagnostic path that bypasses the ring + role.**~~ **CLOSED at #76 — this bullet was the accepted seam, and it was wrong twice over.** P1-F deliberately left `sys_puts_handler`'s byte-by-byte `uart_putc` loop in place, reasoning that "the diagnostic channel must work independent of the console Dev," and accepted a stated cost of *interleaving at the FIFO*. Both halves failed:
  - **The predicted cost arrived, on the trusted path.** "Two programs one on each path can interleave" is precisely what LS-CI caught: a login prompt rendered `patapestrssyd: mworodd:e` — `"password: "` (login, via fd 1 → `cons_output_write`, role-held) shredded byte-for-byte by `"tapestryd: mode "` (tapestryd, via `t_putstr`). SYS_PUTS is not a niche channel: **83 binaries** reach it via `libthyla_rs::t_putstr` / `libt::t_puts`, so it is *the* native diagnostic stream, and a role only some writers take excludes nobody.
  - **The unstated cost was worse than the stated one.** `cons_drain_tap` fires from `cons_emit`/`cons_emit_wait` only, so nothing written via SYS_PUTS ever reached the G-4 renderer. The seam's own justification inverts under a graphical console: the direct path does not make the diagnostic channel *more* reliably available, it makes it available **never** — invisible on the framebuffer while looking perfectly normal on serial.
  - **The independence rationale did not require the direct path.** `cons_output_write` is the shared cons-layer function (#57b), not the Dev: reaching it needs no fd, Spoor, handle, namespace, or open. SYS_PUTS keeps every bit of its independence by calling it, and the pre-GIC boot path still falls through to the direct bounded `uart_putc` inside `cons_tx_push_nowait`. Extinction and Halls are unaffected — they use `uart_puts`/HX-I direct paths, and a dying kernel never issues an EL0 syscall.
  The ONLCR that arrives with the routing is **required, not incidental**: closing the tap gap newly exposes this output to aurora, whose VT does not synthesize CR on LF (#36), so bare-LF writes would staircase the moment they became visible.
- **A debug / job-control stop of a thread parked mid-`cons_output_write` holds the writer role for the stop's duration**, so other console writers park on `g_cons_tx.role_waiters` until it resumes (safety is clean — the role releases on resume or death; contenders park, never spin or extinct). This is a new liveness seam (the role did not exist before #75), the console analog of the #89/8c-3 elected-9P-reader freeze; it is bounded to one write call and accepted as a **v1.x** release-on-stop item, not merge-blocking. **#76 widens whom this seam can block** — SYS_PUTS is now a role client, so a stopped role holder parks `t_putstr` callers too, not just `/dev/cons` writers. That is the price of the writer set being complete, and it is the correct direction (a channel that could not be *blocked* by the role was equally a channel that could not be *protected* by it); the seam's bound and disposition are unchanged.
...
| REVENANT file-backed demand-paged exec (the Image cache + the `BURROW_TYPE_FILE` fault arm) | `kernel/exec.c` (`exec_setup_from_spoor` + `exec_read_header` [the OOB header-buffer guard] + `map_text_file_backed` + `map_eager_from_file` + the `PT_LOAD` dispatch gate; the legacy blob path `exec_setup`/`exec_setup_with_argv` kept test-only), `arch/arm64/fault.c` (the `BURROW_TYPE_FILE` demand-page arm: `demand_page_locked` [the `burrow_ref` pin] + `file_demand_page_slow` [the LOOPED death-interruptible `dev->read`] + `file_install_locked` [install-once re-validate]), `kernel/image.c` + `kernel/include/thylacine/image.h` (the qid-keyed Image cache + the eviction-safety proof), `kernel/burrow.c` (`burrow_create_file` + the FILE free arm + `burrow_acquire_mapping` FILE liveness), `arch/arm64/mmu.c` (`arch_icache_sync_range`), `kernel/syscall.c` (`exec_resolve_from_namespace` + the 5 spawn bodies + 3 thunks -- the pinned-Spoor ref ledger across `rfork`), `kernel/include/thylacine/{exec.h,page.h,syscall.h}` (`EXEC_FILE_MAX` / `EXEC_ELF_HEADER_MAX`); **cross-subsystem**: `kernel/torpor.c` (the pre-fault before `torpor_lock` -- R-5 F1) | REVENANT (`docs/REVENANT.md` = `docs/EXEC-LOAD-DESIGN.md`; ARCH §6.5 + §28 **I-36**). **Realizes I-36 (the 7 file-backed-exec soundness conditions); retires the 1-MiB whole-ELF slurp -- a binary of any size execs.** No new §28 invariant beyond I-36 (composes I-7 dual-refcount / I-9+#811 death-interruptible / I-12 W^X / I-14 ecode-bounding / I-30 pin-at-exec). Prosecute: the spoor/burrow ref ledgers across the 5 spawn paths + `image_lookup_or_create` + `map_text_file_backed` + the FILE free arm (no double-clunk / leak / UAF -- one `spoor_clunk` per adopted ref on every path); the demand-page install-once race (the `burrow_ref` pin closes UAF/ABA across the blocking read; two CPUs fault one page -> install-once, loser frees); the Image eviction-safety (only a `handle_count==1 && mapping_count==0` idle victim, stable under `g_image_lock`; lock order `g_image_lock -> v->lock`); the I-cache #317 (sync before the R+X PTE; the race-loser's page synced by its creator; `ic ivau` IS-broadcast); the OOB ELF header guard (`exec_read_header` bounds `elf_load`'s ehdr+phdr deref within the 16-KiB buffer; `elf_load` never derefs `e_shoff`/segment bytes; `elf_image` is all scalars -> kfree-before-use safe); the dispatch-gate tail (`round_up(filesz)==round_up(memsz)` routes whole-bss-page text to eager-KP_ZERO); W^X (text R+X); death-interruptible + `snare:bus` fail-close (never zero-fill text). **CLOSED CLEAN at R-5** -- two independent prosecutors (Opus-4.8-max `holotype-reviewer` + concurrent self-audit) caught DIFFERENT P1s: **F1** torpor `uaccess_load_u32`-under-`torpor_lock` newly sleeps on a file-backed text page -> system-wide futex stall (fixed: pre-fault before the lock) + **SA-F1** `file_demand_page_slow` single-read corrupts an interior dev9p text page on a partial Rread (fixed: loop the read; regression `demand_page.file_short_read_fills_page`); **F2** [P3, unreachable at v1.0] stale `freq->slot` reuse -> closed-with-justification. v1.x seams: anon-COW data, per-page I-32 charge, the blob-path unification, the Image pressure-reclaim pass (which must re-establish the torpor non-blocking-uaccess-under-lock property). **No new spec** per the 2026-05-23 broadening -- prose validation in `docs/REVENANT.md` + ARCH §6.5/§28 I-36 + `docs/reference/126-revenant.md` + this row + the focused audit (`memory/audit_revenant_closed_list.md`) + the kernel tests (`burrow.file_*` / `demand_page.file_*` incl. `file_short_read_fills_page` / `image.*` / `exec_ns.*`) + the boot proof (the unstripped >1.11-MiB `net-echo` file-backed exec + full TLS-over-/net E2E) + default 986/986 + the SMP gate. **Read-ahead cluster addendum (go-arc)**: `file_demand_page_slow` becomes a thin entry -> `file_demand_page_cluster` (ONE batched `dev->read` of up to `REVENANT_READAHEAD_PAGES`=64 pages via a KP_ZERO staging buffer -> per-slot order-0 pages, word-copied + I-cache-synced BEFORE any PTE can back them -> per-slot install-once under `vma_lock`+`v->lock` [`file_install_cluster_locked`: sibling-won slots keep the resident page, ours freed outside `v->lock`; only the faulting slot's PTE installs -- cluster-mates resident-hit on their own faults]) degrading to `file_demand_page_single` (the audited R-5 body, renamed) on any alloc shortfall / degenerate cluster -- read-ahead never fails a fault; byte-identical to N sequential single reads (same offsets, EOF tail stays zero); `filepages[]` semantics unchanged. Closes the 4-KiB-fault-vs-8-MiB-Stratum-extent amplification. **Companion torpor change (the same go-arc pass)**: after the R-5-F1 pre-fault load, `sys_torpor_wait_for_proc` returns `TORPOR_OK` on a `prefault != expected` mismatch WITHOUT taking `torpor_lock` (the Linux-futex compare-before-queue shape) -- sound because no waiter is registered on that path (I-9's window exists only between register and sleep, reached only by the equal path); the mismatch return provides only the plain aligned-u32 load's ordering, and every consumer (musl / Go runtime / libthyla-rs) re-checks with its own atomics per the universal futex contract. Both prosecuted by the focused read-ahead+torpor audit (`memory/audit_readahead_torpor_closed_list.md`) + the `demand_page.*` cluster tests; prose `docs/reference/126-revenant.md` + `docs/reference/80-torpor.md`. **Rodata file-backing addendum (#45, CHASE 2026-07-11)**: the dispatch gate generalizes `PF_X` -> `NOT PF_W` (`file_shareable`), so the R-only rodata `PT_LOAD` (roughly half a Go binary's bytes: compile 7.89 of 17.5 MiB) rides the SAME file-backed demand-paged Image-cached path as text; writable data stays eager-copied anon (I-36 condition 4 untouched); text byte-identical (`elf_load` rejects `W\|X`). NO fault-arm change (installs already at `vma->prot`; the #317 I-cache sync already `freq->exec`-gated -- sound because each FILE Burrow backs ONE segment at ONE prot, so no page migrates non-exec -> exec) and NO Image-cache change beyond `IMAGE_CACHE_MAX` 64->128 (2 entries/binary now). The per-Burrow-single-prot property is ENFORCED (not assumed) by the `exec` component of the Image key (`(dc,devno,qid.path,qid.vers,file_offset,size,exec)` -- #45 audit F1): a crafted ELF's aliased R+X + R-only PT_LOADs (identical file window, different X-ness) get DISTINCT Burrows, so no FILE Burrow is dual-prot and a non-exec fill never leaves an executable resident-hit unsynced (the #317 stale-I-cache hazard, closed); the gate also requires `PF_R` (#45 audit F2 -- a no-access flags==0 PT_LOAD stays eager). Prosecute on any change: the gate must keep routing `PF_W` eager (a file-backed WRITABLE mapping violates I-36-4 + the section-6.5 refusal); the exec component must stay in the Image key (both search passes + the install) so the single-exec-class property holds; the I-36 conditions' segment-generality (EXEC-LOAD-DESIGN section 4.6 soundness walk incl. the torpor pre-fault composition + the I-32 posture). Validated by the section-4.6 prose + `docs/reference/126-revenant.md` + the #45 focused audit (Fable-5-max prosecutor + concurrent self-audit CONVERGED on F1; 0 P0 / 1 P1 / 0 P2 / 2 P3, NOT dirty; `memory/audit_45_rodata_closed_list.md`) + the `exec.from_spoor_rodata_dispatch` + `exec.from_spoor_aliased_window_distinct` (the F1 regression) + `image.exec_discriminates_key` + `demand_page.file_rodata_prot` tests + boot OK (every boot binary's rodata now takes this path) + the SMP gate; measured (CHASE): the ~792 MiB of per-exec rodata re-reads in a cold `go build` collapse to first-touch demand paging. **Unaligned-`PT_LOAD` addendum (#149, LINEAGE L-6c 2026-08-03) -- THIS is the authoritative prosecution copy.** ELF does not require a page-aligned `p_vaddr`, only `p_vaddr == p_offset (mod p_align)`; aarch64's 64 KiB `p_align` means every real linker emits an unaligned data segment (busybox's at `0x51d2e0`; 20 of 20 stock Alpine ELFs have one, 0 of 20 have an unaligned NON-writable one, 0 of 20 share a page between `PT_LOAD`s). The blanket reject held for years because everything OUR toolchain emits is aligned -- #136's shape, met by the first foreign ELF. AS-BUILT: one `seg_geometry()` derives `{floor, lead, size}` and BOTH eager arms (`exec_map_segment` blob-path, `map_eager_from_file` production) map from `floor` with the segment's bytes at Burrow offset `lead`; the leading slack reads ZERO (KP_ZERO eagerly / demand-zero sparsely). Prosecute on any change: (a) **the leading slack must stay ZERO, never file-filled** -- those bytes are outside `[p_vaddr, p_vaddr+p_memsz)` so no segment claims them, and reading from `floor(file_offset)` (Linux's mmap-fidelity answer) maps section headers / symtab / debug bytes the program never asked for; the regressions POISON the file there so the assertion discriminates rather than passing on an incidentally-zero blob; (b) **the file-backed arm keeps requiring `lead == 0`**, gated in `exec_load_into`'s `file_shareable` -- the R-2 fault arm derives each page's file position as `v->file_offset + (burrow_byte_off & ~(PAGE_SIZE-1))`, which is the segment's own bytes ONLY when Burrow offset 0 IS the segment start, so gating at the dispatch keeps §4.6's `burrow-offset-0 == seg->vaddr` identity true BY CONSTRUCTION rather than by assumption; `map_file_backed` re-checks defensively and fails CLOSED (probe-verified: dropping the gate makes the load fail, never mis-map); an unaligned non-writable segment degrades to the private eager arm (correct, merely unshared); (c) **two `PT_LOAD`s sharing a page are refused** -- the page would need the earlier segment's permissions AND this one's, which for text-then-data is the W+X union I-12 forbids; Linux lets the later mapping win (trailing text silently loses X) and we decline instead. The explicit check adds the DIAGNOSTIC and an early-out, not the refusal (`vma_insert` already rejects the overlap -- verified by probe); (d) the eager arms need NO `file_offset` alignment gate (they copy from an explicit `dev->read`/blob offset), so do not re-add one. **The diagnostic is part of the fix, not decoration**: the reject ran in the child thunk after the pid was returned, so a refused load was indistinguishable from a program that ran and exited non-zero -- `exec_report_fail` now names every refusal and `exec_setup_from_spoor` names the pid; **any new refusal on this path must report itself**. Validated by `exec.unaligned_segment_loads` / `exec.unaligned_lazy_segment_loads` / `exec.shared_page_segments_refused` / `exec.unaligned_stays_off_image_cache` (one per obligation, each revert-probed to fail at its OWN assertion) + `execve.failed_load_leaves_target_drainable` (re-vehicled onto the shared-page refusal) + 1306/1306 + the in-guest A/B at the L-6c gate: busybox went from ZERO syscalls (executed nothing) to 13 distinct ones (through musl startup into its own logic). **Environment-on-the-frame addendum (#140, LINEAGE L-6c 2026-08-04) -- THIS is the authoritative prosecution copy.** Until this landed the frame builder wrote a LONE NULL for `envp`, so no Thylacine process had ever had a POSIX environment (musl's `__libc_start_main` does `__environ = envp`); `/env` was the only channel and only the Go fork read it. AS-BUILT: the frame collapses from TWO shapes to ONE, and that is the fix rather than a tidy-up -- the fixed "no argv" Shape A and the variable Shape B each wrote their own envp NULL (`w[2] = 0` and `w[2+argc] = 0`), so the defect had TWO HOMES and repairing either alone would have left the other; the general arithmetic (`EXEC_INIT_STRUCTURED` / `EXEC_INIT_FRAME_SIZE` in `exec.h`) reduces to the old fixed frame exactly at `argc == 0, envc == 0`, pinned by a `_Static_assert` rather than by prose. The same reasoning gives ONE `exec_fill_ptr_vector` for both pointer vectors and ONE `viv_pack_strv` for both guest walks. Prosecute on any change: (a) **the builder must keep taking env DATA, never a Proc** -- `exec_load_into` builds into a DETACHED address space and commits last (L-2a), so at that moment the Proc still holds the OLD environment and reading `p->env` there projects the wrong one; `exec_stage_env` does the projection for the callers whose answer is "whatever this Proc already has" (every `SYS_SPAWN_*` thunk, where `env_clone_into` already ran, and the native `SYS_EXECVE`, whose ABI has no envp argument and therefore means PRESERVE), while a phenotyped `execve` packs its own; (b) **one `env_render_environ` call, never a size-query-then-render** -- the renderer walks under `e->lock` and returns one atomic snapshot, so a two-call shape opens a window in which a peer thread's `/env` write changes the answer between them (peer threads sharing one `p->env` is the ordinary case, not a corner), and the `EXEC_ENV_DATA_MAX + 1` buffer is what keeps "exactly the bound" distinguishable from "longer than it"; (c) **the env bounds must stay CHOSEN, not derived from the /env maxima** -- `ENV_MAX_ENTRIES x ENV_VALUE_MAX` is a quarter-megabyte, which beside the 64 KiB argv bound puts ~330 KiB of frame under a 1 MiB stack, still PASSING the `_Static_assert` (which only proves the frame FITS) while leaving ~690 KiB of program stack against a call graph G-7b measured overflowing 256 KiB, so the failure the multiplication buys is a squeezed stack the build cannot see; `EXEC_ENV_MAX` additionally bounds the POINTER VECTOR, which the data bound alone does not (32 KiB of minimal `A=\0` records is ~10900 entries and 87 KiB of `envp[]`), and is unreachable from a `/env` projection by construction -- it exists for the vivarium's user-supplied walk; (d) **an oversize environment is REFUSED with `T_E_2BIG`, never truncated** (appended here under the #142 signoff, in the commit wiring its first consumer, per that task's own sequencing rule), while the neighbouring ARGV bounds still answer `EINVAL` -- a LANDED native ABI whose error code is its own deliberate decision, reserved to the #142 rollout rather than changed as a side effect of adding envp; the asymmetry is tracked, not accidental; (e) the packing contract is validated in `sys_execve_core` BELOW both front ends, because the builder EXTINCTS on a count that disagrees with its block. NOT ASSERTED, and deliberately: that `env::var()` and `env::vars()` agree -- they do not, and libthyla-rs says so, because a phenotyped `execve`'s envp lands on the frame WITHOUT rewriting `/env` (the live surface stays `/env`; the frame is a snapshot taken at exec). The runtime needed no change to stay correct: `capture_auxv` already WALKED the envp array to reach the auxv rather than assuming one NULL, so the vDSO was never at risk -- it was written against the ABI, not against the buggy kernel. Validated by `exec.setup_env_frame` + `exec.stage_env_bounds` + the `viv-pheno-probe` L166 (the decline is gone) and L182-L184 (delivery across a REAL execve, read back through `env::vars()`), and by TWO revert probes that are blind to each other in OPPOSITE directions: forcing the builder back to a lone NULL fails `exec.setup_env_frame` at its own assertion (1317/1318) and kills the boot at the suite so the guest leg never runs, while sabotaging only `viv_execve`'s envp walk leaves the unit suite at a FULL 1318/1318 and fails only in-guest at `L185` -- the marker meaning NOTHING ARRIVED, distinct from `L186` ARRIVED WRONG. The gate moved four legs (B-external-exec, C-status-zero, D-status-nonzero) and now fails at `L6C-E-pipeline` on #155. |
...
| Scheduler | `kernel/sched.c`, `arch/arm64/context.c`, `kernel/smp.c` (IPI logic), `kernel/include/thylacine/spinlock.h` + `Thread.preempt_count` (the #360 preemption discipline: plain locks disable preemption per-thread; the gate in `preempt_check_irq`; the sched() lock-across-sleep assert; the raw pending-release pair -- ARCH 8.11) | EEVDF correctness, SMP, wakeup atomicity. **prowl-1 addendum (PROWL-DESIGN.md section 3.1; on the ctx-switch hot path):** `Thread.run_ns`/`switched_in_at` (thread.h) accumulate per-thread on-CPU time stamped at the `sched()` switch boundary (`kernel/sched.c` right before `cpu_switch_context`: one `timer_now_ns()` read + a guarded `__atomic_store` fold-out + a plain stamp-in). **READ-ONLY telemetry -- NO scheduling decision reads `run_ns`, so EEVDF placement / `vd_t` / I-8 liveness / I-17 latency are byte-unchanged.** Prosecute (at prowl-5, the focused audit): the single-writer run_ns store (a thread runs on <=1 CPU per I-21, so its switch-out is the only writer; `__atomic` RELAXED for the lockless cross-Proc reader -- the page_count pattern); the `switched_in_at != 0` guard (drops the boot/idle threads' never-stamped first switch-out so no ~uptime bogus delta lands); `proc_cpu_ns` sums `run_ns` over `p->threads` under `g_proc_table_lock` (the #95 walk-safety: a proc reachable via `proc_for_each` is not being reaped, so its threads are stable). NO new §28 invariant (composes I-1 / I-26 / I-39 / the `/ctl` visibility posture); NO new syscall (the FS is the API); the visibility gate is the prowl-5 obligation. SMP-gated (default+UBSan x smp4/smp8) at prowl-1. **prowl-3a addendum (PROWL-DESIGN.md section 3.3/3.4; same ctx-switch hot path):** four per-thread scheduler counters (`Thread.nsched`/`nsleeps`/`nmigrations`/`last_cpu`, the `/proc/<pid>/sched` substrate) stamped at the SAME single switch chokepoint as `run_ns`, plus one per-CPU `CpuSched.idle_ns` (charged in `sched_idle_park` next to the global `g_wc_idle_ns`; read via `sched_cpu_idle_ns`, the `/ctl/cpu` meter denominator). **READ-ONLY telemetry -- again NO scheduling decision reads any of them, so EEVDF / vd_t / I-8 / I-17 / the tickless machinery are byte-unchanged.** Prosecute (at prowl-5): the single-writer discipline (nsched/nmigrations/last_cpu written at switch-IN by the one CPU that picked `next`, whose `on_cpu` is already true at the stamp so no peer can touch it; nsleeps at `prev`'s switch-OUT on its one CPU; the run_ns pattern); the first-dispatch migration guard (`nsched == 0` skips the spurious move off the KP_ZERO `last_cpu`); the cross-CPU `last_cpu` read (written by the previous dispatcher -- the run-tree lock chain supplies visibility, and a stale read is at worst a cosmetically-off telemetry count, never unsafe); `CpuSched.idle_ns` single-writer-per-CPU (cs cpu-pinned-stable across the park); not rfork-propagated (fresh KP_ZERO). Validated by `scheduler.prowl_counters` + the SMP gate. NO new §28 invariant / syscall; the FS surfaces (`/proc/<pid>/sched` + `/ctl/cpu` + the OQ-4 owner-or-CAP_HOSTOWNER visibility gate) are prowl-3b; the focused audit is prowl-5. |
...
| Handle table | `kernel/handle.c` | Rights monotonicity, transfer rules, hardware-handle non-transferability. **#151 CLOSE-ON-EXEC ADDENDUM (LINEAGE L-6c).** `struct HandleTable` gained `u64 cloexec[HANDLE_CLOEXEC_WORDS]` -- a bitmap PARALLEL to the slot array, plus `handle_dup_posix` / `handle_set_cloexec` / `handle_get_cloexec` / `handle_close_on_exec`, and a sweep call in `sys_execve_core` after the commit point. PARALLEL rather than a field in `struct Handle` because POSIX close-on-exec is a property of the DESCRIPTOR, not of the open file description behind it (`dup(fd)` yields a second descriptor onto the same description with the flag CLEAR, and F_SETFD on either does not touch the other) -- a bit in `struct Handle` would be shared by exactly the two things POSIX requires to differ; Linux keeps `close_on_exec` beside `fd[]` in `struct fdtable` for the same reason. Also because `struct Handle` has no slack (8+4+4+8 is exactly the 24 its `_Static_assert` pins; a u32 there takes the table 6152 -> 8200 bytes, across the 2-page boundary into 3, for one bit per slot). **Prosecute:** the ESTABLISH-NEVER-INHERIT discipline -- the bit is cleared in `handle_install_locked`, the SINGLE point every fd-creating path goes through, which is what makes a REUSED index unable to carry the previous occupant's flag (the reused-identity class: L1f F1's reused inode, net-3d's slot re-mint); the four lifecycle sites as a SET (install clears/sets per argument, close clears, `handle_table_copy_into` COPIES [POSIX: fork preserves -- a shell sets it once and forks per command], `handle_replace` LEAVES ALONE [the fd number persists, so the fd's flag persists; a SOCK_CLOEXEC socket must stay close-on-exec across connect]) -- a missing site is silent; the sweep's PLACEMENT, forced from both ends (the closes may SLEEP via a Spoor's Dev close hook -> no lock may be held, and a FAILED exec must leave the process unchanged -> nothing closing the caller's fds may precede the last thing that can fail; Linux places `do_close_on_exec()` after its own point of no return for the second reason), and BEFORE the trapframe rewrite so no instruction of the new image observes an fd that was supposed to be gone; the sweep's snapshot-and-clear-then-close form (the snapshot cannot go stale in the only caller -- execve refuses unless this is the Proc's sole live thread -- but the form does not DEPEND on that); `handle_dup_posix`'s rights read under the SAME lock hold as the install (a `handle_get` first would be the TOCTOU the function is shaped to avoid) and its rights-verbatim carry satisfying I-6 by non-increasing exactly as `handle_table_copy_into` does; the min_idx argument is LOAD-BEARING, not a convenience (a shell's `savefd()` does `F_DUPFD_CLOEXEC(fd, 10)` precisely to move its bookkeeping fd out of the low range a user redirection could collide with -- returning the first free slot would break the guarantee the call was made to obtain, silently and only under a redirection). **NATIVE BEHAVIOUR IS BYTE-UNCHANGED and that falls out rather than being arranged**: nothing outside the phenotype can set the flag, so a native Proc's bitmap is empty and the sweep closes nothing. No new section-28 invariant (composes I-2/I-6: the flag is not a right and confers nothing). Coverage: `handles.cloexec_lifecycle` / `cloexec_exec_sweep` / `cloexec_fork_preserves` / `dup_posix` + `vivarium.fcntl_domain`; prose `docs/reference/145-vivarium.md` "Close-on-exec". |
...
| VIVARIUM: the syscall-entry phenotype branch + the spawn-time declaration (V-1b; I-43) | `kernel/syscall.c` (`syscall_dispatch`'s phenotype branch -> `viv_linux_dispatch`; the TIER-2 shells `viv_tier2` / `viv_measure_user_path` / `viv_stat_copy_out`; the extracted `sys_fstat_for_proc`; `SPAWN_PHENO_LINUX` in `sys_spawn_args.pheno_flags` -> the thunk stamp), `kernel/include/thylacine/syscall.h` (the ABI: `pheno_flags` consumed the must-be-0 `_pad_allow` slot at offset 92), `kernel/vivarium.{c,h}` (the PURE table + translators, landed at V-2), `kernel/exec.c` (the section-12.1 rule-4 advisory brand diagnostic -- `elf_brand_hint`'s first caller), the mirrors (libt / libthyla-rs `T_SPAWN_PHENO_LINUX`), `usr/viv` (the manifest's `org.thylacine.phenotype` annotation -> the entrypoint spawn) | **Invariant I-43 -- the privilege boundary of the whole Vivarium arc; prosecute hard at V-8.** A mis-branded Proc decodes every syscall number wrong, so the fail-safe direction is forced: outside a vivarium the phenotype is ALWAYS native (section 12.1 rule 3), no ELF byte may decide (rule 4 -- the hint is consulted only on an already-FAILED load, where it can explain an outcome but never change one), and the declaration rides ONLY `SYS_SPAWN_FULL_ARGV`. Prosecute: **I-43 completeness** -- NO translated path may bypass a gate (the T1 rows must keep FALLING THROUGH to the native `switch` rather than growing a parallel handler; each T2 shell must keep calling the same `sys_*_for_proc` core, which is why `sys_fstat_for_proc` was extracted rather than duplicated); the ungated declaration's soundness (every Linux number in the table ALSO exists as a live native number -- 56 openat vs SYS_READDIR, 64 write vs SYS_CONSOLE_OPEN, 94 exit_group vs SYS_TTY_SIGNAL -- so a mis-declaration mis-decodes the mis-declarer's OWN calls behind its OWN gates and reaches nothing new; a NEW table row must be re-checked against this argument); the **argument-arity** property (the branch copies all six argument registers verbatim, so a T1 row is sound only while its native target reads NO MORE arguments than the Linux call supplies -- true of all five current rows [read/write/close/lseek/exit_group], and a property a new row could break silently, since the extra register would carry whatever the Linux caller left there); the T2 uaccess shells (`viv_measure_user_path` is bounded by `SYS_OPEN_PATH_MAX` and validates every byte's VA before loading it, since the length is unknown up front; the `newfstatat` re-read rejects an embedded NUL exactly as `sys_stat_handler` does, so a peer thread rewriting the buffer between measure and copy cannot produce a malformed kernel path); the DECIDE-BEFORE-MEASURE order (a forwarded call must not take a user fault in the kernel fast path); the 128-byte `viv_linux_stat` copy-out is I-13-clean (zeroed wholesale by `vivarium_stat_to_linux` before filling); the phenotype is set ONCE in the thunk before EL0 and never by a running Proc (no runtime self-modification of the ABI); the FORWARD arm stays a distinct case from ENOSYS so V-3's supervisor is a one-line change. **No new spec** -- prose in `docs/VIVARIUM.md` section 8/12.1 + `docs/reference/145-vivarium.md` + this row + the V-8 focused audit + the `vivarium.*` + `sys_spawn_full_argv.validate_req_pheno_flags` tests + the boot-fatal two-vantage gate + the SMP gate. **V-2d ADDENDUM (the memory rows; `mmap`/`munmap` T2, `mprotect` ENOSYS).** V-3 is DEFERRED (VIVARIUM.md section 4.1: no Proc can mutate another's address space, handle table or process tree, so a peer-supervisor's servable set is EMPTY -- the peers work only because a compat supervisor either IS the guest [Starnix restricted mode] or OWNS it [Plan 9 linuxemu over /proc; gVisor]), so FORWARD now means ENOSYS and a too-narrow T2 domain is a COMPATIBILITY FAILURE rather than a slow path -- the inverse of section 6.19's 'declining is always safe'. Prosecute: the mmap admission is an ALLOW-LIST of PROT_READ\|PROT_WRITE, never a PROT_EXEC deny-list (aarch64 musl also defines PROT_BTI/PROT_MTE and generic musl PROT_GROWSDOWN/GROWSUP -- a deny-list admits all four silently); PROT_EXEC stays REFUSED not degraded (I-12 W^X + I-42/CAP_JIT -- a phenotype must never mint an executable anonymous mapping); the flags word is an EXACT match (MAP_FIXED/FIXED_NOREPLACE make `addr` a REQUIREMENT and so must never be admitted while the target picks the address); the PROT_NONE-yields-RW degradation is FIDELITY not AUTHORITY (the guest's own pages, no gate changed, nothing crosses a Proc boundary) and must stay published in section 9's DEGRADED tier + pinned by prover leg L23; the shell must keep translating the target's bare -1 into -ENOMEM (a Linux libc reads -1 as -EPERM) and keep answering EINVAL for len==0 / a misaligned munmap addr, so Linux's own argument errors are not collapsed into the decline; munmap must keep DECLINING everything outside the exact match rather than faking Linux's unmapping-nothing-succeeds (telling that apart from a PARTIAL overlap needs a range scan the VMA API does not expose -- vma_lookup is a point probe blind to a VMA strictly inside the range -- and claiming success on a partial overlap would leave a mapping the guest believes is gone). Coverage: `vivarium.mmap_domain` + the mmap/munmap-are-T2 and mprotect-ENOSYS rows in `rejects_are_deliberate` + prover legs L16-L23 (map/write/read-back/unmap, PROT_EXEC and MAP_FIXED refused in-guest, mprotect ENOSYS which musl DEPENDS on being ENOSYS specifically). Revert-probed three ways: reverting the table row or admitting PROT_EXEC fails exactly two unit tests; breaking only the SHELL leaves the unit suite at a full 1239/1239 while the in-guest leg fails at L16 -- the pure tests prove the DECISION, the guest legs prove the PLUMBING, and neither sees the other's bugs. RE-CHECK PERFORMED (the obligation this row states): 215/222/226 collide with NO native number -- **CORRECTED 2026-07-31 (task #97): the highest assigned native SYSCALL number is 100 (`SYS_WEFT_UNSHARE`), not "256, sparsely" as this addendum first said.** 256 is `SYS_ATTACH_ANAME_MAX` (a max-aname-length bound) and 4096 is `SYS_RW_STACK` (a buffer size); both share the `SYS_` prefix and a naive grep reads them as syscall numbers. The corrected ceiling makes the check STRONGER and cheaper: every Linux number above 100 is free by construction, so the re-check this row mandates is a one-line argument rather than a survey. The collision-table clause therefore does not apply and the argument holds by a different one -- a mis-declared Proc issuing 222 receives an anonymous mapping where it would have received -1, which is NOT new authority because both targets (SYS_BURROW_ATTACH_LAZY 83, SYS_BURROW_DETACH 38) are UNGATED syscalls acting only on the caller's OWN address space and the Proc could call either directly; mprotect dispatches nothing. Arity: the mmap shell reads exactly args[0..5] (six supplied), the munmap shell args[0..1] (two supplied). **V-5 ADDENDUM (sockets as a T2 family; `handle_replace` + `Proc.socktab`; VIVARIUM.md section 5.5, user-voted 2026-07-31).** The Linux socket family translates to `/net` file ops IN THE KERNEL -- there is no supervisor, and section 4.1.1 records the measurement that dissolved that expectation. Two new mechanisms, and the first lands on an EXISTING audit-trigger surface (the Handle table row: I-2/I-4/I-5/I-6), so prosecute it there as well as here. **`handle_replace(p, fd, kind, rights, obj)`** -- swaps what a live fd denotes, in place, because `connect` must turn the guest's socket fd from `ctl` into `data` and `handle_dup` cannot serve (it allocates a NEW slot while the guest holds a specific number). Prosecute: it MUST NOT launder rights (the replacement's rights are derived from the new object's omode exactly as `sys_open_handler` derives them, NEVER inherited from the outgoing slot -- inheriting would let a READ-only socket fd become a WRITE-capable data fd for free, an I-6 monotonicity break); it MUST refuse every hardware kind (`KOBJ_KIND_HW_MASK` + `KOBJ_LOOM`) because replacing a hardware slot is a transfer wearing another name (I-5); it MUST release the outgoing object exactly once under the table lock (the #844 discipline: snapshot + `handle_put` OUTSIDE the lock, since a Spoor release may sleep); and it MUST have NO EL0 caller -- it is phenotype-internal, reached only from the socket translators, because exposing it as a syscall would be a new ABI with none of these gates applied by the caller. **`Proc.socktab`** -- the `(proto, N, state)` tuple per socket, the `Proc.sigtab` shape: CAS-installed outside every lock, freed at `proc_free`, NOT rfork-inherited, bounded by `VIV_SOCK_MAX`. Prosecute: the bound is a real I-32 posture (a guest must not grow kernel memory without bound; the socket count is charged to its own handle table besides); the `close` hook drops the entry (a phenotype-gated check in the close path -- WITHOUT it, fd reuse serves a stale `(proto, N)` and the next socket writes its connect verb to a stranger's connection, which is the sharpest bug in this chunk); the lock-free entry access is sound ONLY because a PHENO_LINUX Proc cannot spawn a thread (`clone` is not a table row), a property of the TABLE not of the data, which EVAPORATES at #93 and must be re-derived there. **The I-43 property for sockets, which is the reason this is kernel-side at all:** every `/net` operation resolves through `stalk` in the CALLER's Territory with the CALLER's authority, so a translated socket call reaches exactly what the guest could reach by opening `/net` by hand -- a container without `/net` gets a walk failure, never a privileged bypass. No path may resolve from anything other than the caller's root, and **no path may decode netd's qid layout** (`CONN_FLAG \| proto<<32 \| N<<8 \| filekind`): `/net` is a mount point and need not be netd, so decoding a foreign server's qid as netd's is precisely the silent mistranslation section 6.19's argument-domain rule exists to forbid -- which is why `proto` is REMEMBERED rather than recovered. **Ordering that netd's refcount makes load-bearing:** the connect swap opens `data` BEFORE releasing `ctl`, because the clone-minted `ctl` fid holds the connection's only reference (`slot_ref` 0->1) and netd frees the connection at zero; and `data` cannot be opened any earlier, because netd DEFERS its `Rlopen` to ESTABLISHED (#257). Both are measured properties of the server, not assumptions -- re-measure them if netd's lifetime model changes. Sockets add NO T1 rows (all T2), so the arity clause above is not engaged; the number re-check is discharged by the corrected ceiling (198-212 and 242 are all above 100). Coverage: the `vivarium.sock_*` pure tests + `handle.replace_*` (rights-not-laundered + hardware-refused + release-exactly-once, each revert-probed) + the in-guest `viv-pheno-probe` TCP round-trip over the resident loopback + the V-5d focused audit + the SMP gate. **V-8 (the arc CLOSE) LANDED.** The focused round covered the arc's genuinely unaudited surface -- V-1b + V-2d + V-6a/b/c + V-7, chosen by measuring what V-4c-3 and V-5d had left rather than by assuming, since V-6 is marked audit-bearing in VIVARIUM.md's own track note and had never received a round -- and returned **0 P0 / 1 P1 / 2 P2 / 4 P3, NOT dirty** (`MODEL(end) == Fable 5`). Every finding is closed: **F1 [P1]** every T1 byte-I/O target returned a flat `-1`, which stock musl's `__syscall_ret` reads as `errno = 1` = EPERM -- and a vivarium guest IS stock musl and cannot carry pouch's patch (FIXED `285acd2c`+`d87c2be2`, task #100, ER-3 applied at the LOCAL gates where the reason is already known exactly; the dispatch is not involved, contra the finding's own prediction). **The sharper thing underneath F1: `T_E_PIPE` had been ABI-pinned since the errno scripture landed and NOTHING IN THE TREE EVER EMITTED IT** -- `devpipe_write`'s EOF arm returned `-1` under a comment asserting a musl wrapper translated it (no such wrapper exists), so every write to a closed pipe reported EIO **tree-wide**, not merely under a phenotype; a registered code with zero emitters is a contract stated and not kept. **F2 [P2]** a phenotyped Proc's handler lives in the sigtab, so the terminate-latch exemption (which reads `handler_va`) never applied to it, and on a frame-push failure the armed latch made every sleep return SLEEP_INTR -> the guest spun at 100% forever (FIXED `ff79386b`). **F3 [P2]** the fixed `/srv/viv-dio` name was first-come-first-served, so a second concurrent `viv run` mounted the FIRST container's diorama and enumerated its processes -- it failed OPEN where the code's own comment promised fail-closed (FIXED `27a11e2c`, task #101: a peer check in the diorama's `h_attach`, NOT the finding's suggested per-runner name, which conflicts with the deliberate fixed-name decision that #33's never-freed registry forces; gating ATTACH rather than each op is what makes the cross-mount impossible rather than merely detectable, since every fid descends from the attach root; **kernel byte-unchanged**). **F4-F7 [P3]** sentinel-as-index, a fault-arm justification naming the wrong memory class, `whence` compared at 64 bits where Linux passes an `int`, and the sigtab not inherited across a fork (FIXED `b3648a2b`, task #102). Landed ahead of the round from its own disposition pass: **#96** (note delivery did not save FP/SIMD -- silent corruption of an interrupted computation, PRE-EXISTING on the native path and made reachable by ordinary compiled C once the phenotype landed) + **#94**, both `a39d2c53`; and a gate failure that surfaced during it was root-caused to a PRE-EXISTING kernel-test race, not V-8, and fixed at `8c1c080e` (#103/#104). **Deliberate residual, published in VIVARIUM.md section 9's DEGRADED tier**: `lseek` on a non-seekable Dev still returns a bare `-1` -> EPERM where POSIX says ESPIPE, because `T_E_SPIPE` (29) is unregistered and appending an errno is signoff-bearing (task #106); EINVAL would be the differently-wrong substitution. **The testing lesson this row should carry forward**: the layers are blind to each other in BOTH directions -- reverting F6's narrowing leaves the unit suite at a full **1272/1272 PASS** while the in-guest leg fails at exactly `hiwhence=-22` (the handler is `static` and takes a user VA, so no kernel test can pose the question), and reverting F3's gate leaves BOTH the selftest and the unit suite green while only the joey leg catches it. A behaviourally-inert fix (F4) additionally needs a test that MANUFACTURES the state, and a leg satisfiable by over-fixing (F6) needs its pair. Gates at the close: 1272/1272 + boot OK + 0 EXTINCTION + the two-vantage phenotype gate + `probe100` boot-fatal; SMP 40/40 PASS 0 corruption (default+UBSan x smp4/smp8, N=10); LS-CI 32/32. No spec gate -- nothing in this arc is modelled in `specs/`, and `specs/phenotype.tla` stays reserved for whichever chunk builds a forward destination (on present evidence process creation, #93). Closed list: `memory/audit_v8_closed_list.md`; as-built `docs/reference/145-vivarium.md` "The V-8 close". **#150 ADDENDUM (the startup batch; LINEAGE L-6c).** The set a real Linux binary issues between `_start` and its first useful instruction, MEASURED off a running Alpine busybox the moment #149's loader fix let it execute -- 13 unserved numbers -> 3, of which two (brk 214, mprotect 226) are declined on purpose and one (fcntl 25) is the new blocker. **Rows:** `getpid` 172 is the ONLY T1; `writev` 66 / `getcwd` 17 / `getppid` 173 / `getuid` 174 / `getgid` 176 / `uname` 160 / `set_tid_address` 96 / `setuid` 146 / `setgid` 144 are T2; `fcntl` 25 is ENOSYS. **THE ARITY RULE'S SHARPEST CASE, and the reason this row restates it:** `writev(fd, iov, iovcnt)` and `SYS_WRITE(fd, buf, len)` line up register for register, and a renumber would make the kernel write `iovcnt` bytes OF THE IOVEC ARRAY ITSELF -- the guest's own pointers -- to the fd. Registers lining up is not arguments meaning the same thing. `getuid`/`getgid` are the standing counterexample from the other side: exact native twins, matching arity, still T2, because the sentinel mapping has nowhere to live in a renumber. **Prosecute:** the writev shell's TWO PASSES (Linux validates the whole array up front and writes nothing on EINVAL/EFAULT; a single pass would leave entries 0..k-1 written -- memory safety does not depend on this, since pass 2 re-validates through `sys_write_handler`, so a peer rewriting the array between passes degrades to a legal short write); its O(1) storage (UIO_MAXIOV 1024 x 16 B = the whole kernel stack, so the array must never be buffered); `iovcnt == 0` reaching the fd check (Linux resolves the descriptor before the array, so `writev(badfd, x, 0)` is EBADF, not 0); the SSIZE_MAX accumulation testing ROOM rather than the sum (a naive `total + add > max` wraps and passes); bytes-already-written winning over a later error. **The identity mapping is a DECISION, not a passthrough:** PRINCIPAL_SYSTEM (0xFFFFFFFE) read raw by a Linux guest is `(uid_t)-2`, historically "nobody" -- so the raw value INVERTS the fact being asked about, telling a Proc that holds the TCB identity that it is the least privileged one. It maps to 0; everything else passes through and cannot collide because PRINCIPAL_INVALID and GID_INVALID are both 0. It confers NOTHING (every authority decision reads the real `principal_id` through perm_check or a CAP_* gate, so this changes what a guest is TOLD, never what it may do -- I-22), and `setuid(getuid())` must stay a no-op compared in the GUEST's number space, which is the case that needs it most. **uname is a fabrication:** `release` claims 4.4.0 as the newest kernel that promises nothing this table lacks (below statx 4.11, io_uring 5.1, clone3 5.3, openat2 5.6, close_range 5.9; above glibc's 3.2 floor, under which a glibc binary aborts before main), while `version` carries "Thylacine" because nothing parses it -- section 9's DEGRADED tier applied inside a single struct. The 390-byte copy-out is I-13-clean (zeroed wholesale before filling). **THE FOUR ROWS BELOW THE NATIVE CEILING each discharge the per-number collision paragraph** the pselect6/ppoll block mandates (17 vs SYS_SET_DUMPABLE, 25 vs SYS_SPAWN_FULL, 66 vs SYS_LOOM_SETUP, 96 vs SYS_TTY_SET_FG); in every case a mis-declared native Proc reaches only its OWN memory and its OWN fds, never authority. **fcntl is ENOSYS by MEASUREMENT:** instrumenting the row showed busybox issues exactly `F_SETFD(FD_CLOEXEC)` and `F_DUPFD_CLOEXEC(10)`, and Thylacine had no close-on-exec at all AT THAT POINT (no bit in handle.h/handle.c; `proc_exec_replace` did not touch the handle table, so exec preserved EVERY fd) -- BUILT at #151, see the addendum below -- serving the dup while dropping the flag would pass the gate and put a known leak in the tree. That is a KERNEL FEATURE, not a translation, and it is **task #151**. It also VOIDED A STATED SCRIPTURE FACT: VIVARIUM.md's ignorable-flags table justified admitting `O_CLOEXEC` on the ground that "there is nothing to opt out of" because spawn endows an explicit fd list -- TRUE when written, and voided by L-2a (execve preserves the table) plus L-3c-1 (rfork copies it), neither of which had any reason to look at an openat flag table. **The recurring shape: a round is scoped to one commit, so a premise a LATER commit voids is invisible to it** -- the defence is re-checking a load-bearing fact when the thing it rests on moves, not a better review of either commit. Coverage: `vivarium.startup_batch_rows` / `writev_domain` / `uname_fill` / `identity_map`, revert-probed FOUR ways in one build -- writev-as-T1, a naive wrapping overflow test, a dropped uname zero-fill and an identity uid map each failed at their OWN named assertion (1306/1310, no cross-talk). 1310/1310 + boot OK + 0 EXTINCTION. **#151 ADDENDUM (close-on-exec; the `fcntl` row goes TIER-2).** The feature was built FIRST and the row served after -- the order is the chunk's whole content, since serving `F_SETFD(FD_CLOEXEC)` by storing nothing would have passed the gate and left a known lie in the tree. The kernel half's prosecution list is the **Handle table** row above (the authoritative copy). Here: `fcntl` serves `F_GETFD` / `F_SETFD` / `F_DUPFD` / `F_DUPFD_CLOEXEC` and declines every other cmd with **ENOSYS, not Linux's EINVAL** -- EINVAL claims the cmd is not a valid fcntl operation, which for `F_GETFL` or `F_SETLK` is false, while ENOSYS says the surface is absent, which is true and is what the whole row returned before any of it was served. It is TIER-2 and NOT a renumber onto SYS_DUP: that call's second argument is a RIGHTS MASK where `F_DUPFD`'s is a MINIMUM FD, so a renumber would read `10` as a set of capability bits and hand back a descriptor with arbitrary authority -- the arity rule at its sharpest after `writev`. `F_SETFD` **masks** `arg & FD_CLOEXEC` rather than validating, because Linux ignores every other bit and being STRICTER than Linux for an input a guest may legally send is its own mistranslation. `F_GETFD`/`F_DUPFD` join the two MEASURED cmds because each is the exact inverse or sibling of one, differing by a line. **`O_CLOEXEC` is now HONOURED FOR REAL** -- `vivarium_openat_decide` gained a third output rather than a bit in the omode, because the flag names the resulting DESCRIPTOR and is applied after the open succeeds; set on every translated path including O_PATH (Linux honours it there -- one of the three flags O_PATH does not ignore). The scripture fact #150 recorded as voided is thereby CLOSED by doing what the flag asks, not by adjusting the claim. **WHAT THE GATE DID NEXT, and it is a measurement not an inference:** serving fcntl moved L-6c from 'busybox speaks' to **'busybox runs a script'** (`L6C-A-shell-runs` fires; `/gate/run.sh` executes), and it now fails at `L6C-B-external-exec`. Instrumenting `viv_execve`'s envp arm shows the guest passing `envc=2` with `env0='SHLVL=1'` -- **ash SYNTHESIZES SHLVL and PWD itself**, so its envp is non-empty even from an empty environment and no container configuration avoids the arm. The blocker is **task #140** (no process has a POSIX environment on its stack) and it wants `exec_build_init_stack`, not the phenotype. The joey gate message was updated to name it: a KNOWN-BLOCKED that outlives its blocker is a disabled test, and one naming the WRONG blocker is worse -- it sends the next reader at the thing already fixed. 1315/1315 + boot OK + 0 EXTINCTION. **#155 ADDENDUM (the `pipe2` row; LINEAGE L-6c).** A shell cannot build a pipeline without it, and on aarch64 there is no second way to ask: the generic syscall table has NO legacy `pipe` (only `__NR_pipe2 59`; x86_64 carries both 22 and 293, which is why the architecture had to be checked rather than inherited), so musl's `pipe()` IS this number with flags 0. **THE ARGUMENT DOMAIN WAS MEASURED, not reasoned from what Linux permits** -- six call sites reach nr 59 in the gate's own busybox, four through `pipe()` with a hardcoded `mov x1, #0` and two through `pipe2()` with `mov w1, #0x80000`, so `{0, O_CLOEXEC}` is not a conservative subset of the domain, it IS the domain. Both are reproducible EXACTLY: 0 is `SYS_PIPE` unchanged, `O_CLOEXEC` is the descriptor flag #151 built, applied to BOTH ends after creation exactly as openat's row applies it (pipe2's flag is not per-end). **An ALLOW-LIST, never a deny-list** -- V-2d's mmap lesson, and here the excluded members are real: Linux's pipe2 genuinely accepts `O_NONBLOCK` and `O_DIRECT`, and devpipe has no non-blocking read and no packet framing, so admitting either by omission would tell a guest something false about the pipe it just received. Prosecute on any change: the domain stays an allow-list; the DECIDE and the RANGE CHECK both precede the allocation (a call that was never going to land its result must not cost a pipe ring and two descriptors first); and **the copy-out cleanup must survive** -- `sys_validate_user_buf` proves only the VA band, so the store can still fault, and Linux's own `do_pipe2` closes both fds on that path. Returning an error with two live descriptors the guest was never told the numbers of is an unreachable leak for the life of the Proc. **Declining `O_CLOEXEC` would ALSO have worked and is not what happens**, which is worth recording because the reasoning inverted once during the work: musl's pipe2 carries an ENOSYS fallback (`pipe()` then `fcntl(F_SETFD)`) that now runs CORRECTLY because #151 made fcntl a served row -- so declining is not unsafe, merely worse (three syscalls instead of one, leaning on a shim that belongs to one libc; a static Go binary calling pipe2 directly has none). **The collision paragraph, since 59 is below the ceiling:** the native occupant is `SYS_WSTAT(fd, valid, ...)`, and a mis-declared native Proc is refused TWICE -- `valid` is a mask of `T_WSTAT_{MODE,UID,GID,SIZE}` == 0xF so every legal wstat lands in [1,15] and the domain admits only {0, 0x80000}, and were a garbage 0 to slip through, args[0] (an fd index) becomes the write VA, which is page zero, so the copy-out faults, both fds are closed, EFAULT. Its own address space, its own two fds, never authority. Coverage: `vivarium.pipe2_domain` + the T2 row in `rejects_are_deliberate` + in-guest prover legs L187-L192 (create / the byte actually round-trips through the pair / O_CLOEXEC on BOTH ends via F_GETFD / the O_NONBLOCK decline / EFAULT on an unmapped-but-in-band VA / **and the leak assertion that one sets up -- 200 failing calls burn 400 descriptors against a 256-slot table, so a leaking shell cannot then make one more pipe**). **Two revert probes, blind to each other in OPPOSITE directions:** removing the cleanup leaves the unit suite at a full **1319/1319** and fails only in-guest at `L192`; turning the allow-list into a deny-list fails `vivarium.pipe2_domain` at **1318/1319** on the bare `1u` case, and the guest legs do not notice because L190 tests `O_NONBLOCK`, which a deny-list still catches. **AND THE NEXT BLOCKER WAS MEASURED BEFORE THIS ONE WAS WRITTEN, so fixing site N could not hide site N+1:** aarch64 has no `dup2` number, so musl compiles dup2 into `dup3`, which is a deliberate FORWARD -- it FREES the fd it overwrites, and `vivarium.c`'s fd-freeing block requires the socktab close hook be extended in the same commit that serves it. pipe2 is fd-CREATING and therefore needs no such extension (it is in openat's class), which is the distinction the two adjacent tests now state together. Task **#157**; the gate log is unambiguous -- `pipe-in` is PRINTED, and busybox names the failure itself: `dup2(4,1): Function not implemented`. **#157 ADDENDUM (the `dup3` row; LINEAGE L-6c -- THE ARC GATE PASSES).** Reached the same way pipe2 was: aarch64 has NO `dup2` number (the arch table defines only `__NR_dup3 24`), so musl's dup2.c compiles `dup2(old,new)` into `__syscall(SYS_dup3, old, new, 0)` and a shell's redirection plumbing has no other route. **THE DOMAIN WAS MEASURED** -- four call sites in the gate's own busybox (`0x4d3ca0` musl dup2 `mov x2,#0`; `0x4d3d0c` musl __dup3 `sxtw x2,w2`, the caller's flags; `0x4db7d8` and `0x4db8cc` busybox-internal, both `mov x2,#0`). **AND THIS ROW'S DOMAIN IS COMPLETE, WHICH PIPE2'S WAS NOT:** `ksys_dup3` rejects everything outside `{0, O_CLOEXEC}` with EINVAL, so the allow-list is EQUAL to Linux's rather than a subset -- which is why an out-of-domain flags word gets **EINVAL, not the ENOSYS decline** (V-2d's munmap distinction: Linux's OWN argument errors are reproduced, because answering ENOSYS would claim the surface is absent, and here that would be false). The row's degraded tier is therefore entirely about STATE, never arguments. **TWO FACTS MEASURED IN musl's dup2.c CONSTRAIN THE IMPLEMENTATION:** `old == new` never arrives from dup2 (musl short-circuits via `fcntl(old, F_GETFD)` first -- which also makes #151's fcntl row load-bearing for `dup2(x,x)`, something nothing else would have shown), and musl **retries on -EBUSY in a bare `while` loop on both paths**, so this row must never produce that errno or a guest spins forever. **A NEW PRIMITIVE, `handle_dup_to`, and it is a genuinely new combination rather than a convenience** -- `handle_dup_posix` picks the index, `handle_replace` demands a live one and takes an object, `handle_table_copy_into` demands a free one in another table; dup3 needs a CALLER-NAMED index that may be free OR live. It is NOT close-then-dup, for the reason `handle_replace`'s header already records for `connect`: the freed index is not reserved, so a peer's fd-creating syscall can take it. Rights carry VERBATIM (POSIX; I-6 by non-increasing); close-on-exec is SET FROM THE FLAG, never inherited from the source (the classic dup2 mistake -- POSIX clears it) nor from the previous occupant (the reused-identity failure); the SOURCE passes the same `handle_slot_may_alias` gate `handle_dup` and the fork copy use, while the DESTINATION is deliberately UNGATED by kind, because it is being CLOSED and `handle_close` places no kind restriction on closing. **THE SOCKET CASE DECLINES, deliberately and with the alternatives written down:** the socktab keys `(proto, N, state)` on the FD NUMBER and is unrefcounted, so COPYING the entry gives two diverging state machines over one connection (a `connect` on A swaps A's handle ctl->data while B still names `ctl` and still believes it is FRESH) and OMITTING it gives an fd that reads but cannot connect/bind/getsockname -- the silent half-service the argument-domain rule forbids. Reproducing Linux exactly needs a REFCOUNTED socktab entry, a real change to a table V-5 audited, and that belongs in a chunk about it; the cost is bounded and named (the inetd `dup2(connfd,0)` idiom), published in VIVARIUM.md section 9's DEGRADED tier. **THE fd-FREEING OBLIGATION IS PAID IN A DIFFERENT ARM FROM `close`'s, and that is the rule a future promotion must follow:** `close` pays in the ENTRY HOOK (sound there only because a close whose fd carries an entry always proceeds), while dup3 pays INSIDE ITS SHELL, after every refusal, because a dup3 can be refused while `new` is a live socket and an entry-time drop would destroy socket state on a failed call. It also drops AFTER the install rather than before -- the OPPOSITE of `viv_sock_accept`'s unwind, whose reason does not transfer: there the number is FREED and reallocatable, here it is never free, so dropping first would only mean a refused dup3 had already destroyed live state. `dup` (23) and `close_range` (436) remain FORWARD and still owe it. **The collision paragraph, since 24 is below the ceiling:** the native occupant is `SYS_SPAWN_WITH_CAPS(name_va, name_len, cap_mask)`, whose arity MATCHES exactly, so the argument is about what is REACHED -- `name_va` is a user VA and therefore out of fd range (EBADF before anything is touched), `cap_mask` lands in `flags` where everything outside `{0, O_CLOEXEC}` is EINVAL, and dup3 acts only on the caller's own handle table, minting no object and consulting no capability. **#158 rode along, because the dup3 socktab work rests on it:** the lock-free socktab/sigtab argument said 'sound because clone/clone3 are not table rows', which LINEAGE L-3d falsified WITHOUT ANYTHING FAILING (L-6a then widened clone to the fork shape). The conclusion survives via a different mechanism -- `vivarium_clone_decide` admits two words by EXACT EQUALITY and neither carries `CLONE_THREAD` -- so the property now evaporates when the clone DOMAIN admits the thread set, not when a row appears. Seven sites carried the claim and two said outright that a guest 'cannot fork', which the passing gate disproves on every boot. **Coverage:** `vivarium.dup3_domain` + `handles.dup_to` (index / rights / cloexec-from-argument-not-from-either-source / the LIVE-vs-FREE counter arithmetic / old==new / the alias gate) + the re-stated `fd_freeing_rows_stay_unserved` (which now asserts the TIERS, since the tier is what decides where each pays) + in-guest legs L193-L199. **Two revert probes, blind to each other in opposite directions:** removing the socktab drop leaves the unit suite at a full **1321/1321 PASS** and fails only in-guest, boot-fatally, at `marker=L199`; turning the allow-list into a deny-list fails `vivarium.dup3_domain` at **1320/1321** on the bare `1u` case, which the guest legs cannot see because L194 tests `O_NONBLOCK` and a deny-list still refuses that. **THE GATE NOW PASSES END TO END** -- `L6C-A` through `L6C-I`, i.e. busybox sh forked, exec'd, piped, substituted, looped, nested and reaped -- so `L6C_GATE_FATAL` flips to 1 in the same commit: a gate that cannot redden is a disabled test, and six landed fixes (#149/#150/#151/#140/#155/#157) now have nothing watching them otherwise. Two things the passing run surfaced are enqueued rather than silently noted: **#159** (`viv_report_unserved` emits its line as SEVEN unbracketed `uart_puts` calls and a live log shows two reporters interleaved mid-word -- the #75/#76/#152 writer-set family, one site further) and **#160** (the next unserved set is `ioctl` 29 and `sendfile` 71, neither blocking, measured off the PASSING run). |
...
| A-4b cross-process kill (`/proc/<pid>/ctl` + `CAP_KILL`) | `kernel/devproc.c` (the `ctl` write parses `kill`/`killgrp` -> `proc_group_terminate` uniformly under `g_proc_table_lock` via the `proc_for_each` resolve+authorize idiom -- the `#811` wake-total primitive; new `stat_native` reporting the target's `principal_id`/`primary_gid`/`0600`; **`perm_enforced = false`** -- the two-axis check (owner [same `principal_id` on the `0600` ctl] OR `CAP_HOSTOWNER` OR `CAP_KILL`; computed DIRECTLY, NOT via the `perm_check` DAC-override -> `CAP_DAC_OVERRIDE` is NOT a kill axis, keeping fs-admin orthogonal to kill) runs at the WRITE site in `devproc_write`, NOT at open: the SHARED open chokepoint hard-rejects pre-`devproc.open` so the gate-at-open sketch could not host the `CAP_KILL` axis -- reconciled 2026-06-01, user vote). USER-REACHABILITY of `/proc` is a Utopia namespace seam (needs the `namec` multi-component mount-crossing resolver + a boot-path mount); A-4b lands + kernel-unit-tests the mechanism + authority (the privilege logic is fully in-kernel testable). | A-4b. **Invariant I-26 (cross-process control two-axis; composes I-22 + I-1).** Privilege boundary -- prosecute hard. Prosecute: no identity bypass of the kill gate; namespace-visibility containment; the parent-of-target case still works; no UAF resolving `<pid>` -> Proc under the lock (resolve+authorize+kill all under `g_proc_table_lock`); multi-thread cascade correctness. **No new spec** -- prose validation in IDENTITY-DESIGN.md §9.8 + this row + the audit + tests. **prowl-4 addendum (job-control suspend/resume via `/proc/<pid>/ctl`; PROWL-DESIGN.md §4.3/§6):** the `ctl` write gains `suspend`/`resume` verbs (`kernel/devproc.c` `CTL_VERB_SUSPEND`/`RESUME` + `devproc_job_walk_cb` + the dispatch) -- the UNCONDITIONAL Plan-9 /proc job-control stop: the walk_cb resolves + refuses kproc/non-ALIVE + applies the SAME I-26 `devproc_kill_authorized` gate as kill (stopping is strictly weaker than the killing that gate already authorizes -> NO new authority, NO new invariant, composes I-26) then calls `proc_job_stop_proc`/`proc_job_cont_proc` (`kernel/proc.c`, non-static). Those REUSE the PTY-1f (audited) `proc_job_stop_one_locked`/`proc_job_resume_one_locked` + the one hoisted `smp_resched_others` -- setting `job_stop_req` (I-20's SECOND stop owner; a target ALSO debugger-stopped stays parked on `debug_stop_req` per StopCompatI39) at the SAME EL0-return-tail / sleep-detour checkpoint the debug stop uses, so a suspended thread holds NO kernel lock while parked. DISTINCT from the DEBUG stop/start (8a-1b, attach-gated + `debug_stop_req`); the /proc job stop is uncatchable exactly as the /proc kill is (NO tty:susp note, NO catchability gate -- those are the pts SIGTSTP path's, `proc_job_stop_pgrp`). `/ctl/procs` STATE shows STOPPED for an ALIVE+job_stopped Proc (`kernel/devctl.c::procs_state_name`, the Unix `ps` T-state -- the debug stop is NOT surfaced, I-39-private) + a new PPID column (`p->parent->pid` under `g_proc_table_lock`) for the tree view. **Prosecute (the prowl-4 focused audit):** the I-26 gate completeness on BOTH suspend AND resume; kproc/non-ALIVE refused before the gate; the death/stop-lineage races (suspend racing `proc_group_terminate` -> death wins at the die-check-before-stop-check tail; the orphan rule hup+conts a suspended orphan); StopCompatI39 (a job resume never runs a debugger-stopped thread); the lock order (the pts-fan precedent, `g_proc_table_lock` across the wake helpers + `smp_resched_others`); the STATE/PPID read safety (`job_stop_req` read atomic, `p->parent` stable under the lock). Known job-control CAVEATS (inherent, documented, NOT kernel-soundness): a suspend of a userspace lock-holder others wait on can wedge that WORKLOAD (resumable); suspend-your-own-shell (the same class as kill-your-own-shell, already allowed via `k`). The tool (`usr/prowl/`) is pure userspace (a buggy prowl corrupts only its own screen; the kernel gates every read + the ctl write). **No new spec / no new §28 invariant** (composes I-26 + I-20's stop leg + I-9) -- prose in this row + PROWL-DESIGN.md §4.3/§6 + `docs/reference/32-devproc.md`/`33-devctl.md`/`144-prowl.md` + the prowl-4 focused audit + the `devproc.ctl_suspend_resume_dispatch` + `devctl.read_procs_format` tests + the `prowl.exp` E2E + the SMP gate. |
...
| JIT code Burrow + `SYS_ICACHE_SYNC` + `CAP_JIT` (the Clade arc, CL-7k; I-42) | **RESERVED — lands at CL-7k** (`docs/LLVM-DESIGN.md` §8 + `docs/JIT-ON-WX-DESIGN.md`): the dual-mappable **code Burrow** (one BURROW attached RW at `VA_w` + RX at `VA_x` in one Proc), `SYS_ICACHE_SYNC(range)` (the `dc cvau`/`ic ivau`/`dsb ish`/`isb` sequence lifted to a syscall), the `CAP_JIT` capability (elevation-only, non-rfork-grantable, the `CAP_HW_CREATE` class), and the userspace ORC `DualMapMemoryMapper` — first consumer Mesa's llvmpipe | **Invariant I-42; W^X-adjacent — prosecute hard (its own focused round, CL-7k).** Realizes JIT-ON-WX-DESIGN.md; composes I-12 (no PTE ever W∧X — two fixed-permission aliases of one physical region, the W1.5 self-patcher's discipline turned outward), I-2/I-6 (`CAP_JIT` monotonic), I-36 (anon code, Image cache untouched). Prosecute: the two-alias W^X-cleanness at PTE granularity (neither alias ever W∧X; the RX alias never precedes the `SYS_ICACHE_SYNC`); the `CAP_JIT` gate at code-Burrow create (unforgeable, non-heritable, elevation-only); the icache/publish SMP protocol (a second CPU sees the emitted bytes only after the sync — the F8 candidate for a focused spec if the CL-7 design pass surfaces genuine subtlety); BTI/PAC well-formedness of JITed code on hardened silicon; the code Burrow lifecycle (#847 dual-count — no UAF/leak across the two aliases + the ORC mapper's working/execution split). **F8 spec posture**: lean prose + audit (the W1.5 precedent — same mechanism, already trusted in-kernel), escalating only on real SMP subtlety. Prose: LLVM-DESIGN.md §8 + this row + the CL-7k focused audit + the `libthyla_rs::jit` prover (emit→sync→call; ungated Proc DENIED) + boot OK + the SMP gate. |
| Spawn-time page-budget (the Clade arc F4; I-32 composition, CL-5) | **RESERVED — lands at CL-5** (`docs/LLVM-DESIGN.md` §7): a `SYS_SPAWN_*` per-child **page budget** (an arg or perm) defaulting to today's `PROC_PAGE_MAX` (256 MiB) and raisable per-child up to a **global hard cap** (e.g. 4 GiB), so a real `clang -O2` TU / a clang-sized link (~2–4 GiB) fits without weakening the per-Proc fork-bomb floor 8× for every Proc | **Invariant I-32 composition — a privilege/DoS surface; own focused round.** The child's budget bounds its anon commit exactly as `PROC_PAGE_MAX` does today; raising it DoSes only that child's own budget, and the per-user *aggregate* quota stays the recorded I-32 seam. Prosecute: the budget is a spawn-time decision (never rfork-widened past the grant — the I-2 monotonic-reduction shape); the global hard cap is unbypassable (preserves the box-cliff protection — an over-budget fault still `proc_fault_terminate`s, never box-extincts); no counter race/underflow on the per-child page charge (the I-32 discipline); the default is unchanged (256 MiB — a non-toolchain Proc is byte-identical); the TCB exemption is unforgeable. No renumber (composes I-32's per-Proc floor). Prose: LLVM-DESIGN.md §7 + this row + the CL-5 focused audit + the per-axis budget tests + the SMP gate. |
| Process creation: `execve` + shared address spaces + COW `fork` (the LINEAGE arc, L-1..L-7; **I-44**) | **L-1 AS-BUILT**: NEW `kernel/include/thylacine/addrspace.h` + `kernel/addrspace.c` (`struct AddrSpace` -- refcounted; `ref` / `lock` [was `Proc.vma_lock`] / `pgtable_root` / `context_id` / `vmas` / the three I-32 axes `page_count`+`vma_count`+`shared_map_pages`; `addrspace_alloc`/`ref`/`unref`), `kernel/include/thylacine/proc.h` (the SEVEN fields out, `struct AddrSpace *as` in; all 23 offset `_Static_assert`s re-baselined, `sizeof(struct Proc)` 408 -> 376), `kernel/proc.c` (`addrspace_alloc` in `proc_alloc`, `addrspace_unref` at exactly the point the inline page-table destroy ran -- after `vma_drain`), + the 239 compiler-enumerated conversion sites across 22 files. **L-2a AS-BUILT**: `SYS_EXECVE = 101` (`kernel/syscall.c::sys_execve_handler` -- the ONE handler that takes `ctx` to rewrite the trapframe and whose success path deliberately writes NO return value), `kernel/exec.c::exec_load_into` (the file-backed load into an EXPLICIT AddrSpace; `exec_setup_from_spoor` becomes its Proc-side wrapper, keeping the two Proc STAMPS -- process name + `exe_path` -- OUT of the shared body so a failed execve leaves a live Proc advertising nothing it is not running), the AddrSpace-taking `vma_insert_in`/`vma_remove_in`/`vma_lookup_in`/`vma_drain_in`/`burrow_map_in` (the Proc forms become one-line wrappers -- ~90 call sites untouched), the I-32 counter arithmetic moved to `kernel/addrspace.c` (`addrspace_charge_pages`/`charge_vma`/`charge_shared_map` + uncharges; `proc.c` keeps the POLICY -- has-an-AS + `proc_resource_exempt`), `kernel/proc.c::proc_exec_alone` + `proc_exec_replace` (the gptl-serialized swap + the per-image resets: handler_va, sigtab, note_mask, TPIDR_EL0, FP/SIMD), `kernel/sched.c::sched_activate_addrspace` (install TTBR0 NOW, IRQ-masked, with an `isb`). **L-3a AS-BUILT**: `kernel/addrspace.c::addrspace_unref` (the VMA drain MOVES here, into the last drop) + `addrspace_ref_count`, `kernel/proc.c::proc_alloc_in(share)` (`proc_alloc()` becomes `proc_alloc_in(NULL)`) + `rfork_internal` accepting `RFPROC\|RFMEM` + the `proc_quiesce_owned_devices` sole-ownership gate + the RFMEM-without-a-space refusal, and the three drain call sites (`proc_free`, `proc_exec_replace`, the execve rollback) collapsing to a bare `addrspace_unref`. **L-3b AS-BUILT**: `SYS_RFORK = 102` (`kernel/syscall.c::sys_rfork_handler` -- the SECOND ctx-taking handler, for the mirror-image reason: execve REWRITES this frame, rfork COPIES it; non-static so the argument gate is reachable from a kernel test with a synthetic frame), `kernel/thread.c::fork_frame_init` (the pure decision) + `thread_create_forked` (the third creation shape -- RESTORES a frame instead of constructing one; carves it off the top of the child's own kstack where KERNEL_ENTRY would have, and inherits FP/SIMD from the LIVE registers via `fp_save_area`), `arch/arm64/vectors.S::thread_fork_trampoline` (release -> `el0_return_die_check` -> `daifset` -> `b .Lexception_return`), `kernel/proc.c::rfork_forked` + `rfork_internal`'s `struct fork_context *fc` arm, and `usr/lib/libthyla-rs::rfork_spawn` (the musl-`clone.s` shim). **L-3c-1 AS-BUILT**: `kernel/handle.c::handle_table_copy_into` (the index-PRESERVING cross-Proc copy) + `handle_slot_may_alias` (`handle_dup`'s admissibility test EXTRACTED so the dup gate and the fork gate cannot drift), `kernel/proc.c::rfork_internal`'s `if (fc) handle_table_copy_into(child, parent)` (gated on the FORK SHAPE, so the whole SYS_SPAWN_* family is byte-unchanged by construction). **L-3c-2 AS-BUILT**: `kernel/proc.c::vfork_child_released` (the release DECISION, non-static so a kernel test can drive all four cases) + `vfork_await_release` (the park -- `wait_pid_for`'s `#344` register-then-observe loop on `child_waiters`, verbatim in discipline) + `rfork_internal`'s `if (fc && (flags & RFMEM)) vfork_await_release(...)` tail + ONE new wake in `proc_exec_replace`, inside the same `g_proc_table_lock` section as the `p->as` swap it reports. NO new struct field, NO new flag: the release condition is `child->as != parent->as \|\| child not ALIVE`. **L-3d AS-BUILT**: `kernel/vivarium.c::vivarium_clone_decide` (PURE -- the DOMAIN, since the mapping itself is a constant) + one `{ VIV_LINUX_CLONE, VIV_TIER2 }` reject-table row + a `viv_tier2` case, and `kernel/syscall.c::sys_rfork_core` EXTRACTED from `sys_rfork_handler` (which becomes the native register-reader) so the phenotype shell and the native path share ONE implementation of the gate -- the V-8 `sys_fstat_for_proc` discipline. `viv_tier2` gains a `ctx` parameter, used by this one case. Also `VIV_NATIVE_CEILING` in `vivarium.h` + a `_Static_assert` pinning it to `SYS_RFORK` in `vivarium.c`, replacing four stale copies of the ceiling literal. **L-2b / L-4 / L-5 RESERVED**: the de_thread primitive (a per-Thread die flag; L-2a REFUSES a multi-threaded execve with `-EAGAIN`), the COW break arm + a real per-page share count, and lifting L-3b's `RFPROC`-alone refusal once COW exists. | **Invariant I-44 -- the address space itself; touches I-7/I-12/I-21/I-24/I-31/I-32/I-36; prosecute hard (the L-7 focused round).** L-1 is a PURE refactor whose gate is byte-for-byte behavioural identity, so prosecute it as such: (a) the `pgtable_root == 0` -> `as == NULL` conversion is COMPLETE **as a set** for the NINE Proc-field predicates -- a missed one does not fail to build, it silently treats a live user address space as a kernel Proc (the four `arch/arm64/mmu.c` sites are parameter validation on a bare `paddr_t` and correctly do NOT convert); (b) the MIRROR-IMAGE hazard, which the compiler also cannot catch: a value read that used to yield 0 for a kernel-only Proc now dereferences NULL -- the reachable ones are the Proc-table walkers (`/ctl/procs` + `/proc/<pid>` walk a tree whose ROOT is kproc), `format_maps`, `vma_drain` and `proc_quiesce_owned_devices` (both reachable from `proc_free`'s rollback path with a Proc that failed before `addrspace_alloc`), and the six I-32 charge/uncharge helpers -- each must yield what the old inline field produced (0, or an empty VMA list), which is what makes the refactor byte-for-byte rather than merely compiling; (c) the AddrSpace refcount balances on every path (alloc, rollback, exit) with no UAF/leak/double-free, and `addrspace_unref`'s last drop requires a drained VMA list + no CPU holding the table in TTBR0 -- both established by `proc_free` (vma_drain runs earlier; every thread reaped + `on_cpu`-spun); (d) I-31 is unaffected in SUBSTANCE by `context_id` moving (`asid.tla` re-runs UNCHANGED as the gate); (e) the I-32 caps stay EXACT (charged under the AS lock) and the per-Proc cap becoming a per-AS cap keeps a fork bomb bounded at N address spaces. **THE CONVERSION METHOD IS FORCED, NOT CHOSEN**: `page_count` and `context_id` are OVERLOADED names in this tree (`struct Burrow.page_count`; `psci_cpu_on(..., u64 context_id)`), so a grep/sed rename corrupts unrelated code silently -- the fields must be deleted from `struct Proc` FIRST and the compiler used as the completeness gate for the value reads (measured: 239 sites, all on `struct Proc`, zero false positives). **L-2a ADDENDUM -- prosecute the swap.** (a) the ORDERING is the safety argument: everything failable (argument copy-in, resolve, the whole ELF load) completes BEFORE `proc_exec_replace`, which is INFALLIBLE by construction -- a rollback path here would be a design smell, not a missing feature, and any change that makes the commit able to fail breaks "a failed execve returns with the caller intact"; (b) the arguments MUST be copied out of the old address space first -- after the swap those user VAs name the new image; (c) `sched_activate_addrspace` must run BEFORE `vma_drain_in(old)` + `addrspace_unref(old)`, and its `isb` is what discharges `addrspace_unref`'s "no CPU still translates through this root" precondition -- note the failure is SILENT, not a fault: the old ASID's TLB entries stay warm at the same stack VA, so removing the activate yields plausible-but-wrong argv read through freed page tables (revert-probed at L-2a); (d) the swap and its single-thread re-check live in ONE `g_proc_table_lock` section, which is also what makes the drop of `old` safe -- every cross-Proc reader of `->as` (`/proc/<pid>/{maps,mem}`, `/ctl/procs`, the weft reaper) resolves its target under that same lock; (e) the per-image resets must touch the HARDWARE, not just `ctx` -- execve erets straight back to EL0 with no context switch, so zeroing `ctx.tpidr_el0` / `ctx.fp_v` alone would leave the live TPIDR_EL0 and FPCR carrying the old image's state into the new one; (f) the multi-thread refusal is not a stub to be quietly relaxed: `group_exit_msg` is Proc-wide and set-once-never-cleared (I-24), so suppressing it for the execer would permanently break a LATER kill of the same Proc -- L-2b needs a genuinely new per-Thread flag. Coverage: `execve.{load_into_detached,load_into_rejects_dirty,failed_load_leaves_target_drainable}` (the detached build; non-vacuous on `vma_count`, since a build into `p->as` would install the same VMAs on the wrong object) + `/exec-probe` (the in-guest gate: re-execs ITSELF, and its post-exec PASS line is written to an fd INHERITED ACROSS THE SWAP, so "handles survive execve" rides the same assertion) + three revert probes with DISTINCT failures. **NOT asserted anywhere, deliberately**: that exec charges the I-32 PAGE axis. Measured -- it does not; that axis is charged at exactly three sites (`SYS_BURROW_ATTACH`, the Loom ring, the lazy fault arm) because it bounds the REPEATABLE anon vectors, while the exec image is one-shot and bounded by `EXEC_FILE_MAX`. A first draft of the test assumed otherwise and failed. **L-3a ADDENDUM -- prosecute the teardown move.** The chunk's substance is that the VMA drain moved from the callers into `addrspace_unref`'s LAST DROP. (a) That is a fix, not a refactor: draining at A PROC'S DEATH and draining at THE LAST REFERENCE coincide only while `ref` cannot exceed 1, and BOTH pre-existing callers had the bug latent -- `proc_free` would have had the first sharer to die unmap the survivor, and `proc_exec_replace` would have drained the outgoing space out from under a suspended vfork parent, which is the ORDINARY posix_spawn case rather than a corner. Any future caller that drains on its own reintroduces it. (b) The counterpart obligation is that nothing may take a reference between the decrement reaching zero and the drain: sound because a reference is only ever taken from a Proc that already holds one (rfork, from a LIVE parent), so zero means no holder is left to hand one out. (c) `proc_free`'s ORDERING is preserved deliberately -- `addrspace_unref` sits exactly where `vma_drain` did, BEFORE `handle_table_free`, because each `Vma` carries a `burrow_unmap` and a Burrow with `mapping_count > 0` must not free even when `handle_count` hits 0; what moves EARLIER is the page-table destroy, which has no stated ordering requirement against handles and is order-independent under the #847 dual refcount. (d) The device quiesce's VMA walk gates on SOLE ownership for the same reason as the drain -- an MMIO mapping belongs to the space, so resetting it while a sharer lives pulls the hardware out from under a running driver; the walk over the per-Proc HANDLE table needs no such gate today only because hardware handles are non-transferable (I-5), which a handle-table copy would break (#119). (e) `RFMEM` from a Proc with no address space is REFUSED rather than downgraded, so "the flag was honoured" and "the flag was ignored" are distinguishable from outside. (f) I-31 needs nothing: `asid_resolve` keys on `as->context_id`, so one space is one ASID however many Procs hold it -- a direct dividend of L-1, and `asid.tla` re-ran unperturbed (443457 distinct; the reported DEPTH is a worker-scheduling artifact -- measured 17 and 18 on back-to-back `-workers auto` runs, stable 17 under `-workers 1` -- so the distinct-state count is the figure to compare). Coverage: `addrspace.share_drains_at_last_ref` (THE I-44 claim -- a non-final unref must leave the mappings, the final one must take them), `addrspace.proc_alloc_in_shares` (pointer identity, not equal contents), `proc.rfork_rfmem_refuses_without_addrspace` (through the REAL rfork, with an RFPROC control leg so a broken rfork cannot pass it) -- each revert-probed to a DISTINCT failure with no collateral. **A successful RFMEM rfork has no kernel-test witness** and deliberately so: the only Proc a kernel test runs on is kproc, which has no address space by construction, and lending it one while a child thread ran would make every `as == NULL` kernel-Proc gate answer the wrong question -- the end-to-end proof lands with the EL0 surface at L-3c, the same split L-2a made. **The probe found a defect in its own test**: `TEST_ASSERT` *returns* on failure, so a cleanup placed below an assertion never runs on precisely the path that needs it -- the leaked child surfaced as `wait_pid pid mismatch` in unrelated tests. Reap before asserting. **spec-first RE-ENABLED for this surface** (re-enabling point (a)): the model lands TLC-green BEFORE the L-4 impl, with buggy cfgs for break-vs-break, break-vs-teardown, and the lost-VFORK-wake. **L-3b ADDENDUM -- prosecute the resume.** The tree's FIRST syscall that returns twice. (a) `fork_frame_init` is the whole decision and must stay exactly two edits: `regs[0] = 0` (the only thing distinguishing the two Procs) and `sp = child_sp` (MANDATORY -- two Procs sharing an address space must not share a stack pointer). Everything else verbatim, and each omission is load-bearing: `elr` verbatim IS 'resumes at the same instruction'; `spsr` verbatim carries the parent's NZCV, live if a conditional follows the syscall, and is not EL0-forgeable because the hardware wrote it on exception entry; x1..x30 verbatim because the child CONTINUES the parent's C frame. (b) The copy must stay FIELD-BY-FIELD -- `*dst = *src` on 288 bytes compiles to a `memcpy` the freestanding kernel does not link (the FP block in `context.h` records the same hazard). (c) `thread_fork_trampoline` must stay IN `vectors.S`: it branches to `.Lexception_return`, which is deliberately a LOCAL label so no kernel caller can eret on whatever sits on the current SP -- branching from inside vectors.S is the use that label's own comment sanctions, and reusing the one audited KERNEL_EXIT beats a second hand-rolled eret on this surface. (d) The ABSENCE of a GPR-zeroing sweep is deliberate and must not be 'fixed': `thread_user_trampoline` zeroes because a fresh thread must not inherit kernel residue, but this child is continuing its parent's EL0 frame, so every register it loads is a value userspace already had -- zeroing would break the fork. (e) The child's frame sits at the TOP of its own fresh kstack (16-aligned by construction, asserted not assumed -- a misaligned SP at the eret faults with the frame half-restored) and the trampoline's own C calls run BELOW it. (f) FP/SIMD must come from the LIVE registers, not the caller's saved `Context`, which holds its last switch-OUT values and is stale while the caller runs. (g) The argument gate lives in the HANDLER, ahead of `rfork_forked`, which is what makes every rejection reachable from a kernel test; `RFPROC` alone is REFUSED (a fresh empty address space + the parent's PC = an instruction-fetch fault on the first cycle), and the `child_sp == ctx->sp` check is a footgun-catcher, NOT a safety property -- an overlapping stack is equally fatal and undetectable here, so non-overlap remains the caller's contract exactly as for a pthread stack. (h) The child's handle table is fresh and EMPTY at L-3b; the copy is L-3c-1 and #119 is its hazard. **CORRECTED at L-3c-1**: this clause originally called the empty table "`CLONE_VM` without `CLONE_FILES`, posix_spawn's shape" -- it is not. Linux's `CLONE_VM` WITHOUT `CLONE_FILES` gives a COPIED fd table, which is precisely why posix_spawn's `dup2`/`close` file_actions do not disturb the parent (LINEAGE section 2.4 measured this one section away). Empty and copied are different shapes, and the wording made a deferral read as a deliberate ABI match. **THE GATE THIS CHUNK WAS GIVEN WAS UNPROVABLE.** L-3a's table said 'a KERNEL-DRIVEN RFMEM child resumes'; measuring showed a kernel test cannot observe an eret to EL0 at all -- kproc has neither an address space to share nor a trapframe to fork from -- so the resume is not merely untested there but unobservable, and `SYS_RFORK` had to land here rather than at L-3c. Coverage: `fork.frame_init` + `fork.rfork_arg_rejection` (the decision + the gate, both from kproc; the rejection test carries a WELL-FORMED leg that must fail LATER, so 'validated' and 'refuses everything' cannot look alike) + `/fork-probe` (six legs, boot-fatal: resume, sharing, own-stack, distinct pid, reapable, and the gate live at EL0). **Revert-probed to the sharpest pairing the arc has produced**: making `thread_create_forked` ignore `child_sp` leaves the unit suite FULLY GREEN while only the in-guest leg fails. **And a probe that fails to sabotage reports the same green as a correct system** -- an earlier attempt short-circuited on a failed `cp` and never applied its edit, so a sabotage must be VERIFIED present before the build, not assumed from the script having run. **L-3c-1 ADDENDUM -- prosecute the copy.** (a) The gate is the FORK SHAPE (`fc != NULL`), and that is a statement about CONTRACTS: `SYS_SPAWN_*` promises "fresh + exactly the endowed `fd_list`" (a capability hand-over) while `SYS_RFORK` promises "the child is the parent" -- so inheritance is what the two calls MEAN, not two settings of one knob. Every existing `rfork`/`rfork_with_caps` call site (joey + the five spawn thunks) passes a kernel `entry` and no `fc`, so the spawn family is untouched BY CONSTRUCTION rather than by a flag a future caller could set wrong; a change that copies unconditionally would hand every spawned child its parent's whole table, defeating the endowment mechanism. (b) SLOT INDICES ARE PRESERVED and a skipped slot leaves a HOLE -- this is why the copy cannot be a loop over `handle_dup`, which installs into the first free slot: one skip would renumber every descriptor after it and the child's inherited stdout would land elsewhere. (c) The admissibility test MUST stay the single shared `handle_slot_may_alias`: `handle_dup` and the fork copy both create a second handle naming one object, so they face the identical hazard, and a new `kobj_kind` is then refused by both until deliberately classified. Its two clauses are independently load-bearing -- the KIND clause (I-5 hardware, Srv, Loom) and the OBJECT clause (a devsrv Spoor, `dc=='s'`, the only case where the kind is admissible and the object is not; a copy written against kinds alone would inherit a `/srv` connection and blur the kernel-stamped SO_PEERCRED origin that handles.tla's SrvHandlesAtOrigin rests on). (d) SKIP, NOT REFUSE, is the stated choice: I-5 is a property of the HANDLE -- pinned to the Proc that created it -- not of forking, so the fork proceeds and the child does not hold what it was never eligible to hold. Refusing would punish a parent for holding a handle it never meant to pass and leave a driver unable to create a process; a child needing hardware authority gets it via the warden's confer path (I-34). The hole is observable (EBADF there; the child's next open lands where Linux's would not) and that is the honest report. (e) Rights are carried VERBATIM -- I-6 is satisfied by non-increasing, and narrowing would make an inherited fd less capable than the one it was forked from. (f) Lock order is SOURCE then DESTINATION, acyclic because this is the only two-table operation and `dst` is an unpublished Proc; holding the source lock across the WHOLE walk is what makes the child's table a coherent snapshot against a peer thread of a multi-threaded parent (the #844 hazard). (g) The copy CANNOT FAIL (no per-slot allocation; destination known-empty and same-sized), so a later rfork failure's `proc_free` rollback is already the correct release path. **The two coverage layers are blind to each other in BOTH directions, demonstrated not asserted** -- three revert probes: admitting every kind fails the unit suite at TWO assertions in TWO tests (the kind clause and the object clause, so a fix to one cannot mask the other); removing `rfork_internal`'s CALL leaves the unit suite at a FULL 1285/1285 PASS while only `/fork-probe` leg H fails; compacting instead of preserving indices fails at the HOLE assertion, a different one again. The kernel test can reach `handle_table_copy_into` directly but nothing kernel-side can reach rfork's CALL to it, which sits behind an RFMEM gate kproc can never pass -- the rule and the wiring need separate proofs. Coverage: `fork.table_copy` (a layout where a skipped handle sits BETWEEN two copied ones, so preservation and compaction are distinguishable; a real Burrow makes the refcount claim measured on both edges) + the `devsrv.open_connect_byte` copy leg + `/fork-probe` leg H (a child writes to a pipe fd only the parent opened). **L-3c-2 ADDENDUM -- prosecute the suspend.** (a) THE SUSPEND IS NOT REQUESTED BY A FLAG, and that is the chunk's decision. Every `RF*` bit answers "what does the CHILD get?" under a stated polarity of `set == share`; suspending the PARENT answers a different question, so a bit there would be a category error and would cost the word the property that makes `RFPROC\|RFMEM` legible as posix_spawn's child. Linux puts `CLONE_VFORK` beside `CLONE_VM` only because its clone word is a grab-bag with no polarity to protect, and Plan 9 has no vfork at all (`RFNOWAIT` is about the children list) -- neither heritage settles it. Keying on `RFMEM` is not an arbitrary coupling: RFMEM is EXACTLY the precondition of the hazard, since sharing the address space is the only way the child can reach the parent's frame. The fail-safe direction is one-sided and decides the default -- an unwanted suspend is a visible, terminating, diagnosable block, while an unwanted concurrency is memory corruption three layers from its cause -- and concurrent shared-memory execution is anyway already served, better, by `SYS_THREAD_SPAWN`. Any change that admits concurrent RFMEM must do so EXPLICITLY, with its own reasoning, exactly as every reserved `RF*` bit will. (b) The gate is `fc && (flags & RFMEM)` rather than plain `fc`, which is what keeps L-5 correct WITHOUT a future edit: stock `fork()` is `RFPROC` alone and both Procs must run. (c) THE RELEASE CONDITION IS NOT A RECORD OF THE RELEASE, IT IS THE RELEASE -- `child->as` is already the fact "the child no longer maps my space", so a flag set at fork and cleared at exec/exit would be strictly worse: it records the release somewhere other than where the release happens, and a third release path added later would silently strand every vfork parent. Three ways out and this covers all of them: exec (`proc_exec_replace` swaps in a fresh AddrSpace), death (not ALIVE), gone from the children list (`child == NULL`). Nothing else can change the pointer, because the only other route to a private space is a fork the child cannot perform (`RFPROC` alone is refused) and an exec the parent cannot perform (it is parked). (d) The POINTER COMPARISON is sound only because the parent still holds a reference to the shared AddrSpace, so the outgoing object cannot be freed while the parent is parked and its address cannot be recycled underneath the comparison -- a direct dividend of L-3a's teardown move; a change that drops the parent's reference earlier reopens an ABA. (e) NOT-FOUND counts as RELEASED deliberately: the only two dispositions are "resume" and "hang forever", and a parent that resumes slightly early corrupts a frame the child has already stopped using, while a parent that hangs looks unkillable and never recovers -- fail towards the one that terminates. (f) Parking on `child_waiters` is what makes the DEATH release free (`proc_become_zombie_locked` already wakes it); only the EXEC release needed a new wake, under the same lock as the swap, and it wakes UNCONDITIONALLY rather than testing whether anyone is suspended, because a spurious wake costs a re-scan whereas a test would be a second place that has to agree with the park about who is waiting. (g) #811: a parent killed while parked returns `SLEEP_INTR` and unwinds to its EL0-return die-check without looping (re-sleeping would re-INTR forever) and leaves nothing behind -- it registered no state anywhere but its own stack. (h) **`child` MUST NOT be dereferenced past `ready(ct)`** -- the pid is captured into a local before it. THE SELF-AUDIT FOUND THIS: `rfork_internal` ended `return child->pid` with no lock held, and a peer thread of a multi-threaded parent in `wait_pid_for(-1)` can reap that child (wait_pid_for unlinks under gptl, DROPS the lock, then proc_frees), so the pointer can dangle. The window PREDATES L-3c-2 (`ready()` -> `return`, a handful of instructions, since L-3b), but the park does worse than widen it: it stretches it to the child's entire lifetime AND **aligns** it, because the park's release edge is the child's death, which is the very edge that makes the child reapable -- the peer's wake and ours are the SAME `child_waiters` wake. Generalizable: **a new park inherits every unsynchronised access that follows it, and converts "narrow" into "aligned with the wake that ends it"** -- so the audit question on any added sleep is not "is this new code correct" but "what did putting a sleep here do to the code after it". **THREE COVERAGE LAYERS, EACH BLIND TO THE OTHERS**, demonstrated by two revert probes: removing `rfork_internal`'s park leaves the unit suite at a FULL 1286/1286 PASS while only `/fork-probe` leg I fails (plus an orphan-adoption line showing the child had not run at all); removing `proc_exec_replace`'s wake leaves the suite green AND HANGS THE BOOT dead after `exec-probe`. That second failure mode is not a probe artifact -- it is the bug's real production symptom, since a `posix_spawn` parent whose child execs would park forever the same way, so a boot that stops between `exec-probe` and `fork-probe` should have this wake checked first. Coverage: `fork.vfork_release` (all four release cases, deterministic -- the two failing directions are opposites and a wrong predicate usually gets only one: never-release hangs every posix_spawn parent, always-release lets the parent run on a frame still in use AND would leave every other fork-probe leg passing, since they all reap before observing) + `/fork-probe` leg I (the death arm; the wiring proof, NOT race-free in its failing direction -- the window is a handful of instructions against a whole context-switch-in, measured by revert probe rather than assumed) + leg J (the exec arm, the only leg covering the new wake: its discriminator is that the child is STILL ALIVE when the parent looks, which is a fact rather than a race because the exec'd successor blocks reading a pipe only the parent can write). **L-3d ADDENDUM -- prosecute the translation.** (a) THE ROW READS ONLY TWO ARGUMENT WORDS, and that is a correctness requirement rather than an economy. arm64 is `CONFIG_CLONE_BACKWARDS` (`flags, stack, parent_tid, tls, child_tid` -- tls BEFORE child_tid, not the x86-64 order), but the sharper fact is that on the call that matters x2/x3/x4 hold GARBAGE: `posix_spawn` invokes `__clone(child, stack, flags, arg)` with FOUR arguments (`posix_spawn.c:198`) and musl's `clone.s` then executes `mov x2,x4 / mov x3,x5 / mov x4,x6`, moving three registers the caller never set. Linux tolerates it because `CLONE_PARENT_SETTID` / `CLONE_SETTLS` / `CLONE_CHILD_SETTID` are clear; so does this row, by REFUSING all three in the admitted flags word and passing a LITERAL 0 for `child_tls`. A translator that reached for `args[3]` would hand the child an uninitialised register as its `TPIDR_EL0` and fault it at its first thread-local access, far from the clone. This is the INVERSE of the arity property this row's V-1b addendum states for T1 rows -- there the target reads MORE words than are supplied; here the words are supplied and MEANINGLESS -- and both reduce to: read a register only when the call's own contract says it holds something. (b) `CLONE_VM` WITHOUT `CLONE_VFORK` IS REFUSED, and this is the one place L-3c-2's reasoning does NOT carry over. L-3c-2 argued the fail-safe direction is one-sided (an unwanted suspend blocks visibly; an unwanted concurrency corrupts silently) -- true for a NATIVE caller reaching `SYS_RFORK` through an ABI whose only shape is the vfork one. A stock Linux binary that sets `CLONE_VM` and clears `CLONE_VFORK` has said "do not suspend me" in the only vocabulary it has, and serving it with a suspend converts a working program into a DEADLOCK whenever the child neither execs nor exits promptly. So the domain is an EXACT equality and the caller gets an honest decline; the genuinely concurrent shape keeps its correct target (`CLONE_THREAD` onto `SYS_THREAD_SPAWN`, whenever that row is written). (c) A ZERO `stack` DECLINES, which is what keeps `vfork()` proper out of scope -- Linux reads it under `CLONE_VM` as "share the parent's stack", safe there only because `CLONE_VFORK` suspends the parent; `SYS_RFORK` refuses a zero `child_sp` by contract and weakening a landed kernel gate to widen a phenotype row is the wrong direction. (d) The exit signal is the LOW BYTE, not a flag, and only SIGCHLD is admitted: `exits()` posts `child_exit` unconditionally (I-19), so any other request -- including 0, "no signal", which is what a detached child asks for -- would get a note it did not ask for. (e) `sys_rfork_core` EXTRACTED rather than duplicated, so the phenotype shell and the native handler share ONE copy of the `child_sp` gate (alignment / range / equal-to-caller); a second copy would drift. (f) CLONE IS A `VIV_TIER2` ROW, NOT AN INTERCEPTION -- unlike `rt_sigreturn`, which is handled ahead of the table because it REWRITES the frame instead of returning a value, clone returns its result the normal way into the PARENT's frame (the child's `regs[0]` was set to 0 in its own COPY by `fork_frame_init`, and the child is a different Thread on a different stack that never returns through the dispatcher). `viv_tier2` therefore gains a `ctx` parameter rather than the row gaining a second dispatcher. (g) THE FLAGS COMPARISON IS FULL 64-BIT WIDTH, unlike every other decide in `vivarium.c`, and the asymmetry is deliberate -- **the SELF-AUDIT found the first draft narrowing to `(u32)` by copying `vivarium_mmap_decide`'s shape without re-deriving its justification.** mmap/openat narrow because their Linux parameters ARE `int`; clone's `flags` is an `unsigned long`, so narrowing is an assumption about Linux's own source rather than about its ABI -- and that source is not in this tree, so it cannot be checked and must not be asserted from memory. Under that uncertainty the stricter reading is the correct one, because this file's own rule is that DECLINING IS ALWAYS SAFE while admitting an unreasoned bit is the failure mode the tier exists to prevent; and it costs nothing measurable, since musl's `clone.s` zero-extends (`uxtw x0,w2`, verified in the built object as `ubfx x0, x2, #0, #32`) so the high half is always 0 from the real consumer. Generalizable: **a shape copied from a sibling carries that sibling's justification, which may not survive the copy** -- re-derive it rather than inheriting it. (h) **WHAT THIS ROW MAKES REACHABLE AND DOES NOT FIX (task #127).** `rfork_internal` copies `phenotype` but NOT `sigtab`, and the comment there -- written at #102 F7 -- justified the gap as UNREACHABLE: "no clone/fork/execve number is a table row, so a PHENO_LINUX Proc cannot create another Proc at all." **L-3d makes clone a table row, so that sentence is now FALSE** and the seam is live: `child->sigtab` stays NULL (all-SIG_DFL) while POSIX `fork(2)` inherits both caught handlers and SIG_IGN. Deliberately not fixed here, for the reason the comment itself gives: `execve(2)` needs the OPPOSITE rule (reset caught to SIG_DFL, PRESERVE ignored), so it is two behaviours and a design decision rather than a copy, and the sigtab is on the V-6 audit surface; v1.0 exposure is narrow because the only admitted clone shape is vfork-then-exec and musl's posix_spawn child resets its own dispositions before exec'ing. It belongs with execve/wait4 at L-6. The comment WAS corrected in this chunk to say reachable-but-unfixed rather than unreachable -- **a stale unreachability claim is precisely what stops the next reader from asking**, and it is the same class as the native-ceiling literal this chunk also retired. **THE GATE WAS NOT THE ONE THIS CHUNK WAS GIVEN**, and measuring is what showed it: section 7 said "a stock `posix_spawn` binary runs in a vivarium", but reading what posix_spawn drives shows the child needs `execve` (221) and the parent usually `wait4` (260) -- neither a table row, and neither a dependency of the CLONE TRANSLATION. They are dependencies of posix_spawn, which is L-6's deliverable and whose translators carry their own reasoning (execve must walk a `char *[]` and repack it into `SYS_EXECVE`'s concatenated blob). Pulling both here would have made L-3d into L-6 rather than completing L-3d, so the gate became: a clone whose child runs, writes into the shared address space, and exits, with the parent suspended until it does. Same correction L-3b's gate needed, for the same reason. Coverage: `vivarium.clone_domain` (every decline BY NAME and BY REASON -- three independent classes of widening, so a mask test would admit all of them at once; plus a `VIV_TIER2` pin so a future edit cannot demote the row to T1 and copy all six words verbatim) + `viv-pheno-probe` legs L155-L163 through `__viv_clone`, a transliteration of musl's `clone.s` (necessary because the child returns on a DIFFERENT STACK holding the parent's x29/x30) that diverges from musl in exactly one way: it loads RECOGNISABLE POISON into x2/x3/x4 rather than leaving them uninitialised, because "uninitialised" is the real hazard but is not a value a test can assert against. **Two revert probes, the layers blind BOTH ways**: gutting the pure domain check fails the unit suite at exactly its own assertion (1286/1287) and never reaches the guest; breaking only the SHELL (reading the garbage `args[3]` as tls) leaves the unit suite at a FULL 1287/1287 PASS while the guest fails at exactly `marker=L162`. **AND THE FIRST TWO RUNS OF THAT SECOND PROBE WERE A FALSE GREEN** -- `viv-pheno-probe`'s CONTAINERED copy lives in the POOL (`/vivarium`, baked by `populate_stratum_pool`), which `THYLACINE_MKFS_PRESERVE=1` skips, so both the production run and the revert probe executed a binary that predated the new legs entirely while the ramfs copy disassembled correctly. A chunk adding viv/diorama/alpine-bundle legs must take one `PRESERVE=0` build before trusting any result (task #126). **L-4 AS-DESIGNED (user-voted 2026-08-02; LINEAGE section 5.4 "As designed at L-4", section 2.8/2.9/2.10 the measurements)**: Q3's "promote `struct page.refcount` or carry a separate structure?" was answered by measuring, and the measurement REFRAMED it. LINEAGE section 2.8's two stated facts were BOTH false -- SLUB writes the field at seven sites (`mm/slub.c:173/190/215/221/251/255/264`) as a slab INUSE count reaching `objects_per_slab` (85 for a 48-byte cache), so it is neither "written only by buddy + magazines" nor "never incremented above 1"; the proof is the very line 2.8 quoted, `page.h:48` = `"BURROW refcount placeholder; slab: inuse count"`, whose half after the semicolon the original text dropped. Two further measured facts: the field is written PER-BLOCK-HEAD (`buddy.c:209` sets it on the returned head only, so all 2^order-1 tail pages hold stale values from previous use), and promotion would mean maintaining it per-page across `alloc_locked`/`free_locked`/`zone_free_chunk`/`buddy_zone_init` -- a loop on both hot allocator paths for a property only user anon pages need. **But the counter was never the obstacle.** A COW break is per-page, and the two anon types own memory oppositely: `BURROW_TYPE_ANON_LAZY` owns sparse order-0 pages in `filepages[]` freed individually (`burrow.c:380`), while eager `BURROW_TYPE_ANON` owns ONE `alloc_pages(order)` block freed by order (`burrow.c:320`) -- and you cannot free one page of a buddy block, nor split an ALREADY-ALLOCATED one. So eager anon has no per-page thing for any count to index, and EVERY writable anon a fork must break is eager: the 1 MiB stack (`exec.c:143`), writable data+bss (`exec.c:519`, `:93`), eager `SYS_BURROW_ATTACH` (`syscall.c:4331`); the lazy path is reached only by `SYS_BURROW_ATTACH_LAZY` (`syscall.c:4457`, the mmap heap). Heritage + SOTA converge unanimously on per-page ownership INSIDE the memory object (Plan 9 `Segment` -> per-page `Page *` with `Page.ref` the share count + `fixfault`->`duppage`; Linux `do_wp_page` over per-page `_refcount`/`_mapcount`; Zircon `VmObjectPaged`+`VmPageList` COW clones; seL4 has no kernel COW), which is the shape Thylacine ALREADY has in ANON_LAZY. Hence the split: **L-4a** moves exec's stack + writable data/bss to `burrow_create_anon_lazy`, pre-populating only the `ceil(filesz/PAGE_SIZE)` leading slots (I-36-4 UNTOUCHED -- the file bytes are still copied in at exec time, just per-page; no userspace mapping becomes file-backed, which would be a private file-backed mapping and a far larger design), and **L-4b** adds the per-page share count + the break arm. **L-4b AS-DESIGNED (user-voted 2026-08-02; LINEAGE section 5.4's L-4b correction)**: the count is PER-PAGE -- a NEW `struct page.cow_share` taking the existing free `_pad` (so `sizeof(struct page)` stays 48 and the per-RAM BSS reservation is unchanged) -- NOT the double-booked `refcount`, and NOT a per-slot array, which is unimplementable: the break must put the private page somewhere, and it can go neither in a shared Burrow's slot (another sharer still needs the pristine page there) nor in the PTE alone (nothing frees user pages from a page table -- `vma_drain` frees `Vma` structs and drops mapping refs, `proc_pgtable_destroy` frees TABLE pages, so a PTE-owned page would leak at teardown), so a fork CLONES the Burrow per address space (Plan 9 `dupseg`: same pages, its own `filepages[]`) -- after which a slot-indexed count describes a page its slot no longer holds, and while take-in-place survives that (the entry is the SUM over the groups sharing it, so `== 1` still implies a sole holder) the FREE decision corrupts (one group drives the entry to zero and frees its page, leaving another group's page at zero to leak or underflow). The Zircon Burrow-tree alternative preserves the per-slot array and was rejected on blast radius: it makes `filepages[]` PARTIAL, which is the assumption all ~20 of its readers in `burrow.c`/`fault.c` rest on. **Prosecute at L-7**: the `cow_share` contract is ESTABLISHED-NEVER-INHERITED at every site that puts a page into an anon slot (measured: a closed set of three -- `burrow_lazy_populate`, the demand-zero install, the break; every other `filepages[]` writer is `BURROW_TYPE_FILE`, never broken), else the field becomes the section-2.8 hazard it was named to avoid (stale count -> premature free or leak); the decide is ONE atomic step under the GLOBAL COW leaf lock (per-Burrow cannot serialise it -- two sharers hold different Burrow locks; `cow.tla::BUGGY_BREAK_UNLOCKED` is the counterexample), held across the decide only and never across the copy or the allocation; the break-vs-teardown pin (`BUGGY_TEARDOWN_NO_PIN`); the fork's write-protect of the parent's ALREADY-INSTALLED writable PTEs (a missed range leaves a sharer writing through a stale writable PTE -- silent cross-address-space corruption, the I-44 violation); W^X across the break (the private page installs RW/XN, never executable -- the L-4a I-cache argument); and the I-32 charge for the breaker's private page. **L-4b-2 AS-BUILT (landed):** `kernel/burrow.c::burrow_clone_cow` (dupseg -- a SEPARATE Burrow, same size/type, its OWN `filepages[]` holding the SAME page pointers, one `cow_page_get` per resident slot under `src->lock -> g_cow_lock`; a non-resident slot stays NULL, which is correct rather than lazy -- an untouched page reads as zero, so each side demand-zeroes its own after the fork) + `burrow_lazy_swap_slot` (the break's commit, keeping `filepages[]` and `v->lock` burrow.c's alone); `kernel/addrspace.c::addrspace_clone` (two passes under `src->lock`: pass 1 builds the child's VMA list, pass 2 -- ON SUCCESS ONLY, so a refused fork leaves the parent untouched -- flags the parent and drops its writable PTEs); `arch/arm64/fault.c` the break arm + the new `install_prot` local (defaults to `vma->prot`, narrowed by exactly one arm) + `copy_page_words` (the REVENANT page-copy helper hoisted above `demand_page_locked` and renamed off its `revenant_` prefix -- COW is its second caller and it is a plain page copy, not REVENANT's). VMA DISPOSITION: `ANON_LAZY` cloned INCLUDING read-only ones (sharing those would be safe, but cloning keeps ONE-Burrow-ONE-address-space uniform, and that property is what lets the break's slot swap be serialised by the faulting address space's own lock); `FILE` SHARED (REVENANT's gate admits only non-writable segments, so nothing to break -- and sharing is the point of the I-36 Image cache; a writable FILE VMA is refused as an impossible shape); guard VMAs reproduced (dropping one silently deletes the child's stack guard); eager `ANON` / `MMIO` / `DMA` / `SHARED_IN` REFUSED, fork fails whole. **THE WRITE-PROTECT IS AN UNINSTALL** (`mmu_uninstall_user_range`), not a permission rewrite -- one extra fault per page, no new MMU primitive. **THE BREAK MUST CLEAR THE STALE PTE FIRST**: `mmu_install_user_pte` REFUSES a mismatching install over a valid leaf (returns -1 rather than overwriting, `mmu.c:1649`) and BOTH break outcomes mismatch -- the copy path changes the PA, take-in-place changes the permission bits -- so, since a read installs read-only, the first COW write after a read would fail its install and kill the Proc without the `mmu_uninstall_user_pte` at the top of the write branch (regression `cow.break_read_then_write_copies`, revert-probed: 1297/1298 failing at exactly that assertion). **THE I-32 CHARGE IS TAKEN AT THE FORK, NOT THE BREAK** (each address space maps the page so each counts it -- the Linux RSS reading; deliberately over-counts physical memory between fork and break, in the safe direction, so the fork fails where the failure can be reported instead of the break OOMing where it cannot). The impl realizes the model's separate `pin` WITH the retained share (the breaker holds its own share across alloc+copy and `cow_page_put`s only when the copy is done) -- strictly stronger, since a held share also keeps the count off zero, so this REFINES `cow.tla` rather than deviating. `VMA_FLAG_COW` is NEVER cleared (the flag is routing, the per-page count is the truth). Regressions `cow.{burrow_clone_shares_resident_pages, addrspace_clone_shares_and_writeprotects, addrspace_clone_refuses_and_leaves_parent_intact, break_read_then_write_copies, break_sole_holder_takes_in_place}`, each revert-probed at its own assertion -- and the refuse test needed a MIX of a lazy and an eager VMA, because with only the eager one the flag-pass-ungated probe FAILED TO FIRE (the sole VMA was not a COW candidate, so pass 2 skipped it either way: the assertion named a property it could not observe). L-4a is a strict improvement independent of COW: measured `llvm-readelf -l`, the writable PT_LOAD is ~all `.bss` (`ut` FileSiz 0 / MemSiz 0x61; `joey` 8 B / 0x56349; **`corvus` 128 B / 0x1802c91 = 24 MiB**), and `map_eager_from_file` sizes the Burrow by MEMSZ -- so corvus's exec today calls `burrow_create_anon(0x1803000)` -> 6147 pages -> `order_for_pages` = 13 -> `alloc_pages(13, KP_ZERO)` = **8192 pages = 32 MiB allocated AND zero-filled for 128 bytes of real data**, 2045 pages of it pure power-of-two rounding (task **#130**); it also subsumes task **#49** (the lazy demand-grown EL0 stack IS a lazy stack Burrow). DMA buffers + Weft rings STAY eager and that is not an inconsistency -- they need PHYSICAL CONTIGUITY (a device DMAs into a PA range; a Weft ring is registered whole), a property of the backing rather than of the ownership model, which exec's segments never needed and had only because `burrow_create_anon` was the one anon constructor when they were written. Prosecute at L-7: the leading-slot pre-population carries the right bytes at every `filesz` (0, sub-page, exactly-page-multiple, and spanning into bss); the lazy stack's init frame (argv/envp/auxv, written at the TOP) is populated or faults correctly; I-32 charge/uncharge stays balanced when a segment's pages arrive by fault instead of at create; W^X (I-12) holds on every newly-lazy segment (RW/XN, never X -- text is NOT converted); and no DMA/Weft caller silently acquires a non-contiguous backing. **L-4a AS-BUILT (landed; the prerequisite half).** The gate is `seg_may_be_sparse()` = **NOT executable** (`(flags & PF_X) == 0`), which is BROADER than the design's "writable" and for a reason the design pass did not have: **the ANON_LAZY demand-zero fault arm performs NO I-cache maintenance**, so an executable `.bss` tail arriving through it would map executable with a prior occupant's lines live in the I-cache -- #107's hazard, which the eager paths close by syncing the span and REVENANT's file-backed arm closes per page. Every executable page therefore stays on a path that already syncs; W^X (I-12) makes `PF_W` imply `!PF_X`, so all writable data is covered either way and the extra reach is only the rare R-only-with-bss-tail segment. Converted: `exec_map_segment` (the blob path -- which loads **joey**, so this is a live boot path, not test-only), `map_eager_from_file` (where #130's 32 MiB lives), and `exec_map_user_stack`. NEW primitives `burrow_lazy_populate(as, exempt, v, first, n)` (all-or-nothing; charges the whole run BEFORE allocating so a run straddling `PROC_PAGE_MAX` is refused whole, not half-populated; `as->lock` per `addrspace_charge_pages`' precondition; the buddy is entered under NEITHER `as->lock` nor `v->lock`, keeping burrow.c's leaf discipline) + `burrow_lazy_slot_kva(v, slot)` (raw page pointer, sound ONLY under the stated caller-privacy precondition: exec creates, populates, fills, and only then maps). `exec_build_init_stack` now stages the argv/auxv frame in a transient `kzalloc` (bounded by `EXEC_INIT_STACK_MAX_SIZE` ~68 KiB worst case; 176 B in the argv-less shape) and copies it in per-page, which makes it FAILABLE -- it returns 0 as the sentinel and both callers turn that into -1. **A SEMANTIC CHANGE worth prosecuting: exec-image pages are now on the I-32 page axis.** `burrow_map_in` charges only the VMA axis, so eager exec pages were UNCHARGED (this section's I-32 row calls the exec image "one-shot bounded"); the lazy path charges per page. `page_count` therefore now tracks true RSS across exec -- what section 6.5 already claims -- but stack growth can now FAIL, gracefully via `proc_fault_terminate`, where before it could not (256 stack pages against `PROC_PAGE_MAX` = 65536, ~0.4%; the TCB is exempt). Prosecute at L-7 IN ADDITION to the L-4-as-designed list: the populate's all-or-nothing unwind returns the WHOLE charge and leaves the Burrow byte-identical to how it was found; `burrow_lazy_slot_kva`'s privacy precondition is never violated by a new caller; the per-page file read in `map_eager_from_file` is byte-identical to the eager one (same offsets, same total, EOF tail still zero); and the frame's populate run is the page holding its FIRST byte through the top of the stack (the frame is 16-aligned, not page-aligned). TWO self-audit notes for the round: (a) the I-32 uncharge on detach is ALREADY correct and was already correct before L-4a -- `sys_burrow_detach_for_proc` reads `burrow_lazy_resident_count` for a lazy Burrow, and the `vaddr_raw < EXEC_USER_BURROW_BASE` bound structurally excludes every exec/stack/guard VMA from detach at all (the handler says so, naming the hazard), so the eager-exec-pages-uncharged asymmetry is unreachable rather than merely unexercised; (b) L-4a puts a copy of the AT_RANDOM block through KERNEL HEAP for the first time (the staged frame), freed without scrubbing -- no leak path exists (kernel heap is not EL0-readable, and the pre-existing `rand` stack local has the same exposure), but it is a new transit of process-secret material and a scrub before `kfree` is nearly free in the 176-byte common case. Regressions `exec.writable_segment_is_sparse` + `exec.stack_is_sparse`, both revert-probed and INDEPENDENTLY so -- forcing every segment eager fails only the first, over-populating the stack fails only the second. **L-5 AS-BUILT (landed):** stock `fork()`. `rfork_internal` gains the THIRD address-space answer -- `entry` alone gets a fresh EMPTY space (spawn execs; copying is waste), `RFMEM` shares, and `fc` without `RFMEM` gets an `addrspace_clone`. The discriminator is `fc`, the SAME one L-3c uses for the handle table and for the same reason rather than by coincidence: `fc` means the child IS the parent continuing, so it must see the parent's memory AND its descriptors. The clone is taken AFTER the cheap rejections (child cap, narrowed allowance) so a fork bomb never pays for one, and its construction reference is dropped after `proc_alloc_in` takes the child's. `sys_rfork_core` admits `RFPROC` alone; the `child_sp == 0` and `child_sp == ctx->sp` refusals become RFMEM-CONDITIONAL (they exist because the two Procs write the same stack), while alignment + VA_TOP still bind any non-zero SP, and `child_sp == 0` now means INHERIT -- resolved at the same layer that already resolves `child_tls == 0`, which keeps `fork_frame_init` at two unconditional edits. The vfork park needed NO edit (`fc && (flags & RFMEM)`, written that way at L-3c-2 on purpose). **TWO DEFECTS SURFACED BY MAKING THE CLONE LIVE, neither visible to L-4b's tests: #136** -- `clone_one_vma` refused eager `ANON`, and the vDSO clock page IS eager anon mapped read-only into EVERY EL0 Proc, so the clone refused every REAL address space while passing on the synthetic ones the tests build; the arm now splits on WRITABILITY (read-only shares on the FILE reasoning; writable still refuses; MMIO/DMA refuse at any prot -- sharing a device window is an authority transfer). **#137** -- `ESR_ISS_WNR_BIT` was 9; WnR is ISS bit 6 (9 is EA), so `fi->is_write` had been ALWAYS FALSE tree-wide since the fault path was written. It survived because nothing before L-4b branched on it for correctness, and its own decode test MIRRORED the constant (set bit 9, asserted bit 9 was read -- a tautology) while every fault CONSUMER test, COW's and the vDSO's alike, builds a synthetic `fault_info` with `is_write` assigned directly and never runs the decode at all. The bug lived exactly in that seam. Prosecute on any change: the fork-shape discriminator stays `fc` (a flag would let a spawn path silently opt in); the clone stays after the DoS rejections; the SP rules stay RFMEM-scoped in BOTH directions; the decode test's bit stays an INDEPENDENT literal, never #included from the kernel header, or the tautology returns. Coverage: `fork.rfork_arg_rejection` (both shapes, both directions), `cow.clone_shares_readonly_eager_anon`, the rewritten `cow.addrspace_clone_refuses_and_leaves_parent_intact` (which now distinguishes all THREE clone phases on the failure path), `fault_decode.kernel_data_permission_write`, and `/fork-probe` legs K (three separately-falsifiable COW claims) + L (a store to READ-ONLY text is DENIED -- the half of #137 leg K does not cover). Five independent revert probes; the sharpest drops the clone WIRING and leaves the unit suite at a full 1300/1300 while only the in-guest leg fails. Prose: `docs/LINEAGE.md` + section 28 I-44 + this row + `docs/reference/{146-addrspace,147-execve,148-fork,145-vivarium}.md` + the L-7 focused audit + the full suite + the SMP gate + the L-6 arc gate (an Alpine `/bin/sh` runs a command). **L-6a AS-BUILT (landed):** the VIVARIUM `execve` row (`VIV_LINUX_EXECVE = 221` -> `VIV_TIER2`) + the `clone` FORK shape. `sys_execve_core` is extracted from `sys_execve_handler` and takes its arguments ALREADY IN KERNEL MEMORY, because the phenotype holds a Linux `char *const argv[]` and has no user VA to hand over -- the V-8 `sys_fstat_for_proc` discipline, one implementation of the ordering with two front ends for the argument shape. Two placements are load-bearing rather than tidy: the blob's PACKING CONTRACT moves into the core (below both builders) because `exec_build_init_stack` EXTINCTS on a NUL-count/argc disagreement, so a mis-built blob must become `-EINVAL` before it reaches the loader; and the blob is the CALLER's on every path -- carrying the pre-split `kfree(argv_kbuf)` calls across the split made it a DOUBLE FREE on every execve, which is a heap corruption that surfaces in an unrelated later spawn rather than near execve (the comment stating the ownership rule was written before the body matched it; do not re-add them). `vivarium_clone_decide` gains `VIV_CLONE_FLAGS_FORK` (`clone(SIGCHLD, 0)`, what musl's `fork()` emits) as a SECOND exact word, with a `bool *share_mem_out` so the shell picks `RFPROC` vs `RFPROC\|RFMEM` without re-deriving the decision; the `stack` rule INVERTS between the shapes (zero is REFUSED under RFMEM, and is the normal INHERIT value under RFPROC alone) -- the kernel's own contract, not a choice made here. **envp DECLINES when non-empty**, by the argument-domain rule: `exec_build_init_stack` writes a lone NULL for envp in BOTH frame shapes (`exec.c:405`/`:438`) and musl does `__environ = envp`, so no process has ever had a POSIX environment on its stack (#140) and writing `/env` would only MOVE the loss; an empty envp is served exactly, and the decline is a DETECTOR for whether L-6c needs the `/env` -> envp projection. Prosecute on any change: no front end may free the core's blob or vice versa; the packing validation stays below both builders; the argv pass-2 copy stays bounded by pass-1's MEASUREMENT rather than by re-finding a NUL (the I-30 snapshot discipline -- no concurrent writer exists today, since a vfork parent is suspended and `CLONE_THREAD` is not a row, but the bound removes the class rather than resting on that); both clone words stay EXACT. Regressions: `vivarium.clone_domain` (both shapes + `share_mem` asserted separately from the verdict, the inverted stack rule, the exact-word declines, NULL-out-param fail-closed) + `viv-pheno-probe` L156/L156b/L164-L169. Two revert probes, complementary: dropping the fork admission fails the UNIT test at its own assertion, while disabling the envp gate leaves the unit suite at a FULL 1300/1300 and fails only in-guest at L166.  **L-6b AS-BUILT**: the VIVARIUM `wait4` row -- `VIV_LINUX_WAIT4 = 260` (over the ceiling, `_Static_assert`ed), `kernel/vivarium.c::vivarium_wait4_decide` (PURE: the rusage gate, the 32-bit narrowing, the ALLOW-LIST, the bit-by-bit map into a `struct viv_wait_opts` DESCRIPTION), `kernel/syscall.c::viv_wait4` (the shell: the description -> native `WAIT_*` word, the pack-when-the-kernel-did-not rule, the up-front buffer validation, the ECHILD map) + `sys_store_user_int` EXTRACTED so the F240 scrub-on-partial-fault discipline has ONE implementation and two callers, `T_E_CHILD` = 10 appended to the errno registry under signoff. **THE OPTION WORDS ARE NOT THE IDENTITY, AND THE GAP IS OCCUPIED**: Linux `WNOHANG`/`WUNTRACED` are 1/2 like Thylacine's BY COINCIDENCE, `WCONTINUED` is 8 against `WAIT_CONTINUED` 4, and Linux's `WEXITED` is NUMERICALLY `WAIT_CONTINUED` -- so a passthrough is wrong in both directions at once (a guest's WCONTINUED sets a bit the native handler rejects; a guest's WEXITED silently opts into continue-reports AND the packed encoding). Prosecute: the allow-list stays an ALLOW-LIST (a mask-and-proceed re-opens exactly this); the map stays written out bit by bit so no line is a passthrough by omission; `kernel_packs` stays derived from the NATIVE flag word rather than from `options`, and stays computed BEFORE the call (the returned value cannot classify itself -- a raw exit status of 5247 and a packed `WAIT_STATUS_STOPPED` are both 0x147f); the buffer is validated BEFORE the reap (a wait that reaps then faults has destroyed the child's exit code); `pid` stays unjudged and narrowed through `s32` (wait_pid_for's selectors ARE Linux's, and an `int` argument must read the same sign- or zero-extended); the pure layer stays free of `proc.h` (the clone row's boundary). The ECHILD collapse of wait_pid_for's TWO `-1` conditions (no matching child; a #811 death-interrupted sleep) is EXACT, not lossy: the death path returns through the sync-from-EL0 tail where `el0_return_die_check` is NORETURN on the die branch, so no guest observes the second. Regressions: `vivarium.wait4_domain` (the collision asserted against the REAL `WAIT_*` constants, each bit alone, the composition, WEXITED/WNOWAIT/__WALL/__WCLONE/__WNOTHREAD declines, one-bad-bit-declines-the-word, rusage, the 32-bit narrowing, NULL-out fail-closed) + `viv-pheno-probe` L170-L176, which reap the children the L-6a legs created and thereby discharge L-6a's stated gap (the fork child RAN, L170) plus the COW-privacy leg (L170c). Two revert probes, opposite directions: dropping the ALLOW-LIST fails the unit test at its own assertion and never reaches the guest, while removing the CONDITIONAL PACK leaves the unit suite at a FULL 1301/1301 and fails only in-guest at L173. v1.x: `WEXITSTATUS` is 0-or-1 because the exit status is boolean (task #91); the `WNOHANG`-returns-0 path has no deterministic in-guest leg (it needs a synchronisation channel the phenotype lacks) and is L-6c's to exercise. **L-7 AS-CLOSED (@`b647a6c4`): the arc-close holotype -- 0 P0 / 0 P1 / 1 P2 / 6 P3, NOT dirty; six fixed, one tracked (#164).** The load-bearing set SURVIVED prosecution and is enumerated in `memory/audit_l7_closed_list.md` -- do not re-derive it. Fixed, each re-verified against source before action: **F1 [P2]** `specs/SPEC-TO-CODE.md::cow.tla` still promised the action-to-site map "lands with L-4b" three commits AFTER L-4b landed, and recorded the decide as running under the Burrow lock when the impl deliberately uses a GLOBAL leaf (`g_cow_lock`) -- necessarily, since after a fork the two sharers hold DIFFERENT Burrows, so no single Burrow lock serializes them; the map is now as-built and the refinement is recorded with its DIRECTION (global-leaf is STRONGER, so every property carries; REVERSING it to a per-Burrow lock would NOT be sound). **F2 [P3]** `execve` left hardware bp/wp armed at the OLD image's VAs -- `proc_exec_replace` reset every other per-image thing and nothing disarmed `debug_hw` (freed only at `proc_free`) while `hwdebug_switch_in` re-arms unconditionally per switch, so a surviving slot fires on whatever now occupies that VA; now cleared alongside the sigtab (the ATTACHMENT survives, the Linux `flush_ptrace_hw_breakpoint` shape). **F5 [P3]** `weft_reap_find_cb` dereferenced `q->as` with no NULL guard on a `proc_for_each` walk that INCLUDES kproc (ALIVE, pid 0, `as == NULL`) -- pre-L-1 that line read an inline `vma_lock` and could not fault. **F4 [P3]** every exec failure AFTER a successful `burrow_lazy_populate` leaked the I-32 page charge; now paired in `lazy_populate_uncharge`/`lazy_populate_unwind`, and the populate's OWN failure arm is deliberately EXCLUDED because it self-unwinds -- uncharging there too would UNDER-count. **F3 [P3]** `addrspace.h` claimed `vma_insert_in` takes the child's lock; it REQUIRES one, and the child's counters are guarded by UNREACHABILITY -- corrected, including what invalidates the whole argument (publishing `dst` earlier). **F6 [P3]** the `as_sole` stability comment named a ZOMBIE caller that #68 retired. **Prosecute on any change:** the `cow.tla` lock refinement stays STRONGER-not-weaker; the F4 exclusion of the populate-failure arm stays (it is an UNDER-count hazard, not an over-count one); the F2 clear stays under the same single-thread guarantee as the sigtab. **Coverage, stated rather than implied:** `weft.reap_kproc_pid_survives_walk` is revert-probed and does not merely fail without the guard -- it EXTINCTS the boot on `translation fault 0x...004` with Halls naming the test frame; **F2 and F4 landed WITHOUT tests** (`proc_exec_replace` is unreachable from a kernel test -- no EL0 trapframe -- and F4 needs an injectable post-populate failure), so the green suite does NOT cover them: task #165. The concurrent two-address-space COW-break harness remains owed (second claimant after #841/#845/#349/Loom). **#68's recorded seam is DISCHARGED**: "EXITING tails never SLEEP" verified mechanically -- zero `uaccess_*` calls inside any `as->lock` critical section. |
...
| I-42 | **JIT-as-a-capability (executable code emission)** (`docs/LLVM-DESIGN.md` §8, the "Clade" LLVM arc; **RESERVED — takes its §28 number now to lock it against I-41; ENFORCED at CL-7k** when the code-Burrow kernel half lands, the `docs/JIT-ON-WX-DESIGN.md` mechanism realized by its first consumer, Mesa's llvmpipe shader JIT): executable JIT memory exists **only** as a code-Burrow **RX alias** whose content arrived via the paired **RW alias** plus an explicit **`SYS_ICACHE_SYNC`**; no PTE is ever W∧X (I-12 holds at page granularity — two aliases of one physical region, the Lazarus W1.5 self-patcher's `.text` discipline turned outward); code-Burrow creation is **`CAP_JIT`-gated** (elevation-only, non-rfork-grantable, the `CAP_HW_CREATE` class) and non-heritable; JITed code is BTI/PAC-well-formed on hardened silicon, and `CAP_JIT` controls *who may emit* while the *namespace* controls *what emitted code can reach* (the JIT runs inside the app's own Proc — no new authority). The sibling of ADVANCED-GO AG-2's **I-41** on the same W^X/I-36-adjacent ground: I-41 governs the *kernel's* one sanctioned text mutation (the debug software-breakpoint COW-break); I-42 governs *userspace's* one sanctioned code-emission path (the capability-gated dual-map). Composes I-12 (W^X) + I-2/I-6 (`CAP_JIT` monotonic) + I-36 (Image cache untouched — JIT code is anon, never a FILE Burrow). | The dual-mapped code Burrow (RW at `VA_w`, RX at `VA_x`) + `SYS_ICACHE_SYNC(range)` (`dc cvau`/`ic ivau`/`dsb ish`/`isb`) + the `CAP_JIT` gate at code-Burrow create; the ORC `DualMapMemoryMapper` (userspace) over the working/execution split | prose + the CL-7k focused audit (W^X-adjacent — prosecute hard; the W1.5 precedent; F8 spec posture — escalate to a focused spec only if the CL-7 design pass surfaces genuine icache/publish SMP subtlety) |
| I-43 | **A phenotype confers ABI SHAPE, never AUTHORITY** (`docs/VIVARIUM.md` §8, the Vivarium Linux-compat arc; **ENFORCED at V-1b** -- the syscall-entry branch): a Proc's `phenotype` changes only how its syscall NUMBERS and ARGUMENT STRUCTURES are *decoded*. Every capability check, namespace resolution, handle right, permission check and resource charge is the **native** one, applied identically to a phenotyped and a native Proc -- a Linux binary reaches exactly what its territory, handles and capabilities allow, no more and by no different path. The property holds **by construction**, not by review: a Tier-1 row is a renumber performed IN PLACE on the trap frame, after which control FALLS THROUGH into the ordinary native `switch`, so the call lands on the very same `sys_*_handler` a native caller reaches (there is no parallel implementation in which a gate could go missing); a Tier-2 row calls the same `sys_*_for_proc` core the native handler calls, with the phenotype-specific work confined to PURE argument reshaping and struct conversion in `kernel/vivarium.c`. Consequently the declaration is deliberately **ungated** -- unlike every `SPAWN_PERM_*` bit, which confers a role -- because a mis-declared child gains nothing: it mis-decodes its OWN numbers, behind its OWN gates, and breaks only itself. The declaration is also the ONLY way to leave `PHENO_NATIVE` (§12.1 rules 1+3: the container declares, and an ELF byte may corroborate but never decide -- the fail-safe direction is forced), and it is inherited by `rfork` like the other per-Proc properties. | `Proc.phenotype` (set once in the spawn thunk before EL0, never by a running Proc); the `syscall_dispatch` branch -> `kernel/vivarium.c`'s pure table + translators; `SPAWN_PHENO_LINUX` in `sys_spawn_args.pheno_flags`; the exec-time brand hint is advisory-only (`elf_brand_hint`, diagnostic on an already-failed load) | prose (`docs/VIVARIUM.md` §8/§12.1 + `docs/reference/145-vivarium.md`) + the V-8 focused audit + the `vivarium.*` translator tests + `sys_spawn_full_argv.validate_req_pheno_flags` + the two-vantage in-guest gate (a native Proc leaves a Linux number untranslated; a declared-Linux container moves real bytes through openat/read/lseek/fstat/newfstatat/write/close and dies through Linux `exit_group`) |
| I-44 | **Address-space integrity under sharing and copy-on-write** (`docs/LINEAGE.md`, the process-creation arc; **RESERVED -- takes its section 28 number now to lock it against I-43; ENFORCED at L-4/L-5** when the COW break and `SYS_RFORK` land): an AddrSpace's pages live until the last referencing Proc drops it AND the last mapping is gone; a COW break yields a PRIVATE page whose contents equal the shared page at the instant of the fault, and leaves every other sharer's view UNCHANGED; no page is ever writable through one mapping and executable through another. Realizes the object the tree does not yet have: today `pgtable_root` / `context_id` / `vmas` / `vma_lock` / `page_count` / `vma_count` / `shared_map_pages` are INLINE fields of `struct Proc` (SEVEN, corrected at L-1 -- `shared_map_pages` belongs by the same argument as `page_count`: `vma.h` states its invariant as the sum over SHARED_IN VMAs, and the uncharge runs off the VMA list), so "two Procs, one address space" is unrepresentable -- which is why the L-1 extraction is the arc's spine and not a detail of COW. Two consequences are decided rather than emergent: **`context_id` belongs to the AddrSpace** (the rolling ASID identifies a TRANSLATION TABLE, which the allocator always semantically meant -- two Procs sharing an AS share one ASID, and the I-31 corruption case [two tables, one ASID] becomes structurally unrepresentable; I-31 is unaffected in substance), and **the I-32 charge follows the AddrSpace** (sharing an AS means sharing the pages, so ONE charge is the honest RSS the field already claims to be; a COW break charges the BREAKER, where the new page lands; a fork bomb stays bounded because N children means N address spaces, each capped). Text needs NO COW: REVENANT already maps text + R-only rodata from a shared read-only Image-cached `BURROW_TYPE_FILE` (I-36), so a forked child inherits it by taking a second `burrow_ref` -- only WRITABLE ANON (data, heap, stack) is copy-on-write, which is what keeps I-12 trivially true across the break. Composes I-1 / I-2 / I-7 (the #847 dual refcount across two address spaces) / I-12 / I-21 / I-24 (`execve`'s peer-thread cascade) / I-28 / I-31 / I-32 / I-36. | `struct AddrSpace` (refcounted; `Proc.as`, with `as == NULL` replacing NINE of the 13 measured `pgtable_root == 0` sites AS A SET -- the other four are `arch/arm64/mmu.c` parameter validation on a bare `paddr_t` and correctly do NOT convert (corrected at L-1)); `execve` (build-new-then-switch); `rfork(RFPROC\|RFMEM)` + the VFORK suspend; the COW fault arm + a real per-page share count; child-context restoration from the parent's trapframe with `x0 = 0` | **spec-first RE-ENABLED for this surface** (re-enabling point (a) -- the break-vs-break / break-vs-teardown SMP class is exactly what `asid.tla` and `death_wake.tla` were re-enabled for): the model lands + is TLC-green BEFORE the L-4 impl, with buggy cfgs for break-vs-break, break-vs-teardown, and the lost-VFORK-wake; + `docs/LINEAGE.md` + the L-7 focused audit + the SMP gate + the L-6 arc gate (an Alpine `/bin/sh` runs a command) |
...
**The LLVM keystone arc (`LLVM-DESIGN.md`, the "Clade" arc; SIGNED OFF 2026-07-23) mints one new invariant — I-42 (JIT-as-a-capability) — RESERVED-with-a-row above (the number is locked NOW against ADVANCED-GO AG-2's I-41; ENFORCED at CL-7k when the code Burrow lands).** Everything else composes existing guarantees: **I-12** (W^X — the dual-mapped code Burrow is two fixed-permission aliases, never a W∧X page), **I-2/I-6** (`CAP_JIT` monotonic reduction), **I-36** (the REVENANT Image cache is untouched — JIT code is anonymous, never a FILE Burrow), **I-32** (the F4 spawn-time page-budget bounds a `-O2`/link RSS under a global hard cap without weakening the fork-bomb floor — the resource axis, no renumber), and **P-1..P-4** (the whole toolchain is a Pouch port — no foreign syscall enters the kernel). Its audit-trigger surfaces — the **code Burrow + `SYS_ICACHE_SYNC` + `CAP_JIT`** (CL-7k, W^X-adjacent — prosecute hard) and the **F4 spawn-time page-budget** (CL-5, its own focused round) — join §25.4 at the sub-chunk that lands each, per LLVM-DESIGN.md §12. `ADVANCED-GO-DESIGN.md`'s **I-41** (software-breakpoint isolation) remains reserved in its own doc, its §28 row landing at AG-2's SB-1 — so the table above jumps I-40 → I-42, the two W^X/I-36-adjacent siblings holding their numbers by construction.
