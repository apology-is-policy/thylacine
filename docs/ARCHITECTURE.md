...
| I-20 | PTY master ↔ slave atomicity | RESERVED-then-enforced (the T-1 precedent) — the PTY master/slave *pair* (`/dev/ptmx` + `/dev/pts/<n>`) is a **Phase-8** deliverable, NOT LS-8 (corrected 2026-06-11, HOLOTYPE RW-12 W6-F2): LS-8 (task #952) delivered single-console line discipline only — pollable cons (8a) + termios/`consctl` ISIG/ECHO (8b) + the shell poll loop (8c) — and explicitly excludes the master/slave mechanism. **The design LANDED 2026-07-17 (`docs/PTY-DESIGN.md`, authoritative; four votes — userspace `ptyfs` server + full POSIX sessions/pgroups + Ctrl-Z-generalizes-the-debug-stop + spec-first), and spec-first is RE-ENABLED for this surface: `specs/pty.tla` is model-first (the 7th instance of re-enable point (a), after Tapestry's T-1).** I-20 (no byte lost/torn/duped across the ldisc; a cooked signal-class char raises exactly-one signal to exactly the fg pgrp, not-also-a-byte; no lost wake per endpoint; teardown drains-then-EOF + SIGHUP-exactly-once) takes its enforced number when PTY-2 lands. **The seam is realized by a kernel `KObj_Pts` (round-1 holotype F1): `tcsetpgrp`/acquisition/`SYS_TTY_SIGNAL` are KERNEL syscalls keyed on it, the `tty:*` notes are kernel-synthetic-only (F4), the job-control stop adds an independent `job_stop_req` owner (F3), and `SYS_WAIT_PID` gains `WUNTRACED`/`WCONTINUED`+pgrp-selectors (F5) — the ABI surface owed to signoff at PTY-1.** (LS-8a's pollable-cons *deferred* poll-wake — a distinct mechanism, cons-only since a RAM-backed PTY runs in process context — IS specced under I-9: `specs/cons_poll.tla`, not this row.) | `specs/pty.tla` (clean + liveness + the buggy cfgs `signal_also_byte` / `lost_teardown_byte` / `double_stop` / `signal_wrong_pgrp`) + `specs/pty_stop.tla` (the stop composition: clean + liveness + `double_stop` / `death_blocked`) + PTY-DESIGN.md prose + the focused audit + tests |
