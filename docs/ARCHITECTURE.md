...
| Wakeup | No wakeup lost between wait-condition check and sleep | Wait/wake state machine | `scheduler.tla`, `poll.tla`, `tsleep.tla`, `death_wake.tla` (torpor leg prose-validated) |
...
**The root seed.** Accumulation roots at a Spoor that already has a Path. The Territory
`root_spoor` is seeded with `/` at every root-set site (`territory_chroot` /
`territory_pivot_root` / the boot root install). A NULL Path means "name unknown" (an
attach root that never flowed through a named walk, or a walk from an fd whose own Path
is unknown); the readers render it empty / `?`, never a wrong name.
...
**Consumers** (#66b): `SYS_FD2PATH(fd, buf, cap)` copies `handle→spoor→path→s`;
`/proc/<pid>/ns` is rewritten to render the real mount/bind list (`PgrpMount` gains a
refcounted `mp_path` so the mount-point name is recoverable — the table keyed on
identity, not name, until #66) + each source's `path`; `/proc/<pid>/fd` lists the
Proc's open fds + their paths (under `g_proc_table_lock`, the #57a F2 read-vs-reap
shape); the native `ns` tool reads `/proc/<pid>/ns`.

**Split**: #66a (the substrate + `SYS_FD2PATH` + kernel tests, one focused audit — the
hot-walk-path surface) → #66b (the `/proc` consumers + the `ns` tool + the
...
`pty` will be an audit-trigger surface when it lands (deferred to the LS-8 line, task #952; not yet built). Master/slave atomicity and `termios` state transitions get spec-or-prose validation at that point — no `pty.tla` exists today.
...
the committed inventory is **17 modules**; three of the planned nine
...
or not yet built (PTY, LS-8) — and nine modules were added beyond the plan.
...
Each module carries its clean cfg(s) plus buggy-cfg counterexamples (71
...
| I-9 | No wakeup is lost between wait-condition check and sleep — *including* the death-wake that group-termination delivers to every rendez sleeper (§8.8.1, task #811), and the terminate-disposition `interrupt` wake that generalizes that same machinery (§8.8.2, LS-5) | Wait/wake protocol; register-then-observe under the per-condition lock (`torpor_lock` for `torpor`, the per-Thread `wait_lock` for `sleep` / `tsleep`) | `scheduler.tla`, `poll.tla`, `tsleep.tla`, `death_wake.tla` (the death-wake generalization, clean + buggy cfg); the `torpor` wait-on-address leg is prose-validated per the 2026-05-23 spec-to-code suspension (no `futex.tla` was written) |
...
| I-29 | Loom completion integrity: every SQE the kernel admits produces **exactly one** terminal CQE (no lost, no double); no CQE is posted whose `user_data` correlation is **stale** (a torn-down / abandoned op never surfaces as a live completion); the CQ is **never overfilled** (back-pressure holds the completion until userspace drains a slot -- the F3/F5 9P-client all-or-nothing discipline) | The SQ/CQ ring state machine: the demux posts at most one CQE per in-flight op + clears its `inflight[]` slot; teardown quiesces every in-flight op (the #811 death-interruptible unwind) before freeing the ring Burrow; `PostCqe` is gated on a free CQ slot | `loom.tla` (clean + the `double_post` / `lost_on_full` / `stale_after_teardown` buggy cfgs); GENERALIZED to a stream in `loom_multishot.tla` (`ExactlyOneTerminal` + `TerminalEndsStream` + per-shot `CqNeverOverfull` + `NoStaleAfterTeardown`) + to a cancellation-complete chain in `loom_order.tla` (`EveryDoneOpPosted` + `EverySubmittedPosts`) |
