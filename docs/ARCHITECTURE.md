...
**The mint-gate widening (the I-27 argument).** consctl's `devdev_open` gate becomes attached-OR-renderer (`cons`, the DATA leaf, is UNCHANGED: attach-gated at open + per-I/O re-gated). Aurora self-serves by name exactly as it opens consdrain/consfeed. The authority argument: the renderer already holds **consfeed** — arbitrary INPUT INJECTION (it can type anything, including a command at a shell prompt) — which strictly dominates the termios+winsize control surface consctl carries, so the widening confers no authority the renderer role did not already have. consctl I/O stays ungated per #94-B (the inherited fd is the capability); the winsize verb is accepted from any consctl holder like the five flags (in practice only aurora writes it; an app cannot mint consctl, so a TIOCSWINSZ-shaped lie from an app is structurally blocked — see the pouch arm below).
