...
- the service **`SYS_CHROOT`s to the handed (non-opened) handle** so its filesystem world *is* the capability — confinement is mechanism-enforced (chroot + `walk_open`'s `..`-reject + the service holding no other Spoor). It then creates/reads its state at `FROM_ROOT` (now the capability) and may `SYS_WALK_CREATE` children (the chrooted root is a non-opened walkable base);
