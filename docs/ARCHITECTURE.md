...
- **Settable wall clock** — `clock_settime` + a userspace timekeeper / NTP client maintaining a UTC clock object (the Fuchsia/Genode userspace-maintained-UTC SOTA). v1.0 is read-only kernel-anchor; settability needs a trusted time authority + a "who may set the clock" capability — a real design pass.
...
the committed inventory is **18 modules**; three of the planned nine
...
or not yet built (the PTY *master/slave* pair, Phase 8) — and ten modules were
...
Each module carries its clean cfg(s) plus buggy-cfg counterexamples (72
...
| I-9 | No wakeup is lost between wait-condition check and sleep — *including* the death-wake that group-termination delivers to every rendez sleeper (§8.8.1, task #811), and the terminate-disposition `interrupt` wake that generalizes that same machinery (§8.8.2, LS-5); and the LS-8a pollable-cons *deferred* poll-wake — the RX IRQ relays through `poll_wake_pending` + the `console_mgr` kthread to the poll-hook walk (§23.5) | Wait/wake protocol; register-then-observe under the per-condition lock (`torpor_lock` for `torpor`, the per-Thread `wait_lock` for `sleep` / `tsleep`, `g_cons.lock` for the cons poll relay) | `scheduler.tla`, `poll.tla`, `cons_poll.tla` (the LS-8a deferred poll-wake, clean + liveness + buggy cfg), `tsleep.tla`, `death_wake.tla` (the death-wake generalization, clean + buggy cfg); the `torpor` wait-on-address leg is prose-validated per the 2026-05-23 spec-to-code suspension (no `futex.tla` was written) |
