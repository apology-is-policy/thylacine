...
**Invariant I-23**: a service's filesystem authority is bounded by the storage capability it is handed. A service granted a storage-root Spoor with rights R reaches only that Spoor's subtree, at only rights R; it holds no ambient FS authority beyond it. Re-handing is blocked by withholding `RIGHT_TRANSFER` at grant time.

**Mechanism (no new kernel surface; verified 2026-05-29).** The substrate composes primitives that already exist:
- the spawner walks/creates the storage directory (`SYS_WALK_CREATE` / `SYS_WALK_OPEN`; `walk_open` returns a handle carrying `R|W|TRANSFER`, so a walk-opened directory is a valid base for mutation);
- it reduces the handle to the grant rights with `handle_dup(h, R|W)` — the subset check in `kernel/handle.c` is the I-6 enforcement point; dropping `RIGHT_TRANSFER` prevents the grantee re-handing its storage;
...
- the service uses that base fd for `walk_open` / `walk_create` / `rename` / `unlink` / `fsync` (every FS-mutation syscall accepts a base Spoor fd, not only the `FROM_ROOT` sentinel);
- optionally the service `SYS_MOUNT`s the handed Spoor into its Territory at a mount point (e.g. `/state`) for ergonomic path I/O confined to the capability (a v1.x ergonomic; the raw base-fd form is the v1.0 default).
...
| Capability-scoped service storage | `usr/joey/joey.c` (post-pivot: create `/var/lib/corvus` → `handle_dup`→`R|W` → endow as corvus's storage fd at spawn), `usr/corvus/src/main.rs` (use the handed fd as `storage_root` for all persistence), `usr/lib/libthyla-rs` (inherited-storage-fd helper if added); **no kernel change** | Convergence detour A-1.7 (ARCH §3.6; NOVEL §3.10 lead angle #10) — the storage-capability substrate, proven on corvus. Invariant **I-23** (FS authority bounded by the handed capability). Prosecute: the spawn-time rights reduction (`R|W`, no `TRANSFER` — grantee cannot re-hand); no-escape confinement (the service cannot name a path outside its storage subtree); reduced-rights monotonicity (I-6) through the spawn-fd endow; corvus ends holding `R|W` on exactly one directory. **No new spec** per the 2026-05-23 spec-to-code broadening — prose validation in ARCH §3.6 + this row + the audit + the corvus confinement smoke. |
...
| I-23 | A service's filesystem authority is bounded by the storage capability it is handed: a service granted a storage-root `KObj_Spoor` with rights R reaches only that subtree, at only rights R; no ambient FS authority beyond it. Re-handing blocked by withholding `RIGHT_TRANSFER` at grant | Spawner endows a `handle_dup`-reduced Spoor at spawn; service confines all FS ops to that base fd; a composition of I-2/I-4/I-6 (handles) + I-1/I-3 (territory) | `ARCHITECTURE.md §3.6` + `docs/detour-status.md A-1.7` (prose + audit + tests per the spec-to-code suspension) |
