...
| I-29 | Loom completion integrity: every SQE the kernel admits produces **exactly one** terminal CQE (no lost, no double); no CQE is posted whose `user_data` correlation is **stale** (a torn-down / abandoned op never surfaces as a live completion); the CQ is **never overfilled** (back-pressure holds the completion until userspace drains a slot -- the F3/F5 9P-client all-or-nothing discipline) | The SQ/CQ ring state machine: the demux posts at most one CQE per in-flight op + clears its `inflight[]` slot; teardown quiesces every in-flight op (the #811 death-interruptible unwind) before freeing the ring Burrow; `PostCqe` is gated on a free CQ slot | `loom.tla` (clean + the `double_post` / `lost_on_full` / `stale_after_teardown` buggy cfgs) |
