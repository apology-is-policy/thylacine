...
- ~~**Kernel prints (`uart_puts`) keep the direct path**, so a kernel print can still interleave with an EL0 write. Post-boot kernel prints are diagnostic and rare; tightening this is a v1.x item.~~ **PARTIALLY CLOSED at #126 — and "rare" was the wrong axis.** The residual is real and stays: pre-GIC boot, IRQ context, and extinction/Halls must keep the direct path, where the ring is unavailable by construction (`cons_tx_push_nowait` falls through to `uart_putc` pre-arm, so that leg is automatic). But frequency was never the hazard. `uart_putc`'s #67 bound is **20 ms per BYTE**, and it does not compose into a per-message bound: `proc_reparent_children`'s ~90-byte #80 orphan line is emitted **while holding `g_proc_table_lock`** — which `proc.c` takes irqsave 40 times and plain 0 times — so against a stalled host consumer one adoption pinned the GLOBAL process-table lock IRQ-masked for ~1.8 s. That is not an interleaving nuisance; it is the interrupt-dead stall #67's bound exists to prevent, reconstituted by iterating it. A **per-item bound is not a per-operation bound.**
  - **The fix is routing, not a tighter bound.** New `cons_diag_puts` / `cons_diag_putdec` / `cons_diag_puthex64` (`kernel/cons.c`) push through the #75 TX ring: never spinning, dropping on a full ring (the echo disposition), kicking once per call, and feeding the G-4 drain tap. Legal under any lock ordered above the cons locks — they take only leaves (`g_cons_drain.lock`; `g_cons_tx.lock`, which nests only `g_uart_imsc_lock`) and wake outside them, which is the *same* path `cons_emit` already takes for echo from the UART RX IRQ. The direction also matches this file's standing obligation that `g_cons.lock` must never be held while taking `g_proc_table_lock`: proc_table_lock sits **above** the cons locks, and `cons_diag_*` touches `g_cons.lock` not at all.
  - **Two sites converted.** `proc_reparent_children` (the defect) and `proc_fault_terminate` (the sibling — off-lock, so never a global stall, but the same class: a steady-state EL0-reachable diagnostic on a 20 ms/byte path, and invisible to the renderer). `proc.c` now has **zero** direct `uart_*` calls, so the file has one sanctioned emit path rather than a mix a future editor must adjudicate.
  - **It closes the #76 gap here too.** `cons_drain_tap` fires from the cons emitters only, so both lines were serial-only — the "user fault: pid=… — terminating Proc" message, the single most user-relevant kernel diagnostic there is, was invisible on the framebuffer console while looking perfectly normal on serial. That is also what makes the fix *testable*: `proc.orphan_diag_uses_cons_path` arms the drain, forces a real adoption, and requires the line verbatim in the tap. Pre-fix the tap captures nothing (revert-probed: `1237/1238 FAIL`). A duration assertion would have been **vacuous** — `uart_test_tx_stall()` gates `uart_tx_try_putc` only, never `uart_putc`, so "elapsed < N ms" passes identically before and after the fix.
...
| Process wait/reap (`SYS_WAIT_PID` v2 / `wait_pid_for`, U-7-pre) | `kernel/proc.c` (`wait_pid_for` + the filtered `wait_pid_cond` + the `wait_pid` wrapper), `kernel/syscall.c` (`sys_wait_pid_handler` 3-arg + dispatch), `kernel/include/thylacine/proc.h` (`WAIT_WNOHANG`), the libt + libthyla-rs wrappers (`t_wait_pid_for` + `Child::try_wait`) | Death-path (#788/#809/#811/#926 lineage). ABI: `SYS_WAIT_PID` (status_out) -> (want_pid, flags, status_out); `pid=-1,flags=0` = legacy reap-any-blocking. Adds a pid filter (`want_pid>0`; closes the `joey.c:2451` wrong-pid extinction) + `WAIT_WNOHANG` (returns 0 = not-ready; pid 0 is never a child). **#94 addendum**: U-7-pre converted the USERSPACE callers but left the KERNEL site that motivated the filter -- `kernel/joey.c`'s kproc-waits-for-joey -- on reap-any, so the hazard stayed live where it mattered most. `proc_reparent_children` splices a dying joey's daemons onto the FRONT of `kproc->children` (ahead of joey) and the scan breaks on the first ZOMBIE, so an already-dead daemon was reaped INSTEAD of joey and the mismatch check discarded joey's exit status -- on exactly the boot-failure branches where a daemon dying is WHY joey is exiting (`usr/joey/joey.c`'s stratumd-died branch even carried a comment asserting immunity that path did not have). Now `wait_pid_for(pid, 0, &status)`; the surviving `reaped != pid` arm is reachable only as -1. Orphan fate unchanged (kproc has never reaped orphans as a service -- init is the reaper; this site did ONE incidental reap then extincted on it). Prosecute on any change: the wait must name joey, never a sibling; both arms must still end the boot. Regression `proc.wait_pid_for_skips_adopted_orphan_zombie` (revert-probed: swapping it back to `wait_pid` FAILS on the target-vs-orphan assert). Reap teardown copied verbatim; only selection + blocking + the cond filter change. I-8 (no starve), I-9 (no lost wake; filtered cond re-checks under r->lock), death-path lifetime (alloc-stable `c->pid` filter, stack-local `wait_cond_ctx`). Closes #856. **No new spec / no new invariant** (2026-05-23 broadening) -- prose in `docs/reference/14-process-model.md` + the focused audit + the 3 `proc.wait_pid_for_*` tests + the SMP gate. |
...
| Resource/DoS floor -- per-Proc page/thread/child caps (#65) | `kernel/include/thylacine/proc.h` (`PROC_PAGE_MAX` / `PROC_THREAD_MAX` / `PROC_CHILD_MAX` + the new `page_count` / `child_count` counters on `struct Proc` + `proc_resource_exempt` decl), `kernel/proc.c` (`proc_resource_exempt` [`principal_id == PRINCIPAL_SYSTEM`] + the `rfork_internal` child-cap early reject + `child_count` ++/-- at `proc_link_child` / `proc_unlink_child` + the `proc_page_charge`/`proc_page_uncharge` helpers), `kernel/syscall.c` (`sys_burrow_attach_for_proc` page-cap check + charge / `sys_burrow_detach_for_proc` uncharge; `sys_thread_spawn_handler` thread-cap check), `kernel/devproc.c` + `kernel/devctl.c` (expose `page_count` / `child_count` in the per-Proc stat surface -- the SEAM counters + the W5-F8 `/ctl/mm/` reconciliation) | RW-12 #65 (HT12.W5-F2 [H2]; BUILD-committed at IDENTITY-DESIGN.md §3.8 / `IDENTITY-DESIGN:769`; D4 pre-rc). **Invariant I-32 (per-Proc resource floor / DoS bound).** A new privilege/DoS surface -- prosecute hard. Prosecute: **cap bypass** (a spawn-variant or a fault path that commits resources without passing its cap check -- rfork_internal is the SINGLE Proc-creation chokepoint so no SYS_SPAWN_* variant escapes; burrow_attach is the SINGLE eager-anon commit point); **exemption forge** (a post-login Proc must NOT reach `principal_id == PRINCIPAL_SYSTEM` -- `CAP_SET_IDENTITY` rejects SYSTEM, proc.h; the boot chain is the only exempt set); **counter race / underflow** (`page_count` written under `vma_lock`, `child_count` + `thread_count` under `g_proc_table_lock`; the uncharge must never underflow past 0; external stat reads are atomic snapshots); **the graceful-OOM backstop preserved** (every user creation path -- `proc_alloc` / `thread_create` / `territory_clone` / `burrow_create_anon` / the demand-page `mmu_install_user_pte` failure -> `proc_fault_terminate` -- returns an error / per-Proc-terminates, NEVER box-extincts; this is the property that bounds a *recursive* cross-Proc bomb at the physical cliff); **the TOCTOU overshoot is bounded** (<= ncpus-1 extra per axis; a floor, not an exact accountant); **the boot chain is not pinched** (the SYSTEM exemption keeps stratumd / joey / corvus / kproc unbounded -- the cap must not break the FS server or the orphan-adopter). Page scope = the repeatable user anon vectors: SYS_BURROW_ATTACH regions + the SYS_LOOM_SETUP ring (audit F1, sys_loom_setup_for_proc charges the ring); pgtable / kstack / exec-image are transitively or one-shot bounded. The global / per-user aggregate quota (cgroup-equivalent reading these counters) is the recorded SEAM. **No new spec** per the 2026-05-23 spec-to-code broadening -- prose validation in IDENTITY-DESIGN.md §3.8 + `docs/reference/110-resource.md` + this row + the focused audit + the runtime tests (per-axis cap-hit + the unforgeable-exemption + graceful-OOM-no-extinction + the charge/uncharge balance) + the SMP gate. **#106/#122 addendum (the charge UNIT + the charge ALLOWLIST; `kernel/burrow.c::burrow_backing_pages` + the four chargers):** two independent ways the floor stood lower than it claims, both fixed together because they meet in one expression. **#106 -- the unit.** Every eager charger billed `length / PAGE_SIZE` while `burrow_create_anon` / `burrow_create_code` allocate `1 << order_for_pages(page_count)`, so a 2049-page region occupied 4096 and a Proc could stand at ~2x its budget (bounded at 2x -- an understated floor, not an unbounded hole). The charge unit is now `burrow_backing_pages(size)`, which SHARES `order_for_pages` with the two creators so charge and allocation cannot drift; the underlying waste STAYS deliberately (a Burrow's backing must be one physically contiguous run -- exec's direct-map alias, `loom_create`'s `ring_kva`, the weft ring view all index `v->pages` as a single chunk -- and buddy contiguity is bought with power-of-two rounding). All four chargers use it: `sys_burrow_attach_for_proc`, `sys_jit_create_*` (+ destroy, recomputed from the VMA span), the Loom ring (`ring_size` is page-rounded but NOT power-of-two rounded), and the detach ANON arm. The lazy-anon fault arm is untouched and must STAY untouched -- it allocates order 0, so its per-page charge is already exact. **#122 -- the allowlist.** `sys_burrow_detach_for_proc` refunded `length / PAGE_SIZE` for everything except CODE (refused) and ANON_LAZY, but page_count was only ever charged for ANON: a SHARED_IN VMA (`SYS_WEFT_MAP`; `burrow_share_into` charges the client's `shared_map_pages` and deliberately leaves page_count alone, yet places the VMA in the burrow-attach window) and an MMIO/DMA map (both take a caller-supplied vaddr, so a `CAP_HW_CREATE` driver can place one in the window) both got refunds they never paid. `proc_page_uncharge` clamps at 0, so this never wrapped -- but it is the "drift, not wrap" shape the CL-7k audit already refused for the CODE alias, and the same sentence applies: a bound a Proc can drive to zero while its real occupancy is unchanged is not a bound. The arm is now a POSITIVE allowlist (ANON rounded / ANON_LAZY resident / everything else 0), so a future Burrow type is uncharged by default -- the fail-safe direction. Prosecute on any change: the charge and the refund must be computed from the SAME input (both take `length`, and `burrow_unmap` exact-matches the VMA, so they agree by construction); the allowlist must stay positive; a new eager charger must use `burrow_backing_pages`, and a new order-0 charger must NOT. Regressions: `burrow.backing_pages_matches_alloc` (the anti-drift pin -- compares the helper against a REAL Burrow's recorded `v->order`, hard-coding no expected table) + `resource.attach_charges_buddy_rounded` + `resource.detach_shared_in_keeps_page_count` (whose nonzero pre-charge is load-bearing: with page_count at 0 the clamp hides the spurious refund and the test would pass against broken code). All three revert-probed. **#131/#132 addendum (the charge RECORD + the release RULE; `kernel/burrow.c::burrow_charge_{record,claim,restore}` + `burrow_is_shared_out` + the `struct Burrow.{charge_pid,charge_pages,shared_out}` fields):** the #122 allowlist decides "was this charged" from the region's SHAPE, which works only while shape determines PAYER -- and stopped doing so once a region became reachable from two Procs. **#132 (introduced by #130's own fix, P1, the DANGEROUS direction).** `loom_register_buffers` + `loom_free` refunded a displaced registered-buffer pin to the Loom's owner on the argument that "registering requires a loom fd from p's own table, and KObj_Loom is neither transferable nor dup-able" -- which proves p owns the LOOM and says NOTHING about who paid for the BUFFER. `loom_resolve_buf` admits any writable ANON VMA, and a weft ring netd allocated + `burrow_share_into`'d satisfies every gate (Weft-6c-1's shipped `WeftFlow::open` registers the WHOLE ring), so a consumer could be refunded for the sharer's pages: an UNDER-count -> budget inflation on a NON-exempt Proc, repeatable through the public `WeftFlow`/`Ring` API. **#131.** Nothing settled the sharer's charge at all -- netd detaches its ring at flow close while the guest's mapping + the binding pin live on, so the LAST drop is the GUEST's `vma_drain`: generic vma code, in another Proc, under that Proc's `vma_lock`, unable to name netd (64 pages leaked per closed zero-copy flow; no bound breach today only because netd inherits PRINCIPAL_SYSTEM -- a coincidence of two independent gates, not an enforced property). **The mechanism**: the Burrow records its payer; `burrow_charge_claim` read-and-CLEARS it so exactly one settler ever refunds; callers claim BEFORE the drop (a freeing drop takes the record with it) and snapshot the BURROW pointer, never the VMA (`burrow_unmap_reporting` frees the `Vma`). `charge_pages` -- not `charge_pid` -- is the held sentinel, because `proc_alloc` stamps pid 0 and `rfork_internal` assigns later, so pid 0 is a legitimate identity. **The release rule (user-voted 2026-08-03; seL4's answer over Linux memcg's):** detach settles on `freed || shared_out`. `freed` is SUFFICIENT but not NECESSARY -- once the region is shared out and this Proc has unmapped it, the Proc cannot reach those pages and the consumer's `shared_map_pages` (the fifth axis) accounts them. `shared_out` rather than "does anything still hold it" is LOAD-BEARING: the Proc's OWN other claim (a Loom regbuf pin on its own buffer) also keeps the region alive and there the charge must STAY. The never-claimed share settles at `weft_share_unregister`/`weft_share_release_owner` (the sharer's pin is its last claim; both know the owner); `weft_binding_release` settles NOTHING -- that pin is the consumer's and the consumer never paid. Prosecute on any change: a new refund site must go through the CLAIM (never recompute from a size), or it re-opens #132 for whatever region shape it happens to face; the claim must stay BEFORE the drop; restore must only ever run where the Burrow provably survives (`!freed`, or nothing dropped); `shared_out` must stay monotonic (a clear would let a sharer reclaim a charge it already released). Recorded window: a share landing between the `shared_out` read and the drop leaves the sharer charged until its next release point -- an over-charge, never a wrong refund. Regressions `loom.regbuf_foreign_charge_not_refunded` + `weft.sharer_charge_released_at_detach`, both revert-probed on DISTINCT assertions (reverting the loom claim fails only the first; reverting the `shared_out` arm fails only the payer-settles legs) -- and the fixture must give its two Procs DISTINCT pids, or both sit at proc_alloc's 0 and the owner check matches by coincidence. |
...
| JIT code Burrow + `SYS_ICACHE_SYNC` + `CAP_JIT` (the Clade arc, CL-7k; I-42) | **AS-BUILT at CL-7k** (`1f0e66c0` kernel + `5633d056` userspace; the focused audit + fixes in the close commit) (`docs/LLVM-DESIGN.md` §8 + `docs/JIT-ON-WX-DESIGN.md`): the dual-mappable **code Burrow** (one BURROW attached RW at `VA_w` + RX at `VA_x` in one Proc), `SYS_ICACHE_SYNC(range)` (the `dc cvau`/`ic ivau`/`dsb ish`/`isb` sequence lifted to a syscall), the `CAP_JIT` capability (elevation-only, non-rfork-grantable, the `CAP_HW_CREATE` class), and the userspace ORC `DualMapMemoryMapper` — first consumer Mesa's llvmpipe | **Invariant I-42; W^X-adjacent — prosecute hard (its own focused round, CL-7k).** Realizes JIT-ON-WX-DESIGN.md; composes I-12 (no PTE ever W∧X — two fixed-permission aliases of one physical region, the W1.5 self-patcher's discipline turned outward), I-2/I-6 (`CAP_JIT` monotonic), I-36 (anon code, Image cache untouched). Prosecute: the two-alias W^X-cleanness at PTE granularity (neither alias ever W∧X; the RX alias never precedes the `SYS_ICACHE_SYNC`); the `CAP_JIT` gate at code-Burrow create (unforgeable, non-heritable, elevation-only); the icache/publish SMP protocol (a second CPU sees the emitted bytes only after the sync — the F8 candidate for a focused spec if the CL-7 design pass surfaces genuine subtlety); BTI/PAC well-formedness of JITed code on hardened silicon; the code Burrow lifecycle (#847 dual-count — no UAF/leak across the two aliases + the ORC mapper's working/execution split). **F8 spec posture**: lean prose + audit (the W1.5 precedent — same mechanism, already trusted in-kernel), escalating only on real SMP subtlety. Prose: LLVM-DESIGN.md §8 + §16.18 (as-built) + `docs/reference/145-jit.md` + this row + the CL-7k-3 focused audit + the `libthyla_rs::jit` prover (emit→sync→call; ungated Proc DENIED) + boot OK + the SMP gate. **AS-BUILT (CL-7k):** `SYS_JIT_CREATE=101` installs BOTH aliases under ONE `vma_lock` hold (either both exist or neither — no RX-without-writer state, no caller-side rollback); `SYS_JIT_DESTROY=102` names the WRITER alias alone (the kernel knows the pairing, so a half-teardown is unrepresentable); `SYS_ICACHE_SYNC=103` syncs the KERNEL DIRECT MAP, never the user VA (a user VA is exactly what a caller can arrange to be unmapped, and `dc cvau`/`ic ivau` can fault; architecturally exact — ARMv8 D-caches are PIPT and `IC IVAU` invalidates all aliases of the PA, the Linux `flush_icache_range`-on-linear-map precedent). DESTROY and SYNC are deliberately NOT cap-gated: authority to CREATE is the scarce thing, and gating release would turn a legate-scope expiry into a leak. `BURROW_TYPE_CODE`'s backing is byte-identical to ANON — the type is an ADMISSIBILITY token minted by the kernel under `CAP_JIT` (the G-2 WEAVE discipline on the W^X boundary), so its arm in every downstream switch IS the ANON arm. **The structural property that makes `jit_find_peer_locked` unambiguous:** all three cross-Proc/registration paths (`burrow_share_into`, `loom_resolve_buf`, `sys_weft_share_for_proc`) are ALLOWLISTS, so CODE is refused everywhere by default and a code Burrow can only ever carry the two aliases `sys_jit_create_region` installs. **Prosecute on any change:** the create-time `arch_icache_sync_range` must stay (audit F1 — `KP_ZERO` zeroes memory, NOT the I-cache, and `free_pages` does no cache maintenance, so a recycled page can hold a prior region's lines; every other executable backing in the tree already syncs at acquisition — `exec.c`'s two eager paths (over the whole page-rounded span since #107) + `fault.c`'s FILE demand-page arms); `SYS_BURROW_DETACH` must keep REFUSING a CODE alias (self-audit — one region carries ONE I-32 charge but TWO aliases, so detaching both refunds twice and a `CAP_JIT` holder could loop create-then-detach-both to zero its `page_count` and reclaim a full `PROC_PAGE_MAX`); destroy must keep validating the exec alias' geometry BEFORE unmapping either (audit F3 — otherwise a failed exec unmap leaves the orphaned RX alias the design forbids). **Cross-PE contract** (audit F2): `IC IVAU` is IS-broadcast so the cache half is cross-CPU correct, but the trailing `ISB` retires prefetch on the CALLING PE only — a peer that has already executed at those addresses needs its own context-synchronization event (any syscall/exception return is one) before branching in; ORC's `DualMapMemoryMapper` is emit-on-one-thread/execute-on-a-worker by nature, so CL-7 must honor it or make the syscall broadcast an ISB. **CL-7k-3 CLOSED 0 P0 / 0 P1 / 0 P2 / 5 P3** (Opus-5-max — the FALLBACK tier, Fable unavailable; lineage independence forfeited, noted in the close — plus a concurrent self-audit that independently found the detach hole the formal round then re-prosecuted as closed). 4 of 5 P3s fixed in-commit; 2 pre-existing tree-wide items enqueued (#106 the I-32 power-of-two charge gap -- OPEN; #107 the `exec.c` PF_X bss-tail sync -- **CLOSED 2026-07-31**, see the REVENANT row's eager-path I-cache span addendum). |
| Spawn-time page-budget (the Clade arc F4; I-32 composition, CL-5) | `kernel/include/thylacine/proc.h` (`Proc.page_budget` @392 [struct 392 -> 400, deliberate] + `Proc.page_peak` @284 [fills existing pad] + `PROC_PAGE_HARD_MAX` = 4 GiB + `PROC_FLAG_MAY_RAISE_PAGE_BUDGET`), `kernel/proc.c` (`proc_spawn_budget_resolve` -- THE authority decision, one place; the `proc_init_fields` seed; the rfork inherit; the `proc_page_charge` budget read + peak stamp; `proc_mark_/may_raise_page_budget`), `kernel/syscall.c` (`SPAWN_PERM_MAY_RAISE_PAGE_BUDGET` in `spawn_perm_grant_check` + `apply_spawn_perms`; the `page_budget` range check in the args validator; `sys_spawn_full_argv_budget_for_proc` + the back-compat `_identity_` wrapper; the `sa->page_budget` stash + the thunk stamp), `kernel/include/thylacine/syscall.h` (`sys_spawn_args.page_budget` @92, REUSING the reserved `_pad_allow` slot -- struct stays 96 B), `kernel/devproc.c` (`peak:` + `budget:` in `/proc/<pid>/status`), the libt / libthyla-rs / pouch ABI mirrors | **Invariant I-32 composition — a privilege/DoS surface; own focused round.** MEASURED BEFORE BUILT (LLVM-DESIGN.md section 7.1): a 1959-byte template-heavy C++ TU costs **64066 pages (250 MiB) = 97.8% of the 256 MiB default**, and a real project TU projects to **500-650 MiB, 2-2.5x it** -- so the collision is real, and 4 GiB covers both a real TU and a clang-sized lld link. The default is UNCHANGED (256 MiB), so a non-toolchain Proc is byte-identical. Prosecute: the budget is INHERITED across rfork/spawn (load-bearing -- `make`/`clang` are pouch ports calling `posix_spawn` and cannot set one, so only inheritance carries a raise down to cc1) yet **rfork alone never widens it** (a raise is a spawn-time decision, the I-2 monotonic-reduction shape); LOWERING needs no authority, RAISING needs `SPAWN_PERM_MAY_RAISE_PAGE_BUDGET` (the audited MAY_POST_SERVICE one-hop gate); the **hard cap is unbypassable by any authority** (this is what preserves the box-cliff protection -- an over-budget fault still `proc_fault_terminate`s, never box-extincts) and an over-cap request is REFUSED, never silently clamped; `page_budget` is never 0 on a live Proc (`proc_init_fields` seeds every Proc incl. kproc; a 0 would refuse every charge) and the resolver treats a 0 parent budget as the default rather than propagating it; the TCB exemption is unforgeable and unchanged (`PRINCIPAL_SYSTEM` ignores the budget entirely -- which is why the clade gate, whose clang++ inherits joey's principal, never consults one); no counter race/underflow on the charge (unchanged `vma_lock` discipline); `page_peak` is pure telemetry that NO policy reads, rises only, and does not move on a REFUSED charge. Known window: the child exists carrying the inherited budget between `rfork` and the thunk's stamp -- sound because nothing charges before `exec_setup`, which runs after the stamp; a future charger placed earlier would break that and must move the stamp. No renumber (composes I-32's per-Proc floor). Prose: LLVM-DESIGN.md section 7.1 + this row + `docs/reference/110-resource.md` + the CL-5 focused audit + the `resource.{page_peak_high_water,spawn_budget_resolve,measured_compile_needs_raise}` tests (the authority + hard-cap legs revert-probed) + joey's always-run `probe_cl5_page_budget` (the real ABI: reduce accepted + stamped, unauthorized raise and over-hard-cap both refused) + the SMP gate. |
