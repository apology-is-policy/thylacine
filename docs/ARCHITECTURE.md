...
| `specs/weft.tla` | Weft-1 (spec; impl OWED Weft-2..7) | Capability network dataplane (I-37): no per-op mediation + the F_NOTIF multi-holder buffer lifetime + the descriptor-ring TOCTOU + the shared-Burrow lifetime bounded by the flow |

Each module carries its clean cfg(s) plus buggy-cfg counterexamples (84
