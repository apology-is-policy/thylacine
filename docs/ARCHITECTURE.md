...
**STATUS**: COMMITTED (scripture-first; implementation follows — `SYS_EXIT_GROUP`, task #809).
...
**Mechanism.** `proc_group_terminate(p, status, msg)` — the unified primitive — under `g_proc_table_lock`:
1. records `group_exiting = true` + `group_exit_status` / `group_exit_msg` on the Proc (so whichever Thread reaps last uses the group status, not `thread_exit_self`'s default 0);
2. sets a per-Thread `die_requested` flag on every Thread of `p`;
3. **wakes** each sleeping peer so it returns toward EL0 — futex / `torpor_wait` sleepers via a new `torpor_wake_all_for_proc(p)`; `devnotes_read` / `poll` sleepers via the existing non-catchable-`kill` wake; deadline-bounded sleepers (9P RPC, `tsleep`) self-resolve at their deadline;
4. **IPI-kicks** each peer currently RUNNING on another CPU (`gic_send_ipi(peer->cpu, IPI_RESCHED)` — needs a new per-Thread `cpu` field, set with `on_cpu`) so it traps and reaches its checkpoint without waiting for a timer tick.

Each flagged Thread, on its next **EL0-return tail**, observes `die_requested` and runs the group-exit Thread-exit path on *itself* (it `sched()`s away — noreturn; it never ERETs to EL0). The **last Thread out** (the one that drives the live-peer count to zero) performs the Proc → ZOMBIE transition with the recorded group status; `wait_pid` already reaps a multi-Thread zombie (walks `p->threads`, `on_cpu`-spins each, `thread_free`s each). The `exit_group` caller is itself flagged and exits the same way, so it need not be the last out.
...
**v1.0 envelope (deferred completion, documented not hidden).** Universal death-interruptibility of *every* kernel sleep is a large, I-9-sensitive, separable arc (the Plan 9 `error(Eintr)`-from-`sleep` lift: a per-Thread wait-lock protecting `rendez_blocked_on` + every blocking syscall propagating an interrupted return). It is **not** required by v1.0's consumers — stratumd's `exit_group` peers are futex / 9P / running, and killed Procs are typically single-thread. So this chunk wakes the sleeper classes above and **documents the residual**: a multi-thread Proc with a peer blocked *indefinitely* in a non-torpor, non-note kernel sleep (at v1.0 only a bare pipe read awaiting data/EOF) that is group-terminated → that peer dies when its blocking call completes/EOFs, not instantly. Universal-interruptible-sleep is the v1.x lift. **[OPEN Q 7.9.A]**
...
| Group termination / cross-thread shootdown (`SYS_EXIT_GROUP`) | `kernel/proc.c` (`proc_group_terminate` + `group_exiting`/`group_exit_status` on `struct Proc`; the group-status variant of the last-Thread-out ZOMBIE reap; `exits`/`thread_exit_self` group-status path), `kernel/include/thylacine/thread.h` (`die_requested` + `cpu` on `struct Thread`), `kernel/include/thylacine/proc.h` (`group_*` fields), `arch/arm64/exception.c` (die-check at the sync-from-EL0 tail + the **new IRQ-from-lower-EL return tail** -- `#713`-sensitive), `arch/arm64/vectors.S` (IRQ-from-EL0 return path if the tail is wired there), `kernel/torpor.c` (`torpor_wake_all_for_proc`), `kernel/smp.c` / `arch/arm64/gic.c` (IPI-kick a running peer via `IPI_RESCHED`), `kernel/syscall.c` (`SYS_EXIT_GROUP = 60` handler + the `kill`-cascade replacing the multi-thread refusal in `sys_postnote`), `kernel/include/thylacine/syscall.h` (`SYS_EXIT_GROUP = 60` ABI), `usr/lib/pouch/patches/0001-pouch-syscall-seam.patch` (`__NR_exit_group` 0 → 60), `usr/lib/libt` + `usr/lib/libthyla-rs` wrappers | SYS_EXIT_GROUP (#809; pulls the documented v1.x lift forward -- #808 audit F3). **Invariant I-24** (group termination atomic + exactly-once + no-lost-wakeup). The privilege/lifetime boundary AND a wait/wake surface -- prosecute hard. Flag-and-self-terminate at the EL0-return checkpoint (Plan 9 / Linux / Zircon convergent model; seL4 synchronous-stall rejected -- see §7.9.1). Invariants: **I-9** (the sleeper-wake reuses register-then-observe under the rendez lock; lock order `g_proc_table_lock → r->lock`, as `proc_become_zombie_locked` already does); **#713** (the new IRQ-from-EL0 die-check sits before the DAIF-masked eret window; the die path is noreturn); **#788** (`on_cpu`-spin before any peer `thread_free`); **I-8** (every flagged Thread eventually reaches its checkpoint -- IPI-kick for running peers, timer-tick floor, wake for sleepers); status ok/fail collapse. Closes the `exits`-with-live-peers extinction (the `tools/test.sh` flake) + `kill → -EIO in multi-thread Proc` (13b R1-F9). **v1.0 envelope** (§7.9.1 [OPEN Q 7.9.A]): a peer blocked indefinitely in a non-torpor/non-note kernel sleep (bare pipe read) dies at call completion, not instantly -- universal-interruptible-sleep is the v1.x lift. The multi-thread **fault** path (`proc_fault_terminate`) is a tracked follow-up, not this chunk. **No new spec** per the 2026-05-23 spec-to-code broadening -- prose validation in §7.9.1 + this row + the audit + the runtime tests. |
