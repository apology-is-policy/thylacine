...
- **Tickless idle (the TI arc, `docs/TICKLESS-IDLE.md`, §8.6):** `scheduler.tla`
  gains an `ArmIdleOneShot` action (an idle CPU, having registered `idle_in_wfi`,
  arms a one-shot to the nearest `g_timerwait` deadline) + the invariant that no
  work-arrival wake nor deadline is lost across it, with a buggy cfg
  (arm-before-register ⇒ a work-arrival in the window is lost ⇒ the idle CPU
  sleeps to the backstop with runnable work pending). Model-first, before the
  TI-2 impl. A sibling `sched_tickless.tla` is the fallback if the extension
  entangles the audited model (the `sched_oncpu`/`sched_alpha` precedent).
