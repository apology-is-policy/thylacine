...
all prior ops are done) — also against the synthetic NOP/FSYNC vehicles. Next is
ONE focused audit over the whole Loom-5 surface (multishot + LINK/DRAIN), then
Loom-6 (registered buffers + the native libthyla-rs API + the bench).** v1.0
