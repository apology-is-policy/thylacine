...
ABI (pinned by `_Static_assert`). Next is **Loom-6b-2** (the metadata-mutation
ops -- SETATTR / MKDIR / SYMLINK / MKNOD / UNLINKAT / RENAMEAT / RENAME / LINK:
names-from-buffer + multi-fid), then **Loom-6c** (the SMP harness + the focused
audit over the whole 6a+6b surface) + **Loom-6d** (the native libthyla-rs API +
the bench). The fid-lifecycle ops (WALK / LOPEN / LCREATE / CLUNK) are the
deferred direct-descriptor seam (task #916). v1.0 dispatches NOP / FSYNC / READ /
WRITE / READDIR / READLINK / GETATTR / STATFS.
