...
COMPLETE; Loom-5 (multishot + linked ops) is next.** v1.0 dispatches the
no-payload opcodes (NOP / FSYNC); the payload opcodes land with Loom-6's
...
- **Loom-4 — SQPOLL** (**design signed off + `loom.tla` extended**; impl in
  ...
  - **Loom-4d** — the focused audit (kthread lifetime + CQ wait/wake) + close
    ...
    seam. DIRTY close (F2 changes the park wait/wake predicate) -> round-2 audit on
    the fixes. The ARCH §25.4/CLAUDE.md Loom row gains the SQPOLL surface. The
