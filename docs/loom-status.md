...
modules `specs/loom_multishot.tla` + `specs/loom_order.tla`) is landed; the
Loom-5 IMPL is next.** v1.0 dispatches the no-payload opcodes (NOP / FSYNC); the
payload opcodes land with Loom-6's registered-buffer surface.
