...
in the build thunk, READ copies the reply into the buffer at completion. Next is
**Loom-6b** (the remaining payload opcodes -- the uniform `p9_client_*` surface),
then **Loom-6c** (the SMP harness + the focused audit) + **Loom-6d** (the native
libthyla-rs API + the bench).** v1.0 dispatches NOP / FSYNC / READ / WRITE; the
metadata/namespace opcodes land with Loom-6b.
