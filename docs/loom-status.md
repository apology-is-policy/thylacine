...
the stop/join lifetime) are COMPLETE; Loom-4d (the focused SQPOLL audit) is
next.** v1.0 dispatches the no-payload opcodes (NOP / FSYNC); the payload
...
- **Loom-4 — SQPOLL** (**design signed off + `loom.tla` extended**; impl in
  ...
  - **Loom-4d** — the focused audit (kthread lifetime + CQ wait/wake) + close +
    the ARCH §25.4/CLAUDE.md SQPOLL audit-trigger row + the OWED deterministic
    two-thread-same-loom_fd + cross-Proc-death SMP harness (shared with #841/#907).
    PENDING.
