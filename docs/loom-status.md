...
one MORE-clear terminal — against the synthetic FSYNC vehicle. Loom-5b
(LINK/DRAIN) is next, then ONE focused audit over the Loom-5 surface, then Loom-6
(registered buffers + the native libthyla-rs API + the bench).** v1.0 dispatches
the no-payload opcodes (NOP / FSYNC); the payload opcodes land with Loom-6's
