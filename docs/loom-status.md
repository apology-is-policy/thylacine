...
exact-concurrent-admission coordination were owed to Loom-6, and are CLOSED
2026-09-23 by `Loom.admitting` -- LOOM.md 8.5.1).
**Loom-6a (registered buffers + the READ/WRITE data path)** is LANDED:
`LOOM_REGISTER_BUFFERS` pins a userspace VA range to its backing anon Burrow (the
io_uring fixed-buffer model + the #847 dual-refcount), and `LOOM_OP_READ` /
`LOOM_OP_WRITE` dispatch against a registered handle + a registered-buffer slice
(two independent I-30 submit-time pins) -- WRITE copies the buffer onto the wire
in the build thunk, READ copies the reply into the buffer at completion.
**Loom-6b-1 (the read-shaped ops -- READDIR / READLINK / GETATTR / STATFS)** is
LANDED: `loom_submit_rw` generalized to `loom_submit_payload` (opcode-selected
builder + required right over the shared pin/bounds machinery); each read-shaped
op copies its reply into the registered buffer like READ, RIGHT_READ-gated.
GETATTR / STATFS expose `struct p9_attr` / `struct p9_statfs` as the Loom output
ABI (pinned by `_Static_assert`). **Loom-6b-2 (the metadata-mutation ops --
SETATTR / MKDIR / MKNOD / SYMLINK / UNLINKAT / RENAMEAT / LINK)** is LANDED: each
reads its name(s) / the input `struct p9_setattr` FROM the pinned buffer (the
WRITE-payload discipline) and replies scalar (0 / -errno); the two-fid ops
(RENAMEAT / LINK) pin a SECOND registered handle (the same I-30 mechanism applied
again, same-session enforced) in `LOOM_SQE_FID2` = `_resv1[3]`; the per-op
scalars + name sub-lengths ride the reserved tail, decoded by the build thunks
from a TOCTOU-safe SQE snapshot; the rights mirror the kernel-syscall handle
gates (RIGHT_WRITE on the mutated dir/file; LINK source RIGHT_READ). This
completes the principled 6b boundary: every op on an ALREADY-OPEN registered fid
is in. **Loom-6c is LANDED**: a deterministic **multi-in-flight** harness (the new
byte-FIFO queueing transport `9p_transport_mq` -- the single-slot loopback
structurally can't stage N in-flight replies, the gap the Loom audits carried
since #841 -- plus two tests driving N FSYNC / N READ ops in flight at once, the
multi-entry reap, and the borrow-guard balance) + the **formal Opus audit over the
whole 6a+6b surface** (one prosecutor + a concurrent self-audit, SOUND-set
cross-confirmed): **0 P0 / 0 P1 / 1 P2 / 3 P3**, all fixed (F1 a >INT32_MAX
count-vs-errno sign-alias -> `loom_count_result` clamp; F2 the two-name `==len`
empty-second-name gate; F3 the BURROW_TYPE_ANON contiguity anchor; F4 ABI field-
offset asserts). The CONCURRENT two-thread reap-vs-pump + cross-Proc-death SMP
stress harness is OWED to **Loom-6d** (the native `libthyla_rs::loom` API = the
first REAL multi-in-flight async driver + restored TSan -- its documented home
across all five Loom closed lists). **Loom-6d-1** (the native `libthyla_rs::loom`
module -- the `Ring` over SYS_LOOM_SETUP/REGISTER/ENTER, the SPSC ring memory
model, register handles/buffers, submit/enter/reap -- plus the `loom-smoke` boot
gate) and **Loom-6d-2** (`usr/loom-stress` -- the concurrent two-thread-same-`loom_fd`
FSYNC stress + the cross-Proc-death quiesce + the positive dev9p round-trip,
joey-spawned post-pivot, witnessed by the ci-smp-gate multi-boot) and
**Loom-6d-3** (`usr/loom-bench` -- the trap-amortization latency bench showing a
~7.7x NOP one-enter-vs-batched speedup, plus the Tapestry seam doc: the one-line
`impl tapestry::Loom for libthyla_rs::loom::Ring` handoff) and **Loom-6d-4** (the
focused Opus audit over the new userspace surface -- 0 P0 / 0 P1 / 3 P3, all
fixed, 2/3 cross-confirmed by self-audit) are **LANDED**. **The whole Loom arc --
Loom-0 through Loom-6d -- is COMPLETE**: the io_uring-inverted 9P ring transport,
end to end (spec suite + ring substrate + the pluggable-completion engine seam +
the batch-enter core + SQPOLL + multishot + LINK/DRAIN + registered buffers + the
uniform `p9_client_*` payload surface + the native `libthyla_rs::loom` API + the
concurrent SMP harness + the trap-amortization bench). Next: Utopia resumes at
U-6d-b. The fid-lifecycle ops (WALK / LOPEN /
LCREATE / CLUNK) are the deferred direct-descriptor seam (task #916). v1.0
dispatches NOP / FSYNC + the whole buffer-backed surface (READ / WRITE / READDIR /
READLINK / GETATTR / STATFS / SETATTR / MKDIR / MKNOD / SYMLINK / UNLINKAT /
RENAMEAT / LINK).
...
| `0c0bf83` | **Loom-5 audit close** (whole 5a + 5b surface) | one **Opus prosecutor + a concurrent self-audit**; **0 P0 / 0 P1 / 2 P2 / 4 P3** -- the mechanism VERIFIED SOUND + memory-safe even under concurrent drivers (the INFLIGHT claim under `l->lock`). The self-audit independently found **F1 (cross-confirmed)**. All 6 share ONE root (F4): the chain scheduler reasons single-drainer; the substrate supports concurrent non-SQPOLL ENTERs. **F1 [P2] FIXED** -- the drain-self gate ignored `rearm_pending`, so a later DRAIN could admit early past a back-pressured FAST multishot stream (`DrainOrdered`); gate is now `async_inflight==0 && rearm_pending==0` + a deterministic regression `9p_client.loom_drain_waits_for_rearm_pending`. **F2 [P2] cancel leg FIXED** (revert-to-HELD + retry on a dropped `-ECANCELED` post; the dispatch-leg over-admit = the inherited Loom-3 residual -> OWED to Loom-6). **F3 [P3] FIXED** (`loom_chain_done` writes `chain->state` under `l->lock`). **F4/F5/F6 [P3] DOCUMENTED** (single-driver-per-ring contract + OOM-degrades-LINK + LINK-group single-producer -- 107-loom.md "Known caveats"). NOT a dirty close (0 P0/P1; localized gate/lock/retry fixes). `memory/audit_loom_closed_list.md` Loom-5 section; ARCH 25.4 / CLAUDE.md audit-trigger row updated. | default **762/762** + UBSan **762/762** (+1 regression) under TCG + 0 EXTINCTION; the 3-module spec suite (22 cfgs) green; **SMP gate (default-smp4 + ubsan-smp4 N=10) 0 corruption** |
| `738bd3e` | **Loom-6a (registered buffers + READ/WRITE)** | `LOOM_REGISTER_BUFFERS` (the io_uring `IORING_REGISTER_BUFFERS` analog): a per-ring registered-buffer table (`struct loom_reg_buffer reg_buf[64]`); `loom_register_buffers` resolves each `(va,len)` against the Proc's VMAs to its backing `BURROW_TYPE_ANON` Burrow (one contiguous chunk -> stable direct-map kva), validates the range lies within ONE writable VMA, and pins the Burrow (the table's `burrow_ref`; the #847 dual-refcount keeps the pages alive past a munmap). All-or-nothing install (resolve under `p->vma_lock`, swap under `l->lock`, unref displaced outside both). `LOOM_OP_READ` / `LOOM_OP_WRITE` (`loom_submit_rw`): resolve + pin the registered file handle (`spoor_ref`) AND the registered buffer (an independent `burrow_ref` = the I-30 buffer pin), bounds-check the `[buf_off, buf_off+len)` slice against the kernel snapshot, rights-gate (READ->`RIGHT_READ` / WRITE->`RIGHT_WRITE`), dispatch the async Tread/Twrite. WRITE's data is read FROM the pinned buffer in `loom_build_write` (under c->lock); READ's reply is copied INTO it in `loom_async_complete` (`loom_payload_result`, under c->lock -- a non-sleeping memcpy into the pinned direct map). The buffer pin is released at reap / #898-quiesce / `loom_free`, symmetric with the spoor pin. New SQE field `_resv1[0]` = the buffer sub-offset (`LOOM_SQE_BUF_OFF`); new ABI struct `loom_buf_reg` (16 B, `_Static_assert`); the register SVC handler branches HANDLES vs BUFFERS. Single-shot only (a real multishot READ needs an event-source dev -> post-Loom graphics). `loom.tla` **unchanged** -- the registered-buffer pin + slice-validation are the abstract `reg`/`sqe_arg` model the Loom-1 spec already reserved (`ObjPinnedToSnapshot` + `ArgPinnedToSnapshot` + `ValidArgs`; the Loom-3 precedent). | default + UBSan 768/768 (+6: `loom.register_buffers{,_rejects,_replace}` + `9p_client.loom_{read,write,rw_rejects}_e2e`) under TCG + 0 EXTINCTION; the 3-module spec suite (loom + loom_multishot + loom_order) re-run clean |
| `2592560` | **Loom-6b-1 (read-shaped ops)** | the **uniform `p9_client_*` breadth** begins -- `LOOM_OP_READDIR` / `LOOM_OP_READLINK` / `LOOM_OP_GETATTR` / `LOOM_OP_STATFS` over the 6a registered-buffer machinery. `loom_submit_rw` generalized to `loom_submit_payload`: the resolve+pin (file `spoor_ref` + buffer `burrow_ref`, the two I-30 pins), the slice bounds-check, and the async submit are shared; only the *builder* + the *required right* are opcode-selected (the read-shaped ops -> `RIGHT_READ`, the `SYS_FSTAT` gate). `loom_payload_result` gains the four cases: READDIR copies the dirent stream + READLINK the link target (`min(reply, op_count)`, the READ path -- a hostile server count is clamped to the dest cap, no overrun); GETATTR / STATFS copy the fixed parsed record -- the **Loom output ABI is `struct p9_attr` (160 B) / `struct p9_statfs` (64 B)**, pinned by `_Static_assert` in `loom.c` (a future kernel field add trips the build). `dr->attr`/`dr->statfs` are by-value (no rmsg dependency); the stream pointers alias rmsg during the callback, like READ's `read_data`. The fid-lifecycle ops (WALK / LOPEN / LCREATE / CLUNK) are DEFERRED to the direct-descriptor seam (task #916; our registered handles wrap already-open fids); the metadata-mutation ops are Loom-6b-2. `loom.tla` **unchanged** (each read-shaped op is one more `Dispatch` against a pinned `reg` slot with a `ValidArgs`-checked `sqe_arg` dest slice). | default + UBSan 773/773 (+5: `9p_client.loom_{readdir,readlink,getattr,statfs}_e2e` + `loom_metaread_rejects`) under TCG + 0 EXTINCTION; the 3-module spec suite (loom + loom_multishot + loom_order) re-run clean; self-audit clean (0 P0/P1; formal audit batched at 6c) |
| `e329126` | **Loom-6b-2 (metadata-mutation ops)** | completes the uniform `p9_client_*` surface -- `LOOM_OP_SETATTR` / `MKDIR` / `MKNOD` / `SYMLINK` / `UNLINKAT` / `RENAMEAT` / `LINK`. Each reads its name(s) / the input `struct p9_setattr` FROM the pinned registered buffer in the build thunk (the WRITE-payload discipline) and replies SCALAR (0 / -errno -- `loom_payload_result`'s `default`). `loom_submit_payload` gains the mutation opcodes in its builder/right table; the per-op scalars + name sub-lengths ride the reserved SQE tail, decoded by the build thunks from a TOCTOU-safe SQE snapshot carried on the op (`op->sqe`). Two **memory-safety gates** at submit (on the kernel snapshot): a two-name op's split sub-length (`_resv1[1]`) `<= len`, and SETATTR's `len >= sizeof(struct p9_setattr)` (56 B, the **Loom SETATTR input ABI**, `_Static_assert`-pinned). The **two-fid ops (RENAMEAT / LINK)** name a SECOND registered handle in `LOOM_SQE_FID2` = `_resv1[3]`: `loom_submit_payload` resolves + pins BOTH (`op->pinned` + the new `op->pinned2` -- two independent I-30 `spoor_ref`s, released symmetrically at reap / `loom_free`), enforces SAME 9P session (`cl1==cl2`, mirroring `sys_rename`'s gate), and rights-gates per op (RIGHT_WRITE on every mutated dir/file; LINK source RIGHT_READ -- mirroring the kernel-syscall handle gates). The borrow-guard refs only `pinned` (sufficient: `cl1==cl2`). The **principled 6b boundary** is now complete: every op on an ALREADY-OPEN registered fid is in; only the fid-lifecycle ops (WALK / LOPEN / LCREATE / CLUNK -- the #916 direct-descriptor seam) + `WIRE_PASSTHROUGH` post `-ENOSYS`. `loom.tla` **unchanged** (a two-fid op is the SAME I-30 pin mechanism applied to a second object; the single-`reg` model abstracts N independent pins). | default + UBSan **777/777** (+4: `9p_client.loom_{mkdir,setattr,renameat}_e2e` + `loom_mutation_rejects`; `loom.enter_submit_rejects` ENOSYS probe repointed SETATTR -> WALK) under TCG + 0 EXTINCTION; the 3-module spec suite (loom + loom_multishot + loom_order) re-run clean; self-audit clean (0 P0/P1; formal audit batched at 6c) |
| `681f744` | **Loom-6c (multi-in-flight harness + the 6a+6b audit close)** | the deterministic **multi-in-flight** harness + the formal Opus audit over the whole 6a+6b kernel surface. NEW byte-FIFO queueing transport `kernel/9p_transport_mq.{c,h}` -- the single-slot `p9_loopback` refuses overlapping sends (can hold only ONE RPC in flight), so the multi-in-flight window stayed reasoned-by-inspection since #841; the queueing transport stages N replies, closing it. 2 deterministic tests: `9p_client.loom_multi_inflight_e2e` (FSYNC x6 all in flight -> N CQEs by tag-demux + the multi-entry `loom_reap_terminal` + the `loom_first_inflight_client` borrow-guard balance asserted by spoor-frees-once) + `loom_multi_inflight_read_e2e` (READ x4 each into a distinct buffer slice -> the completion-copy `loom_payload_result` under multi-in-flight, no crosstalk). **Audit (one Opus prosecutor + a concurrent self-audit, SOUND-set cross-confirmed): 0 P0 / 0 P1 / 1 P2 / 3 P3, all fixed** -- F1 [P2] a >INT32_MAX byte-count CQE result (WRITE's unclamped server count the realistic leg) aliasing onto -errno -> `loom_count_result` clamp to 0x7FFFFFFF; F2 [P3] the two-name split `==len` empty-second-name (SYMLINK accepted, RENAMEAT rejected -- inconsistent) -> gate tightened to `0 < split < len`; F3 [P3] the BURROW_TYPE_ANON contiguity rested on a comment -> a "do not widen this gate" anchor; F4 [P3] the ABI asserts pinned size not field offsets -> `offsetof` asserts on `p9_setattr`/`p9_attr`/`p9_statfs`. NOT a dirty close (0 P0/P1; a clamp + a gate constant + a comment + asserts). The CONCURRENT two-thread reap-vs-pump + cross-Proc-death SMP stress harness is OWED to **Loom-6d** (the native `libthyla_rs::loom` API = the first REAL multi-in-flight async driver + restored TSan -- its documented home across all five Loom closed lists). `memory/audit_loom_closed_list.md` Loom-6 section; CLAUDE.md / ARCH 25.4 audit-trigger row updated. | default + UBSan **779/779** (+2: `9p_client.loom_multi_inflight_{e2e,read_e2e}`) under TCG + 0 EXTINCTION; the 3-module spec suite (loom 2429 + loom_multishot 2940 + loom_order 1505) re-run clean; **SMP gate (default + UBSan x smp4/smp8 N=10) PASS, 0 corruption** (one benign-classified #810 v1.x multi-thread-fault extinction on 1/40 ubsan-smp8 boots under host oversubscription -- causally independent of this chunk, proc.c not touched) |
| `28967e7` | **Loom-6d-1 (native `libthyla_rs::loom` module + smoke)** | the native userspace ring API: `usr/lib/libthyla-rs/src/loom.rs` -- a `Ring` over `SYS_LOOM_SETUP` / `_REGISTER` / `_ENTER` (66/67/68; setup maps the SQ/CQ Burrow at `params.ring_va` + reports geometry, one syscall yields the loom_fd + the mapping), the `repr(C)` `Sqe` (64 B) / `Cqe` (16 B) / `BufReg` (16 B) / `Params` (88 B) byte-pinned to `loom.h` (compile-time `size_of` asserts), the **SPSC ring memory model** (userspace owns `sq_tail` / `cq_head`, the kernel owns `sq_head` / `cq_tail`; a produced SQE is release-published before the `sq_tail` bump, a posted CQE acquire-loaded via `cq_tail` -- the liburing model, paired field-for-field with the kernel's `__atomic_*`), `register_handles` / `register_buffers` (the I-30 pin substrate from userspace), `try_submit` / `enter` / `reap` / `submit_one_wait`, and a `Drop` that detaches the ring VA + closes the loom_fd (the #847 dual-refcount). `pub mod loom` + `T_SYS_LOOM_*` + `t_loom_{setup,register,enter}` svc wrappers (the anticipated aux shared-file touchpoint -- the surface the libtapestry `Loom` seam trait targets). New `usr/loom-smoke` native binary (joey-spawned, gates the boot): a NOP round-trip (the whole ring path, no fid) + register a heap buffer + a `/system.key` handle + a clean payload-op rejection on the non-9P (devramfs) handle -- pre-pivot, so dev9p is unreachable; the **positive** dev9p async round-trip is Loom-6d-2. **No new kernel mechanism** -> the 3-module spec suite is unchanged (re-run clean as the gate; userspace client code cannot violate a kernel invariant -- the kernel copies every SQE + keeps `sq_head`/`cq_tail` private). | default + UBSan **779/779** under TCG + 0 EXTINCTION + `loom-smoke: PASS` + the 3-module spec suite (loom 2429 + loom_multishot 2940 + loom_order 1505) re-run clean |
| `caedb60` | **Loom-6d-2 (concurrent + cross-Proc-death SMP stress harness)** | the OWED concurrency harness (carried since #841 across all five Loom closed lists), driving the kernel's concurrent async paths from real userspace threads. New `usr/loom-stress` native binary, joey-spawned POST-pivot (the disk 9P FS is live -- Loom payload ops are dev9p-only, the kernel 9P client drives them). Three phases: **(1)** a positive dev9p READ/WRITE/FSYNC round-trip over the ring (the real async-op coverage `loom-smoke` could not reach pre-pivot); **(2)** the **concurrent two-thread-same-`loom_fd`** FSYNC stress -- two sibling threads share ONE ring, serialize their SQ/CQ access under a spinlock but enter the kernel WITHOUT it, so both are in `loom_enter` concurrently with async FSYNCs in flight over the shared dev9p client (the #841 elected-reader + the per-ring `loom_first_inflight_client` borrow-guard + the Loom-4b CQ wait-list); every submitted op completes exactly once; **(3)** **cross-Proc-death quiesce** -- submit K FSYNCs, dispatch them async (NONBLOCK, no reap), then `mem::forget` the ring + exit so the K ops are in flight when the Proc tears down, exercising `loom_free`'s #898 abandon-on-death via the Proc-teardown handle-drain (not a clean Drop). Sandwiched before corvus bringup, so the subsequent FS-dependent bringup (corvus + login on the SAME shared dev9p client) doubles as a "client still healthy after the stress" check. The harness runs every ci-smp-gate boot under -smp 4/8 -- the multi-boot's 0-corruption verdict is the real concurrency assertion. The EXACT borrow-guard free-race window stays reasoned + round-2-audited (inherent to the bug class: a clean harness holds its registration constant, so the guard's ref is never the last). Self-audit clean (0 P0/P1; formal Opus audit batched at Loom-6d-4). | default + UBSan **779/779** under TCG + 0 EXTINCTION + `loom-stress: PASS` (phase1 round-trip + phase2 every-op-once + phase3 cross-Proc-death) + the 3-module spec suite re-run clean; **SMP gate (default + UBSan x smp4/smp8 N=10) PASS, 0 corruption across all 4 configs** (37 PASS + 3 benign ubsan-smp8 host-timing timeouts under host oversubscription; **0 EXTINCTION** -- the concurrent + cross-Proc-death stress triggered no #810 multi-thread-fault and no corruption across 40 boots) |
| `57ce416` | **Loom-6d-3 (latency bench + Tapestry seam doc)** | `usr/loom-bench` -- the trap-amortization + present-path latency bench (docs/LOOM.md section 5), joey-spawned post-pivot, CNTVCT_EL0-timed (the irq-bench idiom). Three numbers: **(1) NOP trap-amortization** -- I/O-free (NOPs complete inline, so the only variable is the trap COUNT): 64 NOPs via one-enter-per-op (64 traps) vs batched into one enter (1 trap) -> **~7.7x** under TCG (3030 ns/op -> 390 ns/op), the section-5 thesis cleanly isolated; **(2) FSYNC** -- the honest counterpoint at **~1.0x** because a 9P fsync is commit-dominated (~1.8 ms) so the ~3 us trap is invisible (the win materializes for trap-dominated / pipelinable ops + SQPOLL's zero-syscall hot path, not a single commit-bound op); **(3) present-proxy** -- a single `LOOM_OP_WRITE` of a rect descriptor (TAPESTRY.md D3: present = a generic WRITE) round-trips ~214 us. Absolute numbers are noisy under QEMU + host load + NOT a budget gate (VISION.md budgets are real-HW); joey gates only on the bench completing correctly. The **Tapestry seam doc** lands in `docs/reference/107-loom.md` ("The native `libthyla_rs::loom` API + the Tapestry seam"): the one-line `impl tapestry::Loom for libthyla_rs::loom::Ring` handoff (present = `Sqe::write` submit, arm_events = a multishot `Sqe::read`, reap = `Ring::reap`). No new kernel mechanism; the 3-module spec suite re-run clean. | default + UBSan **779/779** under TCG + 0 EXTINCTION + `loom-bench: PASS` (NOP ~7.7x + FSYNC ~1.0x + present ~214 us) + the 3-module spec suite re-run clean |
| `0a8617b` | **Loom-6d-4 (focused audit + the Loom arc close)** | one Opus prosecutor (scoped to the NEW userspace surface -- `loom.rs` + the 3 svc wrappers + loom-smoke/stress/bench; the kernel side was audited at 6c + re-validated by this chunk's SMP gate) + a concurrent self-audit. **CLEAN: 0 P0 / 0 P1 / 3 P3, all FIXED**; the prosecutor independently CROSS-CONFIRMED 2/3 of the self-found findings. NOT a dirty close (the fixes are compile-time asserts + never-firing loop guards + a comment -- behaviorally inert, no wait/wake-predicate change). **F1** [P3] the `repr(C)` structs lacked the field-offset asserts the kernel added at 6c F4 (a same-size reorder would silently corrupt the ABI the Rust mirror reads/writes) -> `offset_of!` asserts (stable since 1.77; toolchain 1.94.1). **F2** [P3] loom-bench's NOP reap-spin loops were unbounded + swallowed the `enter` Result -> a `guard < 100_000` cap + propagate (the `loom_fsync_batch` policy). **F3** [P3] loom-stress phase-3's in-flight-at-death was timing-dependent -> documented best-effort with the measured ~1000x margin (the dev9p FSYNC ~1.8 ms vs the us exit). SOUND set (repr(C) byte-match, the SPSC release/acquire ordering, the `unsafe impl Sync` justification, the Drop lifetime, the AtomicU32 view, integer/bounds) verified + cross-confirmed. **The OWED concurrent SMP harness (carried since #841 across all five Loom closed lists) is DELIVERED; the Loom arc is COMPLETE.** `memory/audit_loom_closed_list.md` Loom-6d section. | default + UBSan **779/779** under TCG + 0 EXTINCTION + loom-{smoke,stress,bench}: PASS + the 3-module spec suite re-run clean; **SMP gate (the full 6d boot path -- all three loom binaries + the audit fixes -- default + UBSan x smp4/smp8 N=10) PASS, 0 corruption across all 4 configs** (2 benign ubsan-smp8 host-timing timeouts; 0 EXTINCTION) |
...
| `fa91fb34` | **8.5.1 -- directory-mutation DAC at submit + exact admission** (aux, 2026-09-23; audit-bearing) | Found while reading for the Haul cape: `MKDIR`/`MKNOD`/`SYMLINK`/`UNLINKAT`/`RENAMEAT`/`LINK` gated on the registered handle's `RIGHT_WRITE` alone, which an `O_PATH` directory handle carries hollow, so any principal that could X-search to a directory could mutate it. `loom_dir_mutation_gate` runs the sync twins' parent W|X against the ring creator's live identity (`Loom.ident`, stamped at setup), refuses the six on an SQPOLL ring before any stat, resolves a create's gid (0 = primary; else the chgrp rule) and sends a create's mode as rwx only. `Loom.admitting` makes CQ admission exact (the Loom-5 F2 residual, CLOSED); a blocking `ENTER` and the `DRAIN` gate count it too, so neither acts past a sibling that is mid-submit. Child names take `sys_copy_component`'s shape on a kernel copy, which is what the build thunk sends. Tests `9p_client.loom_dirmut_dac` / `loom_dirmut_sqpoll` / `loom_dirmut_names` / `loom_create_gid`, `loom.admission_counts_admitting` / `wait_counts_admitting` / `drain_waits_for_admitting`, and symlink-probe leg O on live Stratum. Audit: two rounds (Fable 5.1), 0 P0 / 0 P1 / 0 P2 / 8 P3; one P3 (a moved directory needs no W on itself, shared with the sync rename) is the operator's call. |
...
`tapestryd`), not in the Loom arc. A native POC RUNS in-tree
(`usr/lib/libtapestry` + `usr/tapestry-demo` + `usr/tapestryd`; promoted off the
aux track, where it merely compiled -- corrected 2026-08-16, aux#237).
