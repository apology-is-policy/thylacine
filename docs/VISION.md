...
| 3 | Pipelined 9P client (out-of-order, flow-controlled) | Low | Phase 5 |
...
| 9 | Formal-verification cadence: 17 TLA+ modules pinning the load-bearing invariants | Low-medium | Continuous |
...
- **Audio at v1.0.** *Relaxed 2026-09-05 by operator direction; the **Nocturne** design (`docs/NOCTURNE.md`) was RATIFIED 2026-09-06 by operator vote and builds the audio system now.* The original rationale is kept as the bar rather than as the deferral: a half-baked audio stack is the kind of stub the project explicitly rejects, so audio ships properly or not at all — the Nocturne design (N-0, ratified §13) and its sequencing (N-1..N-7) are held to that. Whether Nocturne rides v1.0 or v1.1 is a ROADMAP call at ratification.
...
- Plan 9's network stack. Thylacine uses a modern TCP/IP stack (smoltcp-Rust port) exposed as a `netd` userspace 9P server serving `/net` (the Plan 9 idiom); the design lands in Phase 8 (ROADMAP §9.1 + the HOLOTYPE #68 net design pass).
...
- **Stratum v2 is feature-complete and shipping (as of 2026 Q2).** All nine phases of Stratum's v2 roadmap have landed: foundations, lock-free Bε-tree, persistence, integrity + crypto (ML-KEM-768 + XChaCha20-Poly1305 hybrid), multi-device + quorum, territories, cold tier, the comprehensive POSIX surface, and the 9P client interfaces. Integration is consumed via Stratum's three stable ABIs documented in `stratum/v2/docs/OS-INTEGRATION.md` — Thylacine binds to the 9P2000.L wire surface (Stratum's recommended OS integration path).
- **Thylacine's Phase 5 integrates against Stratum v2's stable 9P interfaces.** The 9P2000.L dialect with Stratum extensions: `Tsync` (explicit sync barrier), `Treflink` (single-dataset reflink; cross-dataset gated on Stratum's rekeying primitive), `Tbind` / `Tunbind` (per-connection subvolume composition), `Txattrwalk` family (POSIX xattrs end-to-end). The 9P2000.L baseline (`Tlopen`, `Tlcreate`, `Tsymlink`, `Tmknod`, `Trename`, `Treaddir`, `Tstatfs`, `Tgetattr`, `Tsetattr`, `Treadlink`, `Tlock`, `Tgetlock`, `Tlink`, `Tmkdir`, `Trenameat`, `Tunlinkat`) provides every modern POSIX primitive Thylacine's syscall layer needs. Thylacine's kernel 9P client (Phase 5) speaks the wire dialect directly; the syscall layer translates Thylacine native calls into 9P messages.
- **The deployment model**: `stratumd` is a userspace daemon, one process per pool, bound to a Unix socket. It exposes both an FS surface (the user-facing filesystem) and a separate `/ctl/` surface (the admin synthetic 9P FS — pools, datasets, snapshots, scrub, events, Prometheus metrics). Thylacine's kernel 9P client mounts the FS surface at `/`; userspace tools dial the `/ctl/` socket directly (typically through a `/srv/stratum-ctl/` mount).
- **The `.key` sidecar discipline is non-negotiable.** Stratum is post-quantum encrypted by default — there is no plaintext mode. The wrapped key lives separate from the pool block device (the separability IS the security boundary). Thylacine's boot path must unwrap the key (passphrase / TPM / hardware token / Argon2id-from-typed-input) before invoking `stratumd`; the unwrapped key is `mlock`ed + `MADV_DONTDUMP`'d and consumed by `stratumd` once at mount. See `stratum/v2/docs/OS-INTEGRATION.md §5` for the canonical key-delivery options.
...
- **POSIX surface delivered by Stratum** (Thylacine consumes via 9P; no per-feature work needed unless the kernel mediates): inodes + dirents + xattrs + file seals (`F_SEAL_*`) + advisory locks (`flock` / OFD locks) + `statx` + `name_to_handle_at` / `open_by_handle_at` + `copy_file_range` (whole-file MVP) + `reflink` (single-dataset) + `rename` family (`RENAME_EXCHANGE` / `_WHITEOUT` / `_NOREPLACE`) + `fallocate` (PUNCH/COLLAPSE/INSERT/ZERO/UNSHARE) + symlinks + hard links + `O_TMPFILE` + `posix_fadvise` + inline-data optimization (≤100 B). Deferred upstream (Thylacine accommodates as Stratum lands): cross-dataset reflink, `inotify`/`fanotify`, FS-verity API, `O_DIRECT`, quotas. None of these are v1.0 blockers per Phase 6 (Linux compat) policy.
- **Stratum's formal verification posture** (TLA+ specs, adversarial audit loop, property-based tests, fuzz harness) is the model for Thylacine's own correctness work. The audit pattern, the spec naming convention, the buggy-config counterexample pattern — all transfer. Stratum has TLA+ specs for sync (3-phase), nonce machinery, allocator, tree commit, territory, inode, dirent, xattr, snapshot, quorum, and others; Thylacine ships **17 TLA+ modules** pinning the §28 load-bearing invariants (the authoritative inventory is `ARCHITECTURE.md §25.2`: scheduler/territory/handles/burrow/9p_client/poll/pipe/tsleep/corvus + the sched_ctxsw/sched_oncpu/sched_alpha/asid/death_wake redesign models + the loom/loom_multishot/loom_order family). The three originally-planned `futex`/`notes`/`pty` specs were dropped 2026-05-23 (torpor + notes are prose-validated; PTY is unbuilt); spec-first design is now applied per-surface (the broad spec-to-code suspension, re-enabled for the SMP/ASID/death-wake/Loom surfaces).

Stratum was not designed for Thylacine. It fits as if it was. The fit is evidence that both projects are built on the same proposition: 9P as the universal composition mechanism.

**Stratum/Thylacine coordination at Phase 5 (the integration phase)**:

Thylacine Phase 5 entry consumes Stratum v2's stable 9P2000.L wire + libstratum-9p (optionally) + 9P-served `/ctl/` admin surface. Phase 5 chunks are no longer gated on Stratum-side delivery — Stratum is ready. The Phase 5 plan is bounded by Thylacine's own deliverables: kernel 9P client (with `specs/9p_client.tla` written first per CLAUDE.md spec-first policy), mount path integration, `.key` sidecar handling, `stratumd` lifecycle inside Thylacine's userspace-driver-as-9P-server pattern, and `/ctl/` consumption.

The encryption / snapshot / MVCC / multi-device / dedup / cold-tier features of Stratum are Thylacine-transparent — the kernel sees only the 9P interface. None of those features gate Thylacine's progress. Only the 9P client correctness, mount integration, and key handling matter for Thylacine-side integration.
