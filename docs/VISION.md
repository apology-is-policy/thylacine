...
- **Audio at v1.0.** *Relaxed 2026-09-05 by operator direction: the **Nocturne** arc (`docs/NOCTURNE.md`) builds the audio system now.* The original rationale is kept as the bar rather than as the deferral: a half-baked audio stack is the kind of stub the project explicitly rejects, so audio ships properly or not at all — the Nocturne design pass (N-0) and its sequencing (N-1..N-7) are held to that. Whether Nocturne rides v1.0 or v1.1 is a ROADMAP call at ratification.
