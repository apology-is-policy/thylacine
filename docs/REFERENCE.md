...
- **Tip**: P2-Cf landed (on_cpu flag + SMP wait/wake race close — wakeup() spin-waits for waiter to be off-CPU before transitioning state, preventing pick of half-saved ctx). Phase 2 in progress. Predecessor: `e9de6c1` (P2-Ce hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A through P2-Cf landed. **P2-Cf landed** (struct Thread.on_cpu; cs->prev_to_clear_on_cpu handoff; wakeup() spin-on-on_cpu; sched_arm_clear_on_cpu + sched_finish_task_switch public helpers). Next: P2-Cg (scheduler.tla SMP refinement with Steal action + IPI ordering invariant I-18). Then P2-D..H.
- **Tests**: **22/22 in-kernel tests PASS** every boot. P2-Cf doesn't add a new test — the existing rendez tests + work_stealing_smoke continue to pass under the on_cpu serialization. `tools/test.sh` PASS (production: ~224 ms boot). `tools/test.sh --sanitize=undefined` PASS. `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack).
- **Specs**: 1 written (`scheduler.tla` — TLC-clean at 283 distinct states; `scheduler_buggy.cfg` produces canonical missed-wakeup counterexample). P2-Cd touches no spec — per-CPU run trees are infrastructure with no concurrency invariant impact at this sub-chunk; cross-CPU spec work begins at P2-Cg (Steal action + IPI ordering invariant I-18). 9 planned. Phase 2 close adds LatencyBound liveness refinement + `namespace.tla` + `handles.tla`.
- **LOC**: ~6320 (P2-Cc) + ~80 sched.c per-CPU refactor + ~30 thread_init_per_cpu_idle + ~30 per_cpu_main wiring + new test + reference doc updates ≈ ~6500 LOC.
- **Kernel ELF**: ~288 KB debug (production) / ~313 KB debug (UBSan). Flat binary: ~50 KB. Page tables: 40 KiB BSS. struct page array: 24 MiB BSS. GIC handler table: 16 KiB BSS. Vector table: 2 KiB in `.text`. `g_cpu_online` + `g_cpu_alive`: 16 bytes BSS. `g_pac_keys`: 64 bytes BSS. `g_secondary_boot_stacks`: 112 KiB BSS. `g_exception_stacks`: 32 KiB BSS. `g_exception_stack_observed`: 64 bytes BSS. **`g_cpu_sched`: ~256 bytes BSS** (P2-Cd). **`g_need_resched[]`: 8 bytes BSS** (P2-Cd, was scalar bool).
