...
- **Tip**: P2-Cdc landed (GIC SGI infrastructure + IPI_RESCHED — secondaries now receive cross-CPU IPIs and wake from WFI). Phase 2 in progress. Predecessor: `af38884` (P2-Cd hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A through P2-Cd (incl. Cdc) landed. **P2-Cdc landed** (per-CPU GIC redistributor + CPU interface init; ICC_SGI1R_EL1 send; IPI_RESCHED handler; secondary idle loop = sched()+wfi). Next: P2-Ce (work-stealing); Cf (finish_task_switch); Cg (scheduler.tla SMP refinement with IPI ordering invariant I-18). Then P2-D..H.
- **Tests**: **21/21 in-kernel tests PASS** every boot. P2-Cdc adds smp.ipi_resched_smoke (boot CPU sends IPI_RESCHED to each secondary; verifies counter increments — direct evidence of GIC SGI delivery + handler invocation). `tools/test.sh` PASS (production: ~104 ms boot, 4-CPU QEMU with 3 secondaries fully online via PSCI HVC + per-CPU sched + idle thread + IPI). `tools/test.sh --sanitize=undefined` PASS (~140 ms). `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack).
