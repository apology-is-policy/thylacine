...
- **Tip**: P2-Cb landed (per-CPU init at high VA — PAC + MMU + VBAR + TPIDR; secondaries reach C runtime). Phase 2 in progress. Predecessor: `0fb1832` (P2-Ca hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A through P2-Bc landed. P2-Ca landed (PSCI bringup). **P2-Cb landed** (secondaries reach high-VA `per_cpu_main` after PAC apply + MMU enable + VBAR + TPIDR). Next: P2-Cc (per-CPU exception stacks); P2-Cd (GIC SGI infrastructure + IPIs + per-CPU run trees + idle threads); Ce (work-stealing); Cf (finish_task_switch); Cg (scheduler.tla SMP refinement). Then P2-D..H.
- **Tests**: **18/18 in-kernel tests PASS** every boot. P2-Cb extends smp.bringup_smoke to verify both `g_cpu_online` (trampoline reached) and `g_cpu_alive` (per_cpu_main reached at high VA). `tools/test.sh` PASS (production: ~112 ms boot, 4-CPU QEMU with 3 secondaries reaching C runtime via PSCI HVC). `tools/test.sh --sanitize=undefined` PASS (~143 ms). `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack — primary's PAC/canary/W^X/BTI all live).
- **Specs**: 1 written (`scheduler.tla` — TLC-clean at 283 distinct states; `scheduler_buggy.cfg` produces canonical missed-wakeup counterexample at depth 4). P2-Cb touches no spec — per-CPU init is hardware coordination, not a kernel concurrency invariant. Cross-CPU spec work begins at P2-Cd. 9 planned. Phase 2 close adds LatencyBound liveness refinement + `namespace.tla` + `handles.tla`.
- **LOC**: ~6000 (P2-Ca) + ~80 pac asm + ~70 per_cpu_main + ~30 trampoline extension + minor edits ≈ ~6200 LOC.
- **Kernel ELF**: ~281 KB debug (production) / ~305 KB debug (UBSan). Flat binary: ~48 KB. Page tables: 40 KiB BSS. struct page array: 24 MiB BSS. GIC handler table: 16 KiB BSS. Vector table: 2 KiB in `.text`. `g_cpu_online`: 8 bytes BSS. `g_cpu_alive`: 8 bytes BSS. `g_pac_keys`: 64 bytes BSS. `g_secondary_boot_stacks`: 112 KiB BSS.
...
| [17-smp-bringup.md](reference/17-smp-bringup.md) | SMP secondary bring-up via PSCI: lib/dtb.c cpu/psci enumeration + arch/arm64/psci.{h,c} HVC/SMC + arch/arm64/start.S secondary_entry trampoline (P2-Ca minimal; P2-Cb extended with PAC + MMU + long-branch to high VA per_cpu_main) + kernel/smp.{h,c} smp_init/per_cpu_main + g_pac_keys/g_secondary_boot_stacks/g_cpu_online/g_cpu_alive. | medium | **P2-Ca + P2-Cb landed**; per-CPU run trees + idle threads + IPIs at P2-Cd |
