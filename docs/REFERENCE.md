...
- **Tip**: P2-Cd landed (per-CPU run trees + per-CPU idle threads; secondaries' WFI park awaiting P2-Cdc IPIs). Phase 2 in progress. Predecessor: `a9757db` (P2-Cc hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A through P2-Cd landed. **P2-Cd landed** (per-CPU CpuSched struct; sched_init(cpu_idx); thread_init_per_cpu_idle for secondaries; per_cpu_main wired to scheduler). Next: P2-Cdc (GIC SGI + IPIs); Ce (work-stealing); Cf (finish_task_switch); Cg (scheduler.tla SMP refinement). Then P2-D..H.
- **Tests**: **20/20 in-kernel tests PASS** every boot. P2-Cd adds smp.per_cpu_idle_smoke (verifies sched_idle_thread for each CPU; CPU 0 = kthread; secondaries = fresh thread in SCHED_BAND_IDLE). `tools/test.sh` PASS (production: ~100 ms boot, 4-CPU QEMU with 3 secondaries reaching C runtime via PSCI HVC + per-CPU sched + idle thread). `tools/test.sh --sanitize=undefined` PASS (~140 ms). `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack).
