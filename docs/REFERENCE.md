...
- **Tip**: P2-Ce landed (work-stealing + finish_task_switch — cross-CPU thread placement via try_steal; pending_release_lock handoff for migrated threads). Phase 2 in progress. Predecessor: `2648053` (P2-Cdc hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A through P2-Ce landed. **P2-Ce landed** (real spinlocks; per-CPU run-tree lock; try_steal in sched()'s no-peer path; cs->pending_release_lock finish_task_switch handoff; sched_finish_task_switch helper for thread_trampoline). Next: P2-Cf (proper finish_task_switch refinement / wait-wake race close); Cg (scheduler.tla SMP refinement with Steal action + IPI ordering invariant I-18). Then P2-D..H.
- **Tests**: **22/22 in-kernel tests PASS** every boot. P2-Ce adds smp.work_stealing_smoke (boot CPU creates N threads, ready()s on its tree, sends IPI_RESCHED to secondaries; verifies at least one secondary's per-CPU counter incremented — direct evidence of try_steal pulling work cross-CPU). `tools/test.sh` PASS (production: ~195 ms boot — busy-waits in the new smoke test extend boot vs P2-Cdc's 104 ms; still well under the 500 ms budget). `tools/test.sh --sanitize=undefined` PASS. `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack).
