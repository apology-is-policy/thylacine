...
- **Tip**: P2-Cc landed (per-CPU exception stacks via SPSel=0 — kernel runs on SP_EL0, exception entry hardware-switches to SP_EL1 = per-CPU slot). Phase 2 in progress. Predecessor: `13fff49` (P2-Cb hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A through P2-Cc landed. **P2-Cc landed** (per-CPU exception stacks; SPSel=0 kernel mode; SP_EL1 re-anchored to HIGH VA after MMU enable). Next: P2-Cd (GIC SGI infrastructure + IPIs + per-CPU run trees + idle threads); Ce (work-stealing); Cf (finish_task_switch); Cg (scheduler.tla SMP refinement). Then P2-D..H.
- **Tests**: **19/19 in-kernel tests PASS** every boot. P2-Cc adds smp.exception_stack_smoke (verifies SPSel=0 + g_exception_stacks layout + observed IRQ-entry SP falls in g_exception_stacks[0]). `tools/test.sh` PASS (production: ~112 ms boot, 4-CPU QEMU with 3 secondaries reaching C runtime via PSCI HVC). `tools/test.sh --sanitize=undefined` PASS (~143 ms). `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack — primary's PAC/canary/W^X/BTI all live).
- **Specs**: 1 written (`scheduler.tla` — TLC-clean at 283 distinct states; `scheduler_buggy.cfg` produces canonical missed-wakeup counterexample). P2-Cc touches no spec — per-CPU exception stacks are hardware/cooperation infrastructure, not a kernel concurrency invariant. Cross-CPU spec work begins at P2-Cd. 9 planned. Phase 2 close adds LatencyBound liveness refinement + `namespace.tla` + `handles.tla`.
- **LOC**: ~6200 (P2-Cb) + ~30 start.S SPSel-dance × 2 (primary + secondary) + ~12 SP_EL1 high-VA re-anchor × 2 + g_exception_stacks BSS decl + smp_cpu_idx_self + observability hook + test ≈ ~6320 LOC.
- **Kernel ELF**: ~282 KB debug (production) / ~306 KB debug (UBSan). Flat binary: ~48 KB. Page tables: 40 KiB BSS. struct page array: 24 MiB BSS. GIC handler table: 16 KiB BSS. Vector table: 2 KiB in `.text`. `g_cpu_online` + `g_cpu_alive`: 16 bytes BSS. `g_pac_keys`: 64 bytes BSS. `g_secondary_boot_stacks`: 112 KiB BSS. **`g_exception_stacks`: 32 KiB BSS** (P2-Cc). `g_exception_stack_observed`: 64 bytes BSS.
