...
- **Tip**: P2-Bb landed (Plan 9 wait/wake — `sleep(r, cond, arg)` + `wakeup(r)` over `Rendez`; `sched()` refactored to respect pre-set `prev->state`). Phase 2 in progress. Predecessor: `8577ec8` (P2-Ba hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A landed (process-model bootstrap) + R4 audit closed. P2-Ba landed (scheduler dispatch). **P2-Bb landed** (wait/wake over Rendez). Next: P2-Bc (preemption + IRQ-mask + EEVDF math refinement); P2-C (SMP + work-stealing); P2-D..H (rfork + namespace + handles + VMO + ELF + closing audit).
- **Tests**: **16/16 in-kernel tests PASS** every boot (Phase 1's 9 + context.{create_destroy, round_trip} from P2-A + scheduler.{dispatch_smoke, runnable_count} from P2-Ba + rendez.{sleep_immediate_cond_true, basic_handoff, wakeup_no_waiter} from P2-Bb). `tools/test.sh` PASS (production: ~42 ms boot). `tools/test.sh --sanitize=undefined` PASS. `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack).
- **Specs**: 1 written (`scheduler.tla` — TLC-clean at 283 distinct states; `scheduler_buggy.cfg` produces canonical missed-wakeup counterexample at depth 4). P2-Bb implements `WaitOnCond` ↔ `sleep` and `WakeAll` ↔ `wakeup` mapping; spec unchanged (already proves `NoMissedWakeup`). 9 planned. P2-Bc refines with EEVDF math + latency bound; Phase 2 close adds `namespace.tla` + `handles.tla`.
- **LOC**: ~4790 (P2-Ba) + ~110 sleep/wakeup impl + ~120 rendez.h + ~180 test_rendez.c ≈ ~5200 LOC.
- **Kernel ELF**: ~253 KB debug (production) / ~278 KB debug (UBSan). Flat binary: ~46 KB. Page tables: 40 KiB BSS. struct page array: 24 MiB BSS. GIC handler table: 16 KiB BSS. Vector table: 2 KiB in `.text`.
