...
- **Tip**: P2-Bc landed (scheduler-tick preemption + IRQ-mask discipline; per-thread slice + `sched_tick()` + `preempt_check_irq()` wired to vectors.S IRQ-return). Phase 2 in progress. Predecessor: `c798381` (P2-Bb hash fixup).
- **Phases**: Phase 0 done; Phase 1 CLOSED at `ceecb26`. **Phase 2 in progress.** P2-A landed (process-model bootstrap) + R4 audit closed. P2-Ba landed (scheduler dispatch). P2-Bb landed (wait/wake over Rendez). **P2-Bc landed** (scheduler-tick preemption + IRQ-mask discipline). Next: P2-C (SMP + work-stealing); P2-D..H (rfork + namespace + handles + VMO + ELF + closing audit). Full EEVDF math (vd_t = ve_t + slice × W_total / w_self) deferred — meaningful only with weight ≠ 1 which is Phase 5+.
- **Tests**: **17/17 in-kernel tests PASS** every boot (Phase 1's 9 + context.{create_destroy, round_trip} from P2-A + scheduler.{dispatch_smoke, runnable_count} from P2-Ba + scheduler.preemption_smoke from P2-Bc + rendez.{sleep_immediate_cond_true, basic_handoff, wakeup_no_waiter} from P2-Bb). `tools/test.sh` PASS (production: ~120 ms boot — preempt smoke contributes ~45 ticks of timer_busy_wait). `tools/test.sh --sanitize=undefined` PASS. `tools/verify-kaslr.sh -n 5` PASS (5/5 distinct). `tools/test-fault.sh` PASS (3/3 protections fire under attack).
- **Specs**: 1 written (`scheduler.tla` — TLC-clean at 283 distinct states; `scheduler_buggy.cfg` produces canonical missed-wakeup counterexample at depth 4). P2-Bc updates the docs preamble + SPEC-TO-CODE.md to map preempt path to existing `Yield(cpu)` action; no model changes (Yield is non-deterministic so it covers cooperative + involuntary). 9 planned. Phase 2 close adds LatencyBound liveness refinement + `namespace.tla` + `handles.tla`.
- **LOC**: ~5200 (P2-Bb) + ~50 sched_tick/preempt_check_irq + ~90 test_sched.c additions + minor edits ≈ ~5400 LOC.
- **Kernel ELF**: ~255 KB debug (production) / ~285 KB debug (UBSan). Flat binary: ~47 KB. Page tables: 40 KiB BSS. struct page array: 24 MiB BSS. GIC handler table: 16 KiB BSS. Vector table: 2 KiB in `.text`.
