...
- **Tip**: P1-I in progress — `*(pending)*` lands the verification-infra subset (10000-iter alloc/free leak check via two new tests `phys.leak_10k` + `slub.leak_10k`; boot-time measurement via `CNTPCT_EL0` capture in `start.S` + boot-banner readout; `tools/verify-kaslr.sh` script that does N boots and PASSes if ≥ 70% of offsets are distinct; `THYLACINE_SANITIZE=undefined` opt-in CMake variable enabling UBSan trapping mode `-fsanitize=undefined -fsanitize-trap=undefined`; `tools/test.sh --sanitize=ubsan` / `tools/build.sh --sanitize=ubsan` honor the alternate build dir). Boot time ≈ 35.5 ms (production) / 64.8 ms (UBSan), well under the 500 ms budget. 9/9 tests PASS each boot. Predecessor: `e8c9c5c` (P1-H + audit R2 close).: stack canaries (`-fstack-protector-strong` + `__stack_chk_guard` initialized from KASLR entropy), PAC return-address signing (`-mbranch-protection=pac-ret`; APIA key + SCTLR_EL1.EnIA from start.S), BTI indirect-branch guards (`-mbranch-protection=bti`; SCTLR_EL1.BT0 + PTE_GP on kernel text), LSE atomics permission (`-march=armv8-a+lse+pauth+bti`), stack-clash protection, NX stack. New `arch/arm64/hwfeat.{h,c}` reads ID_AA64ISAR0/1 + PFR1 to populate `g_hw_features` for banner reporting. CFI / MTE / `_FORTIFY_SOURCE` deferred with explicit rationale. Predecessor: `39eafb4` (P1-G + audit R1 close).
- **Phases**: Phase 0 done; Phase 1 in progress (P1-A through P1-H complete; P1-I in flight — verification subset landed, deliberate-fault matrix pending).
- **Tests**: **9/9 in-kernel tests PASS** every boot (kaslr.mix64_avalanche, dtb.chosen_kaslr_seed_present, phys.alloc_smoke, **phys.leak_10k**, slub.kmem_smoke, **slub.leak_10k**, gic.init_smoke, timer.tick_increments, hardening.detect_smoke). 1 integration check (`tools/test.sh`); also `tools/test.sh --sanitize=ubsan` (UBSan trapping mode boot — clean, no UB triggered). `tools/verify-kaslr.sh` — 10/10 boots produced 10 distinct KASLR offsets. Deliberate-fault matrix (canary smash, BTI fault, W^X violation; PAC mismatch deferred) at P1-I-C.
- **Specs**: 0 written; 9 planned.
- **LOC**: ~3030 C99 + ~430 ASM + ~75 linker-script + ~280 CMake/shell ≈ 3815 LOC.
- **Kernel ELF**: ~213 KB debug (production) / ~228 KB debug (UBSan). Flat binary: 29 KB. Page tables: 40 KiB BSS. struct page array: 24 MiB BSS. GIC handler table: 16 KiB BSS. Vector table: 2 KiB in `.text`.
