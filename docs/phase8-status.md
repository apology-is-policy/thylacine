...
userspace; kernel byte-unchanged. **net-4b..net-8** (the `/net/dns` resolver +
cs→dns, `/net/ipifc`+ipconfig, the net-4 audit, socket-compat, dev9p.poll,
...
- **net-4a** `/net/cs` + the compiled-in ndb. **DONE.** **net-4b** `/net/dns` +
  cs→dns. **net-4c** `/net/ipifc` + ipconfig + DHCP-into-config. **net-4d** the
  focused net-4 audit + close. **net-5** socket-compat pouch patch.
