...
no in-guest peer). Then **net-6b** (the `dev9p.poll` bridge —
spec-first `net_poll.tla` + focused audit; the one ABI surface), net-7 (TLS/NTP),
net-8 (exit criteria + arc audit) follow.
