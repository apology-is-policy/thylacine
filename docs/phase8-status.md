...
its live `N/` directories). A slot is "N assigned" — no smoltcp socket yet
(net-2c-2). net-2c-2..net-8 (the live `connect`/`data` path + `ctl` verbs +
NIC-IRQ poll, then listen/accept, cs/dns, socket-compat, dev9p.poll, TLS/NTP,
exit criteria) resume from there; net-2d is the focused audit.
