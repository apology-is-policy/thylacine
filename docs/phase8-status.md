...
(gated on the persistent lifecycle). net-2c..net-8 (the `/net/tcp`
clone/connect/data fid machine, listen/accept, cs/dns, socket-compat,
dev9p.poll, TLS/NTP, exit criteria) resume from there.
...
- **net-2** netd: embed smoltcp, serve `/net`, the `/net/tcp` clone/connect/data
  client path + the fid state machine (NET-DESIGN §3.4). Split:
  **net-2a** (LANDED) smoltcp + the `phy::Device` over `VirtioNetPci` + the
  DHCP-lease proof (`usr/netd`, warden-bound `virtio-pci:1`, one-shot);
  **net-2b** the persistent 9P `/net` server (corvus + ninep, mount `/net`);
  **net-2c** the `/net/tcp` clone/connect/data fid state machine;
  **net-2d** the focused audit + ARCH §25.4 enumeration + close.
