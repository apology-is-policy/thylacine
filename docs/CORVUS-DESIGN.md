...
Posting *or rebinding* the name `corvus` is gated on a one-way `proc_flags` bit (`PROC_FLAG_MAY_POST_SERVICE`) that **joey** stamps on the corvus Proc it spawns. The mechanism is `SYS_SPAWN_WITH_PERMS` (ARCH §11.2c) — a spawn variant that takes a `perm_flags` argument: the kernel stamps each requested `SPAWN_PERM_*` bit on the child *atomically inside the spawn thunk, before `exec_setup`*, so the child's very first user-mode instruction observes the final flags. There is no window in which the child could call `SYS_POST_SERVICE` before the stamp lands. Granting any `SPAWN_PERM_*` bit is itself gated on the calling Proc being `PROC_FLAG_CONSOLE_ATTACHED` (joey is the v1.0 console anchor, §5.5); an ordinary Proc that tries to confer the bit is rejected. The bit is never propagated across `rfork` (the same discipline as `PROC_FLAG_CONSOLE_ATTACHED`). An ordinary Proc cannot post or hijack `/srv/corvus`. When corvus dies, the kernel **tombstones** the name rather than freeing it: only a Proc carrying joey's marker may rebind it, so a malicious Proc cannot race corvus's restart to claim `/srv/corvus`. (One mechanism, not two: the marker *is* the rebind authority.)
...
| 20 | CLEARANCE_LIST_SELF | *(IM-4, `IMPERIUM-DESIGN.md` §11.6)* the SELF form of `CLEARANCE_LIST` (14): NO payload and NO token -- identity is the connection's kernel-stamped principal (`SYS_SRV_PEER`), the verb-18 shape. Read-only disclosure of the caller's OWN eligibility ladder ("what could I become"), so NO re-auth (listing is not activation): a live corvus user reads its own ladder; `PRINCIPAL_SYSTEM` and any principal without a corvus user record get `PermissionDenied`. The `OK` reply is byte-identical to verb 14 (`count u8`, then per eligible level `name_len u8` + `name` + `auth_required u8` + `time_bound u64 LE` + `caps_tlv_len u16 LE` + `caps_tlv`) -- the two verbs share the `emit_eligible_levels` encoder so the wire forms cannot drift. Backs `imperium --list`'s eligibility half |
