...
### LS-6 — Login + prompt UX (echo username, mask password) [DONE via #94-B-a — the real LS-8b termios echo, not the hand-rolled interim]

**Userspace** (`usr/login/src/main.rs`). The raw cons echoes nothing, so the user
types **blind** — fine (masked) for the password, wrong for the username. The
original plan was a hand-rolled `read_line(echo: bool)` flag; instead LS-6 folded
into **#94-B-a** and uses the **real LS-8b kernel termios** via an inherited
`/dev/consctl` fd (the "B = inherited consctl fd" vote, 2026-06-12): login sets
**cooked + echo** (`+icanon +echo +isig +icrnl +onlcr`) for the username read,
**cooked + NO-echo** for the passphrase (the kernel's HARD ECHO-off mask), then
restores the default (ISIG-only) before spawning the shell. login is NOT
console-attached — it drives the line discipline through the consctl fd joey
(console-attached) opens pre-relinquish and hands down (#94-B; the inherited fd is
the capability). No hand-rolled echo interim was needed. Tests: the kernel
`devdev.cons_gate` regression (a non-attached caller's consctl I/O now succeeds) +
...
- **LS-8b — termios / consctl (B, fine-grained).** The kernel-side line discipline (cooking in the cons layer — kernel-owned for the I-27 trusted path, NOT a userspace `consd`), toggled by stty-style `/dev/consctl` writes — **not a cons ioctl** (Thylacine has no ioctl; Plan 9 lineage + the modern capability-microkernel SOTA both use a control channel, not ioctl). **Five independent flags** (granularity B, user-voted 2026-06-12 over the Plan 9 coarse `rawon/rawoff`): `ICANON`, `ECHO` (the enforced password mask, a hard kernel guarantee — supersedes LS-6's interim), `ISIG` (Ctrl-C → the `interrupt` note vs a `0x03` byte), `ICRNL`, `ONLCR`. Independent bits make cbreak representable + the Phase-8 Pouch `tcsetattr` mapping 1:1. Accepted cost: icrnl/onlcr-independence + cbreak have no native v1.0 consumer (unit-tested; driven when Pouch lands). Audit-bearing (the termios state machine; prose + unit-tested per the suspension). **As-built (kernel mechanism landed):** the five-flag cooking in `cons_rx_input` + `/dev/consctl` parse/render + the ECHO-off hard guarantee + the `CONS_ISIG`-only boot default (breaks nothing), 9 unit tests. **The login-echo consumer (the LS-6 fold-in) landed in #94-B-a** — the console-mode-access fork was resolved **B (inherited consctl fd)** (user-voted 2026-06-12) over C (`SYS_CONSOLE_MODE(fd)`, an ioctl-in-disguise that deviates from the consctl-file decision). The kernel `devdev` change DROPS the consctl-only I/O re-gate (keeping the cons I/O re-gate + both open-mint gates; sound because `CWALKONLY`/#81 closes the O_PATH bypass and a consctl fd reaches a non-attached Proc ONLY by deliberate spawn-inheritance from the trusted chain — the inherited fd is the capability). joey opens `/dev/consctl` while console-attached and hands it down to each login via spawn-fd inheritance (child fd 3) + `--consctl-fd 3`; login does the cooked/echo username + masked passphrase + restore dance. The consctl-file scripture (this row) is **preserved** — no ARCH change. **The #94-B-b mechanism LANDED** (PURE USERSPACE; the kernel is byte-identical): the new `libthyla-rs Command::inherit_fd(fd)` lets `/sbin/login` forward its inherited consctl fd to the session shell `ut` (`--consctl-fd 3`); `ut` — a USER-identity, NON-console-attached Proc — now OWNS the console line discipline, holding the fd PRIVATELY on its `Repl` (`Env.consctl_fd`) and establishing its prompt mode via `Repl::console_apply_default` (the boot witness `ut: consctl ok`). This is the controlling-terminal model, I-27-safe (the inherited consctl fd confers no console-attach; `ut` never re-forwards it to a user child; consctl cannot read console INPUT). The **ut raw/cooked dance** itself (set the mode a foreground child needs + switch its stdin `Piped`→`Inherit` + restore `ut`'s mode after) is **LS-7** — the editor arc, where the child's mode needs are known (`stmt.rs:84` marks interactive-child stdin LS-5/LS-8); `console_apply_default` is the primitive LS-7's `console_cooked`/`console_raw` build on. `nora`'s raw mode rides the same enabler; see the task queue.
