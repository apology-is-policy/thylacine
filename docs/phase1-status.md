...
| e8c9c5c | **P1-H**: hardening flag enablement (canaries, PAC, BTI, LSE, stack-clash, NX stack). Toolchain bumped to `-march=armv8-a+lse+pauth+bti` + `-fstack-protector-strong` + `-fstack-clash-protection` + `-mbranch-protection=pac-ret+bti` + `-Wl,-z,noexecstack`. New `kernel/canary.{c}` + `kernel/include/thylacine/canary.h` provide `__stack_chk_guard` (volatile u64; initialized in `kaslr_init` from KASLR-mixed entropy) + `__stack_chk_fail` (extincts). New `arch/arm64/hwfeat.{h,c}` reads `ID_AA64ISAR0/1_EL1` + `ID_AA64PFR1_EL1` to populate `g_hw_features`; `hw_features_describe` produces a banner-friendly string. `arch/arm64/start.S` between BSS clear and `bl kaslr_init` programs APIA / APIB / APDA / APDB keys from cntpct rotations and sets `SCTLR_EL1.{EnIA, EnIB, EnDA, EnDB, BT0}` (RES0 on pre-ARMv8.3/8.5 — zero-cost there). `arch/arm64/mmu.h`: `PTE_GP` (bit 50) added to `PTE_KERN_TEXT` so kernel text is BTI-guarded. `arch/arm64/start.S`: long-branch into TTBR1 changed BR→BLR (PSTATE.BTYPE=01 matches compiler-emitted `bti c` at boot_main's prologue). `arch/arm64/kaslr.c`: `kaslr_init` marked `__attribute__((no_stack_protector))`; calls `canary_init(mixed)` after entropy is ready. Banner adds `hardening: ...+canaries+PAC+BTI+LSE (P1-H)` + `features: PAC,BTI,MTE1,LSE,CRC32 (CPU-implemented)` + `canary: initialized (fold 0x...)` (presence-only diagnostic; full cookie is not leaked, per audit F14). New test: `hardening.detect_smoke` (canary cookie non-zero + ID-register self-check + PAC/BTI presence assertion). Reference doc 12-hardening.md (new). **Deferred with explicit rationale**: CFI (needs LTO + indirect-call audit), MTE (Phase 8 SLUB tag-aware integration per ARCH §24.3), `_FORTIFY_SOURCE` (needs hosted libc), deliberate-fault test matrix (P1-I). | `tools/test.sh` PASS. 5 consecutive boots produced 5 distinct KASLR offsets AND 5 distinct canary cookies; 7/7 in-kernel tests PASS each boot. ELF grew 190→206 KB; flat binary 25→29 KB; reserved 26240→26248 KiB. QEMU virt with `-cpu max` reports PAC,BTI,MTE1,LSE,CRC32 as CPU-implemented. |
...
10. ✅ **P1-H: Hardening enablement.** Landed. Toolchain hardening flags (canaries, stack-clash, PAC, BTI, LSE permission, NX stack). `kernel/canary.{c}` + `kernel/include/thylacine/canary.h` for `__stack_chk_guard` + `__stack_chk_fail`; canary_init from KASLR entropy in kaslr_init (no_stack_protector). `arch/arm64/start.S` PAC keys + SCTLR enable. `arch/arm64/hwfeat.{h,c}` for ID-register inspection + banner reporting. PTE_GP on kernel text. BR→BLR on long-branch. New test hardening.detect_smoke. 7/7 tests PASS every boot. **CFI / MTE / `_FORTIFY_SOURCE` deferred with explicit rationale** (no LTO infra; Phase 8 MTE measurement; freestanding kernel). Deliberate-fault test matrix (canary smash, PAC mismatch, BTI fault, W^X violation) deferred to P1-I.
11. **P1-I (next)** — Phase 1 exit verification: `ROADMAP.md §4.2` criteria + ASan + UBSan instrumentation + 10000-iteration alloc/free leak check + host-side sanitizer matrix + CI workflow + deliberate-fault test target + Phase 1 closing audit pass.
...
- [x] **LSE atomic ops permitted via `-march=...+lse`; runtime detection in place via `g_hw_features.atomic`.** Runtime-patching LL/SC fallback path lives with the spinlock implementation (Phase 2). v1.0 kernel has no live atomic ops to exercise the fallback; P1-H satisfies the detection-only criterion.
- [x] **PAC return-address signing enabled.** `-mbranch-protection=pac-ret`; APIA key + SCTLR_EL1.EnIA set in start.S; `g_hw_features.pac_apa`/`pac_api` queryable. Forge-and-verify PAC mismatch test deferred to P1-I deliberate-fault matrix.
- [x] **BTI enabled.** `-mbranch-protection=bti`; SCTLR_EL1.BT0 set in start.S; PTE_GP on kernel text via `PTE_KERN_TEXT`; long-branch into TTBR1 changed BR→BLR for BTYPE=01 match against compiler-emitted `bti c`. `g_hw_features.bti` queryable. Deliberate-fault test deferred to P1-I.
- [ ] MTE enabled where supported (deliberate UAF detected by MTE). **Deferred per ARCH §24.3 to Phase 8** (needs SLUB tag-aware integration). `g_hw_features.mte` reports the level (1..3) detected at boot but the kernel doesn't tag allocations.
...
| Hardening boot setup | `arch/arm64/start.S` (PAC keys + SCTLR enable, BR→BLR), `arch/arm64/mmu.h` (PTE_GP), `arch/arm64/hwfeat.{h,c}`, `kernel/canary.c`, `kernel/include/thylacine/canary.h`, `arch/arm64/kaslr.c` (canary_init slot + no_stack_protector attribute) | PAC key seed + enable ordering; canary cookie initialization timing; ID-register field extraction; BTI guard-page coverage; the BR→BLR contract for boot_main entry | P1-H |
| Hardening compile flags | `cmake/Toolchain-aarch64-thylacine.cmake` | `-march`, `-mbranch-protection`, `-fstack-protector-strong`, `-fstack-clash-protection`, `-Wl,-z,noexecstack`. Changes affect every TU; regression in any of these silently weakens the binary. | P1-H |
| LSE detection | `arch/arm64/atomic.S` (Phase 2) | Runtime patching correctness | (planned) |
