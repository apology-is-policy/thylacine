...
type: sub
title: "The permission-ceiling witnesses -- the in-kernel suite, the EL0 probe, and the expect-fault guard child"
...
- `usr/viv-pheno-probe/src/main.rs` is UNOWNED (3856 lines, hundreds of legs);
  B-1a's legs L22 / L22b (the `mprotect` errnos over a never-mapped range) and
  L23..L23h (the reserve-then-commit ladder: `mmap` PROT_NONE, `mprotect` RW,
  write, R, read back, RX -> EACCES, none, a zero length, an unaligned address,
  `munmap`) are described in [[sub-kernel-syscall-dispatch]]'s B-1a section;
  the probe's own dossier is owed by the sweep.
- The range detach and its EL0 witness are B-1a' ([[arc-boosty]]).

## Caveats

The probe's `piece-detach` leg depends on the exact-match detach: when B-1a'
lands the range form, the leg should keep detaching one piece at a time -- it
is the piece-detach witness -- and gain a second leg for the range.
